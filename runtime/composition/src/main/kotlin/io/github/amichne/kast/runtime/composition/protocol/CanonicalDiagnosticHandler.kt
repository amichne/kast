...
) : OperationHandler<DiagnosticCheckRequest, DiagnosticCheckResult, DiagnosticCheckQualification, DiagnosticCheckRejection> {
    private val protocol = CanonicalDiagnosticCheckProtocol(operations, authority, scopes)
    override suspend fun execute(request: DiagnosticCheckRequest): OperationOutcome<DiagnosticCheckResult, DiagnosticCheckQualification, DiagnosticCheckRejection> {
        ...
        val maximum = when (val admitted = ResultLimit.parse(request.limit.value)) {
