...
private sealed interface InstalledIndexerStartupFailure {
    data class Launch(val failures: Set<IndexerLaunchFailure>) : InstalledIndexerStartupFailure

    data class Transport(val failure: IndexerTransportFailure) : InstalledIndexerStartupFailure

    data class Runtime(val failures: Set<InstalledKastRuntimeFailure>) : InstalledIndexerStartupFailure

    data class TransportExecution(val failure: IndexerTransportExecutionFailure) : InstalledIndexerStartupFailure
...
class KastIndexerApplicationStarter : ApplicationStarter {
     ...
     *
     * Establishes closed command admission, a bound exact socket, canonical owned state, and the sole composition
     * dispatch capability before serving. [InstalledIndexerStartupFailure] closes expected startup rejection. Raw
     * arguments and paths leave only at their named outer process, filesystem, platform, and transport boundaries.
     */
    override fun main(args: List<String>) {
        val options =
            when (val admission = IndexerLaunchOptions.admit(args)) {
                is IndexerLaunchAdmission.Admitted -> admission.options
                is IndexerLaunchAdmission.Rejected -> reject(InstalledIndexerStartupFailure.Launch(admission.failures))
            }
        val bootstrapState =
            when (val admission = IndexerBootstrapStatePublisher.admit()) {
        ...
        if (
            IndexerCacheStatePublisher.publish(IndexerCacheState.REFRESHING) != IndexerCacheStatePublication.Published
        ) {
            bootstrapState.publishTerminalFailure(
                InstalledSemanticRuntimeBootstrapTerminalFailure.CACHE_STATE_PUBLICATION
            )
            reject(InstalledIndexerStartupFailure.CacheState)
        }
        val endpoint =
            when (val preparation = PreparedIndexerEndpoint.prepare(options)) {
        ...
        val bootstrap =
            InstalledIndexerBootstrapReporter(
                        ...
                        when (state) {
                            ...
                            is InstalledIndexerBootstrapState.Rejected ->
                                when (val failure = state.failure) {
                                    is InstalledIndexerBootstrapTerminalFailure.Runtime ->
                                        when (bootstrapState.publishRejection(failure.failures)) {
                                    ...
                                    is InstalledIndexerBootstrapTerminalFailure.Transport ->
                                        bootstrapState.publishTerminalFailure(
                                            InstalledSemanticRuntimeBootstrapTerminalFailure.TRANSPORT_ACTIVATION
                                        )
                                }
                    ...
                    InstalledKastRuntime.create(
                        ...
                        object : InstalledRuntimeBootstrapObserver {
                            ...
                            override fun observeGradleJvm(report: InstalledGradleJvmSelectionReport) {
                                if (
                                    bootstrapState.observeGradleJvm(report) !=
                                        IndexerBootstrapStatePublication.PUBLISHED
                                ) {
                                    reject(InstalledIndexerStartupFailure.BootstrapState)
                                }
        ...
        val wireIdentity =
            when (
                val identity =
                    io.github.amichne.kast.distribution.contract.WireRuntimeIdentity.admit(
                        options.workspaceRoot,
                        options.runtimeId.value,
                        bootstrapState.attemptId,
                    )
            ) {
        ...
        if (IndexerCacheStatePublisher.publish(IndexerCacheState.SMART) != IndexerCacheStatePublication.Published) {
            bootstrapState.publishTerminalFailure(
                InstalledSemanticRuntimeBootstrapTerminalFailure.CACHE_STATE_PUBLICATION
            )
            reject(InstalledIndexerStartupFailure.CacheState)
        ...
        transport.use { installedTransport ->
            when (
                ...
                is IndexerTransportExecution.Completed ->
                    reject(InstalledIndexerStartupFailure.TransportExecution(IndexerTransportExecutionFailure.RETURNED))
                IndexerTransportExecution.Failed ->
                    reject(InstalledIndexerStartupFailure.TransportExecution(IndexerTransportExecutionFailure.FAILED))
                IndexerTransportExecution.Interrupted ->
                    reject(
                        InstalledIndexerStartupFailure.TransportExecution(IndexerTransportExecutionFailure.INTERRUPTED)
                    )
            }
        }
    }
...
private fun reject(failure: InstalledIndexerStartupFailure): Nothing {
    ...
    val cause =
        when (failure) {
            ...
            is InstalledIndexerStartupFailure.TransportExecution ->
                "transport-execution-${failure.failure.name.lowercase()}"
