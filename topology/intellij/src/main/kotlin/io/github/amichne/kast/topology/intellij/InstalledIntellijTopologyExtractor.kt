...
/**
 * Proof transition: `(CanonicalSemanticProjectRoot, WorkspaceInspectionOperations) -> TopologyFileExtractor`.
 *
 * Establishes one exact-root installed K2 boundary. Project lookup and current workspace observation occur for each
 * explicit extraction call; no foreground IDE control or autostart is performed and no live platform value is retained.
 */
fun installedIntellijTopologyExtractor(
    ...
    return TopologyFileExtractor { request ->
        val project =
            when (val lookup = exactProject(projectRoot)) {
                is ExactTopologyProjectResolution.Found -> lookup.project
                is ExactTopologyProjectResolution.Rejected ->
                    return@TopologyFileExtractor failed(
                        request.file,
                        lookup.failure,
                    )
            }
        val current =
            when (val state = workspaces.inspect()) {
                ...
                WorkspaceRuntimeState.Stopping -> return@TopologyFileExtractor unavailable(request.file)
            }
...
internal sealed interface ExactTopologyProjectResolution {
    ...
    data class Rejected(val failure: TopologyFileExtractionFailure) : ExactTopologyProjectResolution
}

/**
 * Proof transition: `CanonicalSemanticProjectRoot -> ExactTopologyProjectResolution`.
 *
 * Found establishes the only live IntelliJ project for the canonical root. Rejected closes absent, ambiguous, and
 * malformed platform project metadata. Raw platform projects are observed only at this adapter.
 */
...
/**
 * Proof transition: `(CanonicalSemanticProjectRoot, Iterable<Project>) -> ExactTopologyProjectResolution`.
 *
 * [ExactTopologyProjectResolution.Found] establishes exactly one live IntelliJ project whose well-formed normalized
 * absolute base path equals the canonical semantic-project root. [ExactTopologyProjectResolution.Rejected] closes
 * missing, ambiguous, malformed, and inaccessible platform paths as
 * [TopologyFileExtractionFailure.PROJECT_UNAVAILABLE]. Raw `Project.basePath` text may enter only from installed
 * IntelliJ project discovery.
 */
internal fun resolveExactTopologyProject(
    ...
    return if (matches.size == 1) ExactTopologyProjectResolution.Found(matches.single()) else projectUnavailable()
}
...
private fun unavailable(file: io.github.amichne.kast.topology.contract.TopologySourceFile): TopologyFileExtraction =
