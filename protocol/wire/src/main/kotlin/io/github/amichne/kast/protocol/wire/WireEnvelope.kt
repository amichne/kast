...
/**
 * A request envelope whose schema identity and canonical operation have been refined and whose body is structurally a
 * request.
 *
 * Construction remains inside [WireRequestEnvelope]. The retained raw payload is available only to a matching
 * [OperationWireBinding], which replaces it with the generated request type.
 */
class AdmittedWireRequest
...
sealed interface WireRequestAdmission {
    data class Admitted(val request: AdmittedWireRequest) : WireRequestAdmission

    data class Rejected(val failure: WireFailure) : WireRequestAdmission
}
...
object WireRequestEnvelope {
     ...
     *
     * Establishes a refined [SchemaIdentity], a known [CanonicalOperation], and a request body without exposing its raw
     * payload. [WireFailure] is the closed expected failure. Raw payload extraction is permitted only inside the
     * matching [OperationWireBinding] decoder.
     */
    fun admit(document: String): WireRequestAdmission =
        when (val admission = admitWireEnvelope(document)) {
            ...
            is WireEnvelopeAdmission.Admitted ->
                when (val body = admission.envelope.body) {
                    is WireBodyDocument.Request ->
                        WireRequestAdmission.Admitted(
                            AdmittedWireRequest(
 ...
 *
 * Establishes a structurally valid envelope carrying a refined schema identity and known canonical operation.
 * [WireFailure] is the closed expected failure. Raw envelope and payload values remain inside the wire module.
 */
internal fun admitWireEnvelope(document: String): WireEnvelopeAdmission {
    ...
    val schema =
        when (val refined = SchemaIdentity.parse(envelope.schema)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected ->
                return WireEnvelopeAdmission.Rejected(WireFailure.InvalidSchemaIdentity(refined.failure))
        }
    val operationId =
        when (val refined = OperationId.parse(envelope.operation)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected ->
                return WireEnvelopeAdmission.Rejected(WireFailure.InvalidOperationIdentity(refined.failure))
        }
    val operation =
        when (val resolution = CanonicalOperation.resolve(operationId)) {
            ...
            is CanonicalOperationResolution.Unknown ->
                return WireEnvelopeAdmission.Rejected(WireFailure.UnknownOperation(resolution.id))
        }
    return WireEnvelopeAdmission.Admitted(AdmittedWireEnvelope(schema, operation, envelope.body))
}
...
internal sealed interface WireEnvelopeAdmission {
    data class Admitted(val envelope: AdmittedWireEnvelope) : WireEnvelopeAdmission

    data class Rejected(val failure: WireFailure) : WireEnvelopeAdmission
}
...
internal sealed interface WireBodyDocument {
    @Serializable @SerialName("request") data class Request(val value: JsonElement) : WireBodyDocument
...
internal fun WireBodyDocument.valueRole(): WireValueRole =
    when (this) {
