...
class BackendPerformanceTest {
    ...
    fun `telemetry inSpan overhead is negligible`() {
        val telemetry = BackendTelemetry.disabled()
        val iterations = 1_000

        // Warmup
        repeat(100) {
            telemetry.inSpan(TelemetryScope.RESOLVE, "warmup") { 42 }
        }

        val elapsed = measureTimeMillis {
            repeat(iterations) { i ->
                telemetry.inSpan(TelemetryScope.RESOLVE, "test-span") { i + 1 }
            }
        }
    ...
    fun `telemetry recordReadAction overhead is negligible`() {
        ...
        repeat(100) {
            telemetry.recordReadAction(TelemetryScope.READ_ACTION, "warmup", 100L, 200L)
        }

        val elapsed = measureTimeMillis {
            repeat(iterations) {
                telemetry.recordReadAction(
                    TelemetryScope.READ_ACTION,
                    "test-read-action",
    ...
    fun `enabled telemetry with exporter stays within overhead budget`() {
        ...
        val elapsed = measureTimeMillis {
            repeat(iterations) { i ->
                telemetry.inSpan(TelemetryScope.DIAGNOSTICS, "perf-test") {
                    telemetry.recordReadAction(
                        TelemetryScope.READ_ACTION,
                        "inner-read",
                        waitNanos = 500_000L,
    ...
    fun `disabled telemetry does not throw`() {
        val telemetry = BackendTelemetry.disabled()
        val result = telemetry.inSpan(TelemetryScope.RENAME, "no-op") { "ok" }
...
@Tag("performance")
@TestApplication
class BackendOperationPerformanceTest {
