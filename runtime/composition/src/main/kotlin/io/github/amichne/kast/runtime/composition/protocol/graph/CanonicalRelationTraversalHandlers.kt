...
internal class CanonicalRelationReadHandler(operations: RelationOperations, private val authority: CanonicalProtocolAuthority) :
    ...
    override suspend fun execute(request: RelationReadRequest): OperationOutcome<RelationReadResult, RelationReadQualification, RelationReadRejection> {
        val lookup = authority.exact(request.exactSelector)
        val current = when (lookup) {
            is ExactSelectorLookup.Found -> lookup.selector.lease
            is ExactSelectorLookup.Rejected -> return OperationOutcome.Rejected(
                if (lookup.reason == SelectorLookupRejection.WRONG_KIND) RelationReadRejection.SELECTOR_WRONG_KIND else RelationReadRejection.SELECTOR_MALFORMED)
        }
        val budget = when (val admitted = relationBudget(request.limit.value)) {
            is RelationBudgetAdmission.Admitted -> admitted.budget
            RelationBudgetAdmission.Rejected -> return OperationOutcome.Rejected(RelationReadRejection.RELATION_UNSUPPORTED)
        }
        return protocol.execute(request, current, budget)
    }
}

internal class CanonicalTraversalRunHandler(operations: TraversalOperations, private val authority: CanonicalProtocolAuthority) :
    ...
    override suspend fun execute(request: TraversalRunRequest): OperationOutcome<TraversalRunResult, TraversalRunQualification, TraversalRunRejection> {
        val lookup = authority.exact(request.exactSelector)
        val current = when (lookup) {
            is ExactSelectorLookup.Found -> lookup.selector.lease
            is ExactSelectorLookup.Rejected -> return OperationOutcome.Rejected(
                if (lookup.reason == SelectorLookupRejection.WRONG_KIND) TraversalRunRejection.SELECTOR_WRONG_KIND else TraversalRunRejection.SELECTOR_MALFORMED)
        }
        val budget = when (val admitted = traversalBudget(request.maximumDepth.value, request.maximumResults.value)) {
            is TraversalBudgetAdmission.Admitted -> admitted.budget
            TraversalBudgetAdmission.Rejected -> return OperationOutcome.Rejected(TraversalRunRejection.PLAN_REJECTED)
        }
        return protocol.execute(request, current, budget)
    }
}
