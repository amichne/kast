...
fun interface IntellijNativeDefinitionProjector<Definition : NativeDetachedDefinition> {
    /**
     * Proof transition: live PsiNamedElement + RevalidatedExactDeclaration to
     * Refinement<IntellijDetachedDefinitionProjection<Definition>, IntellijFastSymbolReadRejection>.
     *
     * A refined projection establishes that the definition is detached and byte-measured under the exact selector
     * proof. [IntellijFastSymbolReadRejection] is the closed expected failure. The live declaration may be consumed
     * only inside this request-local callback.
     */
    fun project(
...
sealed interface IntellijFastSymbolReadResult<out Definition : NativeDetachedDefinition> {
    ...
    data class Rejected(val reason: IntellijFastSymbolReadRejection) : IntellijFastSymbolReadResult<Nothing>
}

class IntellijFastSymbolReadAdapter<Definition : NativeDetachedDefinition>
private constructor(
    private val scopeQuery: IntellijSearchScopeQueryAdapter = IntellijSearchScopeQueryAdapter(),
    ...
    /**
     * Proof transition: Project + SymbolDiscoveryRequest + WorkspaceSearchScopeModelCompilation +
     * IntellijNativeDefinitionProjector to IntellijFastSymbolReadResult.
     *
     * A completed result establishes bounded native discovery, exact selector issue and revalidation, and detached
     * definition projection in one cancellable IntelliJ read action. [IntellijFastSymbolReadRejection],
     * [IntellijSearchScopeFailure], and [SymbolDiscoveryQualification] close expected failure and partial coverage.
     * Live project, provider, scope, VFS, and PSI values never leave the read action.
     */
    suspend fun read(
        project: Project,
        request: SymbolDiscoveryRequest,
        ...
        itemAdmission: IntellijDiscoveryItemAdmissionPolicy = AdmitEveryIntellijDiscoveryItem,
        ...
        val scoped =
            scopeQuery.execute(
                ...
                when (val execution = query.discoverNative(project, compiledScope, request)) {
                    is IntellijNativeDiscoveryExecution.Rejected ->
                        IntellijFastSymbolReadResult.Rejected(IntellijFastSymbolReadRejection.DISCOVERY_REJECTED)
                    is IntellijNativeDiscoveryExecution.Produced -> {
                        ...
                        for (ordinal in batch.candidates.indices) {
                            ...
                            val selector =
                                when (val resolved = exactQuery.resolve(compiledScope, selection)) {
                                    is IntellijExactSelectorResolution.Resolved -> resolved.selector
                                    is IntellijExactSelectorResolution.Rejected -> {
                                        extraQualifications += SymbolDiscoveryQualification.EXACT_DEFINITION_UNAVAILABLE
                            ...
                            val live =
                                when (val found = lookup.findLive(compiledScope, selector.lookupKey())) {
                            ...
                            val proof =
                                when (val revalidated = RevalidatedExactDeclaration.validate(selector, live.evidence)) {
                            ...
                            when (val projected = projector.project(live.declaration, proof)) {
                                is Refinement.Refined -> {
                                    ...
                                    if (
                                        ...
                                        extraQualifications += SymbolDiscoveryQualification.BYTE_LIMIT_REACHED
                                        break
                                    }
                                    definitions += projected.value.definition
                                    projectionBytes = saturatedAdd(projectionBytes, projected.value.encodedBytes.value)
                                }
                                is Refinement.Rejected ->
                                    return@execute IntellijFastSymbolReadResult.Rejected(projected.failure)
                            }
                        }
                        val measuredBytes =
                            when (val parsed = NativeProjectionByteCount.parse(projectionBytes)) {
        ...
        when (scoped) {
            ...
            is IntellijScopedQueryResult.Rejected ->
                IntellijFastSymbolReadResult.Rejected(IntellijFastSymbolReadRejection.SCOPE_REJECTED)
        }
    }

    private fun elapsedSince(startedAt: Long): Long = (clock.now() - startedAt).coerceAtLeast(0L)
