...
enum class PublicToolIdentity(val toolName: String, val operation: CanonicalOperation, val description: String, val loading: HostedToolLoading) {
    SEARCH_CLASSES("search_classes", CanonicalOperation.QUERY_RUN,
        "Find named class-like declarations supported by Kast. This searches declaration names, not use sites or source text. Constructors are not included. Returns names, locations, opaque symbol references, and completeness evidence. Request signatures with query_symbols using the returned references. Uses the session workspace's saved, indexed IntelliJ state; unavailable or unready hosts reject the read. No hidden fuzzy retry or scope widening.", HostedToolLoading.EAGER),
    SEARCH_FUNCTIONS("search_functions", CanonicalOperation.QUERY_RUN,
        "Find named functions and methods. This searches declaration names, not call sites or source text. Returns every matching overload separately. Returns names, locations, opaque symbol references, and completeness evidence. Request signatures with query_symbols using the returned references. Uses the session workspace's saved, indexed IntelliJ state; unavailable or unready hosts reject the read. No hidden fuzzy retry or scope widening.", HostedToolLoading.EAGER),
    SEARCH_DECLARATIONS("search_declarations", CanonicalOperation.QUERY_RUN,
        "Find named declarations when the kind is unknown or when searching properties or type aliases. Prefer search_classes or search_functions when that kind is known. Not a source-text or usage search. Returns names, locations, opaque symbol references, and completeness evidence. Request signatures with query_symbols using the returned references. Uses the session workspace's saved, indexed IntelliJ state; unavailable or unready hosts reject the read. No hidden fuzzy retry or scope widening.", HostedToolLoading.EAGER),
