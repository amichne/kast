...
class IntellijChangeSourceAdapter(private val project: Project) :
     ...
     * [SourceObservationFailure] closes expected platform failures. Raw paths, PSI, documents, and bytes exist only
     ...
     *
     * Applied establishes the authority's exact singleton postimage after applied-write durability and physical save.
     * [SourceWriteFailure] closes expected platform failures. Raw authority extraction and all live IntelliJ values
     * remain inside this call.
     */
    override fun write(
        authority: MutationAuthority,
        durability: MutationDurabilityBarrier,
    ): SourceWriteResult =
        when (authority.intent) {
            is ChangeIntent.AddFile -> addFile.write(authority, durability)
            is ChangeIntent.AddDeclaration,
            is ChangeIntent.RenameSymbol,
            is ChangeIntent.ReplaceDeclaration -> writeExisting(authority, durability)
        }
    ...
    private fun writeExisting(
        ...
        val prepared =
            when (val result = prepare(authority)) {
                ...
                is IntellijSourcePreparation.Rejected -> return SourceWriteResult.RejectedBeforeMutation(result.failure)
            }
        ...
        return try {
            val input =
                when (val result = authority.toIntellijInput()) {
            ...
            when (
                ...
                is IntellijWriteProtocolResult.RejectedBeforeMutation -> {
                    CHANGE_LOG.info("Kast source write rejected before mutation: ${result.failure}")
                    SourceWriteResult.RejectedBeforeMutation(result.failure)
                }
                is IntellijWriteProtocolResult.RejectedAfterRollback -> {
                    CHANGE_LOG.info("Kast source write rejected after local rollback: ${result.failure}")
     ...
     *
     * RolledBack establishes the exact authority preimage and overwrites only its exact postimage or accepts an already
     * restored preimage. `AddDeclarationRollbackFailure` closes expected mismatch and platform failures. Raw recovery
     * bytes leave only inside this adapter call.
     */
    override fun rollback(
        ...
        if (
            ...
            return AddDeclarationRollbackResult.Rejected(AddDeclarationRollbackFailure.CONTENT_DIVERGED)
        }
        ...
        return when (val expected = authority.preconditionAtIntellijBoundary()) {
            ...
            is MutationPreconditionAtIntellijBoundary.Existing ->
                rollbackExisting(
                    authority,
                    expected,
                    preimage,
     ...
     * Proof transition: `WorkspaceFile -> SourceObservationResult`.
     *
     * Establishes a live valid Kotlin file with a document, exact physical bytes, and closed writability state.
     ...
     *
     * Ready establishes cancellation, smart-mode, and read-action preparation for the exact authority.
     * [SourceWriteFailure] closes expected rejection. Raw platform state remains inside the returned request-local
     * capability and never crosses the public writer boundary.
     */
    private fun prepare(authority: MutationAuthority): IntellijSourcePreparation =
        try {
     ...
     * Proof transition: `MutationAuthority -> IntellijSourcePreparation`.
     *
     * Ready establishes an exact valid writable Kotlin file, available document, byte-identical preimage,
     * compiler-grounded anchor range, and parseable declaration. [SourceWriteFailure] closes expected rejection. Raw
     * PSI and document extraction is confined to this adapter.
     */
    private fun prepareRead(authority: MutationAuthority): IntellijSourcePreparation {
        ...
        val preimage =
            when (val expected = authority.preconditionAtIntellijBoundary()) {
                MutationPreconditionAtIntellijBoundary.Absent ->
        ...
        when (val intent = authority.intent) {
            is ChangeIntent.AddFile -> return IntellijSourcePreparation.Rejected(SourceWriteFailure.MUTATION_FAILED)
            is ChangeIntent.AddDeclaration -> {
                val anchor =
                    target.declarations.filter { declaration ->
                ...
                try {
                    KtPsiFactory(project, false)
                        .createDeclaration<org.jetbrains.kotlin.psi.KtDeclaration>(intent.declaration.value)
                } catch (cancellation: ProcessCanceledException) {
                    throw cancellation
                } catch (_: Exception) {
            ...
            is ChangeIntent.RenameSymbol -> {
                if (
                    authority.mutationsAtIntellijBoundary().any { mutation ->
            ...
            is ChangeIntent.ReplaceDeclaration -> {
                ...
                try {
                    KtPsiFactory(project, false)
                        .createDeclaration<org.jetbrains.kotlin.psi.KtDeclaration>(intent.replacement.value)
                } catch (cancellation: ProcessCanceledException) {
                    throw cancellation
                } catch (_: Exception) {
                    return IntellijSourcePreparation.Rejected(SourceWriteFailure.MUTATION_FAILED)
    ...
    private fun postimageObservationDetails(
        ...
        val firstMismatch =
            (0 until sharedLength).firstOrNull { index ->
