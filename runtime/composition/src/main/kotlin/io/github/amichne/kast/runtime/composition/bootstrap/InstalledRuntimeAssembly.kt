 ...
 *
 * Establishes one production assembler that opens/imports the exact IntelliJ workspace, detaches and refines its Gradle
 * model, then constructs only exact-root compiler and change ports around the resulting publication coordinator. Every
 * bootstrap failure remains closed assembly data.
 */
internal fun productionInstalledRuntimeAssembler(): InstalledRuntimeAssembler = InstalledRuntimeAssembler { request ->
    val workspaceModel =
        when (
            val opened =
                InstalledIntellijWorkspace.open(
            ...
            is InstalledIntellijWorkspaceOpening.ModelInputRejected ->
            ...
            is InstalledIntellijWorkspaceOpening.Rejected ->
                return@InstalledRuntimeAssembler rejected(
                    ...
                    when (val observed = workspaceModel.observeCurrentSemanticIdentity()) {
                        ...
                        is Refinement.Rejected ->
                            when (observed.failure) {
                                io.github.amichne.kast.workspace.intellij.InstalledGradleModelCaptureFailure
                                    .MODEL_INPUTS_CHANGED ->
                                    io.github.amichne.kast.workspace.contract.WorkspaceSourceObservation
                                        .ModelInputsChanged
                                io.github.amichne.kast.workspace.intellij.InstalledGradleModelCaptureFailure
...
internal fun publishInstalledRuntimeIndexScope(
    ...
    observer.observeIndexScope(InstalledRuntimeIndexScope(model.root, model.sourceRoots))
}

private fun InstalledIntellijWorkspaceBootstrapPhase.runtimePhase(): InstalledRuntimeBootstrapPhase =
    when (this) {
        InstalledIntellijWorkspaceBootstrapPhase.GRADLE_JVM_SELECTION ->
            InstalledRuntimeBootstrapPhase.GRADLE_JVM_SELECTION
        InstalledIntellijWorkspaceBootstrapPhase.MODEL_INPUT_CAPTURE ->
            InstalledRuntimeBootstrapPhase.MODEL_INPUT_CAPTURE
        InstalledIntellijWorkspaceBootstrapPhase.PROJECT_IMPORT -> InstalledRuntimeBootstrapPhase.PROJECT_IMPORT
        InstalledIntellijWorkspaceBootstrapPhase.INDEXING -> InstalledRuntimeBootstrapPhase.INDEXING
        InstalledIntellijWorkspaceBootstrapPhase.MODEL_CAPTURE -> InstalledRuntimeBootstrapPhase.MODEL_CAPTURE
    }
...
private class RefreshingInstalledGradleModelReads(
    ...
    override fun read(): InstalledGradleModelRead =
        synchronized(this) {
    ...
    private sealed interface State {
        ...
        data class Refreshing(val workspace: InstalledIntellijWorkspaceModel) : State
    }
}

private fun InstalledIntellijWorkspaceModel.currentModelRead(): InstalledGradleModelRead =
    when (val current = captureCurrentModel()) {
        is Refinement.Refined ->
            projectInstalledGradleModel(
                InstalledGradleModelBoundary(
                    current.value.root,
 ...
 *
 * Establishes one assembler that owns exact-root workspace reconciliation, both durable SQLite authorities, every
 * target service, and all canonical handlers. [InstalledRuntimeAssembly] closes every expected bootstrap failure. Raw
 * paths are extracted only while opening persistence and the physical workspace adapter.
 */
internal fun productionInstalledRuntimeAssembler(inputs: InstalledRuntimeAssemblyInputs): InstalledRuntimeAssembler =
...
private fun assembleInstalledRuntime(
    ...
    val publication =
        when (
            val opened =
                SqliteWorkspacePublicationDatabase.open(
            ...
            is SqliteWorkspacePublicationDatabaseOpening.Rejected ->
                return rejected(InstalledRuntimePersistenceFailure.WORKSPACE_PUBLICATION_UNAVAILABLE)
        }
    val recovery =
        when (
            val opened =
                SqliteMutationRecoveryJournal.open(request.stateDirectory.path.resolve("mutation-recovery.sqlite"))
        ) {
            is SqliteMutationRecoveryJournalOpenResult.Opened -> opened.journal
            is SqliteMutationRecoveryJournalOpenResult.Rejected ->
                return rejected(InstalledRuntimePersistenceFailure.MUTATION_RECOVERY_UNAVAILABLE)
        }
    val topologySnapshots =
        when (val opened = SqliteTopologySnapshotStore.open(request.stateDirectory.path.resolve("topology.sqlite"))) {
            is SqliteTopologySnapshotStoreOpening.Opened -> opened.store
            is SqliteTopologySnapshotStoreOpening.Rejected ->
                return rejected(InstalledRuntimePersistenceFailure.TOPOLOGY_SNAPSHOT_UNAVAILABLE)
        }
    val workspaceModel = InstalledWorkspaceModelAdapter(modelRead)
    val reconciliation =
        IntellijWorkspaceReconciliationPort(
            { request.workspaceRoot.path },
            workspaceModel,
            workspaceModel,
        )
    val workspace = WorkspacePublicationCoordinator(reconciliation, publication)
    val publishedWorkspace =
        when (val publicationRun = workspace.reconcile()) {
            ...
            WorkspacePublicationRun.NoWork -> return rejected(InstalledRuntimeWorkspaceFailure.NoPublication)
            WorkspacePublicationRun.Invalidated -> return rejected(InstalledRuntimeWorkspaceFailure.Invalidated)
            is WorkspacePublicationRun.Blocked -> return rejected(InstalledRuntimeWorkspaceFailure.Blocked)
        }
    ...
    return when (
        val composition =
            KastRuntimeComposition.bind(
                graph.operations,
                handlers,
                platform.semantic.diagnosticScopes,
            )
    ) {
        is KastRuntimeCompositionConstruction.Created -> InstalledRuntimeAssembly.Assembled(composition.composition)
        is KastRuntimeCompositionConstruction.Rejected ->
            InstalledRuntimeAssembly.Rejected(InstalledRuntimeAssemblyFailure.Composition(composition.failures))
    }
}
...
private fun productionPlatformPorts(
    ...
    val symbols =
        InstalledIntellijSymbolPorts.create(
            semanticProjectRoot,
            workspace,
            InstalledSymbolScopeOperations(model::searchScope),
        )
    val relation =
        installedIntellijRelationCompiler(
            semanticProjectRoot,
            workspace,
    ...
    return InstalledRuntimePlatformPorts(
        SemanticRuntimePorts(
            ...
            io.github.amichne.kast.diagnostic.intellij.installedIntellijDiagnosticScopes(
                semanticProjectRoot,
                workspace,
            ),
        ),
...
private fun rejected(failure: InstalledRuntimePersistenceFailure): InstalledRuntimeAssembly.Rejected =
    InstalledRuntimeAssembly.Rejected(InstalledRuntimeAssemblyFailure.Persistence(failure))

private fun rejected(failure: InstalledRuntimeWorkspaceFailure): InstalledRuntimeAssembly.Rejected =
    InstalledRuntimeAssembly.Rejected(InstalledRuntimeAssemblyFailure.WorkspacePublication(failure))
