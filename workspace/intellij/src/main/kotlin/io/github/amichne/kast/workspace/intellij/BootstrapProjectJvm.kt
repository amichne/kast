...
enum class BootstrapProjectJvmFailure {
    PLATFORM_REJECTED
}

sealed interface BootstrapProjectJvmAssignment {
    data class Assigned(val projectJvm: AssignedBootstrapProjectJvm) : BootstrapProjectJvmAssignment

    data class Rejected(val failure: BootstrapProjectJvmFailure) : BootstrapProjectJvmAssignment
}

/** Temporary IntelliJ project SDK-table identity derived only from the sidecar Java home. */
class BootstrapProjectJvm private constructor(internal val home: Path) {
    /**
     * Proof transition: `BootstrapProjectJvm + Project -> BootstrapProjectJvmAssignment`.
     *
     * Establishes a project SDK backed by a Java SDK created from the admitted physical home before startup waits. This
     * assignment is bootstrap-only and must not be reasserted after Gradle import. [BootstrapProjectJvmFailure] is the
     * closed expected platform failure. Raw SDK names and home paths leave only at the process-local IntelliJ SDK-table
     * and project-model boundary.
     */
    fun assign(project: Project): BootstrapProjectJvmAssignment = AssignedBootstrapProjectJvm.establish(this, project)

    companion object {
        /**
         * Proof transition: `InstalledSidecarJvm -> BootstrapProjectJvm`.
         *
         * Preserves the admitted physical Java home for resolution under the project's existing SDK identity. Raw home
         * extraction is permitted only by [assign] at the isolated SDK-table boundary.
         */
        fun from(jvm: InstalledSidecarJvm): BootstrapProjectJvm = BootstrapProjectJvm(jvm.home)
    }
}
...
class AssignedBootstrapProjectJvm private constructor() {
    companion object {
        /**
         * The sole constructor authority for this proof. A value is returned only after the exact project model has
         * accepted the admitted SDK-table identity.
         */
        internal fun establish(
            ...
            try {
                ...
                BootstrapProjectJvmAssignment.Assigned(AssignedBootstrapProjectJvm())
            } catch (failure: RuntimeException) {
...
internal sealed interface InstalledProjectOpenPreparationState {
    ...
    data class Prepared(val projectJvm: AssignedBootstrapProjectJvm) : InstalledProjectOpenPreparationState

    data class Rejected(val failure: InstalledProjectOpenPreparationFailure) : InstalledProjectOpenPreparationState
}

/** Installs the admitted project JVM before project-open state is loaded without configurators. */
internal class InstalledProjectOpenPreparation(private val bootstrapJvm: BootstrapProjectJvm) {
    private var state: InstalledProjectOpenPreparationState = InstalledProjectOpenPreparationState.Pending

    /**
     * Proof transition: `Project -> InstalledProjectOpenPreparationState`.
     *
     * [InstalledProjectOpenPreparationState.Prepared] establishes the admitted project SDK before the project is opened
     * without configurators. Gradle settings are deliberately resolved only after project state has loaded.
     * [InstalledProjectOpenPreparationFailure] closes platform rejection. Live project access remains inside this
    ...
    fun prepare(project: Project): InstalledProjectOpenPreparationState {
        ...
        val assigned =
            when (val assignment = bootstrapJvm.assign(project)) {
                is BootstrapProjectJvmAssignment.Assigned -> assignment.projectJvm
                is BootstrapProjectJvmAssignment.Rejected ->
                    return reject(InstalledProjectOpenPreparationFailure.PROJECT_JVM_REJECTED)
            }
        return InstalledProjectOpenPreparationState.Prepared(assigned).also { prepared -> state = prepared }
    }
    ...
    private fun reject(failure: InstalledProjectOpenPreparationFailure): InstalledProjectOpenPreparationState.Rejected =
...
internal sealed interface InstalledGradleLinkPresenceResolution {
    data class Resolved(val presence: InstalledGradleLinkPresence) : InstalledGradleLinkPresenceResolution

    data object Rejected : InstalledGradleLinkPresenceResolution
}

internal sealed interface InstalledGradleLinkPresence {
    data class Linked internal constructor(internal val settings: GradleProjectSettings) : InstalledGradleLinkPresence

    data class Unlinked internal constructor(internal val settings: GradleProjectSettings) : InstalledGradleLinkPresence
}
...
internal sealed interface InstalledGradleImportApplication {
    data class Applied internal constructor(val operation: InstalledGradleImportOperation) :
 ...
 * [InstalledGradleLinkPresenceResolution.Rejected] closes malformed platform paths. Raw platform settings remain inside
 * the Gradle import boundary.
 */
internal fun linkedGradleProject(
    ...
    gradleSettings.linkedProjectsSettings.forEach { settings ->
        ...
        if (candidate == workspaceRoot) {
            return InstalledGradleLinkPresenceResolution.Resolved(InstalledGradleLinkPresence.Linked(settings))
        }
    }
...
/**
 * Proof transition: `InstalledGradleLinkPresence + SelectedGradleJvm -> InstalledGradleImportApplication`.
 *
 * [InstalledGradleImportApplication.Applied] establishes the only valid exact-workspace import operation after the
 * admitted Gradle JVM selector has been applied. Rejected closes platform settings mutation failure. Raw selector
 * extraction occurs only at this Gradle project-settings boundary.
 */
...
@JvmInline
private value class ProjectJvmTableName private constructor(val value: String) {
    companion object {
