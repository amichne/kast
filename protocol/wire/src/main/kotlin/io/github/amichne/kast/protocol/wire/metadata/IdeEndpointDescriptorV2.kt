...
value class IdeEndpointCanonicalRoot private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<IdeEndpointCanonicalRoot, IdeEndpointPathFailure>`.
         *
         * Establishes a bounded, absolute, normalized POSIX root with no NUL or dot segment. [IdeEndpointPathFailure]
         * is the closed expected failure. Raw text may leave only at the endpoint descriptor serialization boundary.
         */
        fun parse(raw: String): Refinement<IdeEndpointCanonicalRoot, IdeEndpointPathFailure> =
            refineEndpointPath(
                raw,
...
value class IdeUnixSocketPath private constructor(val value: String) {
    companion object {
         ...
         *
         * Establishes a macOS UDS-safe, absolute, normalized POSIX path with no NUL or dot segment.
         * [IdeEndpointPathFailure] is the closed expected failure. Raw text may leave only at the endpoint descriptor
         * or Unix-domain-socket boundary.
         */
...
value class IdeProcessId private constructor(val value: Long) {
    companion object {
         ...
         *
         * Establishes a positive IDE process identity. [IdeProcessIdFailure] is the closed expected failure. The raw
         * number may leave only at process observation or endpoint serialization boundaries.
         */
        fun parse(raw: Long): Refinement<IdeProcessId, IdeProcessIdFailure> =
            if (raw > 0) {
...
value class IdeRuntimeEpoch private constructor(val value: Long) {
    companion object {
         ...
         *
         * Establishes a non-negative, monotonically comparable IDE runtime epoch. [IdeRuntimeEpochFailure] is the
         * closed expected failure. The raw number may leave only at IDE epoch observation or endpoint serialization
         * boundaries.
         */
        fun parse(raw: Long): Refinement<IdeRuntimeEpoch, IdeRuntimeEpochFailure> =
            if (raw >= 0) {
...
sealed interface IdeEndpointDescriptorFailure {
    ...
    data class InvalidCanonicalRoot(val failure: IdeEndpointPathFailure) : IdeEndpointDescriptorFailure

    data class InvalidSocketPath(val failure: IdeEndpointPathFailure) : IdeEndpointDescriptorFailure

    data class InvalidProcessId(val failure: IdeProcessIdFailure) : IdeEndpointDescriptorFailure

    data class InvalidRuntimeEpoch(val failure: IdeRuntimeEpochFailure) : IdeEndpointDescriptorFailure

    data class CompatibilityRejected(val failure: IdeHostCompatibilityFailure) : IdeEndpointDescriptorFailure

    data class HostedCapabilitiesRejected(val failure: HostedCapabilitySetFailure) : IdeEndpointDescriptorFailure
}

sealed interface IdeEndpointDescriptorAdmission {
    data class Admitted(val descriptor: IdeEndpointDescriptorV2) : IdeEndpointDescriptorAdmission

    data class Rejected(val failure: IdeEndpointDescriptorFailure) : IdeEndpointDescriptorAdmission
}

class IdeEndpointDescriptorV2
private constructor(
     ...
     *
     * Preserves every admitted endpoint invariant in one canonical generated-serializer document. Raw JSON is exposed
     * only here, at endpoint publication and transport boundaries.
     */
    fun encode(): EncodedIdeEndpointDescriptor = EncodedIdeEndpointDescriptor.from(this)

    companion object {
        /**
         * Proof transition: `String + IdeHostCompatibilityPolicy -> IdeEndpointDescriptorAdmission`.
         *
         * Establishes the exact canonical v2 schema, root, IDE host, process, compatibility tuple, UDS path, framing,
         * epoch, and generated hosted capability set. Malformed, noncanonical, stale, invalid, and incompatible inputs
         * remain [IdeEndpointDescriptorFailure] data. Raw JSON and primitive fields are extracted only inside this wire
         * boundary.
         */
        fun admit(
            document: String,
         ...
         *
         * Refines all raw endpoint publication fields into one [IdeEndpointDescriptorV2]. Every expected syntax,
         * identity, path, process, epoch, and capability failure remains closed [IdeEndpointDescriptorFailure] data.
         * Raw values may enter only at an endpoint publisher or descriptor decoder boundary.
         */
        fun create(
            ...
            val root =
                when (val refined = IdeEndpointCanonicalRoot.parse(candidate.canonicalRoot)) {
                    is Refinement.Refined -> refined.value
                    is Refinement.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.InvalidCanonicalRoot(refined.failure))
                }
            ...
            val processId =
                when (val refined = IdeProcessId.parse(candidate.processId)) {
                    is Refinement.Refined -> refined.value
                    is Refinement.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.InvalidProcessId(refined.failure))
                }
            val hostedCapabilities =
                when (val admitted = HostedCapabilitySet.admit(candidate.capabilities)) {
                    is HostedCapabilitySetAdmission.Admitted -> admitted.capabilities
                    is HostedCapabilitySetAdmission.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.HostedCapabilitiesRejected(admitted.failure))
                }
            val compatibility =
                when (val admitted = policy.admit(candidate.compatibility())) {
                    is IdeHostCompatibilityAdmission.Admitted -> admitted.compatibility
                    is IdeHostCompatibilityAdmission.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.CompatibilityRejected(admitted.failure))
                }
            val socket =
                when (val refined = IdeUnixSocketPath.parse(candidate.socketPath)) {
                    is Refinement.Refined -> refined.value
                    is Refinement.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.InvalidSocketPath(refined.failure))
                }
            ...
            val epoch =
                when (val refined = IdeRuntimeEpoch.parse(candidate.runtimeEpoch)) {
                    is Refinement.Refined -> refined.value
                    is Refinement.Rejected ->
                        return rejected(IdeEndpointDescriptorFailure.InvalidRuntimeEpoch(refined.failure))
                }
...
private fun IdeEndpointDescriptorCandidate.compatibility() =
...
private fun rejected(failure: IdeEndpointDescriptorFailure) = IdeEndpointDescriptorAdmission.Rejected(failure)

/**
 * Proof transition: `String + Int + ((String) -> Strong) -> Refinement<Strong, IdeEndpointPathFailure>`.
 *
 * Establishes that the raw path is bounded by its UTF-8 byte length, NUL-free, absolute, and lexically normalized
 * before the supplied private constructor creates [Strong]. [IdeEndpointPathFailure] is the closed expected failure.
 * Raw text may leave only through the endpoint descriptor or Unix-domain-socket boundary owned by the caller.
 */
private inline fun <Strong> refineEndpointPath(
    ...
    when {
        ...
        raw != "/" && (raw.endsWith('/') || raw.split('/').drop(1).any { it.isEmpty() || it == "." || it == ".." }) ->
