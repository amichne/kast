...
internal sealed interface IntellijSymbolSelectorLeaseAdmission {
    ...
    data class Rejected(val reason: IntellijSymbolSelectorRejection) : IntellijSymbolSelectorLeaseAdmission
}

internal sealed interface IntellijSymbolSelectorResolution {
    data class Resolved(val selector: SymbolSelector) : IntellijSymbolSelectorResolution

    data class Rejected(val reason: IntellijSymbolSelectorRejection) : IntellijSymbolSelectorResolution
}

internal sealed interface IntellijSymbolDescriptionResolution {
    data class Described(val description: SymbolDescription) : IntellijSymbolDescriptionResolution

    data class Rejected(val reason: IntellijSymbolSelectorRejection) : IntellijSymbolDescriptionResolution
}
...
private sealed interface IntellijSymbolSelectorEnvironmentAdmission {
    ...
    data class Rejected(val reason: IntellijSymbolSelectorRejection) : IntellijSymbolSelectorEnvironmentAdmission
}
...
internal class IntellijSymbolSelectorQuery(
    ...
    /**
     * Proof transition: `(CompiledIntellijSearchScope, SymbolDiscoverySelection) -> IntellijSymbolSelectorResolution`.
     *
     * Establishes same-lease/scope admission, a request-local K2 compiler lookup, and issuance of one exact
     * [SymbolSelector]. [IntellijSymbolSelectorRejection] is the closed expected failure. Platform cancellation
     * propagates; live project, scope, PSI, VFS, and K2 values remain in the lookup call.
     */
    fun resolve(
        ...
        when (val admission = admitEnvironment()) {
            ...
            is IntellijSymbolSelectorEnvironmentAdmission.Rejected -> return rejectedResolution(admission.reason)
        }
        val evidence =
            when (val result = find(compiledScope, selection.lookupKey())) {
                is IntellijCompilerSymbolLookupResult.Found -> result.evidence
                is IntellijCompilerSymbolLookupResult.Rejected -> return rejectedResolution(result.reason)
            }
        when (val admission = admitEnvironment()) {
            IntellijSymbolSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijSymbolSelectorEnvironmentAdmission.Rejected -> return rejectedResolution(admission.reason)
        ...
        return when (val selector = SymbolSelector.issue(selection, evidence)) {
            ...
            is Refinement.Rejected -> rejectedResolution(IntellijSymbolSelectorRejection.COMPILER_EVIDENCE_MISMATCH)
        }
    }

    /**
     * Proof transition: `(CompiledIntellijSearchScope, SymbolSelector) -> IntellijSymbolDescriptionResolution`.
     *
     * Establishes identical current K2 evidence under the selector's retained lease and scope, then projects a detached
     * description. [IntellijSymbolSelectorRejection] is the closed expected failure. Platform cancellation propagates;
     * no live IntelliJ or compiler value crosses this call.
     */
    fun describe(
        compiledScope: CompiledIntellijSearchScope,
        ...
        when (val admission = admitEnvironment()) {
            IntellijSymbolSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijSymbolSelectorEnvironmentAdmission.Rejected -> return rejectedDescription(admission.reason)
        }
        val evidence =
            when (val result = find(compiledScope, selector.lookupKey())) {
                is IntellijCompilerSymbolLookupResult.Found -> result.evidence
                is IntellijCompilerSymbolLookupResult.Rejected -> return rejectedDescription(result.reason)
            }
        when (val admission = admitEnvironment()) {
            IntellijSymbolSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijSymbolSelectorEnvironmentAdmission.Rejected -> return rejectedDescription(admission.reason)
        ...
        return when (val proof = RevalidatedSymbolSelector.validate(selector, evidence)) {
            is Refinement.Refined ->
                IntellijSymbolDescriptionResolution.Described(SymbolDescription.from(proof.value.selector))
            is Refinement.Rejected -> rejectedDescription(IntellijSymbolSelectorRejection.DECLARATION_MOVED_OR_CHANGED)
        }
    }
    ...
    private fun find(
        ...
        try {
            ...
            observation.unexpected(
                IntellijReadUnexpectedFailure.capture(IntellijReadStage.EXACT_REFINEMENT, failure, limits)
            )
            IntellijCompilerSymbolLookupResult.Rejected(IntellijSymbolSelectorRejection.NATIVE_FAILURE)
        }

    /**
     * Proof transition: `live IntelliJ environment -> IntellijSymbolSelectorEnvironmentAdmission`.
     *
     * Establishes a ready project/index state or a closed dumb/disposed rejection before and after compiler work.
     * Platform cancellation propagates from [cancellationCheck].
     */
    private fun admitEnvironment(): IntellijSymbolSelectorEnvironmentAdmission {
        cancellationCheck()
        return when (environmentState()) {
            IntellijDiscoveryEnvironmentState.READY -> IntellijSymbolSelectorEnvironmentAdmission.Ready
            IntellijDiscoveryEnvironmentState.DUMB ->
                IntellijSymbolSelectorEnvironmentAdmission.Rejected(IntellijSymbolSelectorRejection.DUMB_MODE)
            IntellijDiscoveryEnvironmentState.DISPOSED ->
                IntellijSymbolSelectorEnvironmentAdmission.Rejected(IntellijSymbolSelectorRejection.PROJECT_DISPOSED)
        }
    }
}
...
internal class IntellijSymbolSelectorResolver(
     ...
     *
     * Establishes exact current-lease admission, recompiles the retained discovery scope, and performs one restartable
     * K2 selector read. [IntellijSymbolSelectorRejection] is the closed expected failure. The live project, native
     * scope, PSI, VFS, and compiler symbols remain request-local.
     */
    suspend fun resolve(
        project: Project,
            ...
            when (
                val scoped =
                    scopeQuery.execute(
                        ...
                        project.query().resolve(compiled, request.selection)
                    }
            ) {
    ...
    /**
     * Proof transition: `(Project, SemanticReadAuthority, ExactSymbolRequest, WorkspaceSearchScopeModelCompilation) ->
     * IntellijSymbolDescriptionResolution`.
     *
     * Establishes exact current-lease admission, recompiles only the selector's retained scope, and revalidates
     * identical K2 evidence before detached description projection. [IntellijSymbolSelectorRejection] is the closed
     * expected failure. Live IntelliJ and compiler values remain request-local.
     */
            ...
            when (
                val scoped =
                    scopeQuery.execute(
                        ...
                        project.query().describe(compiled, request.selector)
                    }
            ) {
                is IntellijScopedQueryResult.Completed -> scoped.value
                is IntellijScopedQueryResult.Rejected ->
...
/**
 * Proof transition: `(SemanticReadAuthority, SemanticReadAuthority) -> IntellijSymbolSelectorLeaseAdmission`.
 *
 * [IntellijSymbolSelectorLeaseAdmission.Admitted] establishes exact canonical root and generation equality.
 * [IntellijSymbolSelectorLeaseAdmission.Rejected] distinguishes root drift from authority movement. No raw root or
 * authority extraction crosses this adapter boundary.
 */
internal fun admitSymbolSelectorLease(
    ...
    when {
        expected.workspaceRoot != current.workspaceRoot ->
            IntellijSymbolSelectorLeaseAdmission.Rejected(IntellijSymbolSelectorRejection.WORKSPACE_ROOT_MISMATCH)
        expected != current ->
            IntellijSymbolSelectorLeaseAdmission.Rejected(IntellijSymbolSelectorRejection.GENERATION_MOVED)
        else -> IntellijSymbolSelectorLeaseAdmission.Admitted
    }

private fun rejectedResolution(reason: IntellijSymbolSelectorRejection): IntellijSymbolSelectorResolution.Rejected =
    IntellijSymbolSelectorResolution.Rejected(reason)

private fun rejectedDescription(reason: IntellijSymbolSelectorRejection): IntellijSymbolDescriptionResolution.Rejected =
