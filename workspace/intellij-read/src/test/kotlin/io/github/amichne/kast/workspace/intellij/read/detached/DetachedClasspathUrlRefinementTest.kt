...
internal object DetachedClasspathUrlRefinementContract {
    ...
    private fun oversizedBoundaryTextIsRejectedBeforeSemanticScans() {
        ...
        assertEquals(
            ...
            rejectedFailure(refineClasspathUrl("\u0000" + "x".repeat(DetachedModelLimits.MAX_CLASSPATH_URL_CHARS))),
        )
    }

    private fun exactObservedRootIsPrivateProofConsumedByModelConstruction() {
        val rootProof = refinedValue(ExactObservedWorkspaceRoot.refineObservedRoot(FIXTURE_ROOT.value, FIXTURE_ROOT))
        val proofClass = ExactObservedWorkspaceRoot::class.java

        assertEquals(proofClass, rootProof.javaClass)
        assertTrue(
            proofClass.declaredConstructors
        ...
        val capturedMethods =
            DetachedIdeWorkspaceModel.Companion::class.java.declaredMethods.filter { method ->
                method.name.startsWith("captured")
            }
        assertTrue(capturedMethods.any { method -> method.parameterTypes.first() == proofClass })
        assertFalse(
            capturedMethods.any { method ->
                method.parameterTypes.first() in
                    setOf(
                        String::class.java,
    ...
    private fun capture(urls: List<String>): DetachedModelCapture =
        captureDetachedFixture(
            DetachedModelObservation.Observed(
                detachedModelBoundary(
                    modules = listOf(detachedModuleBoundary(classpath = urls.map(::DetachedClasspathBoundary)))
                )
            )
        )
}
