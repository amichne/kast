...
sealed interface SqliteDurableChangeAuthorityOpenResult {
    data class Opened(val authority: SqliteDurableChangeAuthority) : SqliteDurableChangeAuthorityOpenResult

    data class Rejected(val failure: SqliteDurableChangeAuthorityOpenFailure) : SqliteDurableChangeAuthorityOpenResult
}
...
sealed interface SqliteHostedMutationAuthorityOpenResult {
    ...
    data class Rejected(val failure: SqliteDurableChangeAuthorityOpenFailure) : SqliteHostedMutationAuthorityOpenResult
}

sealed interface HostedDurableMutationAudit {
    data object Clean : HostedDurableMutationAudit

    data class RecoveryRequired(val bindings: Set<MutationPlanBinding>) : HostedDurableMutationAudit

    data class Rejected(val failure: DurableChangeAuthorityFailure) : HostedDurableMutationAudit
}

/** Durable public plan/application/receipt authority sharing mutation.sqlite with recovery. */
class SqliteDurableChangeAuthority
private constructor(
    ...
    /**
     * Distinguishes an interrupted applied write from one whose public application identity was durably issued. Only
     * the former withdraws plan/apply/verify authority after restart.
     */
    fun auditMutationState(): HostedDurableMutationAudit =
        storage(rejected = { HostedDurableMutationAudit.Rejected(it) }) {
            ...
            rows.forEach { row ->
                val binding =
                    when (val parsed = MutationPlanBinding.parse(row.binding)) {
                ...
                if (row.stage == "RECOVERY_REQUIRED" || row.applicationIdentity == null) {
                    ...
                    val identity =
                        ChangeApplicationIdentity.parse(row.applicationIdentity)
                            ?: return@storage HostedDurableMutationAudit.Rejected(
                                DurableChangeAuthorityFailure.CORRUPT_RECORD
                            )
    ...
    fun loadPlanForRecovery(binding: MutationPlanBinding): ChangePlanLookup =
        storage(rejected = { ChangePlanLookup.Rejected(it) }) {
            val identity =
                connections.use { connection ->
                    connection.prepareStatement("SELECT identity FROM hosted_change_plan WHERE plan_id = ?").use {
                        statement ->
    ...
    override fun issuePlan(plan: ChangePlan): ChangePlanIssuance {
        val supported =
            plan as? AddDeclarationChangePlan
        ...
        return storage(rejected = { ChangePlanIssuance.Rejected(it) }) {
            connections.use { connection ->
                connection
                    .prepareStatement(
                ...
                when (val stored = connection.planRow(identity)) {
                    null -> ChangePlanIssuance.Rejected(DurableChangeAuthorityFailure.STORAGE_UNAVAILABLE)
                    else ->
                        if (
    ...
    override fun loadPlan(identity: ChangePlanIdentity): ChangePlanLookup =
        storage(rejected = { ChangePlanLookup.Rejected(it) }) {
            connections.use { connection ->
                ...
                val plan =
                    when (val decoded = HostedAddDeclarationPlanCodec.decode(row.document)) {
                        is Refinement.Refined -> decoded.value
                        is Refinement.Rejected ->
                            return@use ChangePlanLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                    }
                if (row.planId != plan.planId.value || planIdentity(plan) != identity) {
    ...
    override fun issueApplication(
        ...
        val supported =
            plan as? AddDeclarationChangePlan
                ?: return ChangeApplicationIssuance.Rejected(DurableChangeAuthorityFailure.UNSUPPORTED_PLAN)
        val plannedWrite =
            supported.writes.entries.singleOrNull()
                ?: return ChangeApplicationIssuance.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
        if (
        ...
        if (!application.recoveryIsApplied()) {
            return ChangeApplicationIssuance.Rejected(DurableChangeAuthorityFailure.RECOVERY_EVIDENCE_UNAVAILABLE)
        }
        ...
        return storage(rejected = { ChangeApplicationIssuance.Rejected(it) }) {
            connections.use { connection ->
                connection
                    .prepareStatement(
                ...
                val row =
                    connection.applicationRow(identity)
    ...
    override fun loadApplication(identity: ChangeApplicationIdentity): ChangeApplicationLookup =
        storage(rejected = { ChangeApplicationLookup.Rejected(it) }) {
            connections.use { connection ->
                val row = connection.applicationRow(identity) ?: return@use ChangeApplicationLookup.Missing
                val planIdentity =
                    ChangePlanIdentity.parse(row.planIdentity)
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                if (
                    applicationDigest(
                        planIdentity,
                        row.planId,
                ...
                val plan =
                    when (val loaded = loadPlan(planIdentity)) {
                ...
                val postimage =
                    WorkspaceSourceContentHash.parse(row.postimage).valueOrNull()
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                val binding =
                    MutationPlanBinding.parse(row.recoveryBinding).valueOrNull()
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                val applicationLease =
                    applicationLease(row)
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                val applicationState =
                    WorkspaceStateIdentity.parse(row.applicationSourceState).valueOrNull()
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                val restored =
                    AppliedUnverified.restore(
                        ...
                        .valueOrNull()
                        ?: return@use ChangeApplicationLookup.Rejected(DurableChangeAuthorityFailure.CORRUPT_RECORD)
                if (applicationIdentity(plan, restored) != identity || !restored.recoveryIsApplied()) {
    ...
    override fun issueReceipt(receipt: VerifiedReceipt): ChangeReceiptIssuance {
        ...
        return storage(rejected = { ChangeReceiptIssuance.Rejected(it) }) {
            connections.use { connection ->
                ...
                val storedDigest =
                    connection
                        .prepareStatement("SELECT record_digest FROM hosted_change_receipt WHERE identity = ?")
                        .use { statement ->
                            statement.setString(1, identity.value)
                            statement.executeQuery().use { rows ->
    ...
    private fun AppliedUnverified.recoveryIsApplied(): Boolean =
        when (val loaded = recovery.load(recoveryBinding)) {
    ...
    companion object {
        ...
        fun openHosted(location: MutationDatabaseLocation): SqliteHostedMutationAuthorityOpenResult {
            val path =
            ...
            val database =
                SqliteMutationRecoveryDatabase.admit(path).valueOrNull()
                    ?: return SqliteHostedMutationAuthorityOpenResult.Rejected(
                        SqliteDurableChangeAuthorityOpenFailure.STORAGE_UNAVAILABLE
                    )
            return try {
 ...
 *
 * Every legacy application necessarily used the plan lease because the former issuance boundary rejected any other
 * lease. The migration therefore strengthens those rows with that already proven relationship and incorporates it into
 * the record digest in one SQLite transaction.
 */
private fun Connection.migrateLegacyApplicationPublicationProof() {
    ...
    rows.forEach { row ->
        ...
        val plan =
            HostedAddDeclarationPlanCodec.decode(row.planDocument).valueOrNull()
                ?: error("Legacy hosted change plan is corrupt")
        val parsedPlanIdentity =
            ChangePlanIdentity.parse(row.planIdentity) ?: error("Legacy hosted change plan identity is corrupt")
        check(
            plan.planId.value == row.planId &&
                planIdentity(plan) == parsedPlanIdentity &&
                row.recoveryBinding == plan.planId.value &&
                row.applicationDigest ==
                    legacyApplicationDigest(
                        parsedPlanIdentity,
                        row.planId,
                        row.postimage,
                        row.recoveryBinding,
...
private fun Connection.planRow(identity: ChangePlanIdentity): PlanRow? =
    prepareStatement("SELECT plan_id, document, document_sha256 FROM hosted_change_plan WHERE identity = ?").use {
        statement ->
        statement.setString(1, identity.value)
        statement.executeQuery().use { rows ->
            if (!rows.next()) null
...
private fun planIdentity(plan: ChangePlan): ChangePlanIdentity =
    checkNotNull(ChangePlanIdentity.parse("plan:${sha256(canonicalFields(plan.planId.value))}"))
...
private fun receiptIdentity(receipt: VerifiedReceipt): ChangeReceiptIdentity =
    checkNotNull(
