...
@JvmInline
value class MutationPlanBinding private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<MutationPlanBinding, MutationPlanBindingFailure>`.
         *
         * Establishes a canonical lowercase SHA-256 plan identity. [MutationPlanBindingFailure] is the closed expected
         * failure. Raw text may enter only from a change-plan identity or SQLite row and may be extracted only at those
         * boundaries.
         */
        fun parse(raw: String): Refinement<MutationPlanBinding, MutationPlanBindingFailure> =
...
@JvmInline
value class RecoverySourcePath private constructor(val value: String) : Comparable<RecoverySourcePath> {
    override fun compareTo(other: RecoverySourcePath): Int = value.compareTo(other.value)

    companion object {
        /**
         * Proof transition: `String -> Refinement<RecoverySourcePath, RecoverySourcePathFailure>`.
         *
         * Establishes a normalized absolute path identity. [RecoverySourcePathFailure] is the closed expected failure.
         * Raw text may enter only from an exact change target or SQLite row and may be extracted only by the recovery
         * source adapter or SQLite boundary.
         */
        fun parse(raw: String): Refinement<RecoverySourcePath, RecoverySourcePathFailure> {
            val path =
                try {
                    Path.of(raw)
...
@JvmInline value class RecoveryContentDigest internal constructor(val value: String)

@JvmInline value class RecoveryEncodedContent internal constructor(val value: String)
...
private constructor(
    ...
    override fun equals(other: Any?): Boolean =
        other is RecoveryPreimage && digest == other.digest && encodedContent == other.encodedContent
    ...
    companion object {
         ...
         *
         * Establishes an immutable Base64 projection and SHA-256 identity for the exact supplied bytes. There is no
         * expected failure because every byte sequence has one canonical representation. Raw bytes may enter at source
         * observation and be extracted only by an admitted recovery adapter.
         */
        fun fromBoundary(bytes: ByteArray): RecoveryPreimage =
            RecoveryPreimage(
        ...
        /**
         * Proof transition: `(String, String) -> Refinement<RecoveryPreimage, RecoveryPreimageFailure>`.
         *
         * Establishes canonical Base64 bytes whose calculated SHA-256 equals the supplied digest.
         * [RecoveryPreimageFailure] is the closed expected failure. Raw columns may enter only at the SQLite decoder
         * and raw bytes may leave only at the recovery adapter.
         */
        fun restore(
            rawDigest: String,
...
class MutationRecoveryPreparation
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes a non-empty, unique, deterministically ordered write set whose exact preimages are bound to one
         * plan. [MutationRecoveryPreparationFailure] is the closed expected failure. Raw content extraction remains
         * confined to recovery and SQLite.
         */
        fun admit(
            binding: MutationPlanBinding,
            plannedWrites: List<PlannedRecoveryWrite>,
        ): Refinement<MutationRecoveryPreparation, MutationRecoveryPreparationFailure> =
            when {
                plannedWrites.isEmpty() -> Refinement.Rejected(MutationRecoveryPreparationFailure.EMPTY_WRITE_SET)
...
class AppliedRecoveryWriteSet private constructor(sources: List<RecoverySourcePath>) {
    ...
    companion object {
         ...
         *
         * Establishes a non-empty, unique, deterministically ordered subset of the exact planned write identities.
         * [AppliedRecoveryWriteSetFailure] is the closed expected failure. Raw path extraction is permitted only at the
         * source or SQLite boundary.
         */
        fun admit(
            planned: List<PlannedRecoveryWrite>,
            applied: List<RecoverySourcePath>,
        ): Refinement<AppliedRecoveryWriteSet, AppliedRecoveryWriteSetFailure> =
            when {
