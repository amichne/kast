...
internal suspend fun KastIndexerBackend.applyEditsOperation(query: ParsedApplyEditsQuery): ApplyEditsResult {
        return mutationAttemptGate.write(query.mutationAttemptId) {
            telemetry.inSpan(IdeaTelemetryScope.APPLY_EDITS, "kast.idea.applyEdits") {
                val applier = IdeaEditApplier(project, workspaceRoot, workspaceIdentity)
                applier.apply(query.toWire(), query.mutationScratchSets)
                // No asyncRefresh needed - IDEA APIs handle VFS updates automatically
            }
        }
    }

internal suspend fun KastIndexerBackend.optimizeImportsOperation(query: ParsedImportOptimizeQuery): ImportOptimizeResult = withContext(readDispatcher) {
...
internal suspend fun KastIndexerBackend.refreshOperation(query: ParsedRefreshQuery): RefreshResult {
...
internal suspend fun KastIndexerBackend.probeSemanticAdmission(filePath: NormalizedPath): SemanticAdmissionStatus {
...
internal fun KastIndexerBackend.pendingSemanticAdmission(
...
internal fun KastIndexerBackend.failedSemanticAdmission(
        filePath: NormalizedPath,
        message: String,
    ): SemanticAdmissionStatus = SemanticAdmissionStatus.incomplete(
        filePath = filePath,
