    ...
    fun `semantic requests carry their original demand through admission before exchange`(@TempDir path: Path) {
        ...
        val cli = KastCli(
            ...
            lifecycle = object : RuntimeLifecycleController {
                override fun status(endpoint: RuntimeEndpoint): RuntimeStatusResult = error("admission owns readiness")
                override fun stop(endpoint: RuntimeEndpoint): RuntimeStopResult = error("semantic requests cannot stop")
            },
