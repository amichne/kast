...
internal suspend fun KastIndexerBackend.callHierarchyOperation(query: ParsedCallHierarchyQuery): CallHierarchyResult = withContext(readDispatcher) {
...
internal suspend fun KastIndexerBackend.callRelationsOperation(query: KastCallersQuery): CallRelationsResult =
        withContext(readDispatcher) {
            ...
            val result = callHierarchy(
                io.github.amichne.kast.api.contract.query.CallHierarchyQuery(
                    ...
                    direction = direction,
                    depth = query.depth,
                    maxTotalCalls = KastIndexerBackend.RELATIONSHIP_STATE_CAPACITY,
                    maxChildrenPerNode = KastIndexerBackend.RELATIONSHIP_STATE_CAPACITY,
            ...
            if (records.size > KastIndexerBackend.RELATIONSHIP_STATE_CAPACITY) {
...
internal suspend fun KastIndexerBackend.typeHierarchyOperation(query: ParsedTypeHierarchyQuery): TypeHierarchyResult = withContext(readDispatcher) {
...
internal suspend fun KastIndexerBackend.hierarchyRelationsOperation(query: KastHierarchyQuery): HierarchyRelationsResult =
        withContext(readDispatcher) {
            val searchScope = GlobalSearchScope.projectScope(project)
            ...
            val records = directions.flatMap { direction ->
                val result = typeHierarchy(
                    io.github.amichne.kast.api.contract.query.TypeHierarchyQuery(
                        ...
                        direction = direction,
                        depth = query.depth,
                        maxResults = KastIndexerBackend.RELATIONSHIP_STATE_CAPACITY,
                    ).parsed(),
                )
