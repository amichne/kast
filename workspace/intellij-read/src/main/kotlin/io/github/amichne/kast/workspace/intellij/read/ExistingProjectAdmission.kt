...
sealed interface ExistingProjectValidation {
    ...
    data class Rejected(val failure: ExistingProjectAdmissionFailure) : ExistingProjectValidation

    companion object {
        /**
         * Proof transition: `(Project, root, host candidate, policy) -> ExistingProjectValidation`.
         *
         * Observes the complete existing-Project admission policy but cannot create a [ProjectReadEpoch.Source].
         * Validation-only hosted factories therefore receive no project-lifetime authority to discard.
         */
        fun validate(
            project: Project,
            expectedRoot: CanonicalWorkspaceRoot,
...
private constructor(
     ...
     */
    fun captureDetachedModel(limits: ReadLimits = ReadLimits.Default): DetachedModelCapture =
     ...
     *
     * Establishes the complete exact-root cached Gradle model through a suspending write-priority read, so the hosted
     * endpoint path never blocks a thread while waiting for read access. Expected model failures remain
     * [DetachedModelCaptureFailure]; raw Project extraction stays inside [LiveDetachedModelCapture].
     */
    suspend fun captureDetachedModelAsync(limits: ReadLimits = ReadLimits.Default): DetachedModelCapture =
        DetachedIdeWorkspaceModel.admit(
            canonicalRoot,
            compatibility,
    ...
    internal suspend fun captureNamedGradleSourceScope(
    ...
    /**
     * `AdmittedIdeProject -> ProjectReadEpochObservation`; returns one opaque retained-source epoch or
     * [ProjectReadEpochObservationFailure], without exposing Project or raw signal values.
     */
    fun observeReadEpoch(): ProjectReadEpochObservation = readEpochSource.observe()

    /**
     * `(AdmittedIdeProject, ProjectReadEpoch<*>) -> VfsPassiveReadAdmission`; one observation proves unchanged
     * same-source state or [VfsPassiveReadAdmissionFailure]. Raw Project stays confined.
     */
    fun admitVfsPassiveRead(expectedEpoch: ProjectReadEpoch<*>): VfsPassiveReadAdmission =
        admitVfsPassiveReadObservation(
            canonicalRoot,
            expectedEpoch,
     ...
     * for unchanged same-source freshness, with Project remaining private.
     */
    internal fun cancellableReadExecution(freshness: VfsPassiveReadCapability): AdmittedProjectReadExecutionAdmission {
        ...
        return when (val admitted = admitVfsPassiveRead(freshness.admittedEpoch)) {
            is VfsPassiveReadAdmission.Admitted ->
                AdmittedProjectReadExecutionAdmission.Admitted(
            ...
            is VfsPassiveReadAdmission.Rejected ->
                AdmittedProjectReadExecutionAdmission.Rejected(
    ...
    companion object {
        /**
         * `(Project, root, host candidate, policy) -> ExistingProjectAdmission`; proves open, initialized, exact-root,
         * complete-model, smart, K2, compatible state or closed failure.
         */
        internal fun admit(
            project: Project,
            expectedRoot: CanonicalWorkspaceRoot,
        ...
        /**
         * `(Project, root, host policy, observation port, epoch factory) -> admission`; proves the [admit] invariants
         * or closed failure while raw Project stays at the observation boundary.
         */
        internal fun admitObserved(
            ...
            when (
                ...
                is ExistingProjectValidationEvidence.Rejected -> ExistingProjectAdmission.Rejected(evidence.failure)
                is ExistingProjectValidationEvidence.Validated ->
                    when (val installed = readEpochSourceFactory.create(project, evidence.exactRoot)) {
                        is Refinement.Refined ->
                            ExistingProjectAdmission.Admitted(
                                AdmittedIdeProject(
...
class AdmittedIdeProjectSession private constructor(private val admissions: ExistingProjectAdmissionOperations) {
    ...
    internal fun admitUsing(
        ...
        if (current != null) {
            return if (
                ...
                ExistingProjectAdmission.Rejected(ExistingProjectAdmissionFailure.RetainedAuthorityMismatch)
            }
        }
        return when (
            val admission =
...
private sealed interface ExistingProjectValidationEvidence {
    ...
    data class Rejected(val failure: ExistingProjectAdmissionFailure) : ExistingProjectValidationEvidence
}
...
private fun validateExistingProject(
    ...
    val disposed =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.DISPOSAL) {
    ...
    val open =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.OPEN) {
    ...
    val initialized =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.INITIALIZATION) {
    ...
    val observedRoot =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.ROOT) {
    ...
    val exactRoot =
        when (observedRoot) {
            ...
            ExistingProjectRootObservation.Unavailable ->
                return validationRejected(ExistingProjectAdmissionFailure.ProjectRootUnavailable)
        }
    ...
    val gradleModel =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.GRADLE_MODEL) {
    ...
    when (gradleModel) {
        ExistingProjectGradleModelState.UNAVAILABLE ->
            return validationRejected(ExistingProjectAdmissionFailure.GradleModelUnavailable)
        ExistingProjectGradleModelState.INCOMPLETE ->
            return validationRejected(ExistingProjectAdmissionFailure.GradleModelIncomplete)
        ExistingProjectGradleModelState.COMPLETE -> Unit
    }
    val indexing =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.INDEXING) {
    ...
    val kotlinMode =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.KOTLIN_MODE) {
    ...
    val hostIdentity =
        when (
            val attempt =
                observe(ExistingProjectObservationStage.HOST_IDENTITY) {
    ...
    val observedCandidate =
        when (hostIdentity) {
            ...
            is ExistingProjectHostIdentityObservation.Rejected ->
                return validationRejected(ExistingProjectAdmissionFailure.HostIncompatible(hostIdentity.failure))
            ExistingProjectHostIdentityObservation.Unavailable ->
                return validationRejected(ExistingProjectAdmissionFailure.HostIdentityUnavailable)
        }
    return when (val admitted = compatibilityPolicy.admit(observedCandidate)) {
        is IdeHostCompatibilityAdmission.Admitted ->
            ExistingProjectValidationEvidence.Validated(
                exactRoot,
                admitted.compatibility,
            )
        is IdeHostCompatibilityAdmission.Rejected ->
            validationRejected(ExistingProjectAdmissionFailure.HostIncompatible(admitted.failure))
    }
}
...
private inline fun <Value> observe(
    ...
    read: () -> Value,
): ExistingProjectObservation<Value> =
    try {
...
private fun ExistingProjectObservation.Failed.rejection() =
    validationRejected(ExistingProjectAdmissionFailure.ObservationFailed(stage))

private fun validationRejected(failure: ExistingProjectAdmissionFailure) =
