...
internal object FrontendWorkerSeedConsent : WorkerSeedConsentAuthority {
    override suspend fun request(disclosure: WorkerSeedDisclosure): WorkerSeedConsent =
        runInterruptible(Dispatchers.IO) {
            val size =
                IndexSeedEstimatedBytes.from(disclosure.estimatedBytes)
                    ?: return@runInterruptible WorkerSeedConsent.ABSENT
            val local =
...
internal fun WorkerSeedConsentAuthority.seedProvider(): IndexSeedConsentProvider =
        ...
        val selected =
            WorkerSeedDisclosure.admit(
                disclosure.categories
                    ...
                    .toSet(),
                disclosure.estimatedBytes.value,
            )
        when (selected) {
            is Refinement.Rejected -> IndexSeedConsent.ABSENT
            is Refinement.Refined ->
                when (runBlocking { request(selected.value) }) {
