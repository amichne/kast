...
<Columns cols={3}>
  <Card title="Sidecar routes" icon="circle-check">
    Index synchronization, topology, symbol discovery and inspection, source
    and relation reads, traversal, diagnostics, planning, verified application,
    and recovery are the eleven public installed capabilities.
  </Card>
  ...
  <Card title="Kast-owned lifecycle" icon="power">
    `start` owns admission and typed cache policy, `status` is passive, and
    `stop` targets only the proven sidecar process and its endpoint artifacts.
  </Card>
</Columns>
