...
class IntellijSourceScopeAdmissionTest {
    ...
    private val model = (WorkspaceSearchScopeModel.compile(root, ImportedWorkspaceModelState.COMPLETE, listOf(
        WorkspaceSourceRootBoundary("app.main", Path.of("/workspace"), ":app", "main", Path.of("/workspace/src"),
            WorkspaceSourceRootKind.PRODUCTION, WorkspaceSourceRootProvenance.AUTHORED),
        WorkspaceSourceRootBoundary("app.test", Path.of("/workspace"), ":app", "test", Path.of("/workspace/src/test"),
            WorkspaceSourceRootKind.TEST, WorkspaceSourceRootProvenance.AUTHORED),
        WorkspaceSourceRootBoundary("app.main", Path.of("/workspace"), ":app", "generatedMain", Path.of("/workspace/src/generated"),
            WorkspaceSourceRootKind.PRODUCTION, WorkspaceSourceRootProvenance.GENERATED),
    )) as WorkspaceSearchScopeModelCompilation.Compiled).model

    @Test
    fun `fresh child candidates retain source-set scope and carry their native declaration kind`() {
        val retained = SymbolDiscoveryConstraints.None.copy(
            directory = SymbolDiscoveryDirectoryConstraint(SymbolDiscoveryDirectory.parse("src").refined(),
                SymbolDiscoveryContainment.DESCENDANTS),
            packageName = SymbolDiscoveryPackageConstraint(SymbolDiscoveryPackage.parse("sample").refined(),
    ...
    fun `retained exact-file scope rejects an otherwise valid different source file`() {
        ...
        val scope = SourceReadScope.Constrained(SymbolSearchScope.ExactFile(
    ...
    fun `most specific test and generated ownership cannot inherit an allowed authored parent`() {
        val scope = SourceReadScope.Constrained(SymbolSearchScope.Workspace(
    ...
    fun `retained module and source-set owners must own the actual source file`() {
        ...
        val scopes = listOf(
            SymbolSearchScope.Module(owner.module, SymbolSourceKindPolicy.PRODUCTION_AND_TEST, SymbolGeneratedSourcePolicy.INCLUDE),
            SymbolSearchScope.SourceSet(owner.project, owner.sourceSet, SymbolSourceKindPolicy.PRODUCTION_AND_TEST, SymbolGeneratedSourcePolicy.INCLUDE),
        )
        for (scope in scopes) {
