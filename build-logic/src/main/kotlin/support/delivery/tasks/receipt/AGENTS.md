...
- `gate/registration/ReceiptProgressionRegistration.kt` is the Gradle registration authority for
  KVP-001 through KVP-009; the convention script retains only projection, authority, and
...
- `gate/` owns KVP-006's positive and negative structural reports over all 129 receipt tasks and its
  ...
  and KVP-009's IDE-read firewall receipt closure. Structural validation never executes the
  registered gate tasks.
