...
internal class IntelliJProjectIndexer(
    ...
    fun indexProject(config: KastConfig) {
        ...
        if (config.indexing.phase2Enabled.value && !environment.isCancelled()) {
            val moduleSpecs = runIntellijReadAction { discoverModuleSpecs() }
            val modulePriorityOrder = computeModulePriorityOrder(
                activeModule = null,
                moduleSpecs = moduleSpecs,
                dependentModuleGraph = buildIntelliJDependencyGraph(moduleSpecs),
    ...
    private fun discoverModuleSpecs(): List<IntellijModuleSpec> =
    ...
    private fun buildIntelliJDependencyGraph(moduleSpecs: List<IntellijModuleSpec>): Map<String, Set<String>> =
        moduleSpecs
            .associate { module ->
    ...
    private fun moduleNameForFile(psiFile: PsiFile): String? = runIntellijReadAction {
        val virtualFile = psiFile.virtualFile
        val module = ModuleUtilCore.findModuleForFile(virtualFile, project) ?: return@runIntellijReadAction null
    ...
    private fun sourceSetForFile(path: String): String? {
        ...
        return when {
            ...
            else -> runIntellijReadAction {
                val virtualFile = LocalFileSystem.getInstance().findFileByNioFile(Path.of(path)) ?: return@runIntellijReadAction null
...
internal data class IntellijModuleSpec(
    val name: String,
...
internal fun computeModulePriorityOrder(
    activeModule: String?,
    moduleSpecs: List<IntellijModuleSpec>,
...
private fun topologicallySortModules(moduleSpecs: List<IntellijModuleSpec>): List<String> {
    val modulesByName = moduleSpecs.associateBy(IntellijModuleSpec::name)
