...
@JvmInline value class WorkspaceRelativeSourceRoot internal constructor(val value: String)

@ConsistentCopyVisibility
data class GradleSourceSetOwner
...
internal constructor(
    ...
    companion object {
        /**
         * Proof transition: `GradleSourceRootEvidence -> Refinement<SourceRoot, Set<SourceRootAdmissionFailure>>`.
         *
         * Establishes normalized workspace-contained root identity, exact Gradle project and source-set ownership, and
         * provenance already supplied by the Gradle model. The closed expected failure is [SourceRootAdmissionFailure].
         * Raw model strings may enter only from the IntelliJ Gradle bridge; raw location extraction is permitted only
         * at a physical source-access boundary.
         */
        fun admit(evidence: GradleSourceRootEvidence): Refinement<SourceRoot, Set<SourceRootAdmissionFailure>> {
         ...
         * [SourceRootAdmissionFailure] is the closed expected failure; the raw string may be consumed only inside
         * [admit].
         */
        private fun admitRelativePath(
            ...
            return if (
                raw == "." ||
                    raw.isNotEmpty() && !path.isAbsolute && segments.none { it.isEmpty() || it == "." || it == ".." }
            ) {
