...
internal class InstalledRuntimeWorkerEffects(
    private val factory:
        (InstalledWorkerStartRequest, (WorkerProcessCapture) -> Unit, IndexSeedConsentProvider) -> RootRuntimeDemander,
    ...
    private class LiveRoute(
        val endpoint: RuntimeEndpoint,
        val query: RuntimeBootstrapProcessQuery,
        val authority: RuntimeBootstrapProcessAuthority,
        val lifecycle: RuntimeLifecycleController,
    )
    ...
    override suspend fun start(request: InstalledWorkerStartRequest): InstalledWorkerStart =
        runInterruptible(Dispatchers.IO) {
            val root =
                when (val discovered = FilesystemCanonicalRootDiscovery.discover(request.root)) {
            ...
            val startup =
                RuntimeStartupRequest.Requested(
                    ...
                    when (val selected = request.startup) {
                        ...
                        is InstalledWorkerStartup.Seed ->
                            StartupCacheIntent.Seed(
                                selected.sourceSystem?.let(StartupIdeaSystem::Explicit) ?: StartupIdeaSystem.Standard,
                                when (selected.consent) {
            ...
            val ready =
                when (val result = demander.demand(root, HostedRuntimeDemand.Lifecycle, startup)) {
                    is RuntimeAdmission.Ready -> result
                    is RuntimeAdmission.Rejected ->
                        return@runInterruptible rejected(WorkerControlFailure.STARTUP_REJECTED)
                }
            val route =
                (live[request.root] as? LaunchOwnership.Entered)?.route
                    ?: return@runInterruptible rejected(WorkerControlFailure.IDENTITY_REJECTED)
            if (ready.endpoint.root != root || ready.endpoint != route.endpoint)
                return@runInterruptible rejected(WorkerControlFailure.IDENTITY_REJECTED)
            val query = route.query
            val bootstrap =
                when (val state = SidecarBootstrapStateFile.observe(query.bootstrapState)) {
                ...
                when (
                    val admitted =
                        InstalledWorkerEndpoint.admit(
                            root.path,
                            ready.endpoint.runtimeId,
                            ready.endpoint.socketPath,
                            bootstrap.attemptId,
                        )
                ) {
    ...
    override suspend fun observe(route: InstalledWorkerEndpoint): InstalledWorkerObservation =
        runInterruptible(Dispatchers.IO) {
            val known = (live[route.root] as? LaunchOwnership.Entered)?.route
            if (known != null && exact(known, route)) InstalledWorkerObservation.EXACT_READY
            else InstalledWorkerObservation.UNPROVEN
        }

    override suspend fun stop(route: InstalledWorkerEndpoint): InstalledWorkerRetirement =
        runInterruptible(Dispatchers.IO) {
            val ownership =
                live[route.root] as? LaunchOwnership.Entered
                    ?: return@runInterruptible InstalledWorkerRetirement.UNPROVEN
            val known = ownership.route
            if (known.endpoint.socketPath != route.socket || known.endpoint.runtimeId != route.runtimeId)
                return@runInterruptible InstalledWorkerRetirement.UNPROVEN
            when (known.lifecycle.stop(known.endpoint)) {
                is RuntimeStopResult.Stopped -> {
                    live.remove(route.root, ownership)
    ...
    override suspend fun retireUnpublished(root: Path): InstalledWorkerRetirement =
        runInterruptible(Dispatchers.IO) {
            ...
            if (ownership == LaunchOwnership.NotEntered) {
                return@runInterruptible if (live.remove(root, ownership)) InstalledWorkerRetirement.EXACT_RETIRED
                else InstalledWorkerRetirement.UNPROVEN
            }
            val known = (ownership as LaunchOwnership.Entered).route
            when (known.lifecycle.stop(known.endpoint)) {
                is RuntimeStopResult.Stopped -> {
                    live.remove(root, ownership)
                    InstalledWorkerRetirement.EXACT_RETIRED
                }
                is RuntimeStopResult.Rejected -> InstalledWorkerRetirement.UNPROVEN
            }
    ...
    private fun exact(known: LiveRoute, route: InstalledWorkerEndpoint): Boolean {
        if (
            known.endpoint.root.path != route.root ||
                known.endpoint.socketPath != route.socket ||
                known.endpoint.runtimeId != route.runtimeId
        )
            return false
        val process = known.authority.observe(known.query)
        if (
            process !is RuntimeBootstrapProcessObservation.Owned ||
                process.attemptId != route.attempt ||
                process.session.observe() != RuntimeSessionObservation.Present
        )
            return false
        val state = SidecarBootstrapStateFile.observe(known.query.bootstrapState)
        if (
            state !is SidecarBootstrapStateObservation.Observed ||
                state.state !is SemanticRuntimeBootstrapState.Ready ||
                state.state.attemptId != route.attempt
        )
            return false
