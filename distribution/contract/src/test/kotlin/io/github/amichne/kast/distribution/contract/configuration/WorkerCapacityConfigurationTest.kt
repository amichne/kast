...
class WorkerCapacityConfigurationTest {
    @Test
    fun `declared worker policy defaults to one resident and accepts explicit concurrency`() {
        val defaults =
            (ResolvedKastConfiguration.resolve(ConfigurationSources()) as Refinement.Refined)
                .value
                .inspection()
                .associateBy { it.key }
    ...
    fun `broker reloads saved assignments without upgrading their provenance`() {
        ...
        assertEquals(
            ...
            configuration.inspection().single { it.key == "KAST_INDEXER_MAX_HEAP" }.source,
        )
    }
}
