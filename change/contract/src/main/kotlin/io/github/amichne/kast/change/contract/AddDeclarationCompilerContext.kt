...
data class AddDeclarationCompilerContextFile
private constructor(
    val path: String,
    ...
    companion object {
        /**
         * Proof transition: raw path and hash to Refinement of AddDeclarationCompilerContextFile or
         * AddDeclarationCompilerContextFileFailure.
         *
         * Establishes one canonical absolute Kotlin or Java source path with an exact lowercase SHA-256 identity. The
         * failure type is closed. Raw values may be extracted only at a compiler-context filesystem boundary.
         */
        fun admit(
            path: String,
            sha256: String,
            ...
            if (
                canonical == null ||
                    !canonical.isAbsolute ||
                    canonical.normalize().toString() != path ||
                    (!path.endsWith(".kt") && !path.endsWith(".java"))
            ) {
                return Refinement.Rejected(AddDeclarationCompilerContextFileFailure.PATH_NOT_CANONICAL_SOURCE)
            }
...
value class AddDeclarationProjectModelFingerprint private constructor(val value: String) {
    companion object {
         ...
         *
         * Establishes one exact lowercase SHA-256 project-model identity. Raw extraction is permitted only at the
         * project-model observation boundary.
         */
        fun parse(
            raw: String
...
value class AddDeclarationClasspathFingerprint private constructor(val value: String) {
    companion object {
         ...
         * AddDeclarationCompilerFingerprintFailure.
         *
         * Establishes one exact lowercase SHA-256 compiler-classpath identity. Raw extraction is permitted only at the
         * compiler-classpath observation boundary.
         */
        fun parse(
            raw: String
...
value class AddDeclarationOutboundReferenceCount private constructor(val value: Int) {
    companion object {
         ...
         *
         * Establishes the finite exact cardinality of compiler-observed outbound references. Raw extraction is
         * permitted only inside the compiler-backed verification adapter.
         */
        fun parse(
            raw: Int
...
sealed interface AddDeclarationCompilerContextAdmissionFailure {
    data class ProjectModel(val failure: AddDeclarationCompilerFingerprintFailure) :
        AddDeclarationCompilerContextAdmissionFailure

    data class Classpath(val failure: AddDeclarationCompilerFingerprintFailure) :
        AddDeclarationCompilerContextAdmissionFailure

    data class ContextFile(val failure: AddDeclarationCompilerContextFileFailure) :
        AddDeclarationCompilerContextAdmissionFailure

    data class OutboundReferenceCount(val failure: AddDeclarationOutboundReferenceCountFailure) :
        AddDeclarationCompilerContextAdmissionFailure

    data class Context(val failure: ExpectedAddDeclarationCompilerContextFailure) :
        AddDeclarationCompilerContextAdmissionFailure
}

@Serializable
@ConsistentCopyVisibility
data class ExpectedAddDeclarationCompilerContext
private constructor(
    ...
    companion object {
        /**
         * Proof transition: typed compiler observations to Refinement of ExpectedAddDeclarationCompilerContext or
         * ExpectedAddDeclarationCompilerContextFailure.
         *
         * Establishes one G-bound deterministic compiler context with exact source identities and outbound-reference
         * cardinality. It does not establish semantic collision, binding, or diagnostic facts; those require a scoped
         * compiler read. The failure type is closed. Raw extraction is permitted only by the IntelliJ planning and
        ...
        /**
         * Proof transition: one raw source observation to Refinement of ExpectedAddDeclarationCompilerContext or
         * AddDeclarationCompilerContextAdmissionFailure.
         *
         * Establishes the same deterministic context as [admit] for a workspace whose observed context contains exactly
         * one source file. It does not establish completeness outside that supplied context or any semantic proof. The
         * failure type is closed and retains the exact failed refinement. Raw values may be extracted only by a
         * compiler-context observation boundary.
         */
        fun admitSingleSource(
            ...
            val model =
                when (val result = AddDeclarationProjectModelFingerprint.parse(projectModelFingerprint)) {
            ...
            val classpath =
                when (val result = AddDeclarationClasspathFingerprint.parse(classpathFingerprint)) {
                    is Refinement.Refined -> result.value
                    is Refinement.Rejected ->
                        return Refinement.Rejected(
            ...
            val source =
                when (val result = AddDeclarationCompilerContextFile.admit(sourcePath, sourceSha256)) {
                    is Refinement.Refined -> result.value
                    is Refinement.Rejected ->
                        return Refinement.Rejected(
            ...
            val outboundCount =
                when (val result = AddDeclarationOutboundReferenceCount.parse(outboundReferenceCount)) {
                    is Refinement.Refined -> result.value
                    is Refinement.Rejected ->
                        return Refinement.Rejected(
            ...
            return when (val result = admit(generation, model, classpath, listOf(source), outboundCount)) {
                is Refinement.Refined -> result
                is Refinement.Rejected ->
                    Refinement.Rejected(AddDeclarationCompilerContextAdmissionFailure.Context(result.failure))
            }
        }
    }
}
