...
object AddDeclarationPlanCodec {
    ...
    fun encode(plan: PlannedAddDeclaration): String = json.encodeToString(PlannedAddDeclaration.serializer(), plan)

    /**
     * Proof transition: String to Refinement of PlannedAddDeclaration or AddDeclarationPlanDecodeFailure.
     *
     * Establishes canonical serialized bytes, every constructor invariant, compiler-evidence digest, and the SHA-256
     * plan identity recomputed from all G0 identity material. AddDeclarationPlanDecodeFailure is the closed expected
     * failure. Raw JSON may be extracted only at a durable journal or process/request transport boundary.
     */
    fun decode(encoded: String): Refinement<PlannedAddDeclaration, AddDeclarationPlanDecodeFailure> {
        val decoded =
            runCatching {
                json.decodeFromString(PlannedAddDeclaration.serializer(), encoded)
            }
                .getOrNull() ?: return Refinement.Rejected(AddDeclarationPlanDecodeFailure.MALFORMED_OR_TAMPERED)
...
private fun PlannedAddDeclaration.revalidatedOrNull(): PlannedAddDeclaration? {
    ...
    val refinedOwner =
        AddDeclarationSourceOwner.admit(
            ...
            .valueOrNull() ?: return null
    val refinedTarget = AddDeclarationTargetCapability.admit(refinedIntent, refinedOwner).valueOrNull() ?: return null
    ...
    val postimage =
        ExactFileContentProof.admit(
            ...
            .valueOrNull() ?: return null
    val refinedFile = ExpectedFileProof.admit(refinedTarget, preimage, postimage).valueOrNull() ?: return null
    ...
    val contextFiles =
        compilerContext.contextFiles.map { file ->
            AddDeclarationCompilerContextFile.admit(file.path, file.sha256.value).valueOrNull() ?: return null
        }
    val modelFingerprint =
        AddDeclarationProjectModelFingerprint.parse(compilerContext.projectModelFingerprint.value).valueOrNull()
            ?: return null
    val classpathFingerprint =
        AddDeclarationClasspathFingerprint.parse(compilerContext.classpathFingerprint.value).valueOrNull()
            ?: return null
    val outboundCount =
        AddDeclarationOutboundReferenceCount.parse(compilerContext.outboundReferenceCount.value).valueOrNull()
            ?: return null
    val expectedCompilerContext =
        ExpectedAddDeclarationCompilerContext.admit(
    ...
    val detachedCompilerEvidence =
        DetachedCompilerEvidence.admit(compilerEvidence.canonicalJson).valueOrNull() ?: return null
