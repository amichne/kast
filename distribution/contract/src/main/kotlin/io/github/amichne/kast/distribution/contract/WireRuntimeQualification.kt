...
enum class WirePeerQualification {
    QUALIFIED,
    REJECTED,
}
...
object WireRuntimeQualification {
    ...
    private fun admit(raw: String, expected: JsonObject): WirePeerQualification =
        try {
            if (
                raw.length <= maximumBytes &&
                    raw.toByteArray(Charsets.UTF_8).size <= maximumBytes &&
                    Json.parseToJsonElement(raw) == expected
            )
