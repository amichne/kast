...
class RunningIndexer internal constructor(
    ...
    fun closeAsync(): CompletableFuture<Unit> {
        ...
        completeOnBackgroundThread(
            ...
            threadName = "kast-indexer-closer",
        ) {
        ...
        completion.whenComplete { _, failure ->
            if (failure != null) {
                LOG.warn("Error closing the Kast indexer", unwrapCloseFailure(failure))
            }
        }
        return completion
    }
    ...
    private companion object {
        val LOG: Logger = Logger.getInstance(RunningIndexer::class.java)
    }
}
