    ...
    override fun configureSettings(settings: GradleExecutionSettings, context: GradleExecutionContext) {
        val raw =
            NetworkProperty.entries
                ...
                .toMap()
        val fallback =
            when (val parsed = NetworkConfiguration.parse(raw)) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected ->
                    throw com.intellij.openapi.progress.ProcessCanceledException(
                        IllegalStateException("network-policy-invariant-lost")
                    )
            }
        val root =
            try {
                java.nio.file.Path.of(context.projectPath).toRealPath()
        ...
        val owned =
            when (
                val read =
                    GradleNetworkConfiguration.read(root, userHome, settings.gradleHome?.let(java.nio.file.Path::of))
            ) {
                is Refinement.Refined -> read.value
                is Refinement.Rejected ->
                    throw com.intellij.openapi.progress.ProcessCanceledException(
                        IllegalStateException("gradle-network-policy-unavailable")
                    )
            }
        val explicit =
            when (
                val parsed =
                    NetworkConfiguration.parse(
                        owned.atJvmBoundary() + GradleNetworkConfiguration.fromArguments(settings.jvmArguments)
                    )
            ) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected ->
                    throw com.intellij.openapi.progress.ProcessCanceledException(
                        IllegalStateException("explicit-network-policy-rejected")
                    )
            }
