...
class HostedListenerTest {
    @Test
    fun `blocked native accept cancellation drains with cancelled observation`() =
    ...
    @Test
    fun `closed native listener rejects with finite accept observations`() =
        runTest(timeout = 5.seconds) {
            ...
            val result =
                acceptHostedConnection(observer(observations), dispatcher = Dispatchers.IO, accept = listener::accept)
    ...
    @Test
    fun `native accepted connection transfers open ownership with completed observation`() =
        runTest(timeout = 5.seconds) {
    ...
    @Test
    fun `cancellation after native accept closes the connection before ownership transfer`() =
        runTest(timeout = 5.seconds) {
            withContext(Dispatchers.IO) {
                ServerSocketChannel.open().use { listener ->
                    listener.bind(InetSocketAddress("127.0.0.1", 0))
                    SocketChannel.open(listener.localAddress).use {
                        ...
                        try {
                            ...
                            assertInstanceOf(
                                ...
                                runCatching { accepting.await() }.exceptionOrNull(),
                            )
                            assertTrue(accepting.isCompleted)
    ...
    private fun observer(observations: MutableList<Pair<HostedEndpointStage, HostedEndpointOutcome>>) =
        HostedEndpointObserver { stage, outcome ->
            observations += stage to outcome
        }

    private fun events(terminal: HostedEndpointOutcome) =
        listOf(
