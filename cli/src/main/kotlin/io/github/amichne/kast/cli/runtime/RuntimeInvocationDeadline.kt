...
sealed interface RuntimeInvocationDeadline {
    ...
    class Running internal constructor(private val limit: ElapsedTimeLimitMillis, private val started: Long, private val clock: () -> Long) : RuntimeInvocationDeadline {
...
internal class RuntimeInvocationBudgetPolicy(
    private val limit: (HostedRuntimeDemand) -> ElapsedTimeLimitMillis = { demand -> when (demand) {
    ...
    fun begin(demand: HostedRuntimeDemand): RuntimeInvocationDeadline.Running = RuntimeInvocationDeadline.Running(limit(demand), clock(), clock)
}
...
sealed interface RuntimeWireAuthority {
    ...
    data class Qualified(val identity: io.github.amichne.kast.distribution.contract.WireRuntimeIdentity) : RuntimeWireAuthority
}
