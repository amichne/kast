...
class CanonicalDiagnosticScopeTest {
    @Test
    fun `a directory resolves every source file under the same lease before diagnostics`(@TempDir temporary: Path) =
        runBlocking {
                    ...
                    DiagnosticScopeResolver { query ->
                        ...
                        val admitted =
                            DiagnosticScope.fromCanonicalPaths(
                                query.lease,
                                listOf(query.path.resolve("B.kt"), query.path.resolve("A.kt")),
                            )
                        when (admitted) {
    ...
    fun `scope admission failures never invoke compiler diagnostics`(@TempDir temporary: Path) = runBlocking {
        ...
        val fixture = InstalledSymbolProtocolFixture.create(root)
        val operations =
            DiagnosticService(fixture.workspace, DiagnosticCompilerPort { error("rejected scope reached compiler") })
        for ((reason, expected) in
            ...
            val handler =
                CanonicalDiagnosticCheckHandler(
                    ...
                    operations,
                    CanonicalProtocolAuthority(),
                    DiagnosticScopeResolver { Refinement.Rejected(reason) },
                )
            assertEquals(OperationOutcome.Rejected(expected), handler.execute(request("src")))
        }
    }

    private fun request(scope: String) =
