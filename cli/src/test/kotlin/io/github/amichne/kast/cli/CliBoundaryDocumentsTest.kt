...
class CliBoundaryDocumentsTest {
    ...
    @Test
    fun `generated lifecycle document preserves fields and canonical artifact order`(@TempDir temporary: Path) {
    ...
    fun `runtime rejection preserves phase attempt and fixed corrective action in one JSON document`() {
        val attempt =
            (io.github.amichne.kast.distribution.contract.bootstrap.SemanticRuntimeBootstrapAttemptId.admit(
                ...
                .value
        val rejected =
            io.github.amichne.kast.distribution.contract.bootstrap.SemanticRuntimeBootstrapState.Rejected(
                attempt,
        ...
        val raw = CliBoundaryDocuments.runtimeRejected(RuntimeAdmissionFailure.IntellijBootstrap(rejected)).value
        val document = kotlinx.serialization.json.Json.parseToJsonElement(raw) as kotlinx.serialization.json.JsonObject
        val bootstrap = document.getValue("bootstrap") as kotlinx.serialization.json.JsonObject
        ...
        assertEquals(
            kotlinx.serialization.json.JsonPrimitive(
                "Select a Gradle-compatible project JVM, then run kast start again."
            ),
            bootstrap["correctiveAction"],
        )
