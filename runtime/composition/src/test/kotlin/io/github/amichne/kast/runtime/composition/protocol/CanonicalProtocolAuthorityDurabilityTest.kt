...
class CanonicalProtocolAuthorityDurabilityTest {
    ...
    fun `candidate and exact selectors survive authority recreation`() {
        ...
        val candidateDocument =
            (first.issueCandidates(fixture.batch) as CandidateSelectorIssuance.Issued).selectors.single()
        val restoredCandidate =
            assertInstanceOf(
        ...
        val exactDocument = (first.issueExact(fixture.selector) as ExactSelectorIssuance.Issued).selector
        val restoredExact =
            assertInstanceOf(
    ...
    fun `relation targets issue exact selectors accepted by every exact consumer`() {
        ...
        val document =
            (CanonicalProtocolAuthority().issueEndpoint(endpoint) as RelationEndpointIssuance.Issued).selector
