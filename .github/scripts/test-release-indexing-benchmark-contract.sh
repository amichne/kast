...
require "$release" 'if: ${{ false }}' 'real-repository release indexing must stay disabled until it is restored deliberately'
reject "$release" 'needs.real-repository-indexing.result' 'release publication must not wait for disabled repository indexing'
