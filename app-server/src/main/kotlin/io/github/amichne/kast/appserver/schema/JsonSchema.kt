...
@JvmInline
internal value class JsonSchemaDigest private constructor(val value: String) {
    companion object {
        internal fun derive(document: JsonObject): JsonSchemaDigest =
            JsonSchemaDigest(
                "sha256:" +
                    HexFormat.of()
                        .formatHex(
...
@JvmInline
internal value class JsonConstraintDescription private constructor(val value: String) {
    companion object {
        internal fun admit(raw: String): JsonConstraintDescription? =
            raw.trim()
                .takeIf { it.isNotEmpty() && it.length <= MAXIMUM_DESCRIPTION_LENGTH }
...
internal class ValidatedJsonValue
private constructor(
...
internal class CompiledJsonSchema
private constructor(
    internal val document: JsonObject,
    internal val digest: JsonSchemaDigest,
    private val validator: Schema,
) {
    internal fun admit(candidate: JsonElement): Validation<ValidatedJsonValue, JsonConstraintViolation> =
    ...
    companion object {
        ...
        internal fun compile(document: JsonObject): Refinement<CompiledJsonSchema, JsonSchemaCompilationFailure> =
            try {
...
internal sealed interface JsonDomainAdmissionFailure<out DomainFailure> {
    data class Constraint(val violation: JsonConstraintViolation) : JsonDomainAdmissionFailure<Nothing>

    data class Domain<DomainFailure>(val failure: DomainFailure) : JsonDomainAdmissionFailure<DomainFailure>
}

/**
 * Applicative admission from arbitrary JSON through one exact schema into a domain value.
 *
 * Domain refinement cannot run until schema proof exists, and callers cannot obtain the strong value by validating a
 * Boolean separately from the candidate.
 */
internal class JsonDomainDefinition<Strong, DomainFailure>(
    ...
    internal fun admit(candidate: JsonElement): Validation<Strong, JsonDomainAdmissionFailure<DomainFailure>> =
        when (val schemaAdmission = schema.admit(candidate)) {
            is Validation.Validated ->
                definition.refine(schemaAdmission.value).mapFailures(JsonDomainAdmissionFailure<DomainFailure>::Domain)
            ...
            is Validation.Rejected ->
                Validation.Rejected(
                    NonEmptyFailures.from(
                        ...
                        remaining =
                            schemaAdmission.failures.drop(1).map { violation ->
