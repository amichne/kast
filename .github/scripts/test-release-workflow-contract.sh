...
require_contains "$release_provenance_assembler" '"headless"' "Release provenance must include the headless backend"
...
require_contains "$release_workflow" '"kast-headless-${tag}.zip"' "Release must require the headless backend asset"
...
require_occurrences "$release_workflow" "if: always() && needs.prepare-release.result == 'success'" 5 "Tag-push release jobs must opt out of skipped bump-version poisoning"
...
require_contains "$release_workflow" "needs.build-headless-backend.result == 'success'" "Release publish job must require a successful headless backend build"
...
require_contains "$release_workflow" "Build headless backend" "Release must build the headless backend"
require_contains "$release_workflow" "Upload headless backend asset" "Release must upload the headless backend asset"
require_contains "$release_workflow" "build-provenance-headless.json" "Release must write headless backend provenance"
...
require_contains "$ubuntu_debian_docker_validator" "KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE" "Ubuntu/Debian Docker validator must create an explicit smoke workspace"
require_contains "$ubuntu_debian_docker_validator" "smoke_source_root=\"\${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}/src/main/kotlin\"" "Ubuntu/Debian Docker validator must create a Kotlin smoke source root"
require_contains "$ubuntu_debian_docker_validator" "kast up \\" "Ubuntu/Debian Docker validator must start the installed standalone backend"
require_contains "$ubuntu_debian_docker_validator" "--workspace-root=\"\${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}\"" "Ubuntu/Debian Docker validator must start the backend for the smoke workspace"
require_contains "$ubuntu_debian_docker_validator" "--source-roots=\"\${smoke_source_root}\"" "Ubuntu/Debian Docker validator must use an explicit smoke source root"
require_contains "$ubuntu_debian_docker_validator" "--module-name=ubuntu-debian-smoke" "Ubuntu/Debian Docker validator must name the smoke module"
require_contains "$ubuntu_debian_docker_validator" "kast capabilities --workspace-root=\"\${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}\"" "Ubuntu/Debian Docker validator must query the installed backend"
