...
value class ProtocolOffset private constructor(val value: Int) {
    companion object {
        /**
         * Proof transition: `Int -> Refinement<ProtocolOffset, ProtocolOffsetFailure>`.
         *
         * Establishes a non-negative source offset. [ProtocolOffsetFailure] is the closed expected failure. Raw
         * extraction is permitted only at the compiler or text-search adapter.
         */
...
internal object ProtocolOffsetSerializer :
    RefiningIntSerializer<ProtocolOffset>(
...
data class SourceRangeDocument
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(ProtocolOffset, ProtocolOffset) -> Refinement<SourceRangeDocument,
         * SourceRangeDocumentFailure>`.
         *
         * Establishes a non-empty half-open source range. The closed expected failure is [SourceRangeDocumentFailure].
         * Raw offsets may be extracted only at a compiler, search, or presentation boundary.
         */
        fun create(
...
sealed interface SymbolTextScopeDocument {
    @Serializable @SerialName("workspace") data object Workspace : SymbolTextScopeDocument

    @Serializable @SerialName("file") data class File(val file: ProtocolText) : SymbolTextScopeDocument
}
...
data class SymbolDiscoverResult(val items: BoundedProtocolList<SymbolDiscoveryDocument>) : OperationResult
...
class SymbolDiscoverQualification private constructor(val limitations: List<SymbolDiscoverLimitation>) :
    OperationQualification {
    companion object {
        /**
         * Proof transition: `Set<SymbolDiscoverLimitation> -> Refinement<SymbolDiscoverQualification,
         * SymbolDiscoverQualificationFailure>`.
         *
         * Establishes a non-empty, deterministically ordered public limitation list, so a qualified discovery outcome
         * cannot be represented without its limitations. [SymbolDiscoverQualificationFailure] is the closed expected
         * failure. Raw limitation sets may be extracted only at the domain-to-protocol composition and wire boundaries.
         */
        fun from(
    ...
    override fun equals(other: Any?): Boolean = other is SymbolDiscoverQualification && limitations == other.limitations
...
@Serializable data class SymbolInspectRequest(val target: SymbolInspectTarget) : OperationRequest
...
sealed interface SymbolQualifiedIdentityDocument {
    data class Available(val value: ProtocolText) : SymbolQualifiedIdentityDocument
...
@JvmInline
value class CompilerTypeParameterCountDocument private constructor(val value: Int) {
    companion object {
        fun parse(raw: Int): Refinement<CompilerTypeParameterCountDocument, CompilerTypeParameterCountDocumentFailure> =
            if (raw < 0) {
...
@ConsistentCopyVisibility
data class CompilerSymbolEvidenceDocument
private constructor(
        ...
        fun fromSignature(
            ...
            val canonical =
                signature.canonicalSignature()
                    ?: return Refinement.Rejected(CompilerSymbolEvidenceDocumentFailure.INVALID_SIGNATURE)
            val identity =
                when (val parsed = ProtocolText.parse(CompilerSymbolIdentity.fromCanonicalSignature(canonical).value)) {
        ...
        fun restore(
            ...
            when (val evidence = fromSignature(signature)) {
                ...
                is Refinement.Refined ->
                    if (evidence.value.identity == identity) {
                        evidence
                    } else {
                        Refinement.Rejected(CompilerSymbolEvidenceDocumentFailure.IDENTITY_MISMATCH)
                    }
            }
    }
}
    ...
    val canonical =
        when (this) {
            ...
            is CompilerSignatureDocument.ClassLike ->
                CanonicalCompilerSignature.classLike(qualifiedIdentity.value).valueOrNull() ?: return null
        }
...
private fun CompilerSignatureDocument.matchesCanonical(canonical: CanonicalCompilerSignature): Boolean =
    when {
        this is CompilerSignatureDocument.Function && canonical is CanonicalCompilerSignature.Function ->
        ...
        this is CompilerSignatureDocument.Property && canonical is CanonicalCompilerSignature.Property ->
            qualifiedIdentity.value == canonical.qualifiedIdentity.value &&
                receiver.matchesCanonical(canonical.receiver) &&
                contextReceivers.values.map(ProtocolText::value) == canonical.contextReceivers.map { it.value } &&
...
private fun CompilerReceiverDocument.matchesCanonical(canonical: CanonicalCompilerReceiver): Boolean =
    when {
...
internal fun CompilerSignatureDocument.supports(kind: SymbolKindDocument): Boolean =
    when (this) {
        is CompilerSignatureDocument.Function ->
