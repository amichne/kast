...
enum class SemanticRuntimeBootstrapPhase(val wireName: String, val displayName: String) {
    @SerialName("discovering-runtime") DISCOVERING_RUNTIME("discovering-runtime", "discovering runtime"),
    @SerialName("selecting-gradle-jvm") GRADLE_JVM_SELECTION("selecting-gradle-jvm", "selecting Gradle JVM"),
    @SerialName("capturing-model-inputs")
    MODEL_INPUT_CAPTURE("capturing-model-inputs", "capturing Gradle model inputs"),
    @SerialName("importing-gradle-model") PROJECT_IMPORT("importing-gradle-model", "importing Gradle model"),
    @SerialName("indexing") INDEXING("indexing", "indexing"),
    @SerialName("capturing-workspace-model") MODEL_CAPTURE("capturing-workspace-model", "capturing workspace model"),
    @SerialName("assembling-runtime") RUNTIME_ASSEMBLY("assembling-runtime", "assembling runtime"),
    @SerialName("activating-transport") TRANSPORT_ACTIVATION("activating-transport", "activating transport");
...
enum class SemanticRuntimeBootstrapCorrectiveAction(val instruction: String) {
    VERIFY_NETWORK_CONFIGURATION(
        "Verify the explicit Kast and Gradle network configuration; no Gradle import was attempted."
    ),
    VERIFY_TRUST_DONOR(
        "Verify that the configured donor Java home contains a readable jssecacerts or cacerts trust store with certificates; do not disable TLS verification."
    ),
    VERIFY_NETWORK_CACHE(
        "Verify that the private Kast cache is writable and contains no conflicting network artifacts, then run kast start again."
    ),
    ...
    VERIFY_GRADLE_TOOLING_PAYLOAD(
        "Update Kast to a build with Gradle tooling payloads compatible with the project JVM, then run kast start again."
    ),
    RECREATE_GRADLE_INIT_SCRIPTS(
        "Stop Kast, verify its runtime cache is writable, then start again to recreate Gradle initialization scripts."
    ),
    VERIFY_GRADLE_IMPORT(
        "Run the repository Gradle wrapper successfully with the admitted import inputs, then run kast start again."
    ),
    SELECT_SUPPORTED_IDE("Select the supported IntelliJ installation with kast start --idea-home."),
    CORRECT_MODEL_INPUTS(
        "Repair the reported Gradle input: targets must exist, remain inside the workspace, be readable, and contain no link cycles; then run kast start again."
    ),
    CORRECT_WORKSPACE_MODEL(
        "Correct the repository Gradle model and source roots, then run kast stop followed by kast start."
    ),
...
fun SemanticRuntimeBootstrapState.Rejected.correctiveAction(): SemanticRuntimeBootstrapRemediation {
    if (cause is SemanticRuntimeBootstrapCause.ModelInput)
        return SemanticRuntimeBootstrapRemediation.Bootstrap(
    ...
    when (val observation = gradleJvm) {
        ...
        is GradleJvmSelectionObservation.Observed ->
            when (val outcome = observation.report.outcome) {
                is GradleJvmSelectionOutcome.Rejected ->
                    return SemanticRuntimeBootstrapRemediation.GradleJvm(outcome.failure)
...
private fun SemanticRuntimeBootstrapState.Rejected.defaultCorrectiveAction(): SemanticRuntimeBootstrapCorrectiveAction =
    when (failure) {
        ...
        SemanticRuntimeBootstrapFailure.PLATFORM_LINKAGE_INVALID ->
            ...
            when (phase) {
                SemanticRuntimeBootstrapPhase.DISCOVERING_RUNTIME ->
                    SemanticRuntimeBootstrapCorrectiveAction.VERIFY_RUNTIME
                SemanticRuntimeBootstrapPhase.GRADLE_JVM_SELECTION ->
                    SemanticRuntimeBootstrapCorrectiveAction.SELECT_GRADLE_JVM
                SemanticRuntimeBootstrapPhase.MODEL_INPUT_CAPTURE ->
                    SemanticRuntimeBootstrapCorrectiveAction.CORRECT_MODEL_INPUTS
                SemanticRuntimeBootstrapPhase.PROJECT_IMPORT ->
                    SemanticRuntimeBootstrapCorrectiveAction.VERIFY_GRADLE_IMPORT
                SemanticRuntimeBootstrapPhase.INDEXING,
                SemanticRuntimeBootstrapPhase.MODEL_CAPTURE ->
                    SemanticRuntimeBootstrapCorrectiveAction.CORRECT_WORKSPACE_MODEL
                SemanticRuntimeBootstrapPhase.RUNTIME_ASSEMBLY,
                SemanticRuntimeBootstrapPhase.TRANSPORT_ACTIVATION ->
                    SemanticRuntimeBootstrapCorrectiveAction.RESTART_RUNTIME
            }
    }
