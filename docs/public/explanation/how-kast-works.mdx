...
- The host is one release-line-compatible local IDEA installation.
- Runtime identity retains the exact observed IDEA, Kotlin plugin, and JBR.
- Config, indexes, plugins, logs, and other mutable state stay under Kast-owned
  paths.
...
sequenceDiagram
    ...
    Caller->>CLI: symbol.inspect JSON on standard input
    ...
    CLI->>Endpoint: bounded SymbolInspectRequest
...
**Bottom line:** `kast start` publishes an endpoint only after the host and
repository are semantically ready.

Before launch, Kast verifies:

- Compatible IDEA and Kotlin plugin release lines.
- Exact observed patch, JBR, and private-payload identities.
- Required bootstrap classes and Gradle integration.

The sidecar then imports the Project, refreshes its private VFS, waits for smart
mode, and runs semantic canaries. Only then does it publish the endpoint.
...
Before readiness, composition constructs the exact nine-operation public capability set.
The [CLI reference](/reference/cli#sidecar-endpoint-operations) lists each
operation and command shape.

Source-root identity comes from IntelliJ's canonical local source-folder URL,
not from an existence-dependent `VirtualFile`.

- An absent resources or generated-source directory can remain declared.
- Until it exists, that directory contributes no source candidates.
- Malformed, non-local, outside-workspace, conflicting, or unproven roots fail
  closed.
...
`kast start --cache seed` is the only lifecycle intent authorized to read an
IDEA system directory.

It requires:

- The exact runtime and target Project.
- A fully stopped IDEA with no live lock or matching process.
- Explicit disclosure of the allowlist, estimated bytes, and shared entries in
  required global VFS/index files.
- `--accept-global-index-copy` for non-interactive consent.

On APFS, Kast copy-on-write clones only:

- Allowlisted global identity data.
- The target Project's dedicated model/index cache.

It excludes Local History, logs, AI data, VCS logs, editor state, temporary
data, and other Projects' dedicated caches. Kast rechecks manifests and source
quiescence before atomic publication. A bad seed becomes a typed failure or
`rebuild-required`, never semantic success.
...
```json
{"target":{"type":"exact","selector":"<exact-selector>"}}
```

The Kotlin control executable selects `symbol.inspect`, then the canonical
`SymbolInspectRequest` serializer parses the standard-input document.
`UnixDomainWireClient` sends that same model as one bounded frame to the
admitted socket.

The request follows four owners:

1. `InstalledIndexerTransport` reads the bounded frame.
2. `KastIndexerHost` passes it to `InstalledKastRuntime`.
3. `CanonicalKastOperationHandlerFactory` dispatches it on the exact workspace
   graph.
4. `InstalledIntellijSymbolPorts` resolves the selector with Project, PSI,
   index, and K2 authority, then detaches host-neutral evidence.
...
<AccordionGroup>
  ...
  <Accordion title="Inspect module ownership" icon="boxes">
    ...
    flowchart LR
        ...
        TRANSPORT --> DISPATCH[Exact nine-route semantic dispatch]
...
Startup publishes one durable semantic generation after it:

- Imports the exact Gradle Project.
- Reaches smart mode and indexing quiescence.
- Captures the admitted source-root model and source identity.

Startup does not build full repository topology. An explicit `topology.build`
can rebind a durable snapshot when its paths, roots, and content hashes still
match; otherwise Kast repeats extraction.
...
| --- | --- |
| **Sidecar routes** | Typed query execution, symbol discovery and inspection, source and relation reads, traversal, diagnostics, planning, verified application, and recovery are the ten public installed capabilities. Index synchronization and topology remain internal prerequisites. |
...
traversal spans are forwarded asynchronously as OTLP JSON Lines. `kast`
exposes the enabled directory, trace file, and format without starting the
runtime.

The span tree separates admission, snapshot reads, enumeration, extraction,
revalidation, publication, snapshot open, and traversal expansion.

Custom attributes include only:

- Closed outcome and failure variants.
- Non-negative file or record counts.

They never include paths, selectors, source text, exception messages, or stack
traces.
