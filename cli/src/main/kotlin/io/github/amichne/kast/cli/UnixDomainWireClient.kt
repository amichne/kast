...
private const val MAX_WIRE_FRAME_BYTES =
...
fun interface WireClient {
     ...
     *
     * Establishes one bounded request/response exchange through the exact-root endpoint. [WireTransportFailure] is the
     * closed expected failure. Raw documents may be extracted only by the canonical wire codec and typed projection.
     */
    fun exchange(
    ...
    fun exchange(
        ...
        budget: io.github.amichne.kast.kernel.ElapsedTimeLimitMillis,
    ): WireExchange = WireExchange.Rejected(WireTransportFailure.UNQUALIFIED_PEER)
}

sealed interface WireExchange {
    data class Received(val document: String) : WireExchange

    data class Rejected(val failure: WireTransportFailure) : WireExchange
}
...
internal constructor(
    ...
    fun exchange(document: String, identity: WireRuntimeIdentity): WireExchange = boundedExchange {
        when (val retained = authority) {
            is Authority.Qualified ->
                if (retained.identity.sameProcess(identity)) transfer(document)
    ...
    private fun qualifyAndTransfer(document: String, identity: WireRuntimeIdentity): WireExchange =
        when (val qualification = transfer(WireRuntimeQualification.request(identity))) {
            ...
            is WireExchange.Received ->
                when (WireRuntimeQualification.admitResponse(qualification.document, identity)) {
                    WirePeerQualification.REJECTED -> {
                        activity.publish(
                            WireRequestActivity(WireRequestStage.PEER_QUALIFICATION, WireRequestOutcome.REJECTED)
                        )
                    ...
                    WirePeerQualification.QUALIFIED -> {
                        ...
                        activity.publish(
                            WireRequestActivity(WireRequestStage.PEER_QUALIFICATION, WireRequestOutcome.COMPLETED)
                        )
                        transfer(document)
                    }
                }
     ...
     *
     * Establishes one bounded request/response exchange on the already-admitted endpoint. [WireTransportFailure] is the
     * closed expected failure. Raw documents remain confined to the framing and canonical wire boundaries.
     */
    fun exchange(document: String): WireExchange = boundedExchange { transfer(document) }

    private fun transfer(document: String): WireExchange =
        when (val written = WireFrameCodec.write(channel, document)) {
            WireFrameWrite.Written ->
                when (val read = WireFrameCodec.read(channel)) {
    ...
    private fun boundedExchange(operation: () -> WireExchange): WireExchange {
        ...
        val observed =
            if (outcome == WireRequestOutcome.TIMED_OUT) WireExchange.Rejected(WireTransportFailure.TIMED_OUT)
            else result
...
sealed interface WireSessionOpening {
    data class Opened(val session: WireSession) : WireSessionOpening

    data class Rejected(val failure: WireTransportFailure) : WireSessionOpening
}
...
class UnixDomainWireClient(
    ...
    override fun exchange(
        ...
        when (val opening = open(endpoint)) {
    ...
    fun open(endpoint: RuntimeEndpoint, allowance: ElapsedTimeLimitMillis = budget): WireSessionOpening {
        ...
        val result =
            try {
                ...
                val remaining =
                    allowance.value - java.util.concurrent.TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - started)
        ...
        activity.publish(
            WireRequestActivity(
                WireRequestStage.CONNECTION,
                if (outcome == WireRequestOutcome.TIMED_OUT) outcome
                else if (result is WireSessionOpening.Opened) WireRequestOutcome.COMPLETED
                else WireRequestOutcome.REJECTED,
            )
        )
        return if (outcome == WireRequestOutcome.TIMED_OUT) WireSessionOpening.Rejected(WireTransportFailure.TIMED_OUT)
        else result
    }
}
...
sealed interface WireFrameWrite {
    ...
    data class Rejected(val failure: WireTransportFailure) : WireFrameWrite
}

sealed interface WireFrameRead {
    data class Received(val document: String) : WireFrameRead

    data class Rejected(val failure: WireTransportFailure) : WireFrameRead
}
...
internal object WireFrameCodec {
     ...
     *
     * Establishes that one bounded UTF-8 frame was completely written. [WireTransportFailure] is the closed expected
     * failure. Raw bytes remain inside this adapter.
     */
    fun write(
     ...
     *
     * Establishes one complete, bounded, length-prefixed UTF-8 response document. [WireTransportFailure] is the closed
     * expected failure. Raw bytes remain inside this adapter.
     */
    fun read(channel: SocketChannel): WireFrameRead {
        ...
        when (readCompletely(channel, header)) {
            ...
            BufferRead.TRUNCATED -> return WireFrameRead.Rejected(WireTransportFailure.TRUNCATED_FRAME)
