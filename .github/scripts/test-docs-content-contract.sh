...
require_contains "$readme" "curl -fsSL https://raw.githubusercontent.com/amichne/kast/main/kast.sh | bash" "README must document the supported curl-pipe installer"
require_not_contains "$readme" "amichne/kast-action@v1" "README must not document a separate hosted-agent installer"
require_contains "$readme" "./kast.sh install" "README must point non-Brew users to the root installer"
require_not_contains "$readme" "scripts/install-ubuntu-debian.sh" "README must not document the deprecated Ubuntu/Debian wrapper"
require_contains "$index_doc" "./kast.sh install" "Docs overview must show the Linux headless tarball install example"
require_contains "$index_doc" "Homebrew install guide" "Docs overview must still link macOS developers to Homebrew"

require_contains "$install_doc" "## Shell installer" "Install docs must document the supported shell installer"
...
require_contains "$install_doc" "curl -fsSL https://raw.githubusercontent.com/amichne/kast/main/kast.sh | bash" "Install docs must document curl-pipe usage"
require_contains "$install_doc" "./kast.sh install --from" "Install docs must document exact artifact installs"
...
require_not_contains "$install_doc" 'scripts/install-ubuntu-debian.sh' "Install docs must not document the deprecated Ubuntu/Debian wrapper"
require_contains "$install_doc" '## Linux headless tarball' "Install docs must document the Linux headless tarball"
require_contains "$install_doc" './kast.sh install' "Install docs must document the canonical non-Brew installer"
...
require_not_contains "$install_doc" 'KAST_UBUNTU_DEBIAN_ARTIFACT_PATH' "Install docs must prefer --from over deprecated artifact environment overrides"
require_not_contains "$install_doc" 'KAST_UBUNTU_DEBIAN_BASE_URL' "Install docs must prefer --from over deprecated release-base environment overrides"
require_not_contains "$install_doc" 'KAST_UBUNTU_DEBIAN_VERSION' "Install docs must prefer --version over deprecated version environment overrides"
...
require_contains "$quickstart_doc" './kast.sh install' "Quickstart must show Linux headless tarball installation"
