...
internal sealed interface IntellijAddFileAbsenceObservation {
    ...
    data class Observed(val result: SourceObservationResult.Observed) : IntellijAddFileAbsenceObservation

    data class Rejected(val failure: SourceObservationFailure) : IntellijAddFileAbsenceObservation
}
...
internal class IntellijAddFileSourcePrimitive(private val project: Project) {
    private val protocol = IntellijAddFileWriteProtocol()

    /**
     * Proof transition: `WorkspaceFile -> IntellijAddFileAbsenceObservation`.
     *
     * Observed establishes exact physical absence and parent-derived creation access. TargetPresent preserves the
     * weaker state for existing-source observation. [SourceObservationFailure] closes unavailable parents. Raw
     * filesystem and VFS values remain inside this call.
     */
    fun observeIfAbsent(source: SymbolDiscoveryFileIdentity.Workspace): IntellijAddFileAbsenceObservation {
        val path = Path.of(source.path.value)
        if (Files.exists(path, LinkOption.NOFOLLOW_LINKS)) {
            return IntellijAddFileAbsenceObservation.TargetPresent
        }
        val parentPath =
            path.parent ?: return IntellijAddFileAbsenceObservation.Rejected(SourceObservationFailure.TARGET_NOT_FOUND)
        val parent =
            LocalFileSystem.getInstance().refreshAndFindFileByNioFile(parentPath)
                ?: return IntellijAddFileAbsenceObservation.Rejected(SourceObservationFailure.TARGET_NOT_FOUND)
        if (!parent.isValid || !parent.isDirectory) {
            return IntellijAddFileAbsenceObservation.Rejected(SourceObservationFailure.TARGET_INVALIDATED)
        }
        val access =
        ...
        return IntellijAddFileAbsenceObservation.Observed(
            SourceObservationResult.Observed(ObservedAbsentMutationSource.fromPhysicalBoundary(source, access))
        )
    }
     ...
     *
     * Applied establishes in-memory staging against exact absence, applied-write durability, singleton physical
     * creation, and exact postimage observation. [SourceWriteFailure] closes expected platform failure. Live IntelliJ
     * values remain inside this call.
     */
    fun write(
        ...
        val prepared =
            when (val result = prepare(authority)) {
                is Refinement.Refined -> result.value
                is Refinement.Rejected -> return SourceWriteResult.RejectedBeforeMutation(result.failure)
            }
     ...
     *
     * RolledBack establishes that the exact AddFile target is absent, deleting it only when its bytes still equal the
     * authority postimage and the durable recovery marker proves prior absence. [AddDeclarationRollbackFailure] closes
     * divergent or unavailable targets.
     */
    fun rollback(
        authority: MutationAuthority,
        ...
        if (
            authority.preconditionAtIntellijBoundary() != MutationPreconditionAtIntellijBoundary.Absent ||
        ...
        return try {
            onEdt {
                WriteCommandAction.writeCommandAction(project).withName("Kast rollback AddFile").compute<
                    Unit,
                    RuntimeException,
                > {
                    file.delete(this)
    ...
    /**
     * Proof transition: `MutationAuthority -> Refinement<IntellijAddFilePreparation, SourceWriteFailure>`.
     *
     * Establishes exact physical absence, a valid writable parent, an AddFile authority, and a syntax-valid detached
     * Kotlin postimage. [SourceWriteFailure] closes stale or unsupported state. Raw PSI, paths, and VFS values remain
     * inside the request-local capability.
     */
    private fun prepare(authority: MutationAuthority): Refinement<IntellijAddFilePreparation, SourceWriteFailure> =
        try {
    ...
    /**
     * Proof transition: `MutationAuthority -> Refinement<IntellijAddFilePreparation, SourceWriteFailure>` inside one
     * smart read action.
     *
     * Establishes exact absence, writable parent identity, AddFile intent, and a Kotlin PSI tree without syntax errors.
     * [SourceWriteFailure] closes stale, unavailable, unsupported, or malformed state. Live PSI and VFS values remain
     * in the returned request-local capability.
     */
    private fun prepareRead(authority: MutationAuthority): Refinement<IntellijAddFilePreparation, SourceWriteFailure> {
        val intent =
            authority.intent as? ChangeIntent.AddFile ?: return Refinement.Rejected(SourceWriteFailure.MUTATION_FAILED)
        if (authority.preconditionAtIntellijBoundary() != MutationPreconditionAtIntellijBoundary.Absent) {
        ...
        val parentPath = path.parent ?: return Refinement.Rejected(SourceWriteFailure.TARGET_NOT_FOUND)
        val parent =
            LocalFileSystem.getInstance().refreshAndFindFileByNioFile(parentPath)
        ...
        return Refinement.Refined(IntellijAddFilePreparation(parent, path.fileName.toString(), path))
    }
    ...
    private fun resolve(
        ...
        when (result) {
            is IntellijWriteProtocolResult.Applied ->
                when (val observed = AppliedSourceWrite.observe(authority, result.bytes, result.changedPaths)) {
                    is Refinement.Refined -> SourceWriteResult.Applied(observed.value)
                    is Refinement.Rejected -> SourceWriteResult.RecoveryRequired(SourceWriteFailure.OBSERVATION_FAILED)
                }
            ...
            is IntellijWriteProtocolResult.RecoveryRequired -> SourceWriteResult.RecoveryRequired(result.failure)
        }
...
private class LiveIntellijAddFileStagingSession(
    ...
    override fun save(staged: IntellijStagedAddFile): IntellijSessionStepResult {
        ...
        return try {
            onEdt {
                WriteCommandAction.writeCommandAction(project).withName("Kast AddFile").compute<
                    VirtualFile,
                    RuntimeException,
                > {
