...
class ProjectBoundIntellijDiagnosticPorts
private constructor(
                ...
                scopes =
                    DiagnosticScopeResolver { query ->
                        readAction {
                            if (
                                project.isDisposed ||
                                    query.lease != authority ||
                                    model.workspaceRoot != authority.workspaceRoot
                            ) {
                                return@readAction Refinement.Rejected(
                                    DiagnosticScopeResolutionFailure.WORKSPACE_NOT_READY
                                )
                            }
                            val paths =
                                when (
                                    ...
                                    is Refinement.Rejected ->
                                        return@readAction Refinement.Rejected(
                                            when (result.failure) {
                                                ProjectSourceFileFailure.INVALID_SCOPE ->
                                                    DiagnosticScopeResolutionFailure.INVALID_SCOPE
                                                ProjectSourceFileFailure.UNAVAILABLE ->
                                                    DiagnosticScopeResolutionFailure.UNAVAILABLE
                                                ProjectSourceFileFailure.LIMIT_EXCEEDED ->
                            ...
                            if (paths.isEmpty())
                                return@readAction Refinement.Rejected(DiagnosticScopeResolutionFailure.EMPTY)
