...
class SidecarCliLifecycleTest {
    ...
    @Test
    fun `status targets exact stale cache endpoint without demanding a runtime`(@TempDir temporary: Path) {
        val fixture = fixture(temporary)
        ...
        assertTrue(exit is CliExit.Complete)
        val document =
            kotlinx.serialization.json.Json.parseToJsonElement(exit.document.value)
    ...
    fun `passive status preserves recorded indexing phase without runtime demand or repair`(@TempDir temporary: Path) {
        ...
        val attempt =
            (io.github.amichne.kast.distribution.contract.bootstrap.SemanticRuntimeBootstrapAttemptId.admit(
                ...
                .value
        val recorded =
            (observedCache(temporary, "sha256:${"c".repeat(64)}") as RootSidecarCacheObservation.Observed)
                .status
                .copy(
                    bootstrap =
                        RuntimeBootstrapObservation.Observed(
                            io.github.amichne.kast.distribution.contract.bootstrap.SemanticRuntimeBootstrapState
                                .Starting(
                                    attempt,
            ...
            fixture.cli(
                ...
                cacheLifecycle =
                    object : RootSidecarCacheLifecycle {
                        ...
                        override fun quarantine(stopped: StoppedSidecarCaches): RootSidecarCacheQuarantine =
                            error("status must not repair")
                    },
            )
        val exit = cli.execute(emptyList(), fixture.root.path)
        assertTrue(exit is CliExit.Complete)
        val document =
            kotlinx.serialization.json.Json.parseToJsonElement(exit.document.value).let {
    ...
    private fun observedLifecycle(state: RuntimeLifecycleState): RuntimeLifecycleController =
        object : RuntimeLifecycleController {
            override fun status(endpoint: RuntimeEndpoint): RuntimeStatusResult = RuntimeStatusResult.Observed(state)

            override fun stop(endpoint: RuntimeEndpoint): RuntimeStopResult = error("stop was not requested")
        }
    ...
    private fun semanticRuntimeId(character: Char = 'b'): SemanticRuntimeId =
                        ...
                        ProductInspection(
                            SidecarProductIdentity(
                                ...
                                (SupportedIdeRuntimePair.admit("262.9437.185", "262.9437.185-IJ")
                                        as SupportedIdeRuntimePairAdmission.Admitted)
                                    .pair,
                                (io.github.amichne.kast.distribution.contract.RuntimeDigest.parse(
                                        "sha256:${"a".repeat(64)}"
                                    ) as Refinement.Refined)
                                    .value,
                            ),
    ...
    private fun fixture(temporary: Path): Fixture {
        ...
        val runtimeId =
            when (val refinement = SemanticRuntimeId.parse("sha256:${"b".repeat(64)}")) {
                is Refinement.Refined -> refinement.value
                is Refinement.Rejected -> error(refinement.failure)
            }
        val endpoint =
            when (val resolution = RuntimeEndpoint.at(root, runtimeId, temporary.resolve("runtime.sock"))) {
    ...
    private companion object {
        fun localMetadata(): CliLocalMetadata =
