...
@JvmInline
value class KotlinIdentifier private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<KotlinIdentifier, KotlinIdentifierFailure>`.
         *
         * Establishes a non-keyword unquoted Kotlin identifier. [KotlinIdentifierFailure] is the closed expected
         * failure. Raw text may enter only at a public change-intent boundary and may leave only at an authority-bound
         * IntelliJ mutation boundary.
         */
        fun parse(raw: String): Refinement<KotlinIdentifier, KotlinIdentifierFailure> =
            when {
...
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(WorkspaceFile, ExactDeclarationTextRange, KotlinIdentifier, RenameSymbolOccurrenceRole)
         * -> Refinement<RenameSymbolOccurrence, RenameSymbolOccurrenceFailure>`.
         *
         * Establishes an exact non-empty range whose UTF-16 length can contain precisely the expected identifier.
         * [RenameSymbolOccurrenceFailure] closes mismatch. Raw offsets may enter only from compiler-grounded PSI
         * evidence and leave only at mutation admission.
         */
        fun admit(
...
class RenameSymbolOccurrenceSet private constructor(occurrences: List<RenameSymbolOccurrence>) {
    ...
    companion object {
         ...
         *
         * Establishes a unique, deterministic set containing exactly one declaration occurrence inside the selected
         * symbol plus zero or more reference occurrences, all bound to its current compiler name and admitted source
         * snapshot. [RenameSymbolOccurrenceSetFailure] is the closed expected failure. Occurrence extraction is
         * permitted only at the compiler-backed planning boundary.
         */
        fun admit(
            ...
            if (
                occurrences.zipWithNext().any { (left, right) ->
                    left.source == right.source && left.range.endExclusive > right.range.startInclusive
                }
            ) {
            ...
            if (declarations.isEmpty()) {
                return Refinement.Rejected(RenameSymbolOccurrenceSetFailure.DECLARATION_OCCURRENCE_MISSING)
            }
            if (declarations.size > 1) {
                return Refinement.Rejected(RenameSymbolOccurrenceSetFailure.DECLARATION_OCCURRENCE_AMBIGUOUS)
            }
            ...
            if (
                ...
                return Refinement.Rejected(RenameSymbolOccurrenceSetFailure.DECLARATION_OCCURRENCE_OUTSIDE_TARGET)
            }
...
sealed interface SourceTextMutation {
    ...
    data class CreateFile internal constructor(val content: KotlinFileSourceText) : SourceTextMutation
...
sealed interface PlannedSourcePrecondition {
    data class Existing(val content: WorkspaceSourceContentHash) : PlannedSourcePrecondition

    data object Absent : PlannedSourcePrecondition
}

class PlannedMutationWrite
internal constructor(
    val source: SymbolDiscoveryFileIdentity.Workspace,
...
class PlannedMutationWriteSet private constructor(entries: List<PlannedMutationWrite>) {
