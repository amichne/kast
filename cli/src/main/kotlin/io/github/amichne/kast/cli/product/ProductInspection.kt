...
sealed interface ProductWorkspaceObservation {
    data class RootRejected(val failure: CanonicalRootFailure) : ProductWorkspaceObservation
...
sealed interface ProductTelemetryObservation {
    data class Enabled(val output: SidecarTelemetryOutput) : ProductTelemetryObservation

    data class EndpointRejected(val failure: RuntimeEndpointFailure) : ProductTelemetryObservation

    data class OutputRejected(val failure: SidecarTelemetryOutputFailure) : ProductTelemetryObservation

    data class CacheRejected(val failure: SidecarCacheLifecycleFailure) : ProductTelemetryObservation
}
...
fun interface ProductInspector {
     ...
     *
     * Preserves the admitted sidecar identity and directly observes only canonical root and Kast-owned cache evidence.
     * No runtime demand or user-IDE endpoint admission is possible.
     */
...
class SidecarProductInspector(
    ...
    override fun inspect(start: Path): ProductInspection {
        val workspace =
            when (val discovery = rootDiscovery.discover(start)) {
                is CanonicalRootDiscovery.Rejected -> ProductWorkspaceObservation.RootRejected(discovery.failure)
    ...
    private fun RuntimeEndpointLocator.telemetryObservation(
        ...
        when (val resolution = locate(root)) {
            is RuntimeEndpointResolution.Rejected -> ProductTelemetryObservation.EndpointRejected(resolution.failure)
                ...
                val exact =
                    when (cache) {
                        is RootSidecarCacheObservation.Identified ->
                            when (
                                val refined =
                                    resolution.endpoint.forSidecarCache(
                                ...
                                is RuntimeEndpointResolution.Rejected ->
                                    return ProductTelemetryObservation.EndpointRejected(refined.failure)
                            }
                        RootSidecarCacheObservation.Absent -> resolution.endpoint
                        is RootSidecarCacheObservation.Rejected ->
                            return ProductTelemetryObservation.CacheRejected(cache.failure)
                    }
                when (val output = SidecarTelemetryOutput.fromSocketPath(exact.socketPath.toString())) {
