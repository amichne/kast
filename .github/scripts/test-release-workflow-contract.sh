...
require_contains "$ci_workflow" "- \"17\"" "CI install-container validation must cover Java 17"
require_contains "$ci_workflow" "- \"21\"" "CI install-container validation must cover Java 21"
...
require_contains "$release_workflow" "for java_version in 17 21" "Release must validate the Ubuntu/Debian bundle on Java 17 and 21"
require_contains "$release_workflow" "KAST_UBUNTU_DEBIAN_JAVA_VERSION=\"\$java_version\"" "Release Docker validation must pass each Java version"
