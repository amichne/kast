...
class CanonicalSourceReadHandlerTest {
    ...
    fun `source selector enters domain service and returns reusable protocol authority`() {
        ...
        assertEquals(
            7,
            (complete.evidence.basis as io.github.amichne.kast.kernel.EvidenceBasis.Published).generation.value,
        )
        ...
        assertTrue(
            SourceSelectorTokenCodec.decode(SourceSelectorToken.parse(returned.selection.selector.value).refined())
    ...
    fun `tampered source selector rejects before source operations execute`() {
        ...
        val tampered = fixture.token.value.dropLast(1) + if (fixture.token.value.last() == '0') '1' else '0'
    ...
    private fun protocolRequest(selector: String): SourceReadRequest =
        SourceReadRequest(
    ...
    private fun fixture(): Fixture {
        ...
        val complete =
            DomainSourceReadResult.Complete.create(
                    snapshot,
                    region,
                    emptyList(),
                    projection,
    ...
    private fun protocolText(raw: String) = io.github.amichne.kast.protocol.contract.ProtocolText.parse(raw).refined()
