...
class CanonicalSourceReadWireBindingTest {
    ...
    fun `returned source line evidence round trips and rejects impossible line spans`() {
        ...
        for (invalid in
            listOf(
                document.replace(
                    "\"lines\":{\"startInclusive\":2,\"endInclusive\":2}",
    ...
    fun `live source snapshot retains its epoch and rejects contradictory publication facts`() {
        ...
        val outcome =
            OperationOutcome.Complete(
                EvidenceEnvelope(
                    CanonicalOperationWireBindings.sourceRead.operation.id,
                    EvidenceBasis.Live(evidence),
                    result,
                )
            )
        ...
        val conflicting =
            document.replace("\"canonicalRoot\":\"/workspace\"", "\"canonicalRoot\":\"/workspace\",\"generation\":4")
        assertEquals(
            WireDecoding.Rejected(WireFailure.InvalidPayload(WireValueRole.RESULT)),
    ...
    private fun sourceReadRequest(): SourceReadRequest =
        SourceReadRequest(
                ...
                SourceEntitySelectionDocument.Matching(
                    ...
                    listOf(
                        SourceEntityFilterDocument.Declarations(
                            ...
                            SourceVisibilitySelectionDocument.Exact(listOf(SourceDeclarationVisibilityDocument.PUBLIC)),
                        )
                    ),
                ),
