...
class HostedPeerCancellationTest {
    @Test
    fun `disconnect drains semantic cancellation before the connection completes`() = runTest {
    ...
    @Test
    fun `successful response drains its disconnect watcher`() = runTest {
    ...
    @Test
    fun `native peer watcher cancellation leaves the response channel writable`() = runBlocking {
        withTimeout(3_000) {
            java.nio.channels.ServerSocketChannel.open().use { listener ->
                ...
                java.nio.channels.SocketChannel.open(listener.localAddress).use { client ->
                    listener.accept().use { server ->
                        val response =
                            dispatchUntilPeerTermination(server::awaitHostedPeerTermination) {
