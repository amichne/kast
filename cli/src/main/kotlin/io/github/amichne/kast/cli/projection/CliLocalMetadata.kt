...
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes non-blank product identity and canonical JSON schema. Clikt generates help from the executable
         * command graph. [CliLocalMetadataFailure] is the closed expected failure. Raw resource text may leave only at
         * this installed-control metadata boundary.
         */
        fun admit(
            ...
            val version =
                when (val admission = versionDocument(productVersion)) {
                    ...
                    is CliLocalVersionAdmission.Rejected -> return CliLocalMetadataAdmission.Rejected(admission.failure)
                }
            val schemaObject =
                when (val admission = CliOpenJsonObject.parse(schema)) {
                    is CliOpenJsonObjectAdmission.Admitted -> admission.value
                    is CliOpenJsonObjectAdmission.Rejected ->
                        return CliLocalMetadataAdmission.Rejected(CliLocalMetadataFailure.SCHEMA_INVALID)
                }
            return CliLocalMetadataAdmission.Admitted(CliLocalMetadata(version, schemaObject.document()))
        }
        ...
        internal fun admit(
            ...
            when (val admission = versionDocument(productVersion)) {
                is CliLocalVersionAdmission.Admitted ->
                    CliLocalMetadataAdmission.Admitted(CliLocalMetadata(admission.document, schema))
                is CliLocalVersionAdmission.Rejected -> CliLocalMetadataAdmission.Rejected(admission.failure)
            }
         ...
         * process document.
         */
        private fun versionDocument(productVersion: String): CliLocalVersionAdmission =
            when {
...
private sealed interface CliLocalVersionAdmission {
    data class Admitted(val document: CliTextDocument) : CliLocalVersionAdmission

    data class Rejected(val failure: CliLocalMetadataFailure) : CliLocalVersionAdmission
}
