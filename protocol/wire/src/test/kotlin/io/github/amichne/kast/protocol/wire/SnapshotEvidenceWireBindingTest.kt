...
class SnapshotEvidenceWireBindingTest {
    ...
    fun `source complete and qualified decoding requires the exact repeated live evidence`() {
        ...
        for (outcome in sourceOutcomes(live, sourceResult(live))) {
            ...
            for (basis in otherBases) {
                val snapshot = source.encodeOutcome(sourceOutcomes(basis, sourceResult(basis)).first())
                    .encoded().getValue("body").jsonObject.getValue("result").jsonObject.getValue("snapshot")
                assertEquals(WireDecoding.Rejected(invalidPayload),
                    source.decodeOutcome(document.replaceSnapshot(snapshot).toString()), basis.toString())
            }
        }
    }
    ...
    fun `source encoder refuses contradictory bases generations and live roots`() {
        ...
        val foreignRoot = sourceResult(live).let { it.copy(snapshot = it.snapshot.copy(canonicalRoot = text("/foreign"))) }
        for ((basis, result) in listOf(
            live to sourceResult(published), published to sourceResult(live),
            published to sourceResult(otherGeneration), live to sourceResult(moved), live to foreignRoot,
    ...
    fun `live traversal wire root agrees with the envelope for complete and qualified outcomes`() {
        val qualification = TraversalRunQualification.terminalIncomplete(
            listOf(TraversalLimitationDocument.DEPTH_LIMIT_REACHED), emptyList()).refined()
        for (root in listOf("/workspace", "/foreign")) {
            ...
            for (outcome in listOf(OperationOutcome.Complete(evidence), OperationOutcome.Qualified(evidence, qualification))) {
                if (root == "/foreign") {
    ...
    private fun sourceOutcomes(basis: EvidenceBasis, result: SourceReadResult): List<OperationOutcome<SourceReadResult, SourceReadQualification, SourceReadRejection>> {
        val evidence = EvidenceEnvelope(source.operation.id, basis, result)
        val qualification = SourceReadQualification.create(SourceEntityCountDocument.parse(0).refined(),
            listOf(SourceReadLimitationDocument.WORK_LIMIT_REACHED), SourceReadContinuationStateDocument.Unavailable).refined()
    ...
    private fun sourceResult(basis: EvidenceBasis): SourceReadResult {
        val snapshot = SourceSnapshotDocument(text(if (basis is EvidenceBasis.Live) basis.evidence.workspaceRoot else "/workspace"),
            when (basis) {
    ...
    private fun live(root: String, host: String, epoch: Long) = EvidenceBasis.Live(
        LiveReadEvidence.create(root, UUID.fromString(host), epoch, LiveReadContentView.SAVED_PSI_COMMITTED, 1).refined())
