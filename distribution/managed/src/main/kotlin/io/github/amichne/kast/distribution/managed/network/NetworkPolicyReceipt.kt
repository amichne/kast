...
sealed interface RecordedNetworkAuthority {
    ...
    class Derived internal constructor(val digest: String, val sources: Set<TrustProvenance>, val path: Path) :
        RecordedNetworkAuthority
}

enum class NetworkPolicyObservationFailure {
    UNREADABLE,
    MALFORMED,
...
object NetworkPolicyReceipt {
    fun observe(directory: Path): NetworkPolicyObservation =
        try {
            ...
            when {
                ...
                Files.isSymbolicLink(directory) ||
                    ...
                    val sources =
                        names
                            .map { name ->
                                TrustProvenance.entries.singleOrNull { it.name == name }
                                    ?: return NetworkPolicyObservation.Rejected(
                                        NetworkPolicyObservationFailure.MALFORMED
                                    )
                            }
                            .toSet()
                    val authority =
                        when (digest) {
                            "explicit-gradle" ->
                                if (sources == setOf(TrustProvenance.EXPLICIT_GRADLE)) RecordedNetworkAuthority.Gradle
                                else return NetworkPolicyObservation.Rejected(NetworkPolicyObservationFailure.MALFORMED)
                            "target-jvm" ->
                                if (sources.isEmpty()) RecordedNetworkAuthority.TargetJvm
                                else return NetworkPolicyObservation.Rejected(NetworkPolicyObservationFailure.MALFORMED)
                            else -> {
                                ...
                                if (!Files.isRegularFile(artifact, LinkOption.NOFOLLOW_LINKS)) {
                                    return NetworkPolicyObservation.Rejected(
                                        NetworkPolicyObservationFailure.ARTIFACT_UNAVAILABLE
                                    )
                                }
