...
The current [public tool contracts](../contracts/public-tools.md) distinguish presentation identity from canonical operation identity. Three ordinary searches and deferred `query_symbols` share `query.run`; `check_diagnostics` shares `diagnostic.check`. Private admission retains each tool's schema identity and typed syntax through its exact CLI binding. Public tools and canonical read commands reach the installed daemon through the versioned `/kast-operation` Unix-socket RPC; the daemon re-admits the public schema or concrete canonical request and exact root, then uses its shared existing-IDE preparation and read path. Operation effects, budgets, reference authority and exhaustive outcomes remain with their existing owners.
