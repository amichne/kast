...
class QueryRetainedCompositionTest {
                        ...
                        QueryExecutionResult.Qualified(
                            ...
                            coverage =
                                QueryCoverage.Qualified.create(
                                        io.github.amichne.kast.query.contract.QueryCount.parse(1).refined(),
    ...
    @Test
    fun `retained failure rows paginate without blocking proven symbols`() = runTest {
        QueryServiceTest().apply {
            val selected = selector(selection())
            val failure = QueryItemFailure.PredicateUnproven(selected)
            val retained =
                QueryRetainedResult.capture(
                        selected.lease,
                        QueryExecutionResult.Qualified(
