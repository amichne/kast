...
class RuntimeEndpointAliasTest {
    @Test
    fun `long runtime endpoint binds a physical installation inode through one admitted alias`() =
        fixture { root, endpoint ->
    ...
    @Test
    fun `replaced runtime alias rejects connect and cleanup without touching foreign state`() =
        fixture { root, endpoint ->
            val prepared = (endpoint.prepareTransport() as RuntimeEndpointResolution.Resolved).endpoint
            ...
            try {
                ...
                assertEquals(
                    ...
                    PosixRuntimeEndpointArtifacts.clean(
    ...
    private fun fixture(test: (Path, RuntimeEndpoint) -> Unit) {
        ...
        val directory =
            (InstalledRuntimeDirectory.admit(logical.toString(), null) as InstalledRuntimeDirectoryAdmission.Admitted)
                .directory
        val runtime = (SemanticRuntimeId.parse("sha256:${"a".repeat(64)}") as Refinement.Refined).value
        val endpoint =
            (Sha256RuntimeEndpointLocator(RuntimeSocketDirectory.from(directory), runtime).locate(CanonicalRoot(root))
