...
class AppServerStatusTest {
    @Test
    fun `passive status observes coordinator and registry without attaching an upstream host`() = withPayload { root, kast ->
        runBlocking {
            val activities = java.util.concurrent.CopyOnWriteArrayList<BrokerStartupActivity>()
            val sink = BrokerStartupActivitySink { activities += it; BrokerStartupActivityPublication.PUBLISHED }
            ...
            val environment = mapOf("BROKER_SERVICE_IDENTITY" to command.identity.value,
                "BROKER_READINESS_FILE" to command.readinessFile.toString())
            val options = (InstalledCoordinatorConfiguration.admit(kast, root, environment, sink) as Refinement.Refined).value
            val workspace = Files.createDirectory(root.resolve("workspace"))
            assertTrue(WorkspaceEnrollmentStore(root.resolve("config/workspaces.json")).enroll(workspace) is Refinement.Refined)
            val running = (InstalledCoordinator.start(options, InstalledWorkerEffects.Unavailable) as InstalledCoordinatorStart.Started).coordinator
            try {
                val result = InstalledAppServerManager(kast, root, emptyMap()).execute(AppServerAction.Status, workspace)
                ...
                assertEquals("ready", document.getValue("coordinator").jsonObject.getValue("state").jsonPrimitive.content)
                assertEquals("registered", document.getValue("registry").jsonObject.getValue("state").jsonPrimitive.content)
    ...
    @Test
    fun `passive status reports absent service and empty registry without creating state`() = withPayload { root, kast ->
        val result = InstalledAppServerManager(kast, root, emptyMap()).execute(AppServerAction.Status, root)
        assertTrue(result is AppServerManagementResult.Completed, result.toString())
        val document = (result as AppServerManagementResult.Completed).document
        assertEquals("unavailable", document.getValue("coordinator").jsonObject.getValue("state").jsonPrimitive.content)
    ...
    @Test
    fun `configuration inspection compares desired revision with passive coordinator acknowledgement`() = withPayload { root, kast ->
        runBlocking {
            val saved = Files.writeString(Files.createDirectory(root.resolve("config")).resolve("environment"), "KAST_WORKER_AGGREGATE_MIB=32768\n")
            val environment = mapOf("KAST_CONFIGURATION_FILE" to saved.toString())
            val command = (BrokerServiceLaunchCommand.resolveCoordinator(kast, root, environment) as BrokerServiceLaunchCommandResolution.Resolved).command
            val launch = environment + mapOf("BROKER_SERVICE_IDENTITY" to command.identity.value, "BROKER_READINESS_FILE" to command.readinessFile.toString())
            val options = (InstalledCoordinatorConfiguration.admit(kast, root, launch) as Refinement.Refined).value
            val running = (InstalledCoordinator.start(options, InstalledWorkerEffects.Unavailable) as InstalledCoordinatorStart.Started).coordinator
            try {
                val acknowledged = InstalledConfigurationAppliedInspection.read(kast, root, environment, command.configuration)
                ...
                val next = (BrokerServiceLaunchCommand.resolveCoordinator(kast, root, environment) as BrokerServiceLaunchCommandResolution.Resolved).command
                val pending = InstalledConfigurationAppliedInspection.read(kast, root, environment, next.configuration)
