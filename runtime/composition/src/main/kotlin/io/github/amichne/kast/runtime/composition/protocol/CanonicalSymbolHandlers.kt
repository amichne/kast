...
) : OperationHandler<SymbolDiscoverRequest, SymbolDiscoverResult, SymbolDiscoverQualification, SymbolDiscoverRejection> {
    private val protocol = CanonicalSymbolDiscoverProtocol(operations, authority)
    override suspend fun execute(request: SymbolDiscoverRequest): OperationOutcome<SymbolDiscoverResult, SymbolDiscoverQualification, SymbolDiscoverRejection> {
        val current = (workspace.inspect() as? WorkspaceRuntimeState.Ready)?.workspace?.readLease
...
internal class CanonicalSymbolInspectHandler(
    ...
    override suspend fun execute(request: SymbolInspectRequest): OperationOutcome<SymbolInspectResult, SymbolInspectQualification, SymbolInspectRejection> {
        val current = when (val target = request.target) {
            is SymbolInspectTarget.Candidate -> when (val lookup = authority.candidate(target.selector)) {
                is CandidateSelectorLookup.Found -> lookup.selector.lease
                is CandidateSelectorLookup.Rejected -> return OperationOutcome.Rejected(lookup.reason.protocolRejection())
            }
            is SymbolInspectTarget.Exact -> when (val lookup = authority.exact(target.selector)) {
                ...
                is ExactSelectorLookup.Rejected -> return OperationOutcome.Rejected(lookup.reason.protocolRejection())
            }
        }
...
private fun SelectorLookupRejection.protocolRejection(): SymbolInspectRejection = when (this) {
