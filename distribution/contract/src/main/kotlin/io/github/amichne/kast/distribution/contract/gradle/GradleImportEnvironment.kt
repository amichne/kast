...
value class GradleImportVariableName private constructor(val value: String) {
    companion object {
        ...
        fun parse(raw: String): Refinement<GradleImportVariableName, GradleImportEnvironmentFailure> =
            when {
                !Regex("[A-Za-z_][A-Za-z0-9_]{0,127}").matches(raw) ->
                    Refinement.Rejected(GradleImportEnvironmentFailure.INVALID_VARIABLE_NAME)
                raw.startsWith("KAST_") || raw.startsWith("DYLD_") || raw.startsWith("LD_") || raw in RESERVED ->
...
/**
 * Explicit import capability. Raw selected values leave only through [processVariables] at the process launch boundary.
 * toString and identity evidence never contain values. Ambient PATH is absent from the model; executable additions
 * preserve their explicit search order.
 */
class GradleImportEnvironment
private constructor(
    private val variables: Map<GradleImportVariableName, String>,
    val executableDirectories: List<GradleImportExecutableDirectory>,
) {
    val evidence: List<GradleImportVariableEvidence> =
        variables.entries.map { (name, value) ->
            GradleImportVariableEvidence(name, GradleImportEnvironmentIdentity.digest(value))
        }
    val identity: GradleImportEnvironmentIdentity =
        GradleImportEnvironmentIdentity.digest(
            buildString {
                append("kast-gradle-import-environment-v1\n")
                evidence.forEach {
                    append("env:").append(it.name.value).append(':').append(it.digest.value).append('\n')
                }
    ...
    override fun toString(): String =
        "GradleImportEnvironment(identity=${identity.value}, names=${evidence.map { it.name.value }})"
    ...
    companion object {
        ...
        /**
         * Refines boundary settings plus the caller environment to an immutable allowlist proof. Missing selected
         * inputs fail closed. No unselected ambient value enters the proof. Raw maps are permitted only at this
         * admission and the process environment boundary.
         */
        fun admit(
            names: String,
            ...
            for (raw in rawNames.distinct().sorted()) {
                val name =
                    when (val parsed = GradleImportVariableName.parse(raw)) {
...
private fun sha256(value: String): String =
    MessageDigest.getInstance("SHA-256").digest(value.toByteArray(Charsets.UTF_8)).joinToString("") { byte ->
