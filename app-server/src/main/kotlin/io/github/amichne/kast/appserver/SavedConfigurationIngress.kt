...
enum class SavedConfigurationIngressFailure { INVALID_SELECTOR, INVALID_INSTALLATION, INVALID_WORKSPACE, UNAVAILABLE, NOT_REGULAR, CHANGED_DURING_READ }
...
@Serializable enum class SavedConfigurationObservation { ABSENT, LOADED }
...
object InstalledSavedConfigurationIngress {
    ...
    fun read(selector: String, environment: Map<String, String>): SavedConfigurationIngress {
        val path = try { Path.of(selector) } catch (_: InvalidPathException) {
        ...
        val bytes = try {
            ...
            if (before.size() > SavedConfigurationDocument.MAXIMUM_BYTES) {
                return SavedConfigurationIngress.Rejected(SavedConfigurationIngressRejection.Document(SavedConfigurationDocumentFailure.TOO_LARGE))
            }
            val content = Files.newInputStream(path, LinkOption.NOFOLLOW_LINKS).use {
            ...
            if (before.fileKey() != after.fileKey() || before.lastModifiedTime() != after.lastModifiedTime() || before.size() != after.size()) {
        ...
        return when (val parsed = SavedConfigurationDocument.parse(bytes)) {
            is Refinement.Refined -> SavedConfigurationIngress.Loaded(parsed.value.configurationSources(environment), SavedConfigurationObservation.LOADED)
            is Refinement.Rejected -> SavedConfigurationIngress.Rejected(SavedConfigurationIngressRejection.Document(parsed.failure))
        }
    }

    private fun rejected(failure: SavedConfigurationIngressFailure) = SavedConfigurationIngress.Rejected(SavedConfigurationIngressRejection.File(failure))
}
