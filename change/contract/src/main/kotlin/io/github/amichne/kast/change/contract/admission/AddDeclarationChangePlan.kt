...
class AdmittedAddDeclarationPlanInput
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `AddDeclarationPlanRequest -> Refinement< AdmittedAddDeclarationPlanInput,
         * AddDeclarationPlanningFailure>`.
         *
         * Establishes an exact editable target, canonical declaration, expected semantic delta, and complete normalized
         * relation, traversal, and diagnostic evidence for the same target lease. [AddDeclarationPlanningFailure] is
         * the closed expected failure. Raw source text and semantic observations must already have crossed their typed
         * boundaries; this pure transition permits no raw extraction.
         */
        fun admit(
            request: AddDeclarationPlanRequest
        ): Refinement<AdmittedAddDeclarationPlanInput, AddDeclarationPlanningFailure> =
            when (val evidence = CompleteAddDeclarationPlanningEvidence.admit(request)) {
...
class AddDeclarationSourceSnapshot
private constructor(
...
sealed interface AddDeclarationPlannedEdit {
    ...
    data class InsertAfterDeclaration
    ...
    @ConsistentCopyVisibility
    data class InsertIntoClassBody
    internal constructor(
        val file: SymbolDiscoveryFileIdentity.Workspace,
        val anchor: ExactDeclarationTextRange,
        val declaration: AddDeclarationSourceText,
 ...
 *
 * This value contains no write method or effect capability. Later admission must bind it to exact current repository
 * state before any physical adapter can act.
 */
class AddDeclarationChangePlan
private constructor(
    ...
    override val intent: ChangeIntent =
        ChangeIntent.AddDeclaration(
            target,
    ...
    companion object {
         ...
         *
         * Establishes a detached plan whose identity covers the exact source snapshot, target, canonical insertion,
         * normalized complete evidence, expected semantic delta, and every closed verification obligation. There is no
         * expected failure because the input already carries complete planning proof. Raw declaration extraction is
         * permitted only at a later mutation adapter after separate mutation-authority admission.
         */
        fun issue(input: AdmittedAddDeclarationPlanInput): AddDeclarationChangePlan {
            val evidence = DurableAddDeclarationPlanningEvidence.from(input.evidence)
            val verification = AddDeclarationVerificationContract.forGeneration(input.target.lease.generation)
            return create(
                input.target,
                input.declaration,
                evidence,
...
sealed interface AddDeclarationPlanResult {
    data class Planned(val plan: AddDeclarationChangePlan) : AddDeclarationPlanResult

    data class Rejected(val failure: AddDeclarationPlanningFailure) : AddDeclarationPlanResult
}
...
fun interface AddDeclarationPlanOperations {
     ...
     *
     * Produces a deterministic plan only when the target and all required detached evidence refine to
     * [AdmittedAddDeclarationPlanInput]. [AddDeclarationPlanningFailure] is the closed expected failure. Raw source and
     * compiler values remain outside this pure operation boundary.
     */
