...
async function runJsonCommand(label, command) {
    ...
    if (!out) {
        return JSON.stringify({
            ...
            command:   label,
            message:   `${label} produced no JSON output (exit ${code})`,
    ...
    try {
        ...
        return JSON.stringify({
            ...
            command:   label,
            message:   `${label} returned non-JSON (exit ${code})`,
            raw:       out,
            errorText: stderr.trim() || null
        });
    }
...
const tools = [
    {
        ...
        handler:     (args) => {
            ...
            const cmd = `bash ${shellArg(scriptPath("gradle", "run_task.sh"))} ${shellArg(args.projectRoot)} ${shellArg(
                args.task)}${extra ? ` ${extra}` : ""}`;
    ...
    },
    {
        ...
        handler:     (args) =>
                         runJsonCommand("gradle_run_hook",
                             `bash ${shellArg(scriptPath("gradle", "run_gradle_hook.sh"))} ${shellArg(
                                 args.projectRoot)}`),
    },
    {
        ...
        handler:     (args) => {
            ...
            return runJsonCommand("gradle_parse_junit",
                `python3 ${shellArg(scriptPath("parse", "junit_results.py"))} ${shellArg(args.projectRoot)}${mod}`);
        },
    },
];
...
const session = await joinSession({
    tools,
    disabledSkills: ["kotlin-gradle-loop"],
});
