...
require_contains "$ci_workflow" "Rust CLI" "CI must include a Rust CLI job"
require_contains "$ci_workflow" "amichne/kast-rs" "Rust CLI job must check out the Rust CLI source"
require_contains "$ci_workflow" "./scripts/build-rust-cli.sh --source-dir kast-rs --output build/rust-cli/kast" "Rust CLI job must build the Rust CLI"
require_contains "$ci_workflow" "./scripts/smoke-native-cli.sh build/rust-cli/kast" "Rust CLI job must smoke embedded agent resources"
require_not_contains "$ci_workflow" "graalvm/setup-graalvm@v1" "CI must not install GraalVM"
require_not_contains "$ci_workflow" ":kast-cli:nativeCompile" "CI must not compile a GraalVM native image"
...
require_contains "$release_workflow" "Build Rust Kast CLI release asset" "Release must build the Rust CLI asset"
require_contains "$release_workflow" "amichne/kast-rs" "Release must check out the Rust CLI source"
require_contains "$release_workflow" "./scripts/build-rust-cli.sh --source-dir kast-rs --output build/rust-cli/kast" "Release must build the Rust CLI before packaging the release asset"
require_not_contains "$release_workflow" ":kast-cli:portableDistZip" "Release must not build the JVM portable zip for native CLI assets"
require_not_contains "$release_workflow" "graalvm/setup-graalvm@v1" "Release must not install GraalVM"
require_not_contains "$release_workflow" ":kast-cli:nativeCompile" "Release must not compile a GraalVM native image"
require_contains "$release_workflow" "--native-binary \"build/rust-cli/kast\"" "Release must package the Rust CLI binary into the CLI asset"
require_contains "$release_workflow" './scripts/smoke-native-cli.sh "$dist_dir/kast-cli/kast-cli"' "Release must smoke the native binary from the actual release asset"
require_contains "$release_workflow" "Release CLI asset must contain only the native launcher" "Release must reject CLI assets that still carry JVM payload directories"
...
require_contains "$release_provenance_assembler" '"headless-agent-linux-x64"' "Release provenance must include the headless agent bundle"
