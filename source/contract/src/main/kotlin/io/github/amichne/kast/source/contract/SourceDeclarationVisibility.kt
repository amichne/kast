...
class SourceDeclarationVisibility private constructor(
    ...
    companion object {
        fun admit(
            ...
            val entity = result.entities.singleOrNull() as? SourceEntity.Declaration
                ?: return Refinement.Rejected(SourceDeclarationVisibilityFailure.MISSING_OR_AMBIGUOUS_DECLARATION)
            val selection = when (val identity = entity.semanticIdentity) {
            ...
            if (result.snapshot.lease != selector.lease || result.snapshot.file != selector.file ||
                result.snapshot.readScope != SourceReadScope.Constrained(selector.scope, selector.constraints) ||
                result.region.kind != SourceRegionKind.DECLARATION || range != result.region.selector.range ||
                range.startInclusive.value != selector.range.startInclusive || range.endExclusive.value != selector.range.endExclusive ||
                entity.nestingDepth.value != 0 || entity.kind.compilerKind() != selector.kind ||
                selection.lease != selector.lease || selection.scope != selector.scope ||
                selection.constraints.copy(declarationKinds = null) != selector.constraints.copy(declarationKinds = null) ||
                selection.candidate.name.value != selector.name.value || location.file != selector.file ||
...
private fun DeclarationKind.compilerKind(): CompilerSymbolKind = when (this) {
