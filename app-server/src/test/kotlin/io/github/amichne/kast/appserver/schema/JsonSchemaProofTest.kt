...
class JsonSchemaProofTest {
    ...
    fun `admitted value carries the exact compiled schema digest`() {
        val document =
            Json.parseToJsonElement("""{"type":"object","additionalProperties":false,"properties":{}}""").jsonObject
        val schema = (NetworkntJsonSchemaCompiler.compile(document) as Refinement.Refined).value
