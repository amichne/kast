...
class QueryVisibilitySelection private constructor(
    val values: List<DeclarationVisibility>,
) {
    companion object {
        fun from(
            raw: Set<DeclarationVisibility>,
        ): Refinement<QueryVisibilitySelection, QueryCollectionFailure> =
            if (raw.isEmpty()) {
    ...
    override fun equals(other: Any?): Boolean =
        other is QueryVisibilitySelection && values == other.values
...
class QueryCandidateFields private constructor(
    ...
    override fun equals(other: Any?): Boolean =
        other is QueryCandidateFields && values == other.values
...
class QuerySymbolFields private constructor(
    ...
    override fun equals(other: Any?): Boolean =
        other is QuerySymbolFields && values == other.values
...
@JvmInline
value class QueryStagePosition private constructor(
    val value: Int,
) {
    companion object {
...
sealed interface QueryPlanAdmissionFailure {
    data class UnsupportedDeclarationKind(
...
object QueryPlanCompiler {
    fun admit(syntax: QueryPlanSyntax): QueryPlanAdmission {
        val discovery = when (val source = syntax.source) {
        ...
        if (
            discovery != null &&
            CompilerSymbolKind.CONSTRUCTOR in discovery.declarationKinds.values
        ) {
        ...
        return when (val source = syntax.source) {
            is QuerySourceSyntax.Candidates -> when (
                val stage = candidateStage(
                    syntax.steps,
                    0,
                    syntax.output,
                )
            ) {
                is CandidateStageAdmission.Admitted -> QueryPlanAdmission.Admitted(
                    AdmittedQueryPlan.Candidates(source.discovery, stage.stage),
                )
                is CandidateStageAdmission.Rejected -> QueryPlanAdmission.Rejected(stage.failure)
            }
            is QuerySourceSyntax.Symbols -> exactPlan(
                source.discovery,
                syntax.steps,
                syntax.output,
            )
            is QuerySourceSyntax.CandidateReferences -> when (
                val stage = candidateStage(syntax.steps, 0, syntax.output)
            ) {
                is CandidateStageAdmission.Admitted -> QueryPlanAdmission.Admitted(
            ...
            is QuerySourceSyntax.ExactReferences -> when (
                val stage = exactStage(syntax.steps, 0, syntax.output)
            ) {
                is ExactStageAdmission.Admitted -> QueryPlanAdmission.Admitted(
                    AdmittedQueryPlan.ExactReferences(source.references, stage.stage),
                )
    ...
    ): QueryPlanAdmission = when (val stage = exactStage(steps, 0, output)) {
    ...
    private fun candidateStage(
        ...
        val step = steps.getOrNull(index) ?: return when (output) {
            is QueryOutputSyntax.Candidates ->
                CandidateStageAdmission.Admitted(CandidateQueryStage.Emit(output.fields))
            is QueryOutputSyntax.Symbols -> CandidateStageAdmission.Rejected(
        ...
        return when (step) {
            QueryStepSyntax.Distinct -> candidateStage(
                steps,
                index + 1,
                output,
            )
                .map { CandidateQueryStage.Distinct(it) }
            QueryStepSyntax.Inspect -> when (val next = exactStage(steps, index + 1, output)) {
                is ExactStageAdmission.Admitted ->
    ...
    private fun exactStage(
        ...
        val step = steps.getOrNull(index) ?: return when (output) {
            is QueryOutputSyntax.Symbols ->
                ExactStageAdmission.Admitted(ExactQueryStage.Emit(output.fields))
            is QueryOutputSyntax.Candidates -> ExactStageAdmission.Rejected(
                outputMismatch(index, QueryElementType.DECLARATION_CANDIDATE, QueryElementType.EXACT_SYMBOL),
            )
        }
        return when (step) {
            QueryStepSyntax.Inspect -> ExactStageAdmission.Rejected(
                stageMismatch(
                    index,
                    QueryElementType.DECLARATION_CANDIDATE,
                    QueryElementType.EXACT_SYMBOL,
            ...
            QueryStepSyntax.Distinct -> exactStage(steps, index + 1, output)
                .map { ExactQueryStage.Distinct(it) }
            is QueryStepSyntax.Where -> exactStage(steps, index + 1, output)
                .map { ExactQueryStage.Where(step.predicate, it) }
            is QueryStepSyntax.Related -> exactStage(steps, index + 1, output)
                .map { ExactQueryStage.Related(step.meaning, it) }
        }
    }
    ...
    ): QueryPlanAdmissionFailure.StageTypeMismatch = QueryPlanAdmissionFailure.StageTypeMismatch(
        position(index),
        required,
        actual,
        correction,
    ...
    private fun outputMismatch(
        ...
        required: QueryElementType,
        actual: QueryElementType,
    ): QueryPlanAdmissionFailure.OutputTypeMismatch = QueryPlanAdmissionFailure.OutputTypeMismatch(
        position(index),
        required,
        actual,
    ...
    private fun position(raw: Int): QueryStagePosition = when (val parsed = QueryStagePosition.parse(raw)) {
...
): CandidateStageAdmission = when (this) {
...
private fun ExactStageAdmission.map(
    transform: (ExactQueryStage) -> ExactQueryStage,
): ExactStageAdmission = when (this) {
