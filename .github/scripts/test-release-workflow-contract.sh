...
require_not_contains "$release_workflow" '"kast-${tag}-linux-x64.zip"' "Kast JVM release must not require Rust CLI assets"
...
require_occurrences "$release_workflow" "if: always() && needs.prepare-release.result == 'success'" 3 "Tag-push release jobs must opt out of skipped bump-version poisoning"
