...
require_block_contains "$ci_workflow" "  rust-cli:" "  build-and-test:" "KAST_LOCAL_SOURCE_SHA256" "CI Rust production must bind the existing release build to the captured source snapshot"
require_contains "$ci_build_and_test_workflow" "kast-local-source-snapshot" "CI Linux backend production must consume the captured source snapshot"
require_contains "$ci_build_and_test_workflow" "packageSourceBoundDevelopmentBackend" "CI Linux backend production must emit the source-bound backend without a second owner"
...
require_contains "$ci_workflow" '--bundle-version "$KAST_UBUNTU_DEBIAN_CI_BUNDLE_TAG"' "CI bundle tests must pass the ready-compatible version to the single generation assembler"
require_contains "$prepared_generation_assembler" '--version "$bundle_version"' "The generation assembler must write the ready-compatible version into the bundle manifest"
...
require_block_order "$ci_workflow" "  kast-action-runtime-contract:" "  analysis-server-transport:" "Free disk for kast-action runtime installation" "Download prepared kast-action runtime inputs" "CI must establish the runtime installation disk budget before downloading artifacts"
