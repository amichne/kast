...
class ExactDeclarationSelectorContractTest {
    ...
    fun `same name and file collisions issue distinct exact selectors by native evidence`() {
        ...
        assertEquals(
            ...
            (firstSelector.qualifiedIdentity as ExactDeclarationQualifiedIdentity.Available).value,
        )
    ...
    fun `selector issuance and revalidation fail closed when evidence does not match`() {
        val selection = SymbolDiscoverySelection.select(batch(listOf(candidate(offset = 7))), 0).refined()
        assertEquals(
    ...
    private fun candidate(offset: Int): SymbolDiscoveryCandidate =
        SymbolDiscoveryCandidate.fromBoundary(
