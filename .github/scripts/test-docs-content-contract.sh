...
require_contains "$readme" "kast agent setup copilot" "README must document repository-local Copilot installation"
...
require_contains "$index_doc" "kast agent setup copilot" "Overview must show repository Copilot install"
require_contains "$index_doc" "Commands" "Overview must route to command docs"
require_contains "$index_doc" "hidden debug transport" "Overview must demote raw RPC"

require_contains "$install_doc" "## Developer machine" "Install docs must distinguish the developer-machine path"
require_contains "$install_doc" "kast agent setup copilot" "Install docs must make repository Copilot install primary"
require_contains "$install_doc" "kast agent setup auto --harness" "Install docs must document harness-neutral agent setup"
require_contains "$install_doc" ".codex/skills" "Install docs must document Codex skill roots"
require_contains "$install_doc" ".codex/instructions" "Install docs must document Codex instruction roots"
require_contains "$install_doc" ".github/instructions" "Install docs must document GitHub instruction roots"
require_contains "$install_doc" ".claude/instructions" "Install docs must document Claude instruction roots"
require_contains "$install_doc" "kast agent up --dry-run" "Install docs must document agent bring-up planning"
require_contains "$install_doc" "--no-onboard" "Install docs must document noninteractive onboarding skip"
require_contains "$install_doc" "automatic IDEA launch" "Install docs must document interactive IDEA onboarding"
require_contains "$install_doc" "this repository only" "Install docs must document repository-scoped onboarding"
require_contains "$install_doc" "kast agent setup auto --dry-run" "Install docs must document harness setup planning"
require_contains "$install_doc" "agentHarness = \"instructions\"" "Install docs must document configured harness preference"
require_contains "$install_doc" "MCP-dependent" "Install docs must call out non-MCP enterprise setup"
require_contains "$install_doc" "kast ready --fix" "Install docs must document repair"
require_contains "$install_doc" "kast inspect paths" "Install docs must document path inspection"
...
require_contains "$headless_doc" 'kast release package ubuntu-debian-bundle' "Headless docs must document the release bundle packager"

require_contains "$quickstart_doc" "kast runtime up --backend=headless" "Quickstart must start a backend"
require_contains "$quickstart_doc" "kast agent raw-resolve" "Quickstart must use agent resolve"
require_contains "$quickstart_doc" "kast agent raw-references" "Quickstart must use agent references"
require_contains "$quickstart_doc" "kast agent workspace-symbol" "Quickstart must show name-to-offset bridge"
...
require_contains "$lifecycle_doc" "kast runtime up" "Lifecycle docs must cover up"
require_contains "$lifecycle_doc" "kast runtime status" "Lifecycle docs must cover status"
require_contains "$lifecycle_doc" "kast runtime restart" "Lifecycle docs must cover restart"
require_contains "$lifecycle_doc" "kast runtime stop" "Lifecycle docs must cover stop"
require_contains "$install_repair_doc" "kast agent setup copilot" "Install command docs must cover Copilot install"
require_contains "$install_repair_doc" "kast agent setup auto --harness" "Install command docs must cover harness-neutral setup"
require_contains "$install_repair_doc" ".codex/skills" "Install command docs must cover Codex skill roots"
require_contains "$install_repair_doc" ".codex/instructions" "Install command docs must cover Codex instruction roots"
require_contains "$install_repair_doc" ".github/instructions" "Install command docs must cover GitHub instruction roots"
require_contains "$install_repair_doc" ".claude/instructions" "Install command docs must cover Claude instruction roots"
require_contains "$install_repair_doc" "kast agent up --dry-run" "Install command docs must cover setup plus runtime bring-up"
require_contains "$install_repair_doc" "--no-onboard" "Install command docs must cover onboarding skip"
require_contains "$install_repair_doc" "kast agent setup auto --dry-run" "Install command docs must cover setup planning"
require_contains "$install_repair_doc" "projectOpen.agentHarness" "Install command docs must cover configured harness preference"
require_contains "$install_repair_doc" "kast ready --fix" "Install command docs must cover repair"
require_contains "$agent_doc" "kast agent call" "Agent command docs must cover structured calls"
require_contains "$agent_doc" "kast agent setup auto --harness" "Agent command docs must cover harness selection"
require_contains "$agent_doc" ".codex/skills" "Agent command docs must cover Codex skill roots"
require_contains "$agent_doc" ".codex/instructions" "Agent command docs must cover Codex instruction roots"
require_contains "$agent_doc" ".github/instructions" "Agent command docs must cover GitHub instruction roots"
require_contains "$agent_doc" ".claude/instructions" "Agent command docs must cover Claude instruction roots"
require_contains "$agent_doc" "kast agent up --dry-run" "Agent command docs must cover agent bring-up planning"
require_contains "$agent_doc" "--no-onboard" "Agent command docs must cover noninteractive onboarding skip"
require_contains "$agent_doc" "this repository only" "Agent command docs must cover repository-scoped onboarding"
require_contains "$agent_doc" "nextActions" "Agent command docs must document next-action guidance"
...
require_contains "$metrics_doc" "kast agent metrics" "Metrics docs must cover agent metrics"
require_contains "$lsp_doc" "kast agent lsp --stdio" "LSP docs must cover the stdio command"
require_contains "$recipes_doc" "kast agent raw-rename" "Recipes must cover safe rename planning"
require_contains "$recipes_doc" "kast agent raw-diagnostics" "Recipes must cover diagnostics"
require_contains "$troubleshooting_doc" "kast --output json ready" "Troubleshooting must start from readiness"
require_contains "$troubleshooting_doc" "kast agent health" "Troubleshooting must include agent health"
require_contains "$distribution_doc" "kast release package ubuntu-debian-bundle" "Distribution docs must cover bundle packaging"
require_contains "$distribution_doc" "kast release activate bundle" "Distribution docs must cover bundle activation"
