...
class EncodedIdeEndpointDescriptor private constructor(val document: String) {
    companion object {
        /**
         * Proof transition: `IdeEndpointDescriptorV2 -> EncodedIdeEndpointDescriptor`.
         *
         * Preserves the admitted descriptor as the sole configured JSON format's canonical bytes using the
         * compiler-generated serializer. Raw JSON leaves only through [document] at the endpoint publication or
         * transport boundary.
         */
...
internal object IdeEndpointDescriptorCodec {
     ...
     *
     * Decodes the generated closed document, refines every primitive through [IdeEndpointDescriptorV2.create], and
     * requires canonical byte equality before returning an admitted descriptor. [IdeEndpointDescriptorFailure] is the
     * closed expected failure. Raw JSON and document fields remain inside this codec boundary.
     */
    fun admit(
        raw: String,
        ...
        val document =
            try {
                ...
                return IdeEndpointDescriptorAdmission.Rejected(IdeEndpointDescriptorFailure.MalformedDocument)
        ...
        return when (val admission = IdeEndpointDescriptorV2.create(document.toCandidate(), policy)) {
            ...
            is IdeEndpointDescriptorAdmission.Admitted -> {
                if (admission.descriptor.encode().document == raw) {
                    ...
                    IdeEndpointDescriptorAdmission.Rejected(IdeEndpointDescriptorFailure.NonCanonicalDocument)
                }
            }
        }
    }
...
private fun IdeEndpointDescriptorDocument.toCandidate() =
...
private fun IdeEndpointDescriptorV2.toDocument() =
    IdeEndpointDescriptorDocument(
