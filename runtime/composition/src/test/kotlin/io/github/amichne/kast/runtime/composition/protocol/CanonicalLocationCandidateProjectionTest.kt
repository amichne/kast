...
class CanonicalLocationCandidateProjectionTest {
    @Test
    fun `relation and traversal distinguish wrong selector family from malformed token`(@TempDir temporary: Path) {
        val fixture =
        ...
        assertEquals(
            ...
            runSuspend {
                relation.execute(RelationReadRequest(wrongKind, RelationKindDocument.REFERENCES, count(4)))
            },
        )
        assertEquals(
            OperationOutcome.Rejected(RelationReadRejection.SELECTOR_MALFORMED),
            runSuspend {
                relation.execute(RelationReadRequest(malformed, RelationKindDocument.REFERENCES, count(4)))
            },
        )
        assertEquals(
            OperationOutcome.Rejected(TraversalRunRejection.SELECTOR_WRONG_KIND),
            runSuspend {
                traversal.execute(TraversalRunRequest(wrongKind, RelationKindDocument.REFERENCES, count(1), count(4)))
            },
        )
        assertEquals(
            OperationOutcome.Rejected(TraversalRunRejection.SELECTOR_MALFORMED),
            runSuspend {
                traversal.execute(TraversalRunRequest(malformed, RelationKindDocument.REFERENCES, count(1), count(4)))
            },
        )
    }

    @Test
    fun `relation traversal and diagnostics retain exact reusable range candidates`(@TempDir temporary: Path) {
        ...
        val traversed =
            runSuspend {
                traversal.execute(TraversalRunRequest(exact, RelationKindDocument.REFERENCES, count(1), count(4)))
            }
        ...
        val restored =
            listOf(
                ...
                .map { token ->
                    (authority.candidate(token) as CandidateSelectorLookup.Found).selector as CandidateSelector.Range
                }
        ...
        val source =
            CanonicalSourceReadHandler(
                SourceReadOperations { request ->
                    admitted += ((request.anchor as SourceReadAnchor.Candidate).selector as CandidateSelector.Range)
                    DomainSourceReadResult.Rejected(DomainSourceReadRejection.REGION_NOT_APPLICABLE)
                },
                authority,
            )
        listOf(relationOccurrence.candidateSelector, diagnosticLocation.candidateSelector).forEach { token ->
            assertInstanceOf(
    ...
    @Test
    fun `zero width compiler diagnostic remains an exact source readable candidate`(@TempDir temporary: Path) {
        val root = Files.createDirectories(temporary.resolve("repo")).toRealPath()
        val fixture = InstalledSymbolProtocolFixture.create(root)
        val authority = CanonicalProtocolAuthority()
        val zeroWidth =
                ...
                DiagnosticCompilerPort { scope ->
                    val fact =
                        DiagnosticFact.fromBoundary(
                            ...
                            .refined()
                    DiagnosticCompilation.complete(DiagnosticBatch.create(scope, listOf(fact)).refined())
                },
            )
        val handler =
            CanonicalDiagnosticCheckHandler(fixture.workspace, zeroWidth, authority, singleFileDiagnosticScopes)

        val outcome =
            runSuspend {
                handler.execute(DiagnosticCheckRequest(text("src/main/kotlin/Sample.kt"), count(4)))
            }
                as OperationOutcome.Complete
        val location = outcome.evidence.payload.diagnostics.values.single().location
        val selector =
            (authority.candidate(location.candidateSelector) as CandidateSelectorLookup.Found).selector
    ...
    private fun discoverRequest(): SymbolDiscoverRequest =
