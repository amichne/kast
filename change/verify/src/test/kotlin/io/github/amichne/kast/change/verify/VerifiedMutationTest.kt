...
class VerifiedMutationTest {
    ...
    fun `verification records exact finite terminal stage while preserving result`() {
        ...
        signals.clear()
        val admission =
            VerifiedMutationService(publication, observation, observability)
            ...
            VerifiedMutationService(
                    ...
                    ChangeVerificationObserver {
                        ChangeVerificationObservation.Observed(
                            fixture.completeEvidence().copy(relations = listOf(fixture.qualifiedResultingRelation()))
                        )
                    },
                    observability,
                )
    ...
    fun `complete resulting proof is the only path to verified receipt`() {
        val publisher =
            FixedResultingGenerationPublisher(ResultingGenerationPublication.Published(fixture.resultingWorkspace))
        val observer = FixedVerificationObserver(ChangeVerificationObservation.Observed(fixture.completeEvidence()))
    ...
    fun `same generation cannot reach semantic observation or success`() {
        val publisher = FixedResultingGenerationPublisher(ResultingGenerationPublication.Published(fixture.workspace))
        val observer = FixedVerificationObserver(ChangeVerificationObservation.Observed(fixture.completeEvidence()))
    ...
    fun `incomplete relation coverage leaves obligations unmet`() {
        val evidence = fixture.completeEvidence().copy(relations = listOf(fixture.qualifiedResultingRelation()))
    ...
    fun `incomplete diagnostics and compiler errors cannot be clear`() {
        val incomplete = fixture.completeEvidence().copy(diagnostics = listOf(fixture.qualifiedResultingDiagnostics()))
        val errored = fixture.completeEvidence().copy(diagnostics = listOf(fixture.erroredResultingDiagnostics()))
    ...
    fun `diagnostics outside the exact mutation source scope cannot discharge the plan`() {
        val evidence = fixture.completeEvidence().copy(diagnostics = listOf(fixture.expandedResultingDiagnostics()))
    ...
    fun `changed existing relation semantics cannot be accepted delta`() {
        val evidence = fixture.completeEvidence().copy(relations = listOf(fixture.changedResultingRelation()))
    ...
    private fun service(evidence: AddDeclarationVerificationEvidence): VerifiedMutationService =
        service(
            FixedResultingGenerationPublisher(ResultingGenerationPublication.Published(fixture.resultingWorkspace)),
...
private class FixedResultingGenerationPublisher(private val result: ResultingGenerationPublication) :
...
private class FixedVerificationObserver(private val result: ChangeVerificationObservation) :
    ChangeVerificationObserver {
    var calls: Int = 0
