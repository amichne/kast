 ...
 *
 * Implementations may retain private persistence and admission handles, but only detached publication evidence and
 * opaque transaction capabilities cross this boundary.
 */
interface WorkspacePublicationAuthority {
    ...
    /**
     * Proof transition: `(OpenWorkspacePublication, WorkspaceStateIdentity, WorkspaceGraphPublication) ->
     * PreparedWorkspacePublication`.
     *
     * Establishes publication completeness and binds the reconciled identity without committing. Persistence failures
     * are unexpected adapter failures; raw store handles remain inside the implementation.
     */
    fun prepare(
