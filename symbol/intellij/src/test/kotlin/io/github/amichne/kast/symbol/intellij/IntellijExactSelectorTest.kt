...
class IntellijExactSelectorTest {
    ...
    fun `compiler signatures keep same name overloads exact`() {
        ...
        val firstSignature = SymbolDescription.from(firstSelector).signature as CanonicalCompilerSignature.Function
        val secondSignature = SymbolDescription.from(secondSelector).signature as CanonicalCompilerSignature.Function
    ...
    fun `root and generation drift are distinct closed selector rejections`() {
        ...
        assertEquals(
            ...
            (admitSymbolSelectorLease(current, moved) as IntellijSymbolSelectorLeaseAdmission.Rejected).reason,
        )
        assertEquals(
            IntellijSymbolSelectorRejection.WORKSPACE_ROOT_MISMATCH,
            (admitSymbolSelectorLease(current, otherRoot) as IntellijSymbolSelectorLeaseAdmission.Rejected).reason,
        )
    }
    ...
    fun `native query round trips identical compiler evidence and rejects changed identity`() {
        ...
        val selector =
            (query.resolve(compiled(selection.lease, selection.scope), selection)
                ...
                .selector
        val described =
            query.describe(compiled(selector.lease, selector.scope), selector)
                as IntellijSymbolDescriptionResolution.Described

        assertSame(selector, described.description.selector)
        val changed =
            query(IntellijCompilerSymbolLookupResult.Found(evidence(selection, "sample.Service.call(kotlin.String)")))
    ...
    private fun query(result: IntellijCompilerSymbolLookupResult): IntellijSymbolSelectorQuery =
