...
private data class WireActivityDocument(val stage: String, val outcome: String, val component: String = "kast-wire")

class JsonLineWireActivitySink(private val output: java.io.PrintStream) : WireActivitySink {
    ...
    override fun publish(activity: WireRequestActivity) {
        output.println(
            json.encodeToString(
                WireActivityDocument.serializer(),
                WireActivityDocument(activity.stage.name.lowercase(), activity.outcome.name.lowercase()),
            )
        )
    }
}
...
class WireIoDeadline(channel: SocketChannel, budget: ElapsedTimeLimitMillis) {
