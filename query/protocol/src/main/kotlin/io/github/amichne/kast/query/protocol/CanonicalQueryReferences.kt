...
class CanonicalQueryReferences private constructor(
    private val restoreSourceSelector: (SourceSelectorToken, SemanticReadAuthority) -> Refinement<SourceSelector, SourceSelectorTokenFailure>,
) : QueryReferenceAuthority {
    constructor() : this({ token, current ->
        when (current) {
    ...
    constructor(model: WorkspaceSearchScopeModel) : this({ token, current ->
        when (current) {
    ...
    override fun restoreSource(token: SourceSelectorToken, current: SemanticReadAuthority): Refinement<SourceSelector, SourceSelectorTokenFailure> {
        val decoded = restoreSourceSelector(token, current)
        return when (decoded) {
            is Refinement.Rejected -> decoded
            is Refinement.Refined -> if (decoded.value.snapshot.lease == current) decoded
                else Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
        }
    }

    override fun restoreCandidate(token: ProtocolText, current: SemanticReadAuthority): CanonicalSelectorDecoding<CandidateSelector> =
        CanonicalSelectorCodec.decodeCandidate(token, current)

    override fun restoreExact(token: ProtocolText, current: SemanticReadAuthority): CanonicalSelectorDecoding<SymbolSelector> =
        CanonicalSelectorCodec.decodeExact(token, current)
        ...
        batch.candidates.forEachIndexed { ordinal, candidate ->
            val selector = when (candidate.location) {
                ...
                is SymbolDiscoveryCandidateLocation.File -> when (
                    val selected = CandidateSelector.file(batch, ordinal)
                ) {
                    is Refinement.Refined -> selected.value
                    is Refinement.Rejected -> return CandidateSelectorIssuance.Rejected(
                        CandidateSelectorIssuanceFailure.CANDIDATE_REJECTED,
                ...
                is SymbolDiscoveryCandidateLocation.Text -> when (
                    val selected = CandidateSelector.range(batch, ordinal)
                ) {
                    is Refinement.Refined -> selected.value
                    is Refinement.Rejected -> return CandidateSelectorIssuance.Rejected(
                        CandidateSelectorIssuanceFailure.CANDIDATE_REJECTED,
    ...
    override fun issueDeclarationCandidate(
    ...
    override fun issueExact(selector: SymbolSelector): ExactSelectorIssuance = when (
