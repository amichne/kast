...
internal sealed interface RuntimeProcessStart {
    ...
    data class ExistingSession(val session: AcceptedRuntimeStartupSession) : RuntimeProcessStart
...
internal object LaunchdRuntimeProcessStarter : RuntimeProcessStarter {
    override fun start(command: IndexerLaunchCommand): RuntimeProcessStart = command.processSession.start(command)
}

/** One launchd service capability derived from an exact admitted runtime endpoint. */
internal class MacOsRuntimeProcessSession
private constructor(
    private val serviceLabel: String,
    private val launchctl: LaunchctlInvoker,
) : RuntimeProcessSession {
    /**
     * Proof transition: `MacOsRuntimeProcessSession + IndexerLaunchCommand -> RuntimeProcessStart`.
     *
     * Establishes that launchd accepted the exact indexer command or already owns its deterministic user-session
     * service, whose lifecycle is independent of the initiating CLI. [RuntimeProcessStart] closes existing ownership,
     * launch rejection, and interruption. Raw labels and arguments leave only at the launchctl boundary.
     */
    fun start(command: IndexerLaunchCommand): RuntimeProcessStart {
        when (observe()) {
            ...
            RuntimeSessionObservation.Rejected ->
                return RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.SessionObservationRejected)
            RuntimeSessionObservation.Absent -> Unit
        }
        val submission =
            when (val construction = launchctlSubmission(command)) {
                is MacOsLaunchctlSubmission.Ready -> construction.arguments
                is MacOsLaunchctlSubmission.Rejected ->
                    return RuntimeProcessStart.Rejected(construction.failure.toProcessStartFailure())
            }
        return when (launchctl.invoke(submission, LaunchctlExitContract.CompletionOnly)) {
            ...
            LaunchctlInvocation.Absent ->
                RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.SessionSubmissionRejected)
            LaunchctlInvocation.TimedOut ->
                RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.SessionSubmissionRejected)
            LaunchctlInvocation.Rejected -> startAfterRejectedSubmission(LaunchctlInvocation.Rejected)
        }
    }

    /**
     * Proof transition: `LaunchctlInvocation.Rejected + MacOsRuntimeProcessSession -> RuntimeProcessStart`.
     *
     * Refines a duplicate-submission race to [RuntimeProcessStart.ExistingSession] only when a second exact-label
     * observation proves launchd ownership. All unproven and interrupted states remain closed [RuntimeProcessStart]
     * failures.
     */
    private fun startAfterRejectedSubmission(rejected: LaunchctlInvocation.Rejected): RuntimeProcessStart =
        when (rejected) {
     ...
     *
     * Establishes the exact launchd submission plus its minimal admitted non-secret environment. The submitted wrapper
     * retires its own exact service after every child exit, so startup failure is bounded and observable instead of
     * acquiring crash-restart authority. [MacOsRuntimeProcessEnvironmentFailure] is the closed expected failure. Raw
            ...
            when (
                val resolution =
                    MacOsRuntimeProcessEnvironment.resolve(
                        ...
                        command.sidecarEnvironment.processVariables(),
                    )
            ) {
     ...
     * [RuntimeSessionObservation.Rejected] closes inaccessible service state. The raw label leaves only at the
     * launchctl boundary.
     */
    override fun observe(): RuntimeSessionObservation =
        when (
            ...
            LaunchctlInvocation.Rejected,
            LaunchctlInvocation.TimedOut -> RuntimeSessionObservation.Rejected
        }
    ...
    override fun terminate(): RuntimeProcessTermination =
        when (val observation = observe()) {
            ...
            RuntimeSessionObservation.Present ->
                when (retire(RuntimeSessionObservation.Present)) {
     ...
     *
     * Establishes that launchd accepted removal of the exact endpoint-derived service. [RuntimeSessionRetirement]
     * closes process rejection and interruption. The raw label leaves only at the launchctl boundary.
     */
        ...
        when (present) {
            RuntimeSessionObservation.Present ->
                when (
                    ...
                    LaunchctlInvocation.Absent,
                    LaunchctlInvocation.Rejected,
                    LaunchctlInvocation.TimedOut -> RuntimeSessionRetirement.Rejected
                }
        }
    ...
    companion object {
         ...
         *
         * Establishes one bounded launchd service identity derived from the exact canonical root, runtime identity, and
         * socket. Raw label extraction is permitted only by this launchctl adapter.
         */
        fun from(endpoint: RuntimeEndpoint): MacOsRuntimeProcessSession = from(endpoint, JdkLaunchctlInvoker)

        /**
         * Proof transition: `RuntimeEndpoint + LaunchctlInvoker -> MacOsRuntimeProcessSession`.
         *
         * Establishes the same exact service identity while substituting only the outer launchctl effect capability.
         * Raw label extraction remains inside this adapter.
         */
        internal fun from(
...
private fun MacOsRuntimeProcessEnvironmentFailure.toProcessStartFailure(): RuntimeProcessStartFailure =
    when (this) {
        MacOsRuntimeProcessEnvironmentFailure.JAVA_HOME_UNAVAILABLE -> RuntimeProcessStartFailure.IdeaJbrUnavailable
        ...
        MacOsRuntimeProcessEnvironmentFailure.USER_HOME_UNAVAILABLE -> RuntimeProcessStartFailure.UserHomeUnavailable
    }

private sealed interface MacOsLaunchctlSubmission {
    data class Ready(val arguments: List<String>) : MacOsLaunchctlSubmission

    data class Rejected(val failure: MacOsRuntimeProcessEnvironmentFailure) : MacOsLaunchctlSubmission
}
...
internal sealed interface LaunchctlExitContract {
    ...
    data class CompletionOrAbsent(val absentExitCode: Int) : LaunchctlExitContract
}
 ...
 *
 * Establishes launchctl's finite exit observation for one already assembled macOS command. [LaunchctlInvocation] closes
 * missing-service, process, security, and interruption states. Raw arguments leave only at [ProcessBuilder].
 */
