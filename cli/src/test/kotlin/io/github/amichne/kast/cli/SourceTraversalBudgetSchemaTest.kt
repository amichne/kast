...
class SourceTraversalBudgetSchemaTest {
    ...
    private val report = ExecutionBudgetReport.from(hostedSchemaBudgetGrant(ExecutionBudgetDocument(maxResults = ResultLimit.parse(999).proven())))

    @Test
    fun `source complete and qualified grants survive wire CLI and installed schemas`() {
        val evidence = EvidenceEnvelope(CanonicalOperation.SOURCE_READ.id, basis, fixture.sourceResult(basis).copy(executionBudget = report))
        val qualification = SourceReadQualification.create(
            SourceEntityCountDocument.parse(0).proven(), listOf(SourceReadLimitationDocument.TIME_LIMIT_REACHED), SourceReadContinuationStateDocument.Unavailable,
        ).proven()
        verify(CanonicalOperationWireBindings.sourceRead, OperationOutcome.Complete(evidence), CanonicalSourceReadCliDocuments::project)
        verify(CanonicalOperationWireBindings.sourceRead, OperationOutcome.Qualified(evidence, qualification), CanonicalSourceReadCliDocuments::project)
    }

    @Test
    fun `traversal complete and qualified grants survive wire CLI and installed schemas`() {
        val evidence = EvidenceEnvelope(CanonicalOperation.TRAVERSAL_RUN.id, basis, fixture.traversalResult().copy(executionBudget = report))
        val qualification = TraversalRunQualification.terminalIncomplete(listOf(TraversalLimitationDocument.TIME_LIMIT_REACHED), emptyList()).proven()
        verify(CanonicalOperationWireBindings.traversalRun, OperationOutcome.Complete(evidence), CanonicalReadCliDocuments::projectTraversal)
        verify(CanonicalOperationWireBindings.traversalRun, OperationOutcome.Qualified(evidence, qualification), CanonicalReadCliDocuments::projectTraversal)
    }

    private fun <Request : OperationRequest, Result : OperationResult, Qualification : OperationQualification, Rejection : OperationRejection> verify(
        ...
        assertRejects(binding.operation, document.with("execution_budget", budget.with("max_results", results.with("effective", JsonPrimitive(0)))))
    }
