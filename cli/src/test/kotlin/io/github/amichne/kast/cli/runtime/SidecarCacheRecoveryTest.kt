...
class SidecarCacheRecoveryTest {
    ...
    fun `rebuild quarantines every root identity only after every exact owner stops`() {
        ...
        assertEquals(
            setOf(fixture.endpoint.runtimeId, runtimeId('1'), runtimeId('2')),
            observed.map { it.runtimeId }.toSet(),
        )
    ...
    fun `an ambiguous process cannot produce quarantine authority`() {
        ...
        assertEquals(
            Refinement.Rejected(RuntimeAdmissionFailure.StopRejected(RuntimeStopFailure.PROCESS_AMBIGUOUS)),
            result,
        )
        ...
        val inventory =
            RootSidecarCacheInventory(
                ...
                listOf(
                    reference,
                    reference.copy(
                        cacheRoot = first.resolveSibling("other"),
                        ideaHome = temporary.resolve("other-idea"),
                    ),
                ),
            )
    ...
    private fun lifecycle(stop: (RuntimeEndpoint) -> RuntimeStopResult) =
    ...
    private fun fixture(): Fixture {
        ...
        val identity =
            (IdeRuntimeIdentity.admit(
                ...
                ) as IdeRuntimeIdentityAdmission.Admitted)
                .identity
        val runtime = InstalledIdeRuntime(home, java, identity)
        val release =
            (SidecarCacheReleaseIdentity.admit(pair, identity.kastPayloadDigest, runtimeId('9'))
                    as SidecarCacheReleaseIdentityAdmission.Admitted)
                .identity
        val lifecycle =
            FilesystemRootSidecarCacheLifecycle(
                cacheRoot,
                release,
                SidecarIdeRuntimeResolver { _, _, _ -> InstalledIdeRuntimeDiscoveryResult.Discovered(runtime) },
                importEnvironment = { Refinement.Refined(GradleImportEnvironment.Empty) },
            )
        val endpoint =
            (RuntimeEndpoint.at(CanonicalRoot(project), runtimeId('9'), temporary.resolve("base.sock"))
    ...
    private data class Fixture(
        ...
        fun addCache(character: Char): Path {
            val cache =
                (KastCacheIdentity.derive(project, runtime, runtimeId(character))
                        as KastCacheIdentityDerivation.Derived)
                    .identity
