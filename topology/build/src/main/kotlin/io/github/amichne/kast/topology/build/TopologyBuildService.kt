...
private class TopologyBuildAuthority {
    suspend fun execute(operation: suspend () -> TopologyBuildResult): TopologyBuildResult = operation()
}

private class ExactDurableTopologySnapshot private constructor(val snapshot: PublishedTopologySnapshot) {
    companion object {
        /**
         * Proof transition: `(TopologyWorkspaceIdentity, PublishedTopologySnapshot) ->
         * Refinement<ExactDurableTopologySnapshot, TopologyBuildFailure.SnapshotContractViolation>`.
         *
         * Establishes that the durable eligibility adapter returned the exact requested workspace identity. The closed
         * expected failure is [TopologyBuildFailure.SnapshotContractViolation]. Raw adapter output is consumed only at
...
private sealed interface TopologyGenerationRevalidationFailure {
    data class SourceEvidenceMoved(val path: WorkspaceSourcePath) : TopologyGenerationRevalidationFailure
...
private class RevalidatedTopologyGeneration private constructor(val generation: CompleteTopologyGeneration) {
    companion object {
        /**
         * Proof transition: `(TopologyCandidateSet, TopologyCandidateSet, CompleteTopologyGeneration) ->
         * Refinement<RevalidatedTopologyGeneration, TopologyGenerationRevalidationFailure>`.
         *
         * Establishes that the exact candidate paths, ownership, and content hashes did not move during extraction and
         * that the generation covers that stable evidence. The closed expected failure is
         * [TopologyGenerationRevalidationFailure]. Fresh physical evidence may enter only through the injected
        ...
        > {
            if (original.workspace != observed.workspace) {
                return Refinement.Rejected(TopologyGenerationRevalidationFailure.WorkspaceMismatch)
            }
            val originalFiles = original.files.associateBy { it.path }
            val observedFiles = observed.files.associateBy { it.path }
            val movedPath =
                (originalFiles.keys + observedFiles.keys).sortedBy(WorkspaceSourcePath::value).firstOrNull {
                    originalFiles[it] != observedFiles[it]
                }
            if (movedPath != null) {
                return Refinement.Rejected(TopologyGenerationRevalidationFailure.SourceEvidenceMoved(movedPath))
            }
            if (
                generation.identity != original.workspace ||
                    generation.files.map(CompleteTopologyFile::file) != original.files
            ) {
                return Refinement.Rejected(TopologyGenerationRevalidationFailure.GenerationMismatch)
            }
...
private constructor(
    ...
    private suspend fun buildAuthorized(trace: KastTraceSpan): TopologyBuildResult {
        val workspace =
            when (val state = workspaces.inspect()) {
                ...
                WorkspaceRuntimeState.Stopping -> return rejected(TopologyBuildFailure.WorkspaceNotReady)
            }
        val identity = TopologyWorkspaceIdentity.from(workspace)
        val eligibilityUse =
            trace.child(KastSpanName.TOPOLOGY_SNAPSHOT_ELIGIBILITY) { span ->
                leaseGuard
                    .whileCurrent(workspace.readLease) {
                        ...
                        span.observe(
                            when (guarded) {
                                SemanticReadLeaseUse.Moved ->
                                    rejectedObservation(KastSpanFailure.TOPOLOGY_WORKSPACE_MOVED)
                                is SemanticReadLeaseUse.Completed ->
                                    when (guarded.value) {
                                        is TopologySnapshotEligibility.Rejected ->
                                            rejectedObservation(KastSpanFailure.TOPOLOGY_SNAPSHOT)
                                        is TopologySnapshotEligibility.Eligible,
                                        is TopologySnapshotEligibility.Stale,
                                        TopologySnapshotEligibility.Unavailable -> completeObservation()
                                    }
                            }
                        )
                    }
        ...
        val prior =
            when (val existing = eligibility) {
                ...
                is TopologySnapshotEligibility.Stale -> PriorTopologySnapshot.Stale(existing.latest)
                TopologySnapshotEligibility.Unavailable -> PriorTopologySnapshot.Unavailable
            }
        val candidateSet =
            when (val enumeration = enumerate(workspace, trace)) {
        ...
        when (prior) {
            is PriorTopologySnapshot.Stale -> {
                val content =
                    when (
                        val read =
                            trace.child(KastSpanName.TOPOLOGY_SNAPSHOT_READ) { span ->
                ...
                when (val reuse = rebindUnchangedTopologyGeneration(workspace, candidateSet, content)) {
                    is TopologyGenerationReuse.Rebound ->
                    ...
                    TopologyGenerationReuse.Rejected -> return rejected(TopologyBuildFailure.SnapshotContractViolation)
        ...
        val extractionFailure =
            trace.child(KastSpanName.TOPOLOGY_EXTRACTION) { span ->
                for (file in candidateSet.files) {
                    val request =
                        when (val admitted = candidateSet.extractionRequest(file)) {
                ...
                span.observe(completeObservation(KastSpanMeasurement.FileCount(exactCount(completed.size))))
                null
            }
        ...
        val generation =
            when (
                ...
                is Refinement.Refined -> admitted.value
                is Refinement.Rejected -> return rejected(TopologyBuildFailure.Coverage(admitted.failure))
            }
        return publishCompleteGeneration(
            workspace,
            candidateSet,
    ...
    private suspend fun publishCompleteGeneration(
        ...
        val validation =
            trace.child(KastSpanName.TOPOLOGY_REVALIDATION) { span ->
                        ...
                        span.observe(
                            when (result) {
                                is Refinement.Refined ->
                                    completeObservation(
                                        KastSpanMeasurement.FileCount(exactCount(result.value.generation.files.size))
                                    )
                                is Refinement.Rejected -> rejectedObservation(KastSpanFailure.TOPOLOGY_EXTRACTION)
                            }
                        )
                    }
            }
        val stableGeneration =
            when (validation) {
                is Refinement.Refined -> validation.value
                is Refinement.Rejected ->
                    return when (val failure = validation.failure) {
                        is TopologyGenerationRevalidationFailure.SourceEvidenceMoved ->
                            rejected(
                                TopologyBuildFailure.Extraction(
                                    failure.path,
        ...
        val publicationUse =
            trace.child(KastSpanName.TOPOLOGY_PUBLICATION) { span ->
                leaseGuard
                    .whileCurrent(workspace.readLease) {
                            ...
                            when (guarded) {
                                ...
                                is SemanticReadLeaseUse.Completed ->
                                    when (guarded.value) {
                                        ...
                                        is TopologyPublicationResult.Unchanged -> completeObservation()
                                        is TopologyPublicationResult.Rejected ->
                                            rejectedObservation(KastSpanFailure.TOPOLOGY_PUBLICATION)
                                    }
                            }
                        )
                    }
        ...
        return when (val guarded = publicationUse) {
            ...
            is SemanticReadLeaseUse.Completed ->
                when (val publication = guarded.value) {
                    is TopologyPublicationResult.Published -> {
                        when (mode) {
                            TopologyPublicationMode.EXTRACTED -> TopologyBuildResult.Published(publication.snapshot)
                            TopologyPublicationMode.REBOUND -> TopologyBuildResult.Reused(publication.snapshot)
                        }
                    }
                    is TopologyPublicationResult.Unchanged -> TopologyBuildResult.Reused(publication.snapshot)
                    is TopologyPublicationResult.Rejected ->
                        rejected(TopologyBuildFailure.Publication(publication.failure))
                }
        }
    }
    ...
    companion object {
        /**
         * Proof transition: `(workspace observation, current-lease guard, admitted-root enumerator, K2 extractor,
         * snapshot reader and publisher) -> TopologyBuildOperations`.
         *
         * Establishes the sole explicit build service carrying private [TopologyBuildAuthority]. Platform and
         * persistence effects remain behind their injected ports. Raw construction is permitted only in runtime
         * composition while binding `topology.build`.
         */
        fun create(
    ...
    private suspend fun enumerate(
        ...
        trace.child(KastSpanName.TOPOLOGY_CANDIDATE_ENUMERATION) { span ->
            candidates.enumerate(workspace).also { result ->
                span.observe(
                    when (result) {
                        is TopologyCandidateEnumeration.Complete ->
                            completeObservation(KastSpanMeasurement.FileCount(exactCount(result.candidates.files.size)))
                        is TopologyCandidateEnumeration.Rejected ->
                            rejectedObservation(KastSpanFailure.TOPOLOGY_ENUMERATION)
                    }
                )
            }
        }
...
private enum class TopologyPublicationMode {
    EXTRACTED,
    REBOUND,
}
...
private fun TopologyBuildResult.traceObservation(): KastSpanObservation =
    when (this) {
        is TopologyBuildResult.Published ->
            completeObservation(KastSpanMeasurement.FileCount(exactCount(snapshot.manifest.cardinalities.files)))
        is TopologyBuildResult.Reused ->
            completeObservation(KastSpanMeasurement.FileCount(exactCount(snapshot.manifest.cardinalities.files)))
        TopologyBuildResult.WorkspaceMoved -> rejectedObservation(KastSpanFailure.TOPOLOGY_WORKSPACE_MOVED)
        is TopologyBuildResult.Rejected ->
            rejectedObservation(
                when (failure) {
                    TopologyBuildFailure.WorkspaceNotReady -> KastSpanFailure.TOPOLOGY_WORKSPACE_NOT_READY
                    TopologyBuildFailure.SnapshotContractViolation,
                    is TopologyBuildFailure.SnapshotRead -> KastSpanFailure.TOPOLOGY_SNAPSHOT
                    is TopologyBuildFailure.Enumeration -> KastSpanFailure.TOPOLOGY_ENUMERATION
                    is TopologyBuildFailure.Extraction,
                    TopologyBuildFailure.ExtractionContractViolation -> KastSpanFailure.TOPOLOGY_EXTRACTION
...
private fun completeObservation(vararg measurements: KastSpanMeasurement): KastSpanObservation =
    KastSpanObservation(
...
private fun TopologyIdentityStage.traceStage(): KastTopologyIdentityStage =
    when (this) {
...
private fun TopologyBindingFailure.traceReason(): KastTopologyBindingFailure =
    when (this) {
...
private fun exactCount(value: Int): KastSpanCount =
    when (val count = KastSpanCount.parse(value.toLong())) {
