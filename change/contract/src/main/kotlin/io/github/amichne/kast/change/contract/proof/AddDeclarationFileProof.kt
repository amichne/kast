...
data class ExactFileContentProof
private constructor(
    ...
    companion object {
        /**
         * Proof transition: String SHA-256 and String Base64 to Refinement of ExactFileContentProof or
         * ExactFileContentProofFailure.
         *
         * Establishes canonical Base64 whose exact decoded bytes hash to the declared SHA-256.
         * ExactFileContentProofFailure is the closed expected failure. Raw bytes may be extracted only by an apply or
         * recovery adapter after later plan admission.
         */
        fun admit(
            sha256: String,
            ...
            val bytes =
                runCatching { Base64.getDecoder().decode(contentBase64) }.getOrNull()
            ...
            return Refinement.Refined(ExactFileContentProof(AddDeclarationSha256.fromProvenRaw(sha256), contentBase64))
        }
    }
}
...
private constructor(
    ...
    companion object {
        /**
         * Proof transition: AddDeclarationTargetCapability and exact images to Refinement of ExpectedFileProof or
         * ExpectedFileProofFailure.
         *
         * Establishes the exact existing target before-image and distinct authorized after-image.
         * ExpectedFileProofFailure is the closed expected failure. Image bytes may be extracted only by later recovery
...
data class DeclaredWriteSet private constructor(val paths: List<AddDeclarationTargetPath>) {
    companion object {
        /**
         * Proof transition: List of AddDeclarationTargetPath to Refinement of DeclaredWriteSet or
         * DeclaredWriteSetFailure.
         *
         * Establishes a non-empty, unique, canonical-order declared write set. DeclaredWriteSetFailure is the closed
         * expected failure. Raw paths may be extracted only by the characterized write-set observer.
         */
        fun admit(paths: List<AddDeclarationTargetPath>): Refinement<DeclaredWriteSet, DeclaredWriteSetFailure> {
