...
class CanonicalQueryRunHandlerTest {
        ...
        val handler =
            CanonicalQueryRunHandler(
                ...
                QueryOperations { request ->
                    ...
                    val names =
                        scope.sourceSets as io.github.amichne.kast.symbol.contract.SymbolDiscoverySourceSets.Exact
    ...
    @Test
    fun `candidate relation is rejected by typed admission before semantic execution`(@TempDir root: Path) = runTest {
        var executed = false
        val handler =
            CanonicalQueryRunHandler(
                InstalledSymbolProtocolFixture.create(root).workspace,
        ...
        assertEquals(
            ...
            handler.execute(
                request(candidateSource(), listOf(QueryStepDocument.Related(RelationKindDocument.CALLERS)))
            ),
        )
    ...
    @Test
    fun `unsupported declaration kind is rejected before semantic execution`(@TempDir root: Path) = runTest {
        var executed = false
        val handler =
            CanonicalQueryRunHandler(
                InstalledSymbolProtocolFixture.create(root).workspace,
    ...
    @Test
    fun `exact reference source distinguishes wrong family from malformed token`(@TempDir root: Path) = runTest {
        val handler =
            CanonicalQueryRunHandler(
                InstalledSymbolProtocolFixture.create(root).workspace,
    ...
    @Test
    fun `returned candidate and exact refs are accepted verbatim by their typed stages`(@TempDir root: Path) = runTest {
        ...
        val candidateResult =
            handler.execute(
                request(
                    ...
                    emptyList(),
                    QueryOutputDocument.Candidates(bounded(listOf(QueryCandidateFieldDocument.NAME))),
                )
            ) as OperationOutcome.Complete
        val candidateRef =
            (candidateResult.evidence.payload.items.values.single() as QueryResultItemDocument.Candidate).ref
        ...
        val first = handler.execute(request(symbolSource(), emptyList())) as OperationOutcome.Complete
        val exactRef = (first.evidence.payload.items.values.single() as QueryResultItemDocument.ExactSymbol).ref
        val second =
            handler.execute(
                request(
        ...
        assertEquals(
            exactRef,
            ((second as OperationOutcome.Complete).evidence.payload.items.values.single()
                    as QueryResultItemDocument.ExactSymbol)
                .ref,
        )
    }
    ...
    private suspend fun assertReferenceRejection(
        ...
        val source = QueryFromDocument.References(bounded(listOf(QueryReferenceDocument.ExactSymbol(text(token)))))
        assertEquals(
    ...
    private fun candidateSymbolSource() =
        QueryFromDocument.Candidates(
            QueryDiscoveryDocument(
                QueryMatchDocument.All,
                QueryScopeDocument(bounded(listOf(text("main"))), null, null),
    ...
    private fun request(
        ...
        output: QueryOutputDocument = QueryOutputDocument.Symbols(bounded(listOf(QuerySymbolFieldDocument.NAME))),
    ) =
        QueryRunRequest(
            from,
            bounded(steps),
