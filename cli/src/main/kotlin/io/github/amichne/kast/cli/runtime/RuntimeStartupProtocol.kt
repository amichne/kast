 ...
 *
 * Raw paths may leave only at indexer process launch. In particular, the installed IDEA home can never be selected as a
 * writable sidecar state or private-plugin path.
 */
class SidecarLaunchContext
private constructor(
    ...
    companion object {
        fun admit(
            ...
            maxHeap: io.github.amichne.kast.distribution.contract.IndexerHeapSize =
            ...
            val root =
                canonicalDirectory(cacheRoot)
                    ?: return SidecarLaunchContextAdmission.Rejected(SidecarLaunchContextFailure.PATH_INVALID)
            val paths =
                listOf(
                        systemDirectory,
                        configDirectory,
            ...
            if (paths.distinct().size != paths.size || pathsOverlap(paths)) {
                return SidecarLaunchContextAdmission.Rejected(SidecarLaunchContextFailure.PATHS_OVERLAP)
            }
            if (paths.take(3).any { it.parent != root }) {
                return SidecarLaunchContextAdmission.Rejected(SidecarLaunchContextFailure.PATHS_OVERLAP)
            }
            if (paths.any { it.startsWith(runtime.home) || runtime.home.startsWith(it) }) {
                return SidecarLaunchContextAdmission.Rejected(SidecarLaunchContextFailure.USER_IDEA_HOME_TARGETED)
            }
        ...
        private fun pathsOverlap(paths: List<Path>): Boolean =
