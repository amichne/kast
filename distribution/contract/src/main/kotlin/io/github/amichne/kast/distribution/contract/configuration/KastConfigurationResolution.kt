...
data class ConfigurationSources(
    ...
    override fun toString(): String = "ConfigurationSources(environmentKeys=${environment.keys.size}, savedInstallationAssignments=${savedInstallation.size}, savedWorkspaceAssignments=${savedWorkspace.size}, commandLineAssignments=${commandLine.size})"
}

@Serializable enum class ConfigurationFailure {
    UNKNOWN_KEY, DUPLICATE_ASSIGNMENT, UNSUPPORTED_SOURCE, TEST_ONLY_INPUT, BUILD_ONLY_INPUT,
    INVALID_VALUE, INVALID_PATH, INVALID_HEAP, DELEGATED_ENVIRONMENT_REJECTED, SAVED_CONFIGURATION_REJECTED,
}

/** Only a bounded setting identity and finite condition leave rejection; never raw values. */
data class ConfigurationRejection internal constructor(
    val key: String,
...
enum class ConfigurationSwitch { ENABLED, DISABLED }
enum class ConfigurationOwner(val module: String) { APP_SERVER(":app-server") }

/** Declared accounting limits; these values do not establish physical memory availability. */
@JvmInline value class WorkerCountLimit private constructor(val value: Int) {
    companion object {
...
@JvmInline value class WorkerMemoryReservationMiB private constructor(val value: Long) {
    companion object {
...
class WorkerCapacityConfiguration internal constructor(
...
@Serializable enum class ConfigurationSemanticAdmission { OWNER_REQUIRED, PHYSICAL_REQUIRED, ADMITTED }
...
class ConfigurationOwnerCandidate internal constructor(
    val parameter: ConfigurationParameter,
    val source: ConfigurationSource,
...
class ResolvedKastConfiguration private constructor(
    ...
    val runtimeStore: ConfigurationPathSelection get() = path(ConfigurationParameter.RUNTIME_STORE)
    val runtimeDirectory: ConfigurationPathSelection get() = path(ConfigurationParameter.RUNTIME_DIRECTORY)
    val cacheRoot: ConfigurationPathSelection get() = path(ConfigurationParameter.CACHE_ROOT)

    private fun path(parameter: ConfigurationParameter): ConfigurationPathSelection = when (
        val assignment = assignments.singleOrNull { it.parameter == parameter }
    ) {
        null -> ConfigurationPathSelection.OwnerDefault
        else -> when (val value = assignment.value) {
    ...
    fun inspection(): List<ConfigurationInspection> = assignments.map { assignment ->
        ...
        ConfigurationInspection(
            declaration.key, assignment.source,
            ...
            assignment.overriddenSources, declaration.applicationBoundary,
    ...
    fun childIdentityInputs(child: ConfigurationChild): Map<String, String> = childProjection(child, reloadSaved = false)
        .mapValues { (key, value) ->
    ...
    private fun childProjection(child: ConfigurationChild, reloadSaved: Boolean): Map<String, String> = buildMap {
        val reloadInstallation = reloadSaved && child == ConfigurationChild.BROKER &&
            assignments.any { it.parameter == ConfigurationParameter.CONFIGURATION_FILE }
        for (assignment in assignments) {
            if (child in assignment.parameter.children && (!reloadSaved || assignment.source != ConfigurationSource.DEFAULT) &&
        ...
        if (delegatedEnvironment.evidence.isNotEmpty() || delegatedEnvironment.executableDirectories.isNotEmpty()) {
            ...
            if (!(reloadInstallation && assignments.any { it.parameter == ConfigurationParameter.GRADLE_IMPORT_VARIABLES && it.source == ConfigurationSource.SAVED_INSTALLATION }))
                put(GradleImportEnvironment.VARIABLES_SETTING, delegatedEnvironment.evidence.joinToString(",") { it.name.value })
            if (!(reloadInstallation && assignments.any { it.parameter == ConfigurationParameter.GRADLE_IMPORT_PATH && it.source == ConfigurationSource.SAVED_INSTALLATION }))
                put(GradleImportEnvironment.PATH_SETTING, delegatedEnvironment.executableDirectories.joinToString(":") { it.path.toString() })
        }
    }

    /** Shadowed syntax-admitted inputs must also receive canonical owner admission; invalid inputs never disappear. */
    fun ownerCandidates(owner: ConfigurationOwner): List<ConfigurationOwnerCandidate> = suppliedCandidates
    ...
    fun ownerInputs(owner: ConfigurationOwner): Map<String, String> = assignments
        ...
        fun resolve(sources: ConfigurationSources): Refinement<ResolvedKastConfiguration, ConfigurationRejection> {
            ...
            for ((source, inputs) in orderedSources) {
                ...
                for ((rawKey, rawValue) in inputs) {
                    ...
                    if (parameter.mutability == ConfigurationMutability.TEST_ONLY) return rejected(rawKey, ConfigurationFailure.TEST_ONLY_INPUT)
                    ...
                    admitted.getOrPut(parameter) { mutableListOf() }.add(ResolvedAssignment(parameter, value, source, emptyList()))
                }
            }
            ...
            for (parameter in ConfigurationParameter.entries) {
                ...
                if (supplied.isNotEmpty()) {
                    ...
                    val value = when (val parsed = parse(parameter, default)) {
                        is Refinement.Refined -> parsed.value
                        is Refinement.Rejected -> return parsed
                    }
            ...
            val environment = if (gradleHome == null) sources.environment else sources.environment + ("GRADLE_USER_HOME" to gradleHome)
            val delegated = when (val parsed = GradleImportEnvironment.admit(names,
                byKey.getValue(ConfigurationParameter.GRADLE_IMPORT_PATH).value.boundaryValue(), environment)) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected -> return rejected(GradleImportEnvironment.VARIABLES_SETTING, ConfigurationFailure.DELEGATED_ENVIRONMENT_REJECTED)
            }
            ...
            val capacity = WorkerCapacityConfiguration(count(ConfigurationParameter.WORKER_RESIDENT_LIMIT),
...
private fun ConfigurationValue.boundaryValue(): String = when (this) {
    ...
    is ConfigurationValue.Switch -> when (value) { ConfigurationSwitch.ENABLED -> "1"; ConfigurationSwitch.DISABLED -> "0" }
...
private fun parse(parameter: ConfigurationParameter, raw: String): Refinement<ConfigurationValue, ConfigurationRejection> {
    ...
    return when (parameter.syntax) {
        ...
        ConfigurationSyntax.MEMORY_MIB -> when (val value = raw.toLongOrNull()?.let(WorkerMemoryReservationMiB::admit)) {
            is Refinement.Refined -> Refinement.Refined(ConfigurationValue.Memory(value.value))
            else -> rejected(parameter.key, ConfigurationFailure.INVALID_VALUE)
        }
        ConfigurationSyntax.HEAP -> when (val heap = IndexerHeapSize.parse(raw)) {
            is Refinement.Refined -> Refinement.Refined(ConfigurationValue.Heap(heap.value))
            is Refinement.Rejected -> Refinement.Rejected(ConfigurationRejection(parameter.key,
                ConfigurationFailure.INVALID_HEAP, ConfigurationRejectionDetail.Heap(heap.failure)))
        }
        ConfigurationSyntax.SWITCH -> when (raw) {
        ...
        ConfigurationSyntax.ABSOLUTE_PATH -> {
            val path = try { Path.of(raw) } catch (_: InvalidPathException) { return rejected(parameter.key, ConfigurationFailure.INVALID_PATH) }
            if (raw.isBlank() || raw.length > 4096 || !path.isAbsolute || path.normalize() != path) rejected(parameter.key, ConfigurationFailure.INVALID_PATH)
            else Refinement.Refined(ConfigurationValue.Directory(path))
        }
        ConfigurationSyntax.OWNER_INPUT -> if (raw.isBlank()) rejected(parameter.key, ConfigurationFailure.INVALID_VALUE)
        ...
        ConfigurationSyntax.VARIABLE_NAMES -> {
            ...
            if (names.size > 64 || names.toSet().size != names.size || names.any { GradleImportVariableName.parse(it) is Refinement.Rejected }) rejected(parameter.key, ConfigurationFailure.DELEGATED_ENVIRONMENT_REJECTED)
            else Refinement.Refined(ConfigurationValue.OwnerInput(raw))
        }
        ConfigurationSyntax.EXECUTABLE_PATH -> {
            val paths = if (raw.isEmpty()) emptyList() else raw.split(':')
            if (paths.size > 32 || paths.toSet().size != paths.size || paths.any { GradleImportExecutableDirectory.parse(it) is Refinement.Rejected }) rejected(parameter.key, ConfigurationFailure.DELEGATED_ENVIRONMENT_REJECTED)
            else Refinement.Refined(ConfigurationValue.OwnerInput(raw))
        }
...
private fun rejected(key: String, failure: ConfigurationFailure): Refinement.Rejected<ConfigurationRejection> =
    Refinement.Rejected(ConfigurationRejection(key.take(128).filter { it.isLetterOrDigit() || it == '_' || it == '.' }, failure))
