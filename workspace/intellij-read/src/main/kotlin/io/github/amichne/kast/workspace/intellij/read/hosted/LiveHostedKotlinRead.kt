...
internal fun readHostedKotlin(
    ...
    val virtualFile =
        LocalFileSystem.getInstance().findFileByPath(selection.file.path.value)
            ?: return rejected(HostedQueryFailure.FILE_UNAVAILABLE)
    if (virtualFile.length > limits[ReadLimitParameter.HOST_FILE_CHARACTERS].value * 4L)
        return rejected(HostedQueryFailure.FILE_TOO_LARGE)
    if (virtualFile.canonicalPath != virtualFile.path) return rejected(HostedQueryFailure.OUTSIDE_SCOPE)
    val document =
        FileDocumentManager.getInstance().getDocument(virtualFile)
            ?: return rejected(HostedQueryFailure.FILE_UNAVAILABLE)
    if (document.textLength > limits[ReadLimitParameter.HOST_FILE_CHARACTERS].value)
        return rejected(HostedQueryFailure.FILE_TOO_LARGE)
    val file =
        PsiManager.getInstance(project).findFile(virtualFile) as? KtFile
            ?: return rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
    val element =
        file.findElementAt(selection.nameOffset.value) ?: return rejected(HostedQueryFailure.INVALID_SELECTION)
    val declaration =
    ...
    val entry =
        declaration.superTypeListEntries.singleOrNull() ?: return rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
    val reference =
        (entry.typeReference?.typeElement as? KtUserType)?.referenceExpression?.mainReference
            ?: return rejected(HostedQueryFailure.UNRESOLVED_SUPERTYPE)
    return analyze(declaration) {
        val subject =
            declaration.symbol as? KaNamedClassSymbol
                ?: return@analyze rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
        val parent =
            reference.resolveToSymbol() as? KaNamedClassSymbol
        ...
        val parentDeclaration =
            parent.psi as? KtClassOrObject ?: return@analyze rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
        val child =
            when (val result = detachDeclaration(project, model, declaration, subject, limits = limits)) {
                is Refinement.Refined -> result.value
                is Refinement.Rejected -> return@analyze rejected(result.failure)
            }
        val base =
            when (val result = detachDeclaration(project, model, parentDeclaration, parent, limits = limits)) {
                is Refinement.Refined -> result.value
                is Refinement.Rejected -> return@analyze rejected(result.failure)
            }
        ProgressManager.checkCanceled()
...
internal fun verifyHostedDeclarations(project: Project, declarations: List<HostedCompilerDeclaration>): SavedDocuments {
    ...
    for (declaration in declarations) {
        val file =
            LocalFileSystem.getInstance().findFileByPath(declaration.symbol.file.stableValue)
                ?: return SavedDocuments.Rejected(HostedQueryFailure.CONTENT_MOVED)
        val document =
            FileDocumentManager.getInstance().getCachedDocument(file)
                ?: return SavedDocuments.Rejected(HostedQueryFailure.CONTENT_MOVED)
        if (
            file.modificationStamp != declaration.content.vfsStamp.value ||
...
internal fun detachDeclaration(
    ...
    val virtualFile =
        declaration.containingFile.virtualFile ?: return Refinement.Rejected(HostedQueryFailure.FILE_UNAVAILABLE)
    ...
    val file =
        when (
            ...
            is Refinement.Refined ->
                result.value as? SymbolDiscoveryFileIdentity.Workspace
    ...
    val scope =
        when (val admitted = HostedSourceScope.admit(model, file)) {
            is Refinement.Refined -> admitted.value
            is Refinement.Rejected -> return admitted
        }
    if (virtualFile.length > limits[ReadLimitParameter.HOST_FILE_CHARACTERS].value * 4L)
        return Refinement.Rejected(HostedQueryFailure.FILE_TOO_LARGE)
    val document =
        FileDocumentManager.getInstance().getDocument(virtualFile)
    ...
    if (FileDocumentManager.getInstance().isFileModified(virtualFile))
        return Refinement.Rejected(HostedQueryFailure.DIRTY_DOCUMENTS)
    if (!PsiDocumentManager.getInstance(project).isCommitted(document))
        return Refinement.Rejected(HostedQueryFailure.UNCOMMITTED_DOCUMENTS)
    val qualifiedName =
        symbol.classId?.asSingleFqName()?.asString()
            ?: return Refinement.Rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
    val signature =
        when (val result = CanonicalCompilerSignature.classLike(qualifiedName)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return Refinement.Rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
        }
    val content =
        when (val revision = HostedContentRevision.observe(document.modificationStamp, virtualFile.modificationStamp)) {
