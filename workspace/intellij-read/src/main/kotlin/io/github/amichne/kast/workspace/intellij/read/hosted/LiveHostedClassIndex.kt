...
internal fun readHostedClassIndex(
    project: Project,
    lookup: HostedClassLookup,
    model: DetachedIdeWorkspaceModel,
    limits: ReadLimits = ReadLimits.Default,
): HostedSemanticRead<HostedIndexedClasses> {
    ...
    val complete =
        KotlinClassShortNameIndex.processElements(lookup.name.value, project, HostedIndexScope(project, model)) {
            declaration ->
            ProgressManager.checkCanceled()
            candidates.accept(declaration) == HostedIndexCollection.CONTINUE
        }
    val found =
        when (val collection = candidates.finish()) {
    ...
    return HostedSemanticRead.Resolved(
        HostedIndexedClasses(
            lookup,
            detached.sortedWith(
                compareBy(
                    { it.symbol.compilerIdentity.value },
                    { it.symbol.file.stableValue },
                    { it.symbol.range.startInclusive },
                )
            ),
        )
    )
...
internal class HostedIndexScope(project: Project, model: DetachedIdeWorkspaceModel) : GlobalSearchScope(project) {
    ...
    override fun contains(file: VirtualFile): Boolean =
        file.isValid && file.fileSystem.protocol == "file" && roots.any { Path.of(file.path).startsWith(it) }
