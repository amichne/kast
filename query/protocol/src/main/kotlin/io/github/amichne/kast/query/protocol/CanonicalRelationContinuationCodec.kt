...
sealed interface CanonicalRelationContinuationDecoding {
    data class Decoded(val continuation: RelationContinuation) : CanonicalRelationContinuationDecoding
...
object CanonicalRelationContinuationCodec {
    ...
    fun decode(
        document: RelationContinuationDocument,
        current: SemanticReadAuthority,
    ): CanonicalRelationContinuationDecoding {
        val authority = current.identity
        val encoded = document.value.split(':').getOrNull(2) ?: return CanonicalRelationContinuationDecoding.Malformed
        val fields =
            try {
        ...
        val subject =
            RelationEndpointFingerprint.parse(fields[0]).refinedOrNull()
                ?: return CanonicalRelationContinuationDecoding.Malformed
        val meaning = fields[1].relationMeaning() ?: return CanonicalRelationContinuationDecoding.Malformed
        val scope =
            RelationScopeFingerprint.parse(fields[2]).refinedOrNull()
                ?: return CanonicalRelationContinuationDecoding.Malformed
        ...
        val provider =
            RelationProviderKind.entries.singleOrNull { it.name == fields[4] }
                ?: return CanonicalRelationContinuationDecoding.Malformed
        val positionRaw = fields[5].toLongOrNull() ?: return CanonicalRelationContinuationDecoding.Malformed
        val position =
            RelationProviderPosition.parse(positionRaw).refinedOrNull()
                ?: return CanonicalRelationContinuationDecoding.Malformed
        val prefix =
            RelationProviderPrefixDigest.parse(fields[6]).refinedOrNull()
                ?: return CanonicalRelationContinuationDecoding.Malformed
        val fingerprint =
...
private fun RelationMeaning.tokenName(): String =
    when (this) {
...
private fun String.relationMeaning(): RelationMeaning? =
    when (this) {
