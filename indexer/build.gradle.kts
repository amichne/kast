...
val extractIdeaDistribution: TaskProvider<ExtractIdeaDistributionTask> by
...
@Suppress("UNCHECKED_CAST") val buildVersion: Provider<String> = extra["buildVersion"] as Provider<String>

val launcherClassEntries =
    listOf(
...
val syncIndexerPluginPayload by
...
tasks.named<WriteWrapperScriptTask>("writeWrapperScript") {
    ...
    scriptContent.set(
        providers
            .fileContents(layout.projectDirectory.file("src/main/scripts/kast-indexer"))
            .asText
            .map(String::trimEnd)
    )
}

val indexerRuntimeRequiredClassEntries =
    listOf(
        "io/github/amichne/kast/indexer/KastIndexerMainKt.class",
        "io/github/amichne/kast/indexer/KastIndexerBootstrap.class",
    )
...
val indexerPluginRequiredClassEntries =
    listOf(
...
val verifyPortableDistLayout by
    tasks.registering(VerifyClasspathLayoutTask::class) {
        group = "verification"
...
val testIndexerLauncherIsolation by
    tasks.registering(Exec::class) {
        ...
        val testScript = layout.projectDirectory.file("src/test/scripts/test-kast-indexer-isolation.sh")
