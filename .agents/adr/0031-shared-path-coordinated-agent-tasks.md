# ADR 0031: Shared serialized-mutation agent tasks
...
scheduling contract by defining one serialized workspace lane.
...
different concurrency needs. Independent worktrees remain independent, while
mutations within one runtime workspace favor predictable ordering over
path-level concurrency.
...
- `repair` is idempotent. For a dead finisher, it releases only the matching barrier token,
...
by the Rust CLI. One in-memory coordinator serializes all applied mutations in
that runtime workspace. It binds each task, idempotency key, and request
fingerprint to its active worker or cached terminal result. The worker remains
server-owned when its waiting client disconnects.

Finish is a generation-coherent barrier in that same coordinator. It rejects
new idempotency keys before reservation and drains active workers. Runtime
restart forgets deduplication state; an in-flight task bound to the replaced
runtime fails closed instead of replaying its mutation.
