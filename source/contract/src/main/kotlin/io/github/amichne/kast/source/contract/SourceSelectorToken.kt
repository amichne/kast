...
@JvmInline
value class SourceSelectorToken private constructor(val value: String) {
    companion object {
        fun parse(raw: String): Refinement<SourceSelectorToken, SourceSelectorTokenFailure> {
...
object SourceSelectorTokenCodec {
    ...
    private fun decodePayload(
        ...
        if (
            (prefix == SOURCE_SELECTOR_TOKEN_PREFIX && version != SOURCE_SELECTOR_PAYLOAD_VERSION) ||
        ...
        if (authorityKind !in setOf("published", "live"))
            return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val rootText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        ...
        val generationText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val sourceStateText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val fileText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val textIdentityText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val lengthText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        val readScope =
            if (extended) {
                decodeReadScope(
                    rootText,
                    fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD),
                    scopes,
                ) ?: return Refinement.Rejected(SourceSelectorTokenFailure.SNAPSHOT_REJECTED)
            } else SourceReadScope.ExactFile
        val depthText = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        ...
        val depth =
            depthText.toCanonicalIntOrNull() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        if (depth < 1) {
            return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
        }
        ...
        repeat(depth) { index ->
            val variant = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val kind = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val start =
                fields.read()?.toCanonicalIntOrNull()
                    ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val end =
                fields.read()?.toCanonicalIntOrNull()
                    ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val nameState = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val nameValue = fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val fingerprintText =
                fields.read() ?: return Refinement.Rejected(SourceSelectorTokenFailure.MALFORMED_PAYLOAD)
            val range =
                admitRange(snapshot, start, end)
                    ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
            val fingerprint =
                when (val parsed = SourceSelectorFingerprint.parse(fingerprintText)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected -> return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                }
            current =
                when (variant) {
                    "root" -> {
                        ...
                        val regionKind =
                            enumValueOrNull<SourceRegionKind>(kind)
                                ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                    ...
                    "nested" -> {
                        val parent = current ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                    ...
                    "entity" -> {
                        val parent = current ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                        val entityKind =
                            enumValueOrNull<SourceEntityKind>(kind)
                                ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                        val name =
                            admitName(nameState, nameValue)
                                ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED)
                        val nonEmpty =
        ...
        return Refinement.Refined(current ?: return Refinement.Rejected(SourceSelectorTokenFailure.SELECTOR_REJECTED))
    }
}
...
private fun admitSnapshot(
    ...
    val root =
        when (val parsed = CanonicalWorkspaceRoot.fromCanonicalPath(rootPath)) {
            is Refinement.Refined -> parsed.value
    ...
    val context =
        if (isLive) {
            ...
            if (
                admitted.workspaceRoot != root ||
                    admitted.identity.revisionKey.value != rawGeneration ||
                    admitted.reference.contentView.name != rawSourceState
            )
                return null
            SourceReadContext.Live(admitted)
        } else {
            val generation =
                when (val parsed = rawGeneration.toCanonicalLongOrNull()?.let(EvidenceGeneration::parse)) {
                    is Refinement.Refined -> parsed.value
                    else -> return null
                }
            val sourceState =
                when (val parsed = WorkspaceStateIdentity.parse(rawSourceState)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected -> return null
                }
    ...
    val file =
        when (val parsed = CanonicalWorkspaceFilePath.fromCanonicalPath(root, filePath)) {
            is Refinement.Refined -> SymbolDiscoveryFileIdentity.Workspace(parsed.value)
            is Refinement.Rejected -> return null
        }
    val textIdentity =
        when (val parsed = SourceTextIdentity.parse(rawTextIdentity)) {
            is Refinement.Refined -> parsed.value
            is Refinement.Rejected -> return null
        }
    val length =
        when (val parsed = rawLength.toCanonicalIntOrNull()?.let(Utf16CodeUnitCount::parse)) {
            is Refinement.Refined -> parsed.value
            else -> return null
        }
...
private fun admitName(state: String, value: String): SourceEntityName? =
    when (state) {
        "unavailable" -> if (value.isEmpty()) SourceEntityName.Unavailable else null
        "present" ->
            when (val parsed = SourceEntityName.present(value)) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected -> null
            }
        else -> null
...
private fun encodeReadScope(scope: SourceReadScope): String =
    when (scope) {
        ...
        is SourceReadScope.Constrained -> {
            ...
            val fields =
                listOf(
                    ...
                    captured.primary ?: "",
                    captured.secondary ?: "",
                    captured.sourceKinds.name,
                    captured.generatedSources.name,
                    captured.libraries?.name ?: "",
...
private fun decodeReadScope(rawRoot: String, raw: String, scopes: SourceScopeRestoration): SourceReadScope? {
    ...
    val libraryPolicy =
        if (libraries.isEmpty()) null else enumValueOrNull<SymbolLibraryPolicy>(libraries) ?: return null
    val captured =
        SymbolSearchScopeSnapshot(
            kind,
            primary.ifEmpty { null },
            secondary.ifEmpty { null },
            sourceKinds,
            generated,
            libraryPolicy,
        )
    val scope =
        when (scopes) {
            is SourceScopeRestoration.Current ->
                when (val restored = SymbolSearchScope.restore(scopes.model, captured)) {
                    is Refinement.Refined -> restored.value
                    is Refinement.Rejected -> return null
                }
            SourceScopeRestoration.Unavailable ->
                when (kind) {
                    SymbolSearchScopeKind.WORKSPACE ->
                        SymbolSearchScope.Workspace(sourceKinds, generated, libraryPolicy ?: return null)
                    SymbolSearchScopeKind.EXACT_FILE -> {
                        ...
                        val root =
                            when (val admitted = CanonicalWorkspaceRoot.fromCanonicalPath(rootPath)) {
                                is Refinement.Refined -> admitted.value
                                is Refinement.Rejected -> return null
                            }
                        val filePath =
                        ...
                        val file =
                            when (val admitted = CanonicalWorkspaceFilePath.fromCanonicalPath(root, filePath)) {
                                is Refinement.Refined -> admitted.value
                                is Refinement.Rejected -> return null
                            }
                    ...
                    SymbolSearchScopeKind.GRADLE_PROJECT -> return null
                }
        }
    if (SymbolSearchScope.snapshot(scope) != captured) return null
    val constraints =
        if (fields.exhausted) SymbolDiscoveryConstraints.None else decodeConstraints(fields) ?: return null
...
private fun decodeConstraints(fields: SourceSelectorFieldReader): SymbolDiscoveryConstraints? {
    ...
    val directory =
        if (directoryText.isEmpty()) {
            ...
            val name =
                when (val parsed = SymbolDiscoveryDirectory.parse(directoryText)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected -> return null
                }
    ...
    val packageName =
        if (packageText.isEmpty()) {
            ...
            val name =
                when (val parsed = SymbolDiscoveryPackage.parse(packageText)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected -> return null
                }
    ...
    val declarationKinds =
        if (kindCount == 0) null
            ...
            when (val parsed = SymbolDiscoveryDeclarationKinds.from(kinds)) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected -> return null
            }
    val sourceSets =
            ...
            "exact-source-sets" -> {
                ...
                val names = buildSet {
                    repeat(count) {
                        when (
                            val name =
                                io.github.amichne.kast.workspace.contract.WorkspaceSourceSetName.parse(
                                    fields.read() ?: return null
                                )
                        ) {
