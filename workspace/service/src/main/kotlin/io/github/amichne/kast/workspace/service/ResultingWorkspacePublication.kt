...
class ResultingWorkspacePublication
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(SemanticReadLease, PublishedWorkspace) -> Refinement< ResultingWorkspacePublication,
         * ResultingWorkspacePublicationAdmissionFailure>`.
         *
         * Establishes the same canonical root and a strictly newer complete workspace generation.
         * [ResultingWorkspacePublicationAdmissionFailure] is the closed expected failure. Raw generation comparison is
         * permitted only at this publication boundary.
         */
        fun admit(
            ...
            when {
                workspace.root != prior.workspaceRoot ->
                    Refinement.Rejected(ResultingWorkspacePublicationAdmissionFailure.WORKSPACE_ROOT_CHANGED)
                workspace.generation.value <= prior.generation.value ->
                    Refinement.Rejected(ResultingWorkspacePublicationAdmissionFailure.GENERATION_NOT_NEWER)
...
sealed interface ResultingWorkspacePublicationFailure {
    ...
    data class Blocked(val blocker: WorkspacePublicationBlocker) : ResultingWorkspacePublicationFailure

    data class InvalidResult(val failure: ResultingWorkspacePublicationAdmissionFailure) :
        ResultingWorkspacePublicationFailure
}

sealed interface ResultingWorkspacePublicationResult {
    data class Published(val publication: ResultingWorkspacePublication) : ResultingWorkspacePublicationResult

    data class Rejected(val failure: ResultingWorkspacePublicationFailure) : ResultingWorkspacePublicationResult
}
