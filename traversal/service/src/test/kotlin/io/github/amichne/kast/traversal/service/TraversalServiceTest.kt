...
class TraversalServiceTest {
        ...
        for (leaf in listOf(false, true)) {
            ...
            val relations = RelationOperations { request ->
                ...
                when (request.subject.name.value) {
                    "a" -> fixture.terminalRelationResult(request, listOf(fixture.endpoint(request.subject, b)),
                        RelationLimitation.UNSUPPORTED_ITEM)
                    "b" -> fixture.completeRelationResult(request,
                        if (leaf) emptyList() else listOf(fixture.endpoint(request.subject, c)))
                    else -> error("Depth two must not read C")
                }
            }
            val result = assertInstanceOf(TraversalResult.Qualified::class.java,
                runSuspend { traversalOperations(relations, TraversalNanoClock { 0L }).run(fixture.plan(a, depth = 2)) })
            assertEquals(listOf("a" to 0L, "b" to 0L), requests.map { it.subject.name.value to it.providerCursor.nextPosition.value })
            assertEquals(if (leaf) listOf("b") else listOf("b", "c"), result.page.records.map { it.fact.target.name.value })
            ...
            assertEquals(if (leaf) setOf(TraversalLimitation.ONE_HOP_INCOMPLETE) else
                setOf(TraversalLimitation.ONE_HOP_INCOMPLETE, TraversalLimitation.DEPTH_LIMIT_REACHED), result.qualification.limitations)
            assertInstanceOf(TraversalQualification.TerminalIncomplete::class.java, result.qualification)
        }
    }
    ...
    fun `terminal omission and recoverable child pages survive repeated targets without replay`() {
        ...
        fun drain(capacity: Int, terminal: Boolean): Pair<List<String>, List<Pair<String, Long>>> {
            ...
            val relations = RelationOperations { request ->
                ...
                if (request.subject.name.value == "a" && terminal) {
                    fixture.terminalRelationResult(request, listOf(fixture.endpoint(request.subject, b)), RelationLimitation.UNSUPPORTED_ITEM)
                } else {
                    val targets = when (request.subject.name.value) {
                        "a" -> listOf(b)
                        "b" -> listOf(c, c, d)
                        else -> emptyList()
                    }
                    val facts = targets.mapIndexed { index, target ->
                        io.github.amichne.kast.relation.contract.RelationFact.create(request, request.subject,
                            fixture.endpoint(request.subject, target),
                            io.github.amichne.kast.relation.contract.RelationOccurrence.fromBoundary(request.subject.file, 100 + index, 101 + index).refined(),
                            io.github.amichne.kast.relation.contract.RelationProvenance.K2_AUTHORED_SOURCE).refined()
                    }.sorted()
                    val page = facts.drop(position.toInt()).take(request.budget.resources.resultLimit.value)
                    val batch = io.github.amichne.kast.relation.contract.RelationBatch.create(request, page,
                        io.github.amichne.kast.relation.contract.RelationByteCount.parse(page.sumOf { it.canonicalProjection().toByteArray().size.toLong() }).refined(),
                        io.github.amichne.kast.relation.contract.RelationWorkCount.parse(page.size.toLong()).refined(),
                        io.github.amichne.kast.relation.contract.RelationResultCount.parse(page.size).refined()).refined()
                    if (position + page.size < facts.size) {
                        val compilation = io.github.amichne.kast.relation.contract.RelationCompilation.qualifiedResumable(batch,
                            setOf(RelationLimitation.RESULT_LIMIT_REACHED), page.fold(request.providerCursor) { cursor, fact ->
                                cursor.advance(io.github.amichne.kast.relation.contract.RelationProviderItemDescriptor.parse(fact.canonicalProjection()).refined())
                            }).refined()
                        ...
                        io.github.amichne.kast.relation.contract.RelationReadResult.Complete(batch, compilation.coverage)
                    }
                }
            }
            ...
            repeat(12) {
                ...
                val page = when (result) {
                    ...
                    is TraversalResult.Qualified -> {
                        assertEquals(terminal, RelationLimitation.UNSUPPORTED_ITEM in result.qualification.relationLimitations)
                        result.page
                    }
                ...
                if (continuation is TraversalQualification.Resumable) {
                    plan = TraversalPlan.resume(a, RelationMeaning.Callees, plan.budget, continuation.continuation).refined()
                } else {
