...
internal sealed interface RuntimeProcessModeAdmission {
    data class Admitted(val mode: RuntimeProcessMode) : RuntimeProcessModeAdmission

    data class Rejected(val failure: RuntimeProcessModeFailure) : RuntimeProcessModeAdmission
}
...
internal object RuntimeProcessModeEnvironment {
     ...
     *
     * Absence and `0` establish direct launch. Only `1` establishes launchd authority. Every other representation
     * remains a closed failure instead of being interpreted heuristically.
     */
    fun admit(configured: String?): RuntimeProcessModeAdmission =
        when (configured) {
            null,
            "0" -> RuntimeProcessModeAdmission.Admitted(RuntimeProcessMode.Direct)
            "1" -> RuntimeProcessModeAdmission.Admitted(RuntimeProcessMode.Launchd)
            else -> RuntimeProcessModeAdmission.Rejected(RuntimeProcessModeFailure.INVALID_ENVIRONMENT_VALUE)
        }
}
...
internal fun RuntimeProcessMode.capabilities(): RuntimeProcessCapabilities =
    when (this) {
        RuntimeProcessMode.Direct ->
            RuntimeProcessCapabilities(
                JdkRuntimeProcessStarter,
                JdkRuntimeProcessAuthority,
                JdkRuntimeBootstrapProcessAuthority,
            )
        RuntimeProcessMode.Launchd ->
            RuntimeProcessCapabilities(
