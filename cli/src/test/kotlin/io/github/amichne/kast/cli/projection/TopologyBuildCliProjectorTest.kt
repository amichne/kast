...
class TopologyBuildCliProjectorTest {
    ...
    fun `coverage rejection retains all exact mismatch evidence`() {
        val compilerEvidence =
            CompilerSymbolEvidenceDocument.fromSignature(CompilerSignatureDocument.ClassLike(text("qualified")))
                .refined()
        val node =
        ...
        val workspaceJson = "{\"root\":\"/workspace\",\"generation\":17," + "\"sourceState\":\"published\"}"
        val sourceRootJson =
            "{\"module\":\"fixture\",\"buildRoot\":\"/workspace\"," +
                "\"projectPath\":\":\",\"sourceSet\":\"main\"," +
                "\"location\":\"src/main/kotlin\",\"provenance\":\"authored\"}"
        fun fileEvidenceJson(hash: Char) =
    ...
    private fun fileEvidence(path: String, hashCharacter: Char): TopologyCoverageFileEvidence =
        TopologyCoverageFileEvidence(
            ...
            path = text(path),
            contentHash = TopologyCoverageSourceHash.parse(hashCharacter.toString().repeat(64)).refined(),
        )
