...
import io.github.amichne.kast.standalone.telemetry.TelemetrySpan
...
internal class AnalysisBackend internal constructor(
    ...
    constructor(
        ...
        limits = limits,
        session = session,
        telemetry = Telemetry.fromEnvironment(workspaceRoot),
    )
    ...
    override suspend fun resolveSymbol(query: SymbolQuery): SymbolResult = withContext(readDispatcher) {
        session.withReadAccess {
            telemetry.inSpan(
                scope = TelemetryScope.SYMBOL_RESOLVE,
    ...
    override suspend fun fileOutline(query: FileOutlineQuery): FileOutlineResult = withContext(readDispatcher) {
        session.withReadAccess {
            telemetry.inSpan(
                scope = TelemetryScope.FILE_OUTLINE,
    ...
    override suspend fun workspaceSymbolSearch(query: WorkspaceSymbolQuery): WorkspaceSymbolResult = withContext(readDispatcher) {
        session.withReadAccess {
            telemetry.inSpan(
                scope = TelemetryScope.WORKSPACE_SYMBOL_SEARCH,
 ...
 * Safe to call while the caller holds [AnalysisSession.withReadAccess] —
