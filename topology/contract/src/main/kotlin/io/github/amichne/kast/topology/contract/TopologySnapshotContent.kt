...
private constructor(
    ...
    companion object {
         ...
         *
         * Re-establishes one workspace identity, unique files and exact location-bearing symbols, closed exact edge
         * targets, exact manifest counts, and byte-for-byte digest equality for persisted content. Compiler identity
         * may repeat at distinct exact locations. [TopologySnapshotContentFailure] is the closed expected failure.
         * Reconstructed rows may enter only from the topology SQLite adapter.
         */
        fun admit(
            ...
            if (
                manifest.digest.value != digest ||
                    manifest.cardinalities.files != ordered.size ||
...
sealed interface TopologySnapshotContentRead {
    data class Loaded(val content: TopologySnapshotContent) : TopologySnapshotContentRead

    data class Rejected(val failure: TopologySnapshotReadFailure) : TopologySnapshotContentRead
}
...
fun interface TopologySnapshotContentReader {
     ...
     *
     * Loaded re-establishes exact persisted content, manifest cardinalities, and digest identity. Rejected carries the
     * closed [TopologySnapshotReadFailure]. Raw storage values may enter only through the concrete persistence adapter
