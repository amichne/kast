...
class QueryCheckpointReplayTest {
    ...
    fun `checkpoint identity retains query scope projection and relationship`() = runTest {
        ...
        val changed = listOf(
            original.copy(from = source.copy(match = QueryMatchDocument.Name(text("Other"), SymbolDiscoveryMatchDocument.EXACT_NAME))),
            original.copy(from = source.copy(scope = source.scope.copy(sourceSets = bounded(listOf(text("main")))))),
            original.copy(from = source.copy(declarationKinds = bounded(listOf(QueryDeclarationKindDocument.FUNCTION)))),
        ...
        val authorities = listOf(
            SemanticReadLease(CanonicalWorkspaceRoot.fromCanonicalPath(Path.of("/other")).refined(), lease.generation),
        ...
        assertEquals(QueryCheckpointRestoration.Unavailable, QueryCheckpointStore().restore(issued.token, original, lease))
        store.clear()
    ...
    fun `checkpoint resumes each larger allowance through canonical admission with retained plan`() = runTest {
        ...
        val grants = listOf(
            budget.copy(resources = budget.resources.copy(elapsedTimeLimit = ElapsedTimeLimitMillis.parse(2000).refined())),
        ...
        grants.forEach { grant ->
            val requested = request().copy(continuation = issued.token, executionBudget = ExecutionBudgetDocument(
            ...
            val protocol = CanonicalQueryProtocol(QueryOperations { admitted ->
                ...
                complete()
            }, CanonicalQueryReferences(), store)
    ...
    private suspend fun checkpoint(authority: SemanticReadAuthority): QueryCheckpoint {
        lateinit var retained: QueryCheckpoint
        CanonicalQueryProtocol(QueryOperations { admitted ->
            retained = object : QueryCheckpoint {
                override val plan = admitted.plan
                override val lease = admitted.lease
                override val retainedBytes = 1024L
            }
            complete()
        }, CanonicalQueryReferences()).execute(request(), authority, budget)
        return retained
    }

    private fun complete() = QueryExecutionResult.Complete(
        QueryResult(QueryResultSet.Symbols(emptyList()), emptyList()),
        QueryCoverage.Complete(QueryCount.parse(0).refined()),
    )
