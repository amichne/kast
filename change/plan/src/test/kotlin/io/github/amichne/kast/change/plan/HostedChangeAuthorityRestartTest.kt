...
class HostedChangeAuthorityRestartTest {
    ...
    fun `plan and application identities survive authority reopen`() {
        ...
        val plan =
            when (val planned = PureAddDeclarationPlanningService().plan(fixture.request())) {
        ...
        val preparation =
            AddDeclarationRecoveryPreparation.fromPlan(
                ...
                .refined()
        val prepared = (recovery.prepare(preparation) as PrepareAddDeclarationRecoveryResult.Prepared).recovery
        val appliedRecovery = (recovery.recordApplied(prepared) as RecordAppliedAddDeclarationResult.Recorded).recovery
        ...
        val applicationIdentity = (first.issueApplication(plan, applied) as ChangeApplicationIssuance.Issued).identity
    ...
    fun `identities are root isolated and corrupt documents fail closed after reopen`() {
        ...
        val otherRoot = CanonicalWorkspaceRoot.fromCanonicalPath(Path.of("/different-workspace")).refined()
        ...
        DriverManager.getConnection("jdbc:sqlite:${state.mutationDatabase.valueAtSqliteBoundary()}").use { connection ->
            connection.prepareStatement("UPDATE hosted_change_plan SET document = ? WHERE identity = ?").use { statement
                ->
    ...
    fun `legacy exact application rows are refined without changing public identity`() {
        ...
        assertEquals(
            applicationIdentity,
            (authority(state).issueApplication(plan, pending.application) as ChangeApplicationIssuance.Issued).identity,
        )
    ...
    private fun authority(state: HostedWorkspaceStateLocation): SqliteDurableChangeAuthority =
        when (val opened = SqliteDurableChangeAuthority.open(state.mutationDatabase)) {
    ...
    private fun journal(state: HostedWorkspaceStateLocation): SqliteMutationRecoveryJournal =
        when (val opened = SqliteMutationRecoveryJournal.open(state.mutationDatabase)) {
    ...
    private fun replaceApplicationTableWithLegacyRow(
        ...
        val digest = sha256(canonicalFields(planIdentity, planId, postimage, recoveryBinding).toByteArray())
        DriverManager.getConnection("jdbc:sqlite:${state.mutationDatabase.valueAtSqliteBoundary()}").use { connection ->
    ...
    private fun sha256(value: ByteArray): String =
        MessageDigest.getInstance("SHA-256").digest(value).joinToString(separator = "") { byte -> "%02x".format(byte) }
