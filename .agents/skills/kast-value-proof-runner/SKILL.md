...
5. Scaffold a fresh iteration workspace with `.agents/skills/kast/value-proof/scripts/run_value_proof.py`, then read `run_manifest.json`.
6. For every run in the manifest, execute the prompt in an isolated child session:
   - `with_skill`: Kast skill and `kast_*` tools enabled
   - `without_skill`: do not use the Kast skill or `kast_*` tools
7. Save each full transcript to `outputs/transcript.md` and keep timing/token data in `timing.json`. Do not fake baseline isolation; if the host cannot create an honest `without_skill` run, stop and report that blocker.
