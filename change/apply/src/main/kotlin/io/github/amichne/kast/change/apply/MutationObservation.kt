...
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(WorkspaceFile, ByteArray, SourceWriteAccess) -> Refinement< ObservedMutationSource,
         * MutationSourceCaptureFailure>`.
         *
         * Establishes strict UTF-8 source text plus one SHA-256-bound immutable recovery preimage for the exact
         * observed file and writability state. [MutationSourceCaptureFailure] is the closed expected failure. Raw bytes
         * may enter only from an IntelliJ physical source observation and may leave only through a later admitted
         * mutation or rollback boundary.
         */
        fun capture(
            ...
            val content =
                when (val parsed = WorkspaceSourceContentHash.parse(preimage.digest.value)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected ->
                        return Refinement.Rejected(MutationSourceCaptureFailure.SOURCE_HASH_UNREPRESENTABLE)
                }
            return Refinement.Refined(ObservedMutationSource(source, content, access, text, preimage))
        }
    }
}

/** Exact absent source identity plus its parent-derived creation access observation. */
class ObservedAbsentMutationSource
private constructor(
    override val source: SymbolDiscoveryFileIdentity.Workspace,
    override val access: SourceWriteAccess,
    override val recoveryPreimage: RecoveryPreimage,
    ...
    companion object {
        /**
         * Proof transition: `(WorkspaceFile, SourceWriteAccess) -> ObservedAbsentMutationSource`.
         *
         * Establishes that the physical boundary observed no directory entry at the exact path, retains parent-derived
         * creation access, and carries the canonical absence recovery marker. There is no expected failure because
         * absence and access were already observed by the physical adapter. Raw filesystem state may enter only at that
         * adapter boundary.
         */
