...
internal value class SqliteTopologySnapshotId private constructor(val value: Long) {
    companion object {
        /**
         * Proof transition: `Long -> Refinement<SqliteTopologySnapshotId, SqliteTopologySnapshotIdFailure>`.
         *
         * Establishes a positive SQLite topology snapshot row identity. The closed expected failure is
         * [SqliteTopologySnapshotIdFailure]. Raw row IDs may enter only from JDBC.
         */
...
internal value class SqliteTopologySymbolId private constructor(val value: Long) {
    companion object {
        /**
         * Proof transition: `Long -> Refinement<SqliteTopologySymbolId, SqliteTopologySymbolIdFailure>`.
         *
         * Establishes one positive location-bearing topology symbol row identity. The closed expected failure is
         * [SqliteTopologySymbolIdFailure]. Raw row IDs may enter only from JDBC inside this adapter.
         */
        internal fun restore(raw: Long): Refinement<SqliteTopologySymbolId, SqliteTopologySymbolIdFailure> =
...
internal fun Connection.findExactTopologySnapshot(identity: TopologyWorkspaceIdentity): SqliteTopologySnapshotLookup =
    prepareStatement(
...
internal fun Connection.findLatestTopologySnapshot(root: CanonicalWorkspaceRoot): SqliteTopologySnapshotLookup =
    prepareStatement(
            """SELECT snapshot_id, workspace_root, generation, source_state, digest,
              file_count, symbol_count, edge_count
...
internal fun Connection.insertTopologySnapshot(generation: CompleteTopologyGeneration): SqliteTopologySnapshotRecord {
 ...
 *
 * Reconstructs and re-admits every persisted file, symbol, edge, manifest count, and digest for the exact snapshot.
 * [TopologySnapshotReadFailure.CORRUPT_SNAPSHOT] closes every malformed or inconsistent row; SQL availability failures
 * remain for the owning store boundary to classify. Raw JDBC values do not leave this SQLite adapter.
 */
internal fun Connection.readTopologyContent(snapshot: PublishedTopologySnapshot): TopologySnapshotContentRead =
    try {
...
private fun Connection.reconstructTopologyContent(snapshot: PublishedTopologySnapshot): TopologySnapshotContentRead {
    val record =
        when (val lookup = findExactTopologySnapshot(snapshot.identity)) {
...
private fun Connection.readFiles(record: SqliteTopologySnapshotRecord): Map<WorkspaceSourcePath, TopologySourceFile> =
    prepareStatement(
            ...
            statement.executeQuery().use { rows ->
                buildMap {
                    while (rows.next()) {
                        ...
                        val hash =
                            WorkspaceSourceContentHash.parse(rows.getString("content_hash")).refined("content hash")
                        put(
                            path,
...
): Map<SqliteTopologySymbolId, TopologySymbol> =
    prepareStatement(
        ...
        .use { statement ->
            ...
            statement.executeQuery().use { rows ->
                buildMap {
                    while (rows.next()) {
                        val filePath =
                            WorkspaceSourcePath.parse(rows.getString("file_path")).refined("symbol file path")
                        ...
                        val compilerIdentity =
                            CompilerSymbolIdentity.parse(rows.getString("compiler_identity"))
                                .refined("compiler identity")
                        val compilerSignature =
                            CanonicalCompilerSignature.restoreCanonicalEncoding(rows.getString("compiler_signature"))
                        ...
                        put(
                            SqliteTopologySymbolId.restore(rows.getLong("symbol_id")).refined("symbol identity"),
        ...
        .use { statement ->
            ...
            statement.executeQuery().use { rows ->
                buildList {
                    while (rows.next()) {
                        val sourceIdentity =
                            SqliteTopologySymbolId.restore(rows.getLong("source_symbol_id"))
                                .refined("edge source identity")
                        val targetIdentity =
                            SqliteTopologySymbolId.restore(rows.getLong("target_symbol_id"))
                                .refined("edge target identity")
                        val source = symbols[sourceIdentity] ?: corrupt("edge source absent")
                        val target = symbols[targetIdentity] ?: corrupt("edge target absent")
                        val occurrenceFile =
                            WorkspaceSourcePath.parse(rows.getString("occurrence_file_path"))
...
private fun ResultSet.snapshot(): SqliteTopologySnapshotRecord {
    val root = CanonicalWorkspaceRoot.fromCanonicalPath(Path.of(getString("workspace_root"))).refined("workspace root")
    val generation = EvidenceGeneration.parse(getLong("generation")).refined("generation")
    val identity =
        TopologyWorkspaceIdentity(
    ...
    val manifest =
        TopologySnapshotManifest.restore(
                digest,
                getInt("file_count"),
                getInt("symbol_count"),
                getInt("edge_count"),
...
private fun ResultSet.snapshotLookup(): SqliteTopologySnapshotLookup =
    if (next()) SqliteTopologySnapshotLookup.Found(snapshot()) else SqliteTopologySnapshotLookup.Absent

private fun String.sourceRootProvenance(): SourceRootProvenance =
    when (this) {
...
private inline fun <reified Value : Enum<Value>> enumValue(raw: String): Value =
    try {
        enumValueOf(raw)
