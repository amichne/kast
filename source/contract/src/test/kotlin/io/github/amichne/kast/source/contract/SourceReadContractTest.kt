...
class SourceReadContractTest {
    ...
    fun `partial source text is derived from the exact full committed document`() {
        ...
        val selector =
            SourceSelector.issueNested(
                    root,
                    range(root.snapshot, start, document.length),
    ...
    fun `request retains orthogonal region entity text and bound decisions`() {
        ...
        val declarations =
            DeclarationKindSelection.from(setOf(DeclarationKind.FUNCTION, DeclarationKind.PROPERTY)).refined()
        val visibility =
            VisibilitySelection.exact(setOf(DeclarationVisibility.PUBLIC, DeclarationVisibility.INTERNAL)).refined()
        val entities =
    ...
    fun `invalid and contradictory request states reject during refinement`() {
        assertIs<Refinement.Rejected<DeclarationKindSelectionFailure>>(DeclarationKindSelection.from(emptySet()))
        assertIs<Refinement.Rejected<VisibilitySelectionFailure>>(VisibilitySelection.exact(emptySet()))
        assertIs<Refinement.Rejected<EntitySelectionFailure>>(EntitySelection.matching(Containment.DIRECT, emptyList()))
        assertEquals(
    ...
    fun `qualification is non-empty and entity truncation carries continuation proof`() {
        ...
        val continuation = SourceReadContinuation.parse("source-read-continuation-v1|${"a".repeat(64)}").refined()
    ...
    private fun textByteLimit(raw: Long): SourceTextByteLimit = SourceTextByteLimit.parse(raw).refined()

    private fun nestingDepth(raw: Int): SourceNestingDepth = SourceNestingDepth.parse(raw).refined()
