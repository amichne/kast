...
demo_doc="${docs_root}/commands/demo.md"
...
require_contains "$readme" 'kast demo --workspace-root "$PWD"' "README must expose the repository-native demo"

require_contains "$index_doc" "Developer-oriented command documentation" "Overview must be command-manual oriented"
require_contains "$index_doc" '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/amichne/kast/main/install.sh)"' "Overview must show the brew-style macOS installer"
...
require_contains "$install_doc" '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/amichne/kast/main/install.sh)"' "Install docs must make the brew-style root installer primary"
require_contains "$install_doc" '-- update --workspace-root' "Install docs must document the explicit update command"
require_contains "$install_doc" '-- verify --workspace-root' "Install docs must document installer verification"
require_contains "$install_doc" "NONINTERACTIVE=1" "Install docs must document unattended installer automation"
require_contains "$install_doc" "--tap-url" "Install docs must document custom-host tap overrides"
require_contains "$install_doc" "plugin can prepare workspace metadata" "Install docs must make plugin-owned setup primary"
...
require_contains "$install_doc" "kast --output json ready --for agent" "Install docs must document structured readiness"
...
require_contains "$quickstart_doc" '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/amichne/kast/main/install.sh)"' "Quickstart must install through the brew-style root macOS installer"
require_contains "$quickstart_doc" '-- update --workspace-root' "Quickstart must expose the explicit update command"
...
require_contains "$quickstart_doc" 'kast demo --workspace-root "$PWD"' "Quickstart must tour the current repository"
require_contains "$quickstart_doc" 'kast --output json demo' "Quickstart must show captured demo output"
...
require_contains "$commands_index" '| Demo | `demo`' "Command overview must list the public demo"
