...
object CanonicalSourceReadCliDocuments {
    fun project(outcome: OperationOutcome<SourceReadResult, SourceReadQualification, SourceReadFailure>) =
        projectClosedOutcome(
            ...
            complete = { result, live ->
                if (result.format == SourceReadFormatDocument.COMPACT) compactSourceComplete(result, live)
                else
            ...
            qualified = { result, qualification, live ->
                if (result.format == SourceReadFormatDocument.COMPACT)
                    compactSourceQualified(result, qualification.toCliDocument(), live)
                else
...
@Serializable
data class SourceReadQualificationCliDocument(
...
@Serializable
sealed interface SourceReadContinuationCliDocument {
...
private val completeFactory = CanonicalJsonDocument.generated(SourceReadCompleteCliDocument.serializer())
private val qualifiedFactory = CanonicalJsonDocument.generated(SourceReadQualifiedCliDocument.serializer())
