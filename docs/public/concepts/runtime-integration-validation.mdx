...
admission, workspace registration, resource admission, and worker retirement.
...
Explicit isolated-runtime workflows use per-workspace IntelliJ sidecars. Their
startup and resource policies are distinct from ordinary existing-IDE reads.
Coalesced startup applies only to the same workspace and lifecycle intent;
conflicting reuse, rebuild, or seed requests reject.
...
| Installed multi-workspace acceptance | Separation, reconnection, lifecycle, and cleanup for the specific fixture |
