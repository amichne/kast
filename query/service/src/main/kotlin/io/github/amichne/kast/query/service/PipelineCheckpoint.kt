...
internal const val MAX_CHECKPOINT_BYTES = 8L * 1024L * 1024L
internal val pageLimits = setOf(QueryLimitation.RESULT_LIMIT_REACHED, QueryLimitation.BYTE_LIMIT_REACHED,
    QueryLimitation.WORK_LIMIT_REACHED, QueryLimitation.TIME_LIMIT_REACHED)
...
internal sealed interface PipelineTask {
    ...
    data class Related(val value: QuerySymbol, val stage: ExactQueryStage.Related, val cursor: RelationContinuation?) : PipelineTask
}
...
internal data class PipelineCheckpoint(
    ...
    val symbolDistinct: Map<ExactQueryStage.Distinct, Set<SymbolSelectorFingerprint>>,
    val limitations: Set<QueryLimitation>,
) : QueryCheckpoint {
    override val retainedBytes: Long = tasks.sumOf { task ->
        512L + when (task) {
            ...
            is PipelineTask.Revalidate -> QuerySymbol(SymbolDescription.from(task.selector), emptyList()).projectedUtf8Size() * 4L
...
internal fun initialTasks(plan: AdmittedQueryPlan): List<PipelineTask> = when (plan) {
    is AdmittedQueryPlan.Candidates -> listOf(PipelineTask.Discover(plan.source, plan.stage))
    is AdmittedQueryPlan.Symbols -> listOf(PipelineTask.Discover(plan.source,
...
internal fun discoverySyntax(plan: AdmittedQueryPlan): QueryDiscoverySyntax? = when (plan) {
    is AdmittedQueryPlan.Candidates -> plan.source
    is AdmittedQueryPlan.Symbols -> plan.source
    is AdmittedQueryPlan.CandidateReferences, is AdmittedQueryPlan.ExactReferences -> null
}

internal fun isCandidateOutput(plan: AdmittedQueryPlan): Boolean {
    fun candidate(stage: CandidateQueryStage): Boolean = when (stage) {
    ...
    return when (plan) {
        ...
        is AdmittedQueryPlan.Symbols, is AdmittedQueryPlan.ExactReferences -> false
    }
}
