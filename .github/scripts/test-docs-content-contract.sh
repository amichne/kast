...
cli_cheat_sheet_doc="${docs_root}/cli-cheat-sheet.md"
packaged_cli_cheat_sheet_doc="${repo_root}/cli-rs/docs/reference/cli-cheat-sheet.md"
json_rpc_operations_doc="${repo_root}/cli-rs/docs/reference/json-rpc-operations.md"
...
require_not_contains "$docs_root" "[paths]" "Docs must not document install-owned path config"
require_not_contains "$docs_root" "[cli]" "Docs must not document install-owned CLI config"
require_not_contains "$docs_root" "[install]" "Docs must not document install state in config"
require_not_contains "$docs_root" "binaryPath =" "Docs must not document install-owned CLI binary config"
require_not_contains "$docs_root" "runtimeLibsDir =" "Docs must not document install-owned runtime library config"
require_not_contains "$docs_root" "ideaHome =" "Docs must not document install-owned IDEA home config"
...
require_not_contains "$docs_root" 'The plugin is optional' "Docs must not describe the macOS IDEA plugin as optional"
require_not_contains "$docs_root" 'optional IDEA plugin' "Docs must not describe the macOS IDEA plugin as optional"
require_not_contains "$docs_root" '--backend-name' "Docs must use --backend as the only backend selector"
...
require_not_contains "$docs_root" 'repository backed' "Docs must use concrete install-scope wording"
require_not_contains "$docs_root" 'kast setup' "Docs must not advertise the retired setup command"
require_not_contains "$packaged_cli_cheat_sheet_doc" 'kast setup' "Packaged CLI cheat sheet must not advertise the retired setup command"
...
require_not_contains "$readme" 'optional IDEA plugin' "README must not describe the macOS IDEA plugin as optional"
require_not_contains "$readme" 'Optional Homebrew cask' "README must not describe the macOS IDEA plugin as optional"
...
require_contains "$readme" "brew install --cask kast-plugin" "README must document the Homebrew-managed IDEA plugin"
...
require_contains "$index_doc" "brew install --cask kast-plugin" "Docs overview must show the Homebrew-managed IDEA plugin install command"
require_contains "$cli_cheat_sheet_doc" 'kast package ubuntu-debian-bundle' "CLI cheat sheet must document the release bundle packager"
require_contains "$cli_cheat_sheet_doc" 'kast install activate-bundle' "CLI cheat sheet must document bundle activation"
require_contains "$packaged_cli_cheat_sheet_doc" 'kast package ubuntu-debian-bundle' "Packaged CLI cheat sheet must document the release bundle packager"
require_contains "$packaged_cli_cheat_sheet_doc" 'kast install activate-bundle' "Packaged CLI cheat sheet must document bundle activation"
require_contains "$json_rpc_operations_doc" 'repository Copilot package extension' "JSON-RPC docs must describe the current repository Copilot package"
require_not_contains "$json_rpc_operations_doc" 'deprecated Copilot SDK extension' "JSON-RPC docs must not describe retired Copilot extension packaging"
require_not_contains "$json_rpc_operations_doc" 'extension-only fallback' "JSON-RPC docs must not preserve retired extension fallback wording"

require_contains "$install_doc" "## Developer machine" "Install docs must distinguish the developer-machine path"
require_contains "$install_doc" "kast doctor --repair" "Install docs must document manifest-backed broad repair"
require_contains "$install_doc" "kast paths" "Install docs must document path inspection"
require_contains "$install_doc" '$HOME/.local/share/kast/install.json' "Install docs must document the install manifest"
...
require_not_contains "$install_doc" "## Headless Linux server" "Install docs must not mix the headless server body into the developer-machine page"
require_contains "$install_doc" "headless-linux.md" "Install docs must link to the separate headless server page"
...
require_not_contains "$install_doc" 'legacy locations' "Install docs must not advertise pre-v1 legacy-location repair"
...
require_not_contains "$headless_doc" 'hidden Rust packager' "Headless docs must not describe release-facing CLI commands as hidden"
...
require_contains "$install_doc" '.github/instructions/Kotlin.instructions.md' "Install docs must document the top-level Copilot instruction output"
require_contains "$install_doc" 'does not recursively traverse instruction subdirectories' "Install docs must explain the Copilot instruction pickup constraint"
require_contains "$install_doc" '.github/extensions/kast/_shared/kast-tools.mjs' "Install docs must document the tool helper output"
require_contains "$install_doc" '.github/extensions/kast/_shared/kast-trace.mjs' "Install docs must document the trace helper output"
...
require_contains "$install_doc" '$HOME/.local/share/kast/install.json' "Install docs must document the manifest-backed resource trust record"
require_contains "$install_doc" 'source bundle checksum' "Install docs must document managed resource checksums"
require_not_contains "$install_doc" '.github/.kast-copilot-version' "Install docs must not document retired Copilot marker files"
require_not_contains "$install_doc" '.github/instructions/kast-kotlin.instructions.md' "Install docs must not document retired static Copilot instruction filename"
require_not_contains "$install_doc" '.github/agents/kast-reader.agent.md' "Install docs must not document retired reader agent output"
require_not_contains "$install_doc" '.github/agents/kast-writer.agent.md' "Install docs must not document retired writer agent output"
require_not_contains "$install_doc" 'kast-agents.mjs' "Install docs must not document retired custom-agent helper output"
...
require_contains "$headless_doc" 'KAST_UBUNTU_DEBIAN_ARTIFACT_PATH' "Headless docs must list the canonical local artifact override"
require_contains "$headless_doc" '$HOME/.local/share/kast/install.json' "Headless docs must name the install manifest"
...
require_contains "$backends_doc" '$HOME/.local/share/kast/current/lib/backends/headless/current/runtime-libs' "Backend docs must use the manifest-managed runtime-libs path"
...
require_contains "$agents_doc" "brew install --cask kast-plugin" "Agent docs must show Homebrew-managed IDEA plugin setup"
