...
class SharedTaskSessionsTest {
    @Test
    fun `ordinary turn completion cannot discharge reconciliation`() {
        ...
        assertEquals(
            ...
            sessions.authorize(thread, controller),
        )
    ...
    @Test
    fun `completed turn retains disconnected controller until approval resolves`() {
        val sessions = SharedTaskSessions()
        val controller = ClientConnectionId.fresh()
    ...
    @Test
    fun `lost upstream requires matching terminal history before a new controller`() {
        val tasks = SharedTaskSessions()
        val a = ClientConnectionId.fresh()
        val b = ClientConnectionId.fresh()
        val thread = checkNotNull(BrokerThreadId.admit("thread-1"))
        tasks.connect(a)
        tasks.connect(b)
        tasks.attach(thread, a)
        tasks.attach(thread, b)
        tasks.begin(thread, "turn-1")
        tasks.upstreamLost(a)
    ...
    @Test
    fun `two observers share a task with one explicit controller`() {
        val sessions = SharedTaskSessions()
        val a = ClientConnectionId.fresh()
        val b = ClientConnectionId.fresh()
        val thread = checkNotNull(BrokerThreadId.admit("thread-1"))
        sessions.connect(a)
        sessions.connect(b)
        sessions.attach(thread, a)
        sessions.attach(thread, b)
    ...
    @Test
    fun `disconnect retains active ownership until terminal evidence`() {
        val sessions = SharedTaskSessions()
        val a = ClientConnectionId.fresh()
        val b = ClientConnectionId.fresh()
        val thread = checkNotNull(BrokerThreadId.admit("thread-1"))
    ...
    @Test
    fun `pending approval prevents a handoff even before turn notification`() {
        val sessions = SharedTaskSessions()
        val a = ClientConnectionId.fresh()
        val thread = checkNotNull(BrokerThreadId.admit("thread-1"))
        sessions.connect(a)
        sessions.attach(thread, a)
        sessions.pending(thread, "request-1")
