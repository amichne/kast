...
class HostedQualifiedClassSelectionTest {
    @Test
    fun `qualified selection retains exact root and structured class identity`() {
        val selected =
            (HostedQualifiedClassSelection.parse(FIXTURE_ROOT, "example.Outer.Child") as Refinement.Refined).value
        ...
        for (invalid in
            ...
            assertEquals(
                ...
                HostedQualifiedClassSelection.parse(FIXTURE_ROOT, invalid),
            )
        }
    }

    @Test
    fun `only a unique complete declaration collection supplies the stronger candidate`() {
        assertEquals(
            Refinement.Rejected(HostedQueryFailure.DECLARATION_NOT_FOUND),
            HostedUniqueDeclaration.select(emptyList<Any>()),
        )
        val declaration = Any()
        ...
        assertEquals(
            Refinement.Rejected(HostedQueryFailure.AMBIGUOUS_DECLARATION),
            HostedUniqueDeclaration.select(listOf(Any(), Any())),
        )
    }

    @Test
    fun `index selection requires the exact compiler class signature`() {
        ...
        for (mismatch in listOf(other, alias)) {
            assertEquals(
                Refinement.Rejected(HostedQueryFailure.DECLARATION_IDENTITY_MISMATCH),
                selected.verify(mismatch),
            )
        }
    }
}
