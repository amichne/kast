...
sealed interface MutationRecoveryPersistResult<out Record : MutationRecoveryRecord> {
    data class Durable<Record : MutationRecoveryRecord>(val record: Record) : MutationRecoveryPersistResult<Record>

    data class Rejected(val failure: MutationRecoveryEvidenceFailure) : MutationRecoveryPersistResult<Nothing>
}

sealed interface MutationRecoveryLoadResult {
    data class Found(val record: MutationRecoveryRecord) : MutationRecoveryLoadResult

    data class Absent(val binding: MutationPlanBinding) : MutationRecoveryLoadResult

    data class Rejected(val failure: MutationRecoveryEvidenceFailure) : MutationRecoveryLoadResult
}
...
interface MutationRecoveryEvidenceStore {
     ...
     * [MutationRecoveryEvidenceFailure] is the closed expected failure. JDBC and raw columns remain inside the
     * persistence adapter.
     */
    fun prepare(
    ...
    /**
     * Proof transition: `(AppliedWritesDurable, Terminal) -> MutationRecoveryPersistResult<Terminal>`.
     *
     * A durable result proves an atomic exact-prior transition to `RolledBack` or `RecoveryRequired`.
     * [MutationRecoveryEvidenceFailure] is the closed expected failure. JDBC and raw columns remain inside the
     * persistence adapter.
     */
     ...
     *
     * A found result carries one fully re-derived digest chain; absence and expected load failures are closed by
     * [MutationRecoveryLoadResult]. Raw row extraction remains inside the adapter.
     */
