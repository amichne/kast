...
enum class WireRequestStage { CONNECTION, PEER_QUALIFICATION, EXCHANGE }
enum class WireRequestOutcome { COMPLETED, REJECTED, TIMED_OUT }
...
fun interface WireActivitySink {
    ...
    object Disabled : WireActivitySink { override fun publish(activity: WireRequestActivity) = Unit }
}

internal class JsonLineWireActivitySink(private val output: java.io.PrintStream) : WireActivitySink {
    @Synchronized override fun publish(activity: WireRequestActivity) {
        output.println("{\"component\":\"kast-wire\",\"stage\":\"${activity.stage.name.lowercase()}\",\"outcome\":\"${activity.outcome.name.lowercase()}\"}")
    }
}
...
internal class WireIoDeadline(channel: SocketChannel, budget: ElapsedTimeLimitMillis) {
    private enum class State { RUNNING, COMPLETED, EXPIRED }
    ...
    private val expiration = timer.schedule({
        if (state.compareAndSet(State.RUNNING, State.EXPIRED)) closeOwnedChannel()
    }, budget.value, TimeUnit.MILLISECONDS)
    ...
    fun finish(): WireRequestOutcome {
        ...
        return if (state.get() == State.COMPLETED) WireRequestOutcome.COMPLETED else {
    ...
    private fun closeOwnedChannel() {
        try { ownedChannel.close() } catch (_: java.io.IOException) { /* Channel is invalidated by close. */ }
    }

    companion object {
        private val timer = ScheduledThreadPoolExecutor(1) { operation ->
