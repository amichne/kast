...
sealed interface SqliteMutationRecoveryJournalOpenResult {
    data class Opened(val journal: SqliteMutationRecoveryJournal) : SqliteMutationRecoveryJournalOpenResult

    data class Rejected(val failure: SqliteMutationRecoveryJournalOpenFailure) : SqliteMutationRecoveryJournalOpenResult
}

/** SQLite owner of atomic clean-slate mutation recovery evidence transitions. */
class SqliteMutationRecoveryJournal
private constructor(
    ...
    private fun <Record : MutationRecoveryRecord.Terminal> Connection.persistTerminal(
        ...
        val requirement =
            when (record) {
                ...
                is MutationRecoveryRecord.RecoveryRequired -> SqlTerminalRequirement.Present(record.requirement)
            }
        val updated =
            prepareStatement(
                    """UPDATE mutation_recovery
    ...
    private fun existingPreparation(
        ...
        when (val loaded = load(record.binding)) {
            is MutationRecoveryLoadResult.Found ->
                if (loaded.record.digest == record.digest) {
                    MutationRecoveryPersistResult.Durable(record)
                } else {
                    rejected(MutationRecoveryEvidenceFailure.PLAN_BINDING_COLLISION)
                }
            is MutationRecoveryLoadResult.Absent -> rejected(MutationRecoveryEvidenceFailure.CORRUPT_RECORD)
    ...
    companion object {
         ...
         *
         * Establishes a canonical database capability with a durable WAL/FULL-synchronous recovery schema.
         * [SqliteMutationRecoveryJournalOpenFailure] is the closed expected failure. Raw path and JDBC access remain
         * inside this SQLite adapter.
         */
        fun open(path: Path): SqliteMutationRecoveryJournalOpenResult =
            open(path, MutationRecoveryFaultInjector.Disabled)

        /**
         * Proof transition: `(Path, MutationRecoveryFaultInjector) -> SqliteMutationRecoveryJournalOpenResult`.
         *
         * Establishes the same database capability with one module-internal deterministic crash injector.
         * [SqliteMutationRecoveryJournalOpenFailure] is the closed expected failure. This boundary exists only for
         * adapter fault proofs; the public boundary installs no callback.
         */
        internal fun open(
            path: Path,
            ...
            val database =
                when (val admitted = SqliteMutationRecoveryDatabase.admit(path)) {
                    ...
                    is Refinement.Rejected -> return SqliteMutationRecoveryJournalOpenResult.Rejected(admitted.failure)
                }
            return try {
                val connections = SqliteMutationRecoveryConnections(database).initialize()
                SqliteMutationRecoveryJournalOpenResult.Opened(SqliteMutationRecoveryJournal(connections, faults))
            } catch (_: Exception) {
        ...
        internal fun retain(connections: InitializedSqliteMutationRecoveryConnections): SqliteMutationRecoveryJournal =
...
private fun <T> storageResult(operation: () -> MutationRecoveryPersistResult<T>): MutationRecoveryPersistResult<T>
    where T : MutationRecoveryRecord =
    try {
        operation()
    } catch (_: SQLException) {
...
private fun <T> rejected(failure: MutationRecoveryEvidenceFailure): MutationRecoveryPersistResult<T>
    where T : MutationRecoveryRecord = MutationRecoveryPersistResult.Rejected(failure)
