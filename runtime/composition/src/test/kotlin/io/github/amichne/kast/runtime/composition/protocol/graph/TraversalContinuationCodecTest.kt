...
class TraversalContinuationCodecTest {
    @Test
    fun `self contained checkpoint round trips pending relation authority`(@TempDir temporary: Path) {
        val fixture =
        ...
        val relationRequest =
            DomainRelationRequest.start(
                selector,
                RelationMeaning.Callees,
                budget.oneHop,
            )
        val relationCursor =
            relationRequest.providerCursor.advance(
        ...
        val document = checkNotNull(CanonicalTraversalContinuationCodec.encode(continuation, authority))
        val decoded =
            assertInstanceOf(
    ...
    @Test
    fun `public continuation restores the exact frontier without repeating its start`(@TempDir temporary: Path) {
        val fixture =
            InstalledSymbolProtocolFixture.create(Files.createDirectories(temporary.resolve("repo")).toRealPath())
        val authority = CanonicalProtocolAuthority()
        val exact = exactSelector(fixture, authority)
        ...
        val start = startOutcome as OperationOutcome.Qualified<TraversalRunResult, TraversalRunQualification>
        val qualification =
            assertInstanceOf(
        ...
        @Suppress("UNCHECKED_CAST")
        val resumed = resumedOutcome as OperationOutcome.Qualified<TraversalRunResult, TraversalRunQualification>
        ...
        val otherFixture =
            InstalledSymbolProtocolFixture.create(Files.createDirectories(temporary.resolve("other-repo")).toRealPath())
    ...
    fun `malformed checkpoint rejects before traversal work`(@TempDir temporary: Path) {
        ...
        var calls = 0
        val handler =
            CanonicalTraversalRunHandler(
                TraversalOperations {
                    calls += 1
                    TraversalResult.Rejected(
                        io.github.amichne.kast.traversal.contract.TraversalRejection.TraversalContractViolation
                    )
                },
                authority,
            )
    ...
    private fun exactSelector(
        ...
        val discover =
            io.github.amichne.kast.runtime.composition.protocol.CanonicalSymbolDiscoverHandler(
                fixture.workspace,
                fixture.discovery,
                authority,
            )
        val inspect =
            io.github.amichne.kast.runtime.composition.protocol.CanonicalSymbolInspectHandler(fixture.exact, authority)
        val candidate =
            (runSuspend {
                    discover.execute(
