...
source_bound_control="${cli_extract}/_kastctl"
source_bound_agent="${cli_extract}/kast"
source_bound_backend="${backend_extract}/backend-headless"
[[ -x "$source_bound_control" ]] || die "CLI archive must contain executable _kastctl at its root"
...
cp "$source_bound_control" "${prepared_generation}/bin/_kastctl"
cp "$source_bound_agent" "${prepared_generation}/bin/kast"
chmod 755 "${prepared_generation}/bin/_kastctl" "${prepared_generation}/bin/kast"
