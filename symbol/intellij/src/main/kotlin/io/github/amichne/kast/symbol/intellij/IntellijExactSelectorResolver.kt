...
internal sealed interface IntellijExactSelectorResolution {
    data class Resolved(val selector: ExactDeclarationSelector) : IntellijExactSelectorResolution

    data class Rejected(val reason: IntellijExactSelectorRejection) : IntellijExactSelectorResolution

    data class ScopeRejected(val failures: Set<IntellijSearchScopeFailure>) : IntellijExactSelectorResolution
}

internal sealed interface IntellijExactSelectorRevalidation {
    data class Revalidated(val proof: RevalidatedExactDeclaration) : IntellijExactSelectorRevalidation

    data class Rejected(val reason: IntellijExactSelectorRejection) : IntellijExactSelectorRevalidation

    data class ScopeRejected(val failures: Set<IntellijSearchScopeFailure>) : IntellijExactSelectorRevalidation
}

internal sealed interface IntellijExactSelectorLeaseAdmission {
    data object Admitted : IntellijExactSelectorLeaseAdmission

    data class Rejected(val reason: IntellijExactSelectorRejection) : IntellijExactSelectorLeaseAdmission
}
...
private sealed interface IntellijExactSelectorEnvironmentAdmission {
    ...
    data class Rejected(val reason: IntellijExactSelectorRejection) : IntellijExactSelectorEnvironmentAdmission
}
...
internal class IntellijExactSelectorQuery(
    ...
    /**
     * Proof transition: CompiledIntellijSearchScope + SymbolDiscoverySelection to IntellijExactSelectorResolution.
     *
     * Establishes that one batch-owned declaration selection resolved through the same exact generation and native
     * scope before an opaque selector was issued. Expected failures are the closed [IntellijExactSelectorRejection]
     * states. Platform cancellation propagates. Live PSI, files, and scopes remain inside [lookup].
     */
    fun resolve(
        ...
        if (compiledScope.lease != selection.lease || compiledScope.scope != selection.scope) {
            return IntellijExactSelectorResolution.Rejected(IntellijExactSelectorRejection.INTERNAL_INVARIANT)
        }
        ...
        val lookupResult =
            try {
                ...
                return IntellijExactSelectorResolution.Rejected(IntellijExactSelectorRejection.DUMB_MODE)
            } catch (_: RuntimeException) {
                return IntellijExactSelectorResolution.Rejected(IntellijExactSelectorRejection.NATIVE_FAILURE)
            }
        val evidence =
            when (lookupResult) {
                is IntellijExactDeclarationLookupResult.Found -> lookupResult.evidence
                is IntellijExactDeclarationLookupResult.Rejected ->
                    return IntellijExactSelectorResolution.Rejected(lookupResult.reason.toPublicRejection())
            }
        when (val admission = admitEnvironment()) {
            IntellijExactSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijExactSelectorEnvironmentAdmission.Rejected ->
        ...
        return when (val issued = ExactDeclarationSelector.issue(selection, evidence)) {
            ...
            is Refinement.Rejected ->
                IntellijExactSelectorResolution.Rejected(IntellijExactSelectorRejection.NATIVE_EVIDENCE_MISMATCH)
        }
    }

    /**
     * Proof transition: CompiledIntellijSearchScope + ExactDeclarationSelector to IntellijExactSelectorRevalidation.
     *
     * Establishes that a selector-only lookup resolved identical declaration evidence through the selector's original
     * authority and native scope. Expected failures are the closed [IntellijExactSelectorRejection] states. Platform
     * cancellation propagates. Live PSI, files, and scopes remain inside [lookup].
     */
    fun revalidate(
        compiledScope: CompiledIntellijSearchScope,
        ...
        if (compiledScope.lease != selector.lease || compiledScope.scope != selector.scope) {
            return IntellijExactSelectorRevalidation.Rejected(IntellijExactSelectorRejection.INTERNAL_INVARIANT)
        }
        when (val admission = admitEnvironment()) {
            IntellijExactSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijExactSelectorEnvironmentAdmission.Rejected ->
        ...
        val lookupResult =
            try {
                ...
                return IntellijExactSelectorRevalidation.Rejected(IntellijExactSelectorRejection.DUMB_MODE)
            } catch (_: RuntimeException) {
                return IntellijExactSelectorRevalidation.Rejected(IntellijExactSelectorRejection.NATIVE_FAILURE)
            }
        val evidence =
            when (lookupResult) {
                is IntellijExactDeclarationLookupResult.Found -> lookupResult.evidence
                is IntellijExactDeclarationLookupResult.Rejected ->
                    return IntellijExactSelectorRevalidation.Rejected(lookupResult.reason.toPublicRejection())
            }
        when (val admission = admitEnvironment()) {
            IntellijExactSelectorEnvironmentAdmission.Ready -> Unit
            is IntellijExactSelectorEnvironmentAdmission.Rejected ->
        ...
        return when (val validated = RevalidatedExactDeclaration.validate(selector, evidence)) {
            is Refinement.Refined -> IntellijExactSelectorRevalidation.Revalidated(validated.value)
            is Refinement.Rejected ->
                IntellijExactSelectorRevalidation.Rejected(IntellijExactSelectorRejection.DECLARATION_MOVED_OR_CHANGED)
        }
    }

    /**
     * Proof transition: live IntelliJ environment observation to IntellijExactSelectorEnvironmentAdmission.
     *
     * Establishes a ready project/index state or one closed dumb/disposed rejection before or after native lookup.
    ...
    private fun admitEnvironment(): IntellijExactSelectorEnvironmentAdmission {
        ...
        return when (environmentState()) {
            IntellijDiscoveryEnvironmentState.READY -> IntellijExactSelectorEnvironmentAdmission.Ready
            IntellijDiscoveryEnvironmentState.DUMB ->
                IntellijExactSelectorEnvironmentAdmission.Rejected(IntellijExactSelectorRejection.DUMB_MODE)
...
/**
 * Proof transition: expected SemanticReadAuthority + current SemanticReadAuthority to
 ...
 * [IntellijExactSelectorLeaseAdmission.Rejected] distinguishes root drift from generation movement. Both inputs are
 * already strong detached lease values, and no raw extraction crosses this admission boundary.
 */
internal fun admitExactSelectorLease(
    ...
    when {
        expected.workspaceRoot != current.workspaceRoot ->
            IntellijExactSelectorLeaseAdmission.Rejected(IntellijExactSelectorRejection.WORKSPACE_ROOT_MISMATCH)
        expected != current ->
            IntellijExactSelectorLeaseAdmission.Rejected(IntellijExactSelectorRejection.GENERATION_MOVED)
...
internal fun exactSelectorResolutionFromScoped(
    ...
    when (scoped) {
        ...
        is IntellijScopedQueryResult.Rejected -> IntellijExactSelectorResolution.ScopeRejected(scoped.failures)
    }
...
internal fun exactSelectorRevalidationFromScoped(
    ...
    when (scoped) {
        is IntellijScopedQueryResult.Completed -> scoped.value
        is IntellijScopedQueryResult.Rejected -> IntellijExactSelectorRevalidation.ScopeRejected(scoped.failures)
    }
