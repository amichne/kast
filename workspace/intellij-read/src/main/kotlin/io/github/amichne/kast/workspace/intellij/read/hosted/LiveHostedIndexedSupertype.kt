...
internal fun readHostedIndexedSupertype(
    ...
    val complete =
        KotlinFullClassNameIndex.processElements(
            selection.signature.qualifiedIdentity.value,
            project,
            HostedIndexScope(project, model),
        ) {
            ProgressManager.checkCanceled()
            candidates.accept(it) == HostedIndexCollection.CONTINUE
        }
    val found =
        when (val result = candidates.finish()) {
    ...
    val unique =
        when (val result = HostedUniqueDeclaration.select(found)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return HostedSemanticRead.Rejected(result.failure)
        }
    ...
    val name =
        declaration.nameIdentifier ?: return HostedSemanticRead.Rejected(HostedQueryFailure.UNSUPPORTED_DECLARATION)
    ...
    val located =
        when (
            val result = HostedKotlinSelection.parse(selection.root, root.relativize(path).toString(), name.textOffset)
        ) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return HostedSemanticRead.Rejected(result.failure)
        }
