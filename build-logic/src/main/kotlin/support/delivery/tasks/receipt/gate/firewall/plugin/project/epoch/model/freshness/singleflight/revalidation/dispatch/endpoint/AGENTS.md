...
- Admit exact `UNPUBLISHED -> BOUND -> READY` service transitions. The separate prepared
  capability already retains the admitted Project root, constructed read runtime, and
  descriptor-v2 inputs before the service leaves `UNPUBLISHED`.
...
- Reject wrong-root, partial-runtime, duplicate, occupied socket, occupied descriptor, bind, and
  publication cases before READY. Preserve occupied descriptor paths; roll back only the owned
  bound socket and owned temporary descriptor.
- Digest the existing typed compatibility boundary and every KVP-024 authority,
  architecture-policy, locator, projection, verifier, product, and test artifact in both gate
  receipts so the exact-head closure remains independently judgeable.
