...
@JvmInline
value class KastUserStateRoot private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<KastUserStateRoot, KastUserStateRootFailure>`.
         *
         * Establishes a bounded absolute lexical path without aliases or traversal. The value is retained as detached
         * text; a physical state adapter alone may turn it into a raw path.
         */
        fun parse(raw: String): Refinement<KastUserStateRoot, KastUserStateRootFailure> =
            when {
                ...
                !isCanonicalAbsolutePath(raw) -> Refinement.Rejected(KastUserStateRootFailure.NOT_NORMALIZED)
...
@JvmInline
value class TopologyDatabaseLocation internal constructor(private val value: String) {
...
@JvmInline
value class MutationDatabaseLocation internal constructor(private val value: String) {
    /** Detached text may leave only when the SQLite adapter creates its physical capability. */
    fun valueAtSqliteBoundary(): String = value
}
...
class HostedWorkspaceStateLocation
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(KastUserStateRoot, CanonicalWorkspaceRoot) -> Refinement< HostedWorkspaceStateLocation,
         * HostedWorkspaceStateLocationFailure>`.
         *
         * Binds both databases to one full SHA-256 root digest under the durable user-state tree. No endpoint socket
         * path or raw filesystem capability enters this representation.
         */
        fun locate(
            ...
            val digest =
                HexFormat.of()
                    .formatHex(
                        MessageDigest.getInstance("SHA-256").digest(workspace.value.toByteArray(StandardCharsets.UTF_8))
                    )
            ...
            if (
                ...
                return Refinement.Rejected(HostedWorkspaceStateLocationFailure.LOCATION_TOO_LONG)
            }
...
private fun KastUserStateRoot.child(relative: String): String = if (value == "/") "/$relative" else "$value/$relative"

private fun isCanonicalAbsolutePath(raw: String): Boolean =
    raw == "/" ||
        !raw.endsWith('/') &&
            raw.split('/').drop(1).none { segment ->
