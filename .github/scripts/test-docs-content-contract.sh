...
require_contains "$readme" "kast setup" "README must document one-touch repository setup"
...
require_contains "$index_doc" "kast setup" "Overview must show one-touch repository setup"
require_contains "$index_doc" "Commands" "Overview must route to command docs"
require_contains "$index_doc" "kast agent call <method>" "Overview must route advanced payloads through agent call"

require_contains "$install_doc" "## Developer machine" "Install docs must distinguish the developer-machine path"
require_contains "$install_doc" "kast setup" "Install docs must make one-touch setup primary"
require_contains "$install_doc" "kast setup --dry-run" "Install docs must document setup planning"
require_contains "$install_doc" ".agents/skills/kast" "Install docs must document the shared skill target"
require_contains "$install_doc" "AGENTS.local.md" "Install docs must document local agent guidance"
require_contains "$install_doc" "--agents-md" "Install docs must document explicit agent guidance targets"
require_contains "$install_doc" '<kast files="*.kt, *.kts" type="instructions" replaceTools="grep,search,write">' "Install docs must document fenced agent guidance ownership"
require_contains "$install_doc" "--no-open-ide" "Install docs must document noninteractive onboarding skip"
...
require_contains "$install_repair_doc" "--agents-md" "Install command docs must cover explicit agent guidance targets"
require_contains "$install_repair_doc" "kast setup --dry-run" "Install command docs must cover setup plus runtime bring-up"
require_contains "$install_repair_doc" "agentsMdTargets" "Install command docs must cover guidance target dry-run output"
require_contains "$install_repair_doc" "--no-open-ide" "Install command docs must cover onboarding skip"
...
require_contains "$agent_doc" "--agents-md" "Agent command docs must cover explicit agent guidance targets"
require_contains "$agent_doc" "--no-open-ide" "Agent command docs must cover noninteractive onboarding skip"
...
require_contains "$agent_doc" "kast agent call <method>" "Agent docs must route catalog methods through agent call"
require_contains "$metrics_doc" "kast developer inspect metrics fan-in" "Metrics docs must cover direct metrics"
require_contains "$metrics_doc" "kast agent call database/metrics" "Metrics docs must cover agent metrics"
require_contains "$lsp_doc" "kast agent lsp --stdio" "LSP docs must cover the stdio command"
require_contains "$recipes_doc" "kast agent call raw/rename" "Recipes must cover safe rename planning"
require_contains "$recipes_doc" "kast agent call raw/diagnostics" "Recipes must cover diagnostics"
require_contains "$troubleshooting_doc" "kast --output json ready" "Troubleshooting must start from readiness"
require_contains "$troubleshooting_doc" "kast agent call health" "Troubleshooting must include agent health"
