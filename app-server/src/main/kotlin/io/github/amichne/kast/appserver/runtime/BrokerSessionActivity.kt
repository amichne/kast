...
internal fun interface SessionActivitySink {
    ...
    data object Disabled : SessionActivitySink { override fun publish(activity: SessionActivity) = Unit }
}
...
internal class JsonLineSessionActivitySink(private val output: PrintStream) : SessionActivitySink {
    @Synchronized override fun publish(activity: SessionActivity) { output.println(activity.document()) }
}
