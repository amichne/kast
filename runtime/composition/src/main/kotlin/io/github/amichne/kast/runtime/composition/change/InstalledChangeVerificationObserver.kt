...
internal class InstalledChangeVerificationObserver(
     ...
     *
     * Observed AddDeclaration evidence carries the exact physical postimage, a freshly discovered G1 selector for the
     * original target, complete G1 relation and diagnostic batches, and a separately compiler-resolved declaration
     * delta. Moved generations, asynchronous compiler effects, incomplete evidence, and unsupported intent families
     * fail closed.
     */
    override fun observe(request: ChangeVerificationObservationRequest): ChangeVerificationObservation {
        ...
        val observedSource =
            when (val result = sources.observe(request.applied.source)) {
                is SourceObservationResult.Observed -> result.source as? ObservedMutationSource ?: return rejected()
        ...
        val relations =
            plan.evidence.relations.map { planned ->
                val compilation =
                    when (
                        val read = awaitCompilerRead {
        ...
        val diagnosticCompilation =
            when (
                val read = awaitCompilerRead {
                    semantic.diagnostic.check(scope)
                }
            ) {
                is InstalledCompilerRead.Completed -> read.value as? DiagnosticCompilation.Complete ?: return rejected()
                is InstalledCompilerRead.Rejected -> return rejected()
            }
        val diagnostic =
            DiagnosticCheckResult.Complete(
    ...
    private fun resolveUnique(
        ...
        val pattern =
            when (val parsed = SymbolDiscoveryPattern.parse(name)) {
        ...
        val compilation =
            when (
                ...
                is InstalledCompilerRead.Completed -> read.value as? SymbolCompilation.Compiled ?: return null
        ...
        val candidates =
            batch.candidates.withIndex().filter { indexed ->
        ...
        val selection =
            when (
                val selected =
                    SymbolDiscoverySelection.select(
                        batch,
                        candidate.index,
                    )
            ) {
...
private sealed interface InstalledCompilerRead<out Value> {
    data class Completed<Value>(val value: Value) : InstalledCompilerRead<Value>

    data class Rejected(val failure: InstalledCompilerReadFailure) : InstalledCompilerRead<Nothing>
}

/**
 * Proof transition: `suspend () -> Value -> InstalledCompilerRead<Value>`.
 *
 * Completed establishes that the compiler read reached its terminal result even when IntelliJ suspended it behind write
 * priority. [InstalledCompilerRead.Rejected] closes interruption and unexpected coroutine failure. Live compiler values
 * remain inside the calling observation.
 */
private fun <Value> awaitCompilerRead(block: suspend () -> Value): InstalledCompilerRead<Value> {
