...
class ExecutionBudgetTest {
    ...
    fun `caller allowances are clamped independently and retain selection evidence`() {
        ...
        val grant =
            AdmittedExecutionBudget.admit(
                ...
                ExecutionBudgetCapacity(
                    ElapsedTimeLimitMillis.parse(300).proven(),
                    ResultLimit.parse(200).proven(),
                    bytes(250),
                ),
            )
        ...
        assertEquals(
            setOf(ExecutionBudgetClamp.OPERATOR_CEILING, ExecutionBudgetClamp.DEADLINE_REMAINING),
            grant.elapsed.clamping,
        )
        assertEquals(setOf(ExecutionBudgetClamp.OPERATOR_CEILING), grant.work.clamping)
        assertEquals(
            setOf(ExecutionBudgetClamp.OPERATOR_CEILING, ExecutionBudgetClamp.TRANSPORT_CAPACITY),
            grant.results.clamping,
        )
    ...
    fun `absent values select configured defaults and smaller caller values are never ignored`() {
        val capacity =
            ExecutionBudgetCapacity(
                ElapsedTimeLimitMillis.parse(1_000).proven(),
                ResultLimit.parse(1_000).proven(),
                bytes(1_000),
            )
        val default =
            AdmittedExecutionBudget.admit(
        ...
        val small =
            AdmittedExecutionBudget.admit(
                RequestedExecutionBudget(work = ExecutionAllowance.Requested(WorkUnitLimit.parse(1).proven())),
                resources(100),
                bytes(100),
                resources(400),
                bytes(400),
    ...
    private fun resources(value: Int) =
        ResourceBudget(
            ResultLimit.parse(value).proven(),
            WorkUnitLimit.parse(value.toLong()).proven(),
            ElapsedTimeLimitMillis.parse(value.toLong()).proven(),
        )
