...
internal class DemoTerminal(
    ...
    val width: Int get() = terminal.size.width.coerceAtLeast(80)
    ...
    ): Widget = SequenceWidget(buildList {
        add(sectionHeading("Act 1 of 3 — Text Search"))
        add(
            Text(
                STYLE_DIM("grep -rn \"$symbolName\" --include=\"*.kt\""),
                whitespace = Whitespace.PRE,
            )
        )
        add(Text(""))
        add(act1CategoryTable(summary))
        add(Text(""))
        if (summary.sampleMatches.isNotEmpty()) {
            add(act1SamplesTable(workspaceRoot, summary))
            add(Text(""))
        }
        add(
            Text(
                "${STYLE_FAIL("${summary.totalMatches} grep hits")}. " +
                    STYLE_DIM("No type information. No scope. Just noise."),
                whitespace = Whitespace.PRE,
            )
        )
        add(
            Text(
                STYLE_DIM(
                    "sed across ${summary.filesTouched} files would touch " +
                        "${summary.falsePositives} non-symbol matches."
                ),
                whitespace = Whitespace.PRE,
            )
        )
    ...
    fun act2Semantic(
        ...
        add(sectionHeading("Act 2 of 3 — Symbol Resolution"))
        add(
            Text(
                "${STYLE_DIM("kast resolve ⇒")} ${STYLE_HEAD(resolvedSymbol.fqName)}",
                whitespace = Whitespace.PRE,
            )
        )
        add(Text(""))
        val declared = "${relativise(workspaceRoot, resolvedSymbol.location.filePath)}:${resolvedSymbol.location.startLine}"
        add(Text("Declared in: $declared", whitespace = Whitespace.PRE))
        resolvedSymbol.visibility?.let {
            add(Text("Visibility:  ${it.toString().lowercase()}", whitespace = Whitespace.PRE))
        }
        add(Text("Kind:        ${resolvedSymbol.kind.name.lowercase()}", whitespace = Whitespace.PRE))
        add(Text(""))
        add(act2ReferenceTable(workspaceRoot, references))
        if (rename.edits.isNotEmpty()) {
            add(Text(""))
            add(act2RenamePanel(workspaceRoot, resolvedSymbol, rename))
        }
        if (callHierarchy.stats.totalNodes > 0) {
            add(Text(""))
            add(act2CallSummary(callHierarchy))
        }
    })
    ...
    fun act3CallerTree(
        ...
        add(sectionHeading("Act 3 of 3 — Caller Graph (depth $depth)"))
        val lines = renderCallTreeLines(workspaceRoot, callHierarchy.root)
        if (lines.isEmpty()) {
            add(
                Text(
                    STYLE_DIM("No callers found — this is a root entry point."),
                    whitespace = Whitespace.PRE,
                )
            )
        } else {
            add(Text(lines.joinToString("\n"), whitespace = Whitespace.PRE))
        }
        add(Text(""))
        add(
            Text(
                "${STYLE_OK("${callHierarchy.stats.totalNodes} symbols")} reachable in $depth hops. " +
                    STYLE_DIM("Every edge is a compiler-verified call site."),
                whitespace = Whitespace.PRE,
            )
        )
    })
    ...
    private fun renderCallTreeLines(workspaceRoot: Path, root: CallNode): List<String> {
        ...
        fun walk(node: CallNode, prefix: String, isLast: Boolean, depth: Int) {
            ...
            val module = inferModule(rel)
            val moduleTag = moduleColor(module)("[$module]")
            lines += "$prefix$branch$styledName  $moduleTag  ${STYLE_DIM("$rel:${sym.location.startLine}")}"
