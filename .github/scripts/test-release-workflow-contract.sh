...
require_contains "$ci_workflow" "Download headless backend distribution" "CI install-container validation must use the headless backend distribution"
require_contains "$ci_workflow" "backend-headless-*-portable.zip" "CI install-container validation must package the headless backend"
...
require_contains "$release_workflow" "Download headless backend artifact" "Ubuntu/Debian bundle packaging must consume the headless backend artifact"
require_contains "$release_workflow" "headless-artifact/dist/headless.zip" "Ubuntu/Debian bundle packaging must use the headless backend zip"
...
require_contains "$release_workflow" "build-headless-backend" "Ubuntu/Debian bundle job must depend on the headless backend build"
...
require_contains "$ubuntu_debian_docker_validator" "kast up \\" "Ubuntu/Debian Docker validator must start the installed headless backend"
require_contains "$ubuntu_debian_docker_validator" "--backend=headless" "Ubuntu/Debian Docker validator must select the headless backend"
...
require_contains "$ubuntu_debian_docker_validator" "kast capabilities --backend=headless --workspace-root=\"\${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}\"" "Ubuntu/Debian Docker validator must query the installed headless backend"
...
require_contains "$ubuntu_debian_packager" "backend-headless" "Ubuntu/Debian packager must consume the headless backend archive"
require_contains "$ubuntu_debian_packager" "headless-backend" "Ubuntu/Debian bundle manifest must identify the headless backend role"
