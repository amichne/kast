        ...
        scope.launch {
            val path =
                try {
                    ...
                    result.accept(
                        HostedQueryWire.encode(HostedIndexResult.Rejected(HostedQueryFailure.INVALID_SELECTION))
                    )
                    return@launch
                }
            ...
            val lookup =
                when (val admitted = HostedClassLookup.parse(parsed.value, name)) {
        ...
        scope.launch {
            val path =
                try {
                    ...
                    result.accept(
                        HostedQueryWire.encode(HostedQueryResult.Rejected(HostedQueryFailure.INVALID_SELECTION))
                    )
                    return@launch
                }
            val root =
                when (val parsed = CanonicalWorkspaceRoot.fromCanonicalPath(path)) {
            ...
            val selection =
                when (val parsed = HostedKotlinSelection.parse(root, relativeFile, nameOffset)) {
                    is Refinement.Refined -> parsed.value
                    is Refinement.Rejected -> {
...
internal fun packagedHostedCompatibility(): Refinement<HostedCompatibility, HostedQueryFailure> {
    ...
    try {
        val input =
            HostedQueryProbe::class.java.getResourceAsStream("/kast-hosted-query.properties") ?: return unavailable
    ...
    val candidate =
        IdeHostCompatibilityCandidate(
            metadata.getProperty("ideBuild"),
            metadata.getProperty("kotlinBuild"),
            metadata.getProperty("version"),
            "kast.ide-hosted.runtime.v1",
            metadata.getProperty("registryDigest"),
            metadata.getProperty("schemaDigest"),
            emptyList(),
        )
