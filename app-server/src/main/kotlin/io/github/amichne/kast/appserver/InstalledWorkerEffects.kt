...
sealed interface InstalledWorkerStartup {
    ...
    data class Seed(
        override val ideHome: Path?,
        val sourceSystem: Path?,
        val consent: WorkerSeedConsentSelection = WorkerSeedConsentSelection.PREGRANTED,
    ) : InstalledWorkerStartup
}

class InstalledWorkerStartRequest
...
private constructor(
    ...
    companion object {
        fun admit(
            ...
            socket: Path,
            attempt: SemanticRuntimeBootstrapAttemptId,
        ): Refinement<InstalledWorkerEndpoint, WorkerControlFailure> =
...
sealed interface InstalledWorkerStart {
    data class Ready(
        val endpoint: InstalledWorkerEndpoint,
        val binding: WorkerRouteBinding = WorkerRouteBinding.EffectBoundary,
    ) : InstalledWorkerStart

    data class Rejected(val failure: WorkerControlFailure) : InstalledWorkerStart
}

enum class InstalledWorkerObservation {
    EXACT_READY,
    UNPROVEN,
}

enum class InstalledWorkerRetirement {
    EXACT_RETIRED,
    UNPROVEN,
}
...
interface InstalledWorkerEffects {
    ...
    data object Unavailable : InstalledWorkerEffects {
        override suspend fun start(request: InstalledWorkerStartRequest) =
            InstalledWorkerStart.Rejected(WorkerControlFailure.UNAVAILABLE)
...
sealed interface WorkerRouteBinding {
    ...
    private constructor(
        ...
        companion object {
            ...
            ): Refinement<Installation, WorkerControlFailure> =
                try {
                    val epoch = java.util.UUID.fromString(stateEpoch)
                    val generation = java.util.UUID.fromString(serviceGeneration)
                    if (
                        !installationId.matches(Regex("[A-Za-z0-9._:-]{1,256}")) ||
                            epoch.toString() != stateEpoch ||
                            generation.toString() != serviceGeneration ||
                            !configurationIdentity.matches(Regex("[0-9a-f]{64}")) ||
                            workspaceRevision < 1
                    )
                        Refinement.Rejected(WorkerControlFailure.IDENTITY_REJECTED)
                    else
                        Refinement.Refined(
                            Installation(installationId, epoch, generation, configurationIdentity, workspaceRevision)
                        )
