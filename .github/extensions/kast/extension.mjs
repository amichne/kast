...
async function readCliVersion(path) {
    ...
    if (!looksLikeKastCliVersion(stdout)) {
        return null;
    }
...
async function resolveKastBinary() {
    ...
    if (pathResult.ok) {
        addCandidate(pathResult.stdout.trim());
    }
...
async function supportsWrapperCommands(path) {
    const cmd = `${JSON.stringify(
        path)} rpc '{"jsonrpc":"2.0","method":"health","id":1}' --workspace-root=${JSON.stringify(REPO_ROOT)}`;
    ...
    },
    {
        ...
        parameters: {
            ...
            properties: {
                ...
                regex:                   {
                    type:        "boolean",
                    description: "When true, treats pattern as a regular expression."
                },
    ...
    },
    {
        ...
        parameters: {
            ...
            properties: {
                ...
                targetSymbol:  {
                    type:        "string",
                    description: "Optional simple symbol name to focus the scaffold within targetFile."
                },
    ...
    },
    {
        ...
        parameters: {
            ...
            properties: {
                ...
                kind:           {
                    type:        "string",
                    description: "Optional discriminator: class, function, property, etc."
                },
                containingType: {type: "string", description: "FQ name of the enclosing type for member resolution."},
                fileHint:       {
                    type:        "string",
                    description: ABS_PATH + " Narrows resolution when the same name lives in multiple files."
                },
                workspaceRoot:  {type: "string", description: ABS_PATH + " Defaults to cwd."},
            },
            required:   ["symbol"],
    ...
    },
    {
        ...
        parameters: {
            ...
            properties:           {
                ...
                expectedHash:  {
                    type:        "string",
                    description: "Optional sha256 of the file before edit; protects against concurrent change."
                },
                workspaceRoot: {type: "string", description: ABS_PATH + " Defaults to cwd."},
            },
...
function extractKotlinPath(toolName, toolArgs) {
    ...
    if (!keys || !toolArgs || typeof toolArgs !== "object") {
        return null;
    }
    for (const key of keys) {
        const v = toolArgs[key];
        if (typeof v === "string" && KOTLIN_PATH.test(v)) {
            return v;
        }
        if (Array.isArray(v)) {
            for (const entry of v) {
                if (typeof entry === "string" && KOTLIN_PATH.test(entry)) {
                    return entry;
                }
            }
        }
        ...
        onSessionStart: async () => {
            ...
            if (cliVersion && installedVersion && cliVersion !== installedVersion) {
                const syncResult = await execBash(
                    `${JSON.stringify(bin)} install copilot-extension --target-dir=${JSON.stringify(
                        join(REPO_ROOT, ".github"))} --yes=true`,
                );
                if (syncResult.ok) {
                    await session.log(
            ...
            await session.log(`kast extension ready (binary: ${bin}, version: ${cliVersion ?? "unknown"})`,
                {ephemeral: true});
            execBash(
