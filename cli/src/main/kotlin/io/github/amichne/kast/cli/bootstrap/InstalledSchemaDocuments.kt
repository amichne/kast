 ...
 *
 * Establishes object-shaped open resource components embedded in one closed generated outer schema document.
 * [InstalledSchemaFailure] retains the exact resource and closed object-admission failure. Raw resource text and the
 * encoded document may leave only at metadata composition.
 */
internal fun installedSchema(
    ...
    val operationRegistryObject =
        when (val admission = CliOpenJsonObject.parse(operationRegistry)) {
            is CliOpenJsonObjectAdmission.Admitted -> admission.value
            is CliOpenJsonObjectAdmission.Rejected ->
                return InstalledSchemaConstruction.Rejected(
                    InstalledSchemaFailure(InstalledSchemaResource.OPERATION_REGISTRY, admission.failure)
                )
        }
    val wireSchemaObject =
        when (val admission = CliOpenJsonObject.parse(wireSchema)) {
            is CliOpenJsonObjectAdmission.Admitted -> admission.value
            is CliOpenJsonObjectAdmission.Rejected ->
                return InstalledSchemaConstruction.Rejected(
