...
class IntellijSourceExecutionTest {
    ...
    fun `admitted caller allowance may exceed the configured default`() {
        val defaults = ReadLimits.resolve(environment = mapOf(ReadLimitParameter.SOURCE_ENTITY_WORK.environmentKey to "1")).proven()
        val execution = IntellijSourceExecution(resources(work = 2), defaults) { 0L }
        repeat(2) { assertEquals(SourceExecutionAdmission.ADMITTED, execution.admitUnit()) }
    ...
    private fun resources(work: Long = 100, time: Long = 100): ResourceBudget = ResourceBudget(
        ResultLimit.parse(10).proven(), WorkUnitLimit.parse(work).proven(), ElapsedTimeLimitMillis.parse(time).proven(),
    )
