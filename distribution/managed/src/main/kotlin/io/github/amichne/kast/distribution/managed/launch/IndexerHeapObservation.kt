...
class IndexerHeapObservation private constructor(
    ...
    companion object {
        fun observe(args: Array<String>, maximumBytes: Long): IndexerHeapObservationResult {
            ...
            if (values.size != 1) return IndexerHeapObservationResult.Rejected(IndexerHeapObservationFailure.ARGUMENT_CARDINALITY)
            val raw = values.single()
            val requested = when (val parsed = IndexerHeapSize.parse("${raw}m")) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected -> return IndexerHeapObservationResult.Rejected(IndexerHeapObservationFailure.INVALID_ARGUMENT)
            }
            if (requested.mebibytes.toString() != raw) return IndexerHeapObservationResult.Rejected(IndexerHeapObservationFailure.INVALID_ARGUMENT)
            if (maximumBytes <= 0) return IndexerHeapObservationResult.Rejected(IndexerHeapObservationFailure.INVALID_OBSERVATION)
            return IndexerHeapObservationResult.Observed(IndexerHeapObservation(requested, maximumBytes))
        }
    }
}

enum class IndexerHeapObservationFailure { ARGUMENT_CARDINALITY, INVALID_ARGUMENT, INVALID_OBSERVATION }
