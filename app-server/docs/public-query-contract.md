...
`bind` snapshots the current ordered symbol stream under a bounded query-local name. `join` reads an earlier binding or a retained symbol result on the right and compares canonical symbol identity. Inner join is terminal and requires `binding_rows` output with two distinct column names; each pair retains both exact references and established relation/occurrence evidence without collapsing multiplicity. Semi and anti join keep the left symbol stream and may feed later stages. An incomplete retained right input rejects anti join; an incomplete named binding yields qualified incomplete coverage rather than unsupported absence. Duplicate, unknown, and forward binding names reject before effects. Read-result pages retained binding rows with `binding_rows` output, preserving pairs and row IDs; binding rows cannot seed symbol stages or serve as a join right input. The public grammar has no alternate join key.
