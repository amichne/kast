...
fun main(args: Array<String>) {
    when (val heap = IndexerHeapObservation.observe(args, Runtime.getRuntime().maxMemory())) {
        is IndexerHeapObservationResult.Observed ->
            System.err.println("kast-indexer-heap: ${heap.observation.boundaryDocument()}")
    ...
    when (val bootstrap = KastIndexerBootstrap.configureSystemProperties(args)) {
        is IndexerBootstrapConfiguration.Configured -> {
            val networkArguments =
                when (val admission = IndexerNetworkArguments.admit(args)) {
            ...
            when (
                val network =
                    InstalledNetworkBootstrap.prepare(
                        networkArguments.root,
                        networkArguments.cache,
            ...
            val ideaMain = Class.forName("com.intellij.idea.Main").getMethod("main", Array<String>::class.java)
...
sealed interface IndexerBootstrapConfiguration {
    data class Configured(val ideaArguments: List<String>) : IndexerBootstrapConfiguration

    data class Rejected(val failure: IndexerBootstrapFailure) : IndexerBootstrapConfiguration
}

internal object KastIndexerBootstrap {
    private const val IDEA_HOME_PREFIX = "--idea-home="
    private val LAUNCHER_OWNED_PREFIXES =
        listOf(
            IDEA_HOME_PREFIX,
            "--max-heap-mib=",
            "--java-executable=",
     ...
     *
     * Establishes one canonical physical IDEA home and the headless platform properties required by the installed
     * launcher. [IndexerBootstrapFailure] is the closed expected failure. Raw property and path extraction is permitted
     * only at this outer JVM bootstrap boundary.
     */
    fun configureSystemProperties(args: Array<String>): IndexerBootstrapConfiguration {
        val ideaHome =
            when (val admission = args.ideaHome()) {
                is IdeaHomeAdmission.Admitted -> admission.path
                is IdeaHomeAdmission.Rejected -> return IndexerBootstrapConfiguration.Rejected(admission.failure)
            }
     ...
     *
     * Removes every launcher-owned installed-runtime and private-path option, then prepends the registered application
     * command. Raw application arguments remain inside the launcher-to-IDEA boundary.
     */
     ...
     *
     * Establishes exactly one absolute, normalized, physically canonical IDEA home directory. [IndexerBootstrapFailure]
     * is the closed expected failure. The raw path may leave only for JDK filesystem admission and the `idea.home.path`
     * platform property.
     */
    private fun Array<String>.ideaHome(): IdeaHomeAdmission {
        val values = filter { it.startsWith(IDEA_HOME_PREFIX) }.map { it.removePrefix(IDEA_HOME_PREFIX) }
        if (values.isEmpty()) {
        ...
        return if (canonical == candidate && Files.isDirectory(canonical, LinkOption.NOFOLLOW_LINKS)) {
...
private sealed interface IdeaHomeAdmission {
    data class Admitted(val path: Path) : IdeaHomeAdmission

    data class Rejected(val failure: IndexerBootstrapFailure) : IdeaHomeAdmission
}

internal enum class IndexerNetworkArgumentFailure {
    ROOT_CARDINALITY,
    CACHE_CARDINALITY,
    INVALID_PATH,
}
...
internal class IndexerNetworkArguments private constructor(val root: Path, val cache: Path) {
    companion object {
        fun admit(args: Array<String>): IndexerNetworkArgumentAdmission {
            ...
            if (roots.size != 1)
                return IndexerNetworkArgumentAdmission.Rejected(IndexerNetworkArgumentFailure.ROOT_CARDINALITY)
            if (caches.size != 1)
                return IndexerNetworkArgumentAdmission.Rejected(IndexerNetworkArgumentFailure.CACHE_CARDINALITY)
            return try {
                ...
                if (
                    !root.isAbsolute ||
                        ...
                        state.fileName.toString() != "cache-state" ||
                        !Files.isDirectory(cache) ||
                        cache.toRealPath() != cache ||
