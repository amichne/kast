...
internal sealed interface CliOpenJsonObjectAdmission {
    data class Admitted(val value: CliOpenJsonObject) : CliOpenJsonObjectAdmission

    data class Rejected(val failure: CliOpenJsonObjectFailure) : CliOpenJsonObjectAdmission
}
...
internal value class CliOpenJsonObject private constructor(internal val value: JsonObject) {
    ...
    companion object {
         ...
         *
         * Establishes syntactically valid object-shaped JSON while preserving its deliberately open fields.
         * [CliOpenJsonObjectFailure] is the closed expected failure. The underlying [JsonObject] may leave this type
         * only when embedded in the installed schema document.
         */
...
class CliTextDocument private constructor(override val value: String) : CliProcessOutput {
    companion object {
         ...
         *
         * Establishes a non-blank process text document. [CliTextDocumentFailure] is the closed expected failure. Raw
         * rendered text is admitted only at local-metadata or Clikt output boundaries.
         */
        internal fun admit(value: String): CliTextDocumentAdmission =
            if (value.isBlank()) {
...
internal enum class CliTextDocumentFailure {
    BLANK
}
