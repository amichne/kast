...
require_not_contains "$release_workflow" "installed-semantic-canary" "Release must not depend on the retired local headless canary"
...
require_block_contains "$ci_workflow" "  source-bound-headless-backend:" "  install-ubuntu-debian-container:" ":backend-headless:portableDistZip" "Headless backend production must reuse the release archive owner"
...
require_block_contains "$ci_workflow" "  kast-action-runtime-contract:" "  analysis-server-transport:" "      - prepared-generation" "kast-action validation must consume the immutable CI runtime input directly"
require_block_contains "$ci_workflow" "  kast-action-runtime-contract:" "  analysis-server-transport:" "--kind kast-action-runtime" "kast-action validation must derive only its owned package family"
...
require_contains "$ci_workflow" ":backend-headless:verifyHeadlessPortableDistLayout" "Headless backend production must reuse the owning Gradle layout verifier"
...
require_not_contains "$prepared_generation_packager" "developer local" "CI runtime packaging must not invoke retired developer-machine authority"
...
require_block_not_contains "$ci_workflow" "  kast-action-runtime-contract:" "  analysis-server-transport:" "Free disk for" "kast-action validation must not spend its critical path deleting unrelated runner SDKs"
...
require_not_contains "$ci_workflow" "installed-semantic-fixture:" "Pull-request CI must not run a developer-machine headless semantic fixture"
