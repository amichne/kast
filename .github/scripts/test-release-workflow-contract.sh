...
require_contains "$release_workflow" "publish-setup-kast-action:" "Release must publish the setup-kast action tag"
require_contains "$release_workflow" "Publish setup-kast action tag" "Release must name setup-kast action publication explicitly"
require_contains "$release_workflow" 'action_tag="v1"' "Release must publish the stable setup-kast v1 action tag"
require_contains "$release_workflow" 'git tag -f "$action_tag" "$release_sha"' "Release must move setup-kast v1 to the published release commit"
require_contains "$release_workflow" 'refs/tags/${action_tag}:refs/tags/${action_tag}' "Release must push the setup-kast action tag ref explicitly"
require_contains "$release_workflow" "needs.publish-setup-kast-action.result" "Final release verification must read the setup-kast action publication result"
require_order "$release_workflow" "Publish draft release with provenance annotation" "publish-setup-kast-action:" "setup-kast action tag must publish only after release assets are published"
...
require_contains "$release_state_verifier" 'tag_commit_sha "v1"' "Release state verifier must prove setup-kast v1 resolves"
require_contains "$release_state_verifier" 'setup-kast/action.yml' "Release state verifier must prove setup-kast action metadata exists at v1"
require_contains "$release_state_verifier" 'setup-kast/dist/index.js' "Release state verifier must prove setup-kast compiled action entrypoint exists at v1"
...
require_contains "$setup_kast_action_doc" 'github.com/<owner>/<repo>/<subpath>@<ref>' "setup-kast action docs must document Devin subpath action pinning"
require_contains "$setup_kast_action_doc" 'github.com/amichne/kast/setup-kast@v1' "setup-kast action docs must document the published stable action tag"
require_contains "$setup_kast_action_doc" 'Product release tags remain available for exact pinning' "setup-kast action docs must distinguish stable action and product release tags"
