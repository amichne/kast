...
@JvmInline
value class SourceTextIdentity private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<SourceTextIdentity, SourceTextIdentityFailure>`.
         *
         * Establishes the versioned lowercase SHA-256 identity format used for committed IntelliJ document text.
         * [SourceTextIdentityFailure] is the closed expected failure. Raw identity text may enter only at
         * selector-token restoration boundaries.
         */
        fun parse(raw: String): Refinement<SourceTextIdentity, SourceTextIdentityFailure> {
         ...
         *
         * Hashes the exact Java [Char] sequence as big-endian UTF-16 code units without a byte order mark. The IntelliJ
         * adapter is the only boundary permitted to supply document text and must first prove that the document is
         * committed and line-ending-normalized.
         */
        fun fromNormalizedCommittedText(text: CharSequence): SourceTextIdentity {
