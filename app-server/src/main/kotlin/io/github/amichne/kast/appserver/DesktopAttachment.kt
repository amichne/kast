...
internal enum class DesktopAttachmentFailure {
    OVERRIDE_CONFLICT,
    VERSION_UNSUPPORTED,
    DESKTOP_UNAVAILABLE,
    INSPECTION_REJECTED,
...
internal object DesktopAttachmentPolicy {
    fun admit(environment: Map<String, String>, version: String, bundledGit: Boolean): DesktopAttachmentAdmission =
        when {
            environment["CODEX_APP_SERVER_FORCE_CLI"] == "1" ||
                !environment["CODEX_CLI_PATH"].isNullOrEmpty() ||
                bundledGit -> DesktopAttachmentAdmission.Rejected(DesktopAttachmentFailure.OVERRIDE_CONFLICT)
...
internal enum class DesktopOverride {
    CODEX_CLI_PATH,
...
internal class DesktopBundle private constructor(val path: Path) {
    companion object {
        fun discover(
            ...
            userHome: Path,
        ): Refinement<DesktopBundle, DesktopAttachmentFailure> {
            ...
            if (explicit != null) {
                ...
                val executable =
                    when (val admitted = CodexDesktopExecutable.admit(candidate)) {
                ...
                if (
                    path.nameCount < 4 ||
                        path.parent.fileName.toString() != "MacOS" ||
...
internal object InstalledDesktopAttachmentProbe {
    ...
    private fun inspectFiles(
        ...
        val bundle =
            when (val selected = DesktopBundle.discover(environment, userHome)) {
        ...
        val version =
            when (
                val read =
                    commands.read(DesktopInspectionCommand.ReadVersion(bundle.path.resolve("Contents/Info.plist")))
            ) {
        ...
        return DesktopAttachmentPolicy.admit(
            effective,
            version,
            Files.exists(bundle.path.resolve("Contents/Resources/git/bin/git")),
        )
    }
}
...
internal object NativeDesktopInspectionCommands : DesktopInspectionCommands {
    override fun read(command: DesktopInspectionCommand): DesktopInspectionRead {
        ...
        return try {
            ...
            if (!acceptedExit || output.size > BrokerOperationalLimits.maximumDesktopInspectionBytes)
                DesktopInspectionRead.Rejected
