...
internal class InstalledGradleModelInputs private constructor(
    ...
    companion object {
            ...
            try {
                if (!Files.isDirectory(workspaceRoot, LinkOption.NOFOLLOW_LINKS) ||
                    ...
                    when (val result = visit(workspaceRoot, Path.of(""), emptySet(), entries)) {
                        ...
                        is Refinement.Refined -> Refinement.Refined(InstalledGradleModelInputs(workspaceRoot, entries.toMap()))
                    }
                }
            } catch (_: IOException) {
                Refinement.Rejected(InstalledGradleModelCaptureFailure.MODEL_INPUTS_UNAVAILABLE)
        ...
        fun readProperties(root: Path, logical: Path): Refinement<InstalledGradleProperties, InstalledGradleModelCaptureFailure> {
            var prefix = Path.of("")
            try {
                val canonicalRoot = root.toRealPath()
                var parent = canonicalRoot
                for (component in logical) {
                    if (!Files.isDirectory(parent, LinkOption.NOFOLLOW_LINKS)) return rejected(logical, ModelInputFailureReason.UNSUPPORTED)
                ...
                val entry = when (val resolved = resolve(canonicalRoot, logical)) {
                    is Refinement.Rejected -> return resolved
                ...
                val bytes = when (val content = readResolvedInput(entry.path, logical) { directory, name ->
                    directory.newByteChannel(name, setOf(java.nio.file.StandardOpenOption.READ, LinkOption.NOFOLLOW_LINKS)).use { channel ->
        ...
        private fun rejected(logical: Path, reason: ModelInputFailureReason): Refinement.Rejected<InstalledGradleModelCaptureFailure> =
            when (val path = ModelInputPath.admit(logical.toString().ifEmpty { "." })) {
                is Refinement.Refined -> Refinement.Rejected(InstalledGradleModelCaptureFailure.ModelInputRejected(
                    ModelInputFailure(path.value, reason),
                ))
                is Refinement.Rejected -> Refinement.Rejected(InstalledGradleModelCaptureFailure.MODEL_INPUTS_UNAVAILABLE)
            }
        ...
        private fun resolve(
            ...
            try {
                for (component in if (candidate == root) Path.of("") else candidate.subpath(root.nameCount, candidate.nameCount)) {
                    if (component.toString().isNotEmpty() && !Files.readAttributes(cursor, BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS).isDirectory) {
                        return rejected(logical, ModelInputFailureReason.UNSUPPORTED)
                    }
                    ...
                    val attributes = Files.readAttributes(cursor, BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS)
                ...
                return Refinement.Refined(ResolvedEntry(
                    cursor, links.toList(), Files.readAttributes(cursor, BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS),
                ))
        ...
        private fun visit(
            ...
            val resolved = when (val result = resolve(root, logical)) {
            ...
            try {
                if (resolved.attributes.isDirectory) {
                    ...
                    if (selected(logical) || resolved.links.isNotEmpty()) {
                        entries[logical] = EntryIdentity(resolved.links, root.relativize(resolved.path), EntryContent.Directory)
                    }
                    val children = when (val read = readResolvedInput(resolved.path, logical) { parent, name ->
                    ...
                    for (name in children) {
                        ...
                        val attributes = Files.readAttributes(physical, BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS)
                        if (name.toString() in excludedDirectories && (attributes.isDirectory || attributes.isSymbolicLink)) continue
                        if (attributes.isDirectory || selected(child) ||
                    ...
                    when (val read = readResolvedInput(resolved.path, logical) { parent, name ->
                        parent.newByteChannel(name, setOf(java.nio.file.StandardOpenOption.READ, LinkOption.NOFOLLOW_LINKS)).use { channel ->
                    ...
                    entries[logical] = EntryIdentity(resolved.links, root.relativize(resolved.path), EntryContent.File(
        ...
        /**
         * Open resolved input content relative to held directory descriptors. Every ancestor and
         * the final entry refuse links, so a concurrent replacement cannot redirect a read outside
         * the proven target. Platforms without descriptor-relative access fail closed.
         */
        private fun <T> readResolvedInput(
            ...
            fun descend(parent: SecureDirectoryStream<Path>, index: Int): Refinement<T, InstalledGradleModelCaptureFailure> =
                if (index == target.nameCount - 1) Refinement.Refined(read(parent, target.getName(index)))
                else parent.newDirectoryStream(target.getName(index), LinkOption.NOFOLLOW_LINKS).use { child ->
                    descend(child, index + 1)
                }
            return try {
                Files.newDirectoryStream(target.root).use { filesystem ->
                    if (filesystem !is SecureDirectoryStream<Path>) rejected(logical, ModelInputFailureReason.UNSUPPORTED)
