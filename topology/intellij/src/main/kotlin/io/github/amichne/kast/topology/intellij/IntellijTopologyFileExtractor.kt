...
class IntellijTopologyFileExtractor internal constructor(private val mismatchRetrier: TopologyVfsMismatchRetrier) {
    constructor() : this(TopologyVfsMismatchRetrier(InstalledTopologySourceRootVfsSynchronizer))

    private val registries = TopologyProjectionRegistryCache()
    private val readEpochs = TopologyReadEpochCache()

    /**
     * Proof transition: `(Project, PublishedWorkspace, TopologyExtractionRequest) -> TopologyFileExtraction`.
     *
     * Complete output establishes detached compiler symbols and repository-internal edges for the exact admitted file.
     * Candidate PSI is projected once per exact content generation; only the requested file is reloaded and receives
     * terminal output. [TopologyFileExtractionFailure] is the ordinary closed expected failure. Cancellation propagates
     * and live Project, VFS, PSI, and K2 values never leave the read action.
     */
    suspend fun extract(
        project: Project,
    ...
    private suspend fun extractOnce(
        ...
        if (
            project.isDisposed ||
                current.readLease != request.candidates.workspace.lease ||
    ...
    private fun extractInReadAction(
        ...
        val resolution =
            readEpochs.resolve(registryKey, request.file) {
    ...
    private fun extractUncached(
        ...
        val requested =
            when (val lookup = load(project, request.file)) {
        ...
        val requestedSymbols = requested.declarations.mapNotNull(symbolByDeclaration::get).distinct().sorted()
        ...
        PsiTreeUtil.collectElementsOfType(requested.psi, KtReferenceExpression::class.java)
            ...
            .forEach { reference ->
                val source =
                    when (val owner = reference.owningSymbol(symbolByDeclaration)) {
                        is OwningTopologySymbol.Found -> owner.symbol
                        OwningTopologySymbol.Unresolved -> return@forEach
                    }
                val occurrence =
                    when (val refined = reference.topologyOccurrence()) {
                ...
                val target =
                    when (val resolved = reference.topologyTarget(registry, identitySource, sourceLookup)) {
                ...
                when (edge) {
                    ...
                    is Refinement.Rejected -> return failed(request.file, TopologyFileExtractionFailure.EDGE_REJECTED)
                }
            }
        requested.declarations.forEach { declaration ->
    ...
    /**
     * Proof transition: `(Project, TopologyExtractionRequest, TopologyProjectionRegistryKey) ->
     * TopologyProjectionRegistryResolution`.
     *
     * Ready establishes one detached symbol registry covering the exact content-identified candidate generation.
     * Rejected preserves [TopologyFileExtractionFailure] for unavailable PSI or an inadmissible detached fact. Live
     * Project, PSI, and K2 values remain inside this read action; only detached symbols enter the registry cache.
     */
    private fun buildRegistry(
        ...
        return when (val registry = TopologyProjectionRegistry.from(key, symbols)) {
            ...
            is Refinement.Rejected ->
                TopologyProjectionRegistryResolution.Rejected(
                    request.file,
    ...
    private fun loadRegistered(
        ...
        val loaded =
            admittedSources[candidate.symbol.file]
                ?: when (val result = load(project, candidate.symbol.file)) {
                    is TopologyFileLoad.Loaded -> result.file
                    TopologyFileLoad.Unavailable ->
                        return TopologyRegisteredSource.LoadFailed(TopologyFileExtractionFailure.FILE_UNAVAILABLE)
                    TopologyFileLoad.DocumentDirty ->
                        return TopologyRegisteredSource.LoadFailed(TopologyFileExtractionFailure.DOCUMENT_DIRTY)
                    TopologyFileLoad.PsiDocumentUncommitted ->
                        return TopologyRegisteredSource.LoadFailed(
                            TopologyFileExtractionFailure.PSI_DOCUMENT_UNCOMMITTED
                        )
                    TopologyFileLoad.VfsContentMismatch ->
                        return TopologyRegisteredSource.LoadFailed(TopologyFileExtractionFailure.VFS_CONTENT_MISMATCH)
                    TopologyFileLoad.NotKotlinPsi ->
                        return TopologyRegisteredSource.LoadFailed(TopologyFileExtractionFailure.NOT_KOTLIN_PSI)
                }
        admittedSources[candidate.symbol.file] = loaded
        val range = candidate.symbol.evidence.range
        val declaration =
            loaded.declarationsByRange[TextRange(range.startInclusive, range.endExclusive)]?.singleOrNull()
                ?: return TopologyRegisteredSource.DeclarationUnavailable
     ...
     *
     * Loaded establishes valid VFS and Kotlin PSI evidence whose live bytes retain the exact admitted source-content
     * identity. Unavailable, dirty document, uncommitted PSI document, VFS mismatch, and non-Kotlin PSI are distinct
     * closed failures. Raw VFS bytes, documents, and PSI extraction stay in this K2 adapter.
     */
    private fun load(project: Project, file: TopologySourceFile): TopologyFileLoad {
        ...
        val virtualFile =
            LocalFileSystem.getInstance().findFileByPath(absolute.toString()) ?: return TopologyFileLoad.Unavailable
        ...
        if (document != null) {
            when (
                TopologyDocumentReadiness.observe(
                ...
                TopologyDocumentReadiness.DOCUMENT_DIRTY -> return TopologyFileLoad.DocumentDirty
                TopologyDocumentReadiness.PSI_DOCUMENT_UNCOMMITTED -> return TopologyFileLoad.PsiDocumentUncommitted
            }
        }
        val content =
            try {
        ...
        val identified =
            when (val validation = LiveTopologySourceContent.validate(file, content)) {
                is Refinement.Refined -> validation.value
                is Refinement.Rejected -> return TopologyFileLoad.VfsContentMismatch
            }
        val psi =
            PsiManager.getInstance(project).findFile(virtualFile) as? KtFile ?: return TopologyFileLoad.NotKotlinPsi
        val declarations =
            PsiTreeUtil.collectElementsOfType(psi, KtNamedDeclaration::class.java)
...
private sealed interface TopologyReferenceTarget {
    data class Found(val binding: ProvenTopologyBinding) : TopologyReferenceTarget

    data object Unresolved : TopologyReferenceTarget

    data class Mismatched(val evidence: TopologyIdentityMismatchEvidence) : TopologyReferenceTarget
...
/**
 * Proof transition: `(KtReferenceExpression, TopologyProjectionRegistry) -> TopologyReferenceTarget`.
 *
 * Found establishes one exact location-bearing topology symbol owned by the exact candidate generation; unresolved and
 * rejected targets stay closed. Raw reference resolution remains inside the request-local K2 analysis session.
 */
private fun KtReferenceExpression.topologyTarget(
    ...
    for (native in references.filterIsInstance<KtReference>()) {
        when (
            val projection =
                analyze(native.element) {
                    val symbol = native.resolveToSymbol() ?: return@analyze TopologyIdentityResolution.Unsupported
                    topologyIdentityProjection(symbol, registry, source, lookup)
                }
        ) {
            is TopologyIdentityResolution.Matched -> return TopologyReferenceTarget.Found(projection.binding)
            ...
            is TopologyIdentityResolution.Mismatched -> return TopologyReferenceTarget.Mismatched(projection.evidence)
 ...
 *
 * Found preserves the nearest enclosing admitted declaration symbol; Unresolved is the closed absence state. Raw PSI
 * parent traversal stays within this file extraction boundary.
 */
private fun KtReferenceExpression.owningSymbol(symbols: Map<KtNamedDeclaration, TopologySymbol>): OwningTopologySymbol {
    val symbol =
        generateSequence(parent) { it.parent }
            .filterIsInstance<KtNamedDeclaration>()
            .mapNotNull(symbols::get)
            .firstOrNull()
    return if (symbol == null) OwningTopologySymbol.Unresolved else OwningTopologySymbol.Found(symbol)
}
 ...
 *
 * Admitted establishes a non-empty direct or enclosing super-type call source range. Rejected closes synthetic
 * references with no physical source anchor. Raw PSI remains inside the request-local IntelliJ extraction boundary.
 */
...
): TopologyFileExtraction.IdentityMismatch = TopologyFileExtraction.IdentityMismatch(evidence, cacheDisposition)

private fun TopologyFileExtraction.withCacheDisposition(disposition: TopologyCacheDisposition): TopologyFileExtraction =
    when (this) {
        is TopologyFileExtraction.Complete,
        is TopologyFileExtraction.Failed -> this
