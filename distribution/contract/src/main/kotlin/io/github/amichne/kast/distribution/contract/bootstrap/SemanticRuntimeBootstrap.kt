...
enum class SemanticRuntimeBootstrapFailure(val wireName: String) {
    ...
    INDEX_BOOTSTRAP_IMPORTED_MODULES_UNAVAILABLE("index-bootstrap-imported-modules-unavailable"),
...
object SemanticRuntimeBootstrapFailureSerializer : KSerializer<SemanticRuntimeBootstrapFailure> {
...
@Serializable
@JvmInline
value class SemanticRuntimeBootstrapAttemptId private constructor(val value: String) {
    init {
...
enum class SemanticRuntimeBootstrapAttemptIdFailure {
    INVALID_IDENTITY
}
...
object SemanticRuntimeBootstrapCodec {
    ...
    /**
     * Proof transition: `String -> Refinement<SemanticRuntimeBootstrapState, SemanticRuntimeBootstrapDocumentFailure>`.
     */
    fun decode(
        document: String
    ): Refinement<
    ...
    > {
        val schemaVersion =
            try {
                val envelope = json.parseToJsonElement(document) as? JsonObject ?: return malformedDocument()
                envelope["schemaVersion"]?.jsonPrimitive?.intOrNull ?: return malformedDocument()
        ...
        if (schemaVersion != BOOTSTRAP_SCHEMA_VERSION) {
            return Refinement.Rejected(SemanticRuntimeBootstrapDocumentFailure.UNSUPPORTED_SCHEMA)
        }
        return try {
            val decoded =
    ...
    private fun malformedDocument(): Refinement.Rejected<SemanticRuntimeBootstrapDocumentFailure> =
...
sealed interface SemanticRuntimeBootstrapCause {
    ...
    data class Standard(val failure: SemanticRuntimeBootstrapFailure) : SemanticRuntimeBootstrapCause {
        init {
            require(failure != SemanticRuntimeBootstrapFailure.MODEL_INPUT_REJECTED)
        }
    }
