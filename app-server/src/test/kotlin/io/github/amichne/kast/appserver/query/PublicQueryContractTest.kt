...
class PublicQueryContractTest {
    ...
    fun `public enum inputs and encoding use caps case`() {
        val input =
            """{"type":"QUERY","from":{"type":"SEARCH","query":"Order","match":"FUZZY","kinds":["TYPE_ALIAS"]},"steps":[{"type":"EXPAND","relation":"TYPE_USES"},{"type":"FILTER","visibility":["PUBLIC"]}],"select":["NAME","SIGNATURE"]}"""
    ...
    fun `pipeline order survives lowering`() {
        val request =
            parse(
                """{"type":"QUERY","from":{"type":"SEARCH","query":"submitOrder"},"steps":[{"type":"EXPAND","relation":"CALLERS"},{"type":"FILTER","visibility":["PUBLIC"]}]}"""
            )
    ...
    fun `public encoding remains acceptable to the public CLI serializer`() {
        val admittedForRoundTrip =
            json.decodeFromString(
                PublicQueryRequestSerializer,
                """{"type":"QUERY","from":{"type":"SEARCH","query":"Order","kinds":["CLASS"]},"steps":[{"type":"EXPAND","relation":"TYPE_USES"}],"select":[]}""",
            )
    ...
    fun `directory containment defaults without dropping source-set restrictions`() {
        val request =
            parse(
                """{"type":"QUERY","from":{"type":"ALL","scope":{"type":"DIRECTORY","value":"services/orders","sourceSets":["main"]}}}"""
            )
    ...
    fun `schema proof from another contract cannot enter query lowering`() {
        val other =
            io.github.amichne.kast.appserver.schema.NetworkntJsonSchemaCompiler.compile(
                Json.parseToJsonElement("""{"type":"object"}""") as kotlinx.serialization.json.JsonObject
            ) as Refinement.Refined
        val value =
            other.value.admit(Json.parseToJsonElement("""{"type":"QUERY","from":{"type":"ALL"}}"""))
    ...
    fun `custom source-set names survive lowering and public round trip`() {
        val input =
            """{"type":"QUERY","from":{"type":"ALL","scope":{"type":"DIRECTORY","value":".","sourceSets":["integrationTest","commonMain","jvmTest"]}}}"""
        ...
        assertEquals(
            listOf("integrationTest", "commonMain", "jvmTest"),
            source.scope.sourceSets.values.map { it.value },
        )
    }
    ...
    fun `containment belongs to either exclusive scope target and round trips`() {
        listOf("DIRECTORY" to "services", "PACKAGE" to "com.example").forEach { (type, value) ->
            listOf("DIRECT" to QueryContainmentDocument.DIRECT, "RECURSIVE" to QueryContainmentDocument.DESCENDANTS)
    ...
    fun `scope discriminator refines value into its own grammar`() {
        ...
        val directRoot =
            parse(
                """{"type":"QUERY","from":{"type":"ALL","scope":{"type":"DIRECTORY","value":".","containment":"DIRECT"}}}"""
            )
    ...
    private fun parse(input: String): QueryRunRequest =
        json.decodeFromString(PublicQueryRequestSerializer, input).canonicalRequest
