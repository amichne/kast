...
fun interface InstalledSymbolScopeOperations {
     ...
     *
     * A compiled result preserves the exact imported Gradle model for the lease; the closed rejected variant remains
     * the only unavailable state. Raw model extraction is prohibited.
     */
...
class InstalledIntellijSymbolPorts
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes ports that admit only the current exact-root publication before locating a live project. Missing
         * projects and moved generations remain closed compiler rejections; every live IntelliJ value is obtained and
         * consumed within one request call.
         */
        fun create(
...
private fun unavailableResolution(): SymbolResolutionCompilation =
    SymbolResolutionCompilation.Rejected(SymbolExactCompilerRejection.WORKSPACE_INDEX_UNAVAILABLE)

private fun unavailableDescription(): SymbolDescriptionCompilation =
    SymbolDescriptionCompilation.Rejected(SymbolExactCompilerRejection.WORKSPACE_INDEX_UNAVAILABLE)
