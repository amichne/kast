...
- On macOS, runtime demand owns one deterministic exact-endpoint launchd service so the indexer
  ...
  is successful admission; duplicate submission races resolve by exact-label re-observation.
  Preserve that accepted session capability while polling: an owned session may continue through
  crash backoff, while terminal absence, rejected observation, and interruption are distinct finite
  startup failures. Lifecycle stop retires the service before endpoint markers.
