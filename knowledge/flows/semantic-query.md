...
The current [public tool contracts](../contracts/public-tools.md) distinguish presentation identity from canonical operation identity. Three ordinary searches and deferred `query_symbols` share `query.run`; `check_diagnostics` shares `diagnostic.check`. Private admission retains each tool's schema identity and typed syntax through its exact CLI binding. `tool query_symbols` reaches the installed daemon through a versioned Unix-socket RPC; the daemon re-admits the exact public schema and canonical root, then uses its shared existing-IDE preparation and read path. Other `tool` commands still use the direct existing-IDE read path. Operation effects, budgets, reference authority and exhaustive outcomes remain with their existing owners.
