...
internal class CanonicalTopologyBuildHandler(private val operations: TopologyBuildOperations) :
    OperationHandler<
    ...
    > {
    override suspend fun execute(request: TopologyBuildRequest) =
        when (val result = operations.build()) {
            ...
            TopologyBuildResult.WorkspaceMoved -> OperationOutcome.Rejected(TopologyBuildRejection.WorkspaceMoved)
    ...
    > {
        val digest =
            when (val text = TopologyBuildDigest.parse(snapshot.manifest.digest.value)) {
                is Refinement.Refined -> text.value
                is Refinement.Rejected ->
                    return OperationOutcome.Rejected(
                        TopologyBuildRejection.PublicationFailed(TopologyPublicationRejection.CONTRACT_VIOLATION)
                    )
            }
...
private fun TopologyBuildFailure.toProtocol(): TopologyBuildRejection =
    when (this) {
        ...
        is TopologyBuildFailure.SnapshotRead ->
            TopologyBuildRejection.SnapshotUnavailable(
                when (failure) {
        ...
        is TopologyBuildFailure.Enumeration -> TopologyBuildRejection.EnumerationFailed(failure.toProtocol())
        is TopologyBuildFailure.Extraction ->
            when (val parsed = ProtocolText.parse(file.value)) {
        ...
        TopologyBuildFailure.ExtractionContractViolation -> TopologyBuildRejection.ExtractionContractViolation
        is TopologyBuildFailure.Coverage ->
            when (val projected = failure.toProtocolCoverage()) {
                is Refinement.Refined -> TopologyBuildRejection.CoverageIncomplete(projected.value)
                is Refinement.Rejected -> TopologyBuildRejection.CoverageProjectionFailed(projected.failure)
            }
        is TopologyBuildFailure.Publication -> TopologyBuildRejection.PublicationFailed(failure.toProtocol())
    }

private fun TopologyCandidateEnumerationFailure.toProtocol(): TopologyEnumerationRejection =
    when (this) {
        TopologyCandidateEnumerationFailure.WORKSPACE_UNAVAILABLE -> TopologyEnumerationRejection.WORKSPACE_UNAVAILABLE
        ...
        TopologyCandidateEnumerationFailure.CANDIDATE_REJECTED -> TopologyEnumerationRejection.CANDIDATE_REJECTED
    }

private fun TopologyExtractionFailure.toProtocol(): TopologyExtractionRejection =
    when (this) {
        TopologyExtractionFailure.PROJECT_UNAVAILABLE -> TopologyExtractionRejection.PROJECT_UNAVAILABLE
        ...
        TopologyExtractionFailure.PSI_DOCUMENT_UNCOMMITTED -> TopologyExtractionRejection.PSI_DOCUMENT_UNCOMMITTED
        TopologyExtractionFailure.VFS_CONTENT_MISMATCH -> TopologyExtractionRejection.VFS_CONTENT_MISMATCH
        ...
        TopologyExtractionFailure.COMPILER_UNAVAILABLE -> TopologyExtractionRejection.COMPILER_UNAVAILABLE
        ...
        TopologyExtractionFailure.COMPILER_IDENTITY_MISMATCH -> TopologyExtractionRejection.COMPILER_IDENTITY_MISMATCH
        TopologyExtractionFailure.REFERENCE_TARGET_REJECTED -> TopologyExtractionRejection.REFERENCE_TARGET_REJECTED
        TopologyExtractionFailure.OCCURRENCE_REJECTED -> TopologyExtractionRejection.OCCURRENCE_REJECTED
        TopologyExtractionFailure.EDGE_REJECTED -> TopologyExtractionRejection.EDGE_REJECTED
        TopologyExtractionFailure.OVERRIDE_REJECTED -> TopologyExtractionRejection.OVERRIDE_REJECTED
        TopologyExtractionFailure.FILE_ADMISSION_REJECTED -> TopologyExtractionRejection.FILE_ADMISSION_REJECTED
    }

private fun TopologyPublicationFailure.toProtocol(): TopologyPublicationRejection =
    when (this) {
        TopologyPublicationFailure.STORAGE_UNAVAILABLE -> TopologyPublicationRejection.STORAGE_UNAVAILABLE
        TopologyPublicationFailure.SNAPSHOT_CONFLICT -> TopologyPublicationRejection.SNAPSHOT_CONFLICT
        TopologyPublicationFailure.CORRUPT_SNAPSHOT -> TopologyPublicationRejection.CORRUPT_SNAPSHOT
    }
