...
class HostedQueryResponseTest {
    ...
    fun `opaque reference expansion retains a qualified prefix inside the actual wire byte limit`() {
        ...
        val envelope = EvidenceEnvelope(CanonicalOperationWireBindings.queryRun.operation.id, EvidenceGeneration.parse(1).refined(), QueryRunResult(BoundedProtocolList.create(items).refined(), BoundedProtocolList.create(emptyList<QueryItemFailureDocument>()).refined()))
        val qualification = QueryRunQualification.create(QueryKnownMinimum.parse(items.size).refined(), listOf(QueryLimitationDocument.TIME_LIMIT_REACHED)).refined()
        ...
        assertTrue(response.document.toByteArray(Charsets.UTF_8).size <= ReadLimits.Default[ReadLimitParameter.HOST_RESPONSE_BYTES].value)
        ...
        assertEquals(listOf(QueryLimitationDocument.BYTE_LIMIT_REACHED, QueryLimitationDocument.TIME_LIMIT_REACHED), retained.limitations)
    }

    private fun item(): QueryResultItemDocument = QueryResultItemDocument.ExactSymbol(
        QueryReferenceDocument.ExactSymbol(ProtocolText.parse("exact:v3:" + "a".repeat(3000)).refined()),
        SymbolKindDocument.CLASSLIKE, null, null, null, BoundedProtocolList.create(emptyList<io.github.amichne.kast.protocol.contract.RelationFactDocument>()).refined(),
    )
