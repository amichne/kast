...
class CanonicalQueryProtocol(
    ...
    suspend fun execute(
        ...
        val syntax = when (val admission = request.admitSyntax(lease, authority)) {
...
private fun QueryRunRequest.admitSyntax(lease: SemanticReadAuthority, authority: QueryReferenceAuthority): QuerySyntaxAdmission {
    val source = when (val value = from) {
        ...
        is QueryFromDocument.References -> when (
            val references = value.values.values.admitReferenceSource(lease, authority)
        ) {
...
private fun List<QueryReferenceDocument>.admitCandidateReferences(
    ...
    forEachIndexed { index, reference ->
        ...
        val selector = when (val decoded = authority.restoreCandidate(reference.token, lease)) {
...
private fun List<QueryReferenceDocument>.admitExactReferences(
    ...
    forEachIndexed { index, reference ->
        ...
        val selector = when (val decoded = authority.restoreExact(reference.token, lease)) {
            is CanonicalSelectorDecoding.Decoded -> decoded.value
            is CanonicalSelectorDecoding.Rejected -> return QueryReferenceSourceAdmission.Rejected(
                index,
...
fun SemanticReadAuthority.evidenceBasis(): EvidenceBasis = when (this) {
    is SemanticReadLease -> EvidenceBasis.Published(generation)
    is LiveSemanticReadAuthority -> when (val admitted = LiveReadEvidence.create(
        workspaceRoot.value, reference.host.value, reference.epoch.value,
        LiveReadContentView.valueOf(reference.contentView.name), reference.version,
    )) {
...
private fun CanonicalSelectorDecodingFailure.queryRejection(token: ProtocolText, expectedExact: Boolean): QueryReferenceRejectionReason = when (this) {
    ...
    CanonicalSelectorDecodingFailure.STALE_AUTHORITY ->
        if (token.value.startsWith("exact:v2:") || token.value.startsWith("candidate:v2:")) QueryReferenceRejectionReason.STALE_GENERATION
    ...
    CanonicalSelectorDecodingFailure.UNSUPPORTED_REFERENCE_VERSION -> QueryReferenceRejectionReason.INCOMPATIBLE_REFERENCE_VERSION
    else -> if (token.belongsToOtherReferenceFamily(expectedExact)) QueryReferenceRejectionReason.WRONG_KIND
