...
object GeneratedOperationWireBindingFactory {
    ...
    /**
     * Proof transition: `OperationDefinition + four generated KSerializer values -> OperationWireBinding`.
     *
     * Establishes one binding whose request, result, qualification, and rejection documents all cross the shared wire
     * JSON boundary through the supplied compiler-generated factories. Raw JSON elements remain private to the
     * resulting codecs.
     */
    fun <
    ...
    ): OperationWireBinding<Request, Result, Qualification, Rejection> =
...
internal constructor(
     ...
     *
     * Establishes that the admitted request names this binding's exact schema and canonical operation before returning
     * a generated-serializer-decoded request. [WireFailure] is the closed expected failure. Raw request fields may
     * leave this type only through the operation handler boundary.
     */
    fun decodeRequest(request: AdmittedWireRequest): WireDecoding<Request> {
        when (val admission = admitBindingIdentity(request.schema, request.operation)) {
            ...
            is BindingIdentityAdmission.Rejected -> return WireDecoding.Rejected(admission.failure)
        }
        return serializers.request.decode(request.value, WireValueRole.REQUEST)
    }

    fun encodeOutcome(outcome: OperationOutcome<Result, Qualification, Rejection>): WireEncoding =
        when (outcome) {
            is OperationOutcome.Complete -> encodeComplete(outcome.evidence)
            is OperationOutcome.Qualified -> encodeQualified(outcome.evidence, outcome.qualification)
            is OperationOutcome.Rejected -> encodeRejected(outcome.reason)
        }

    /**
     * Proof transition: `String -> WireDecoding<OperationOutcome<Result, Qualification, Rejection>>`.
     *
     * Establishes this binding's schema, canonical operation, evidence generation, generated payload types, and closed
     * semantic outcome variant. [WireFailure] is the closed expected failure. Raw fields may leave this type only
     * through the external result-projection boundary.
     */
    fun decodeOutcome(document: String): WireDecoding<OperationOutcome<Result, Qualification, Rejection>> {
        val envelope =
            when (val admission = admitWireEnvelope(document)) {
                ...
                encodeEnvelope(
                    when (val basis = evidence.basis) {
                        ...
                        is EvidenceBasis.Live ->
                            WireBodyDocument.Complete(result = result.value, live = basis.evidence.document())
                    }
                )
                ...
                encodeEnvelope(
                    when (val basis = evidence.basis) {
                        is EvidenceBasis.Published ->
                            WireBodyDocument.Qualified(basis.generation.value, result, encoded.value)
                        is EvidenceBasis.Live ->
                            WireBodyDocument.Qualified(
                                result = result,
                                qualification = encoded.value,
                                live = basis.evidence.document(),
                            )
                    }
                )
            is WireValueEncoding.Rejected -> WireEncoding.Rejected(encoded.failure)
    ...
    private fun decodeQualified(
        ...
        val evidence =
            when (val decoded = decodeEvidence(body.generation, body.live, body.result)) {
                is WireDecoding.Decoded -> decoded.value
                is WireDecoding.Rejected -> return decoded
            }
        return when (
            val qualification =
                serializers.qualification.decode(
                    body.qualification,
                    WireValueRole.QUALIFICATION,
                )
        ) {
            is WireDecoding.Decoded -> WireDecoding.Decoded(OperationOutcome.Qualified(evidence, qualification.value))
     ...
     * generated-serializer-decoded result. [WireFailure] is the closed expected failure. Raw wire values do not escape
     * this boundary.
     */
    private fun decodeEvidence(
        ...
        val basis =
            when {
                rawGeneration != null && rawLive == null ->
                    when (val refined = EvidenceGeneration.parse(rawGeneration)) {
                        is Refinement.Refined -> EvidenceBasis.Published(refined.value)
                        is Refinement.Rejected ->
                            return WireDecoding.Rejected(WireFailure.InvalidEvidenceGeneration(refined.failure))
                    }
                rawGeneration == null && rawLive != null ->
                    when (val admitted = rawLive.admit()) {
        ...
        return when (val result = serializers.result.decode(rawResult, WireValueRole.RESULT)) {
            is WireDecoding.Decoded ->
                if (result.value.retainsEvidenceBasis(basis)) {
     ...
     * [WireFailure.UnexpectedOperation] are the closed expected failures. Raw identity extraction is permitted only at
     * [WireRequestEnvelope] and outcome-envelope admission.
     */
    private fun admitBindingIdentity(
        ...
        if (observedOperation != operation) {
            return BindingIdentityAdmission.Rejected(WireFailure.UnexpectedOperation(operation, observedOperation))
        }
        return BindingIdentityAdmission.Admitted
    }

    private fun encodeEnvelope(body: WireBodyDocument): WireEncoding =
        try {
    ...
    private fun evidenceOperationMismatch(evidence: EvidenceEnvelope<Result>): EvidenceOperationAdmission =
        if (evidence.operation == operation.id) {
            ...
            when (val resolution = CanonicalOperation.resolve(evidence.operation)) {
                is CanonicalOperationResolution.Known ->
                    EvidenceOperationAdmission.Rejected(
                        WireFailure.UnexpectedOperation(operation, resolution.operation)
                    )
                is CanonicalOperationResolution.Unknown ->
                    EvidenceOperationAdmission.Rejected(WireFailure.UnknownOperation(resolution.id))
            }
        }
}
...
private sealed interface BindingIdentityAdmission {
    ...
    data class Rejected(val failure: WireFailure) : BindingIdentityAdmission
}

private sealed interface EvidenceOperationAdmission {
    data object Admitted : EvidenceOperationAdmission

    data class Rejected(val failure: WireFailure) : EvidenceOperationAdmission
}

internal fun LiveReadEvidence.document() =
...
internal fun LiveEvidenceDocument.admit(): Refinement<LiveReadEvidence, Unit> {
    val hostIdentity =
        try {
            UUID.fromString(host).takeIf { it.toString() == host }
        } catch (_: IllegalArgumentException) {
            null
        } ?: return Refinement.Rejected(Unit)
    val view = LiveReadContentView.entries.singleOrNull { it.name == contentView } ?: return Refinement.Rejected(Unit)
