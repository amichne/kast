...
import io.github.amichne.kast.standalone.hierarchy.CallEdgeResolver
...
class AnalysisBackendCallHierarchyTest {
    @TempDir
    ...
    fun `outgoing hierarchy skips resolved symbols without backing file`() = runTest {
        ...
        session.use { currentSession ->
            val resolver = CallEdgeResolver(
                ...
                normalizedWorkspaceRoot = io.github.amichne.kast.standalone.normalizePath(workspaceRoot),
            )
    ...
    private suspend fun withBackend(
        ...
        block: suspend (AnalysisBackend) -> Unit,
    ) {
        val session = AnalysisSession(
            workspaceRoot = workspaceRoot,
            sourceRoots = emptyList(),
