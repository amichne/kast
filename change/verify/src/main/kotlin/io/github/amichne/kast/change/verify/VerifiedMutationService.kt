...
class AdmittedVerifiedMutationRequest
private constructor(
    val plan: ChangePlan,
    val applied: AppliedUnverified,
) {
    companion object {
        /**
         * Proof transition: `VerifiedMutationRequest -> Refinement< AdmittedVerifiedMutationRequest,
         * VerifiedMutationAdmissionFailure>`.
         *
         * Establishes exact plan identity, the retained plan-to-application publication proof, and source identity
         * before any publication effect begins. [VerifiedMutationAdmissionFailure] is the closed expected failure. Raw
         * extraction is prohibited; only typed KCS-015 and KCS-017 proof enters.
         */
        fun admit(
            request: VerifiedMutationRequest
        ): Refinement<AdmittedVerifiedMutationRequest, VerifiedMutationAdmissionFailure> =
            when {
                ...
                request.plan.priorLease != request.applied.publication.plannedLease ||
                    ...
                    request.plan.writes.entries.singleOrNull()?.sourceRoot != request.applied.publication.sourceRoot ||
                ...
                else -> Refinement.Refined(AdmittedVerifiedMutationRequest(request.plan, request.applied))
            }
    }
}
...
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes final success with distinct generation, complete coverage, discharged obligations, clear
         * diagnostics, unchanged existing relations, and accepted semantic delta. There is no expected failure because
         * the input already carries exhaustive proof. Raw extraction is prohibited; persistence or transport may
         * project only typed fields.
         */
...
sealed interface VerifiedMutationBeforePublicationFailure {
    data class Admission(val failure: VerifiedMutationAdmissionFailure) : VerifiedMutationBeforePublicationFailure

    data class Publication(val rejection: ResultingGenerationPublicationRejection) :
        VerifiedMutationBeforePublicationFailure
}

sealed interface VerifiedMutationResult {
    data class Verified(val receipt: VerifiedReceipt) : VerifiedMutationResult
...
fun interface VerifiedMutationOperations {
     ...
     *
     * Verified establishes a distinct complete publication and exhaustive semantic proof. Expected admission,
     * publication, observation, and proof failures are closed by [VerifiedMutationResult], retaining the strongest
     * state reached. Platform effects remain in the injected ports.
     */
...
class VerifiedMutationService(
    ...
    private fun verifyObserved(request: VerifiedMutationRequest): VerifiedMutationResult {
        val admitted =
            when (val result = AdmittedVerifiedMutationRequest.admit(request)) {
        ...
        val published =
            when (val result = publisher.publishAfter(admitted.applied.priorLease)) {
        ...
        val resulting =
            when (
                val result =
                    DistinctResultingWorkspace.admit(
                ...
                is Refinement.Refined -> result.value
                is Refinement.Rejected ->
                    return VerifiedMutationResult.RejectedAfterPublication(
                        admitted.applied,
                        published,
                        result.failure,
                    )
        ...
        return when (val proof = complete(admitted, resulting, evidence)) {
            ...
            is Refinement.Rejected ->
                VerifiedMutationResult.RejectedAfterObservation(
                    admitted.applied,
                    resulting,
                    evidence,
                    proof.failure,
    ...
    private fun VerifiedMutationResult.observation(): KastChangeVerificationOutcome =
        when (this) {
            ...
            is VerifiedMutationResult.RejectedBeforePublication ->
                when (val reason = failure) {
                    is VerifiedMutationBeforePublicationFailure.Admission ->
                        KastChangeVerificationOutcome.ADMISSION_REJECTED
                    is VerifiedMutationBeforePublicationFailure.Publication ->
                        when (reason.rejection) {
            ...
            is VerifiedMutationResult.RejectedAfterPublication ->
                KastChangeVerificationOutcome.RESULTING_PUBLICATION_REJECTED
    ...
    ): Refinement<CompleteChangeVerification, Set<ChangeProofFailure>> =
        when (val plan = admitted.plan) {
            is AddFileChangePlan ->
                when (evidence) {
                    is AddFileVerificationEvidence ->
                        when (
                            val proof =
                                CompleteAddFileVerification.admit(
                                    plan,
                                    admitted.applied,
                                    resulting,
                                    evidence,
            ...
            is AddDeclarationChangePlan ->
                when (evidence) {
                    is AddDeclarationVerificationEvidence ->
                        when (
                            val proof =
            ...
            is RenameSymbolChangePlan ->
                when (evidence) {
                    is RenameSymbolVerificationEvidence ->
                        when (
                            val proof =
            ...
            is ReplaceDeclarationChangePlan ->
                when (evidence) {
                    is ReplaceDeclarationVerificationEvidence ->
                        when (
                            val proof =
