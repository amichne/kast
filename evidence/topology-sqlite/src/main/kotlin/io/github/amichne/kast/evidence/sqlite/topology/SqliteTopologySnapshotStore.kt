...
private constructor(
    ...
    companion object {
        ...
        fun open(location: TopologyDatabaseLocation): SqliteTopologySnapshotStoreOpening =
            when (val path = admitTopologyDatabasePath(location.valueAtSqliteBoundary())) {
                is Refinement.Refined -> open(path.value)
                is Refinement.Rejected -> SqliteTopologySnapshotStoreOpening.Rejected(path.failure)
            }
