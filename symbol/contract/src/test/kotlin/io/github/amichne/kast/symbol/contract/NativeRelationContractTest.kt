...
class NativeRelationContractTest {
    ...
    fun `terminal and qualified coverage cannot exchange exact and minimum counts`() {
        ...
        val qualified =
            NativeRelationOutcome.qualified(
                    batch,
    ...
    fun `batch rejects wrong subject family order and byte claims`() {
        ...
        val bytes = NativeRelationByteCount.parse(ordered.sumOf { it.projectedUtf8Size().value }).refined()
    ...
    private fun selector(libraries: SymbolLibraryPolicy = SymbolLibraryPolicy.EXCLUDE): ExactDeclarationSelector {
        val scope =
    ...
    private fun timings(): NativeRelationTimings =
