...
usage() {
  printf '%s\n' \
    'Usage: scripts/packaging/package-prepared-local-generation.sh --source-root <checkout> --prepared-generation <directory> --output <tar.zst>' \
    >&2
}
...
[[ -x "${prepared_generation}/libexec/kastctl" ]] || die "Prepared generation has no executable libexec/kastctl"
[[ -x "${prepared_generation}/bin/kast" ]] || die "Prepared generation has no executable bin/kast"
cmp -s "${prepared_generation}/libexec/kastctl" "${prepared_generation}/bin/kast" \
