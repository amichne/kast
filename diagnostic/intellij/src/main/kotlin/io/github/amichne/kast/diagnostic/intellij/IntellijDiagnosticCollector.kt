...
internal class IntellijDiagnosticCollector(private val scope: DiagnosticScope) {
    ...
    /**
     * Proof transition: `(IntellijDiagnosticCollector, DiagnosticFact) -> IntellijDiagnosticCollectionAdmission`.
     *
     * Accepted proves that the detached fact belongs to the identical exact scope while the collector is open. Rejected
     * is the closed collector-contract failure. Raw K2 values must be refined into [DiagnosticFact] before this
     * boundary.
     */
    fun accept(fact: DiagnosticFact): IntellijDiagnosticCollectionAdmission =
        when {
            state != IntellijDiagnosticCollectorState.COLLECTING -> IntellijDiagnosticCollectionAdmission.REJECTED
    ...
    /**
     * Proof transition: `(IntellijDiagnosticCollector, DiagnosticSourceFile) -> IntellijDiagnosticCollectionAdmission`.
     *
     * Accepted proves that the exact scope file completed analysis without a limitation and was recorded once. Rejected
     * is the closed collector-contract failure. Live compiler completion state remains inside the request-local
     * adapter.
     */
    fun recordAnalyzed(file: DiagnosticSourceFile): IntellijDiagnosticCollectionAdmission =
        when {
            state != IntellijDiagnosticCollectorState.COLLECTING -> IntellijDiagnosticCollectionAdmission.REJECTED
            ...
            limitations.any { limitation -> limitation.file == file } -> IntellijDiagnosticCollectionAdmission.REJECTED
            else -> {
                analyzedFiles += file
                IntellijDiagnosticCollectionAdmission.ACCEPTED
            }
    ...
    /**
     * Proof transition: `(IntellijDiagnosticCollector, DiagnosticSourceFile, DiagnosticLimitationReason) ->
     * IntellijDiagnosticCollectionAdmission`.
     *
     * Accepted proves that one exact scope file remains explicitly incomplete and cannot be counted as analyzed.
     * Rejected is the closed collector-contract failure. Raw provider state is retained only as the finite
     * [DiagnosticLimitationReason].
     */
     ...
     *
     * Complete proves every exact file analyzed without limitation. Qualified proves total file accounting with at
     * least one explicit limitation. Unaccounted files, invalid facts, or repeated finalization become
     * [DiagnosticCompilerRejection.COMPILER_CONTRACT_VIOLATION]. Live compiler values never enter the returned detached
     * output.
     */
    fun finish(): DiagnosticCompilation {
        ...
        val batch =
            when (val admission = DiagnosticBatch.create(scope, facts)) {
        ...
        return when (val qualified = DiagnosticCompilation.qualified(batch, analyzedFiles, limitations)) {
...
private fun contractRejected(): DiagnosticCompilation.Rejected =
