...
class IdeEndpointDescriptorTest {
    ...
    fun `the exact v2 descriptor retains every refined endpoint authority`() {
        ...
        assertEquals(
            ...
            descriptor.capabilities.capabilities.single { it.operation == CanonicalOperation.CHANGE_PLAN }.intents,
        )
    }
...
internal const val FIXTURE_REGISTRY_DIGEST = "sha256:1111111111111111111111111111111111111111111111111111111111111111"
internal const val FIXTURE_WIRE_DIGEST = "sha256:2222222222222222222222222222222222222222222222222222222222222222"

internal fun fixtureEndpointCandidate() =
...
internal fun fixtureEndpointPolicy(): IdeHostCompatibilityPolicy =
    when (
        val refinement =
            IdeHostCompatibilityPolicy.define(
                IdeHostCompatibilityCandidate(
                    ...
                    operationRegistryDigest = FIXTURE_REGISTRY_DIGEST,
                    wireSchemaDigest = FIXTURE_WIRE_DIGEST,
                    capabilities = fixtureEndpointCandidate().capabilities.map(HostedCapabilityCandidate::operationId),
                )
            )
    ) {
