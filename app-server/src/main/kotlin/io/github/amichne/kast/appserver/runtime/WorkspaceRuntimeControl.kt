...
internal sealed interface WorkerConfigurationExpectation {
    ...
    class Selected private constructor(val identity: String) : WorkerConfigurationExpectation {
        companion object {
            fun admit(identity: String): Refinement<Selected, WorkerControlFailure> =
                if (identity.matches(Regex("[0-9a-f]{64}"))) Refinement.Refined(Selected(identity))
...
internal class WorkspaceRuntimeControl
private constructor(
    ...
    private enum class RootRetirementState {
        IN_PROGRESS,
        UNCERTAIN,
    }
    ...
    private sealed interface StopSelection {
        ...
        class Reserved(val reservation: WorkerReservation, override val binding: WorkerRouteBinding.Installation) :
            StopSelection
    ...
    suspend fun stop(root: Path): InstalledWorkerStop {
        val canonical =
            try {
                ...
                return InstalledWorkerStop.Rejected(WorkerControlFailure.IDENTITY_REJECTED)
            }
        val selection = admission.withLock {
            if (rootRetirements[canonical] == RootRetirementState.IN_PROGRESS)
                return InstalledWorkerStop.Rejected(WorkerControlFailure.RETIREMENT_UNPROVEN)
            val record =
                ledger.snapshot().workers.singleOrNull { it.reservation.identity.workspace.root.path == canonical }
            val selected =
                if (record != null) {
                    val binding =
                        when (val admitted = binding(record.reservation)) {
                    ...
                    val snapshot =
                        when (val observed = registry.snapshot()) {
                            is WorkspaceRegistryRead.Read -> observed.snapshot
                            is WorkspaceRegistryRead.Rejected ->
                                return InstalledWorkerStop.Rejected(WorkerControlFailure.REGISTRATION_REJECTED)
                        }
                    if (snapshot.workspaces.none { it.root.path == canonical })
                        return InstalledWorkerStop.Rejected(WorkerControlFailure.REGISTRATION_REJECTED)
                    val selected =
                        when (
                            val resolved =
                                InstalledWorkspaceConfigurationIngress.resolve(
                                    installationRoot,
                                    canonical,
                                    sourceEnvironment,
                                )
                        ...
                        when (
                            val admitted =
                                WorkerRouteBinding.Installation.admit(
                                    ...
                                    workerConfigurationIdentity(selected),
            ...
            try {
                when (selection) {
                    is StopSelection.Reserved -> {
                        ...
                        val settled =
                            jobs[reservation.id]?.let { job ->
                                job.cancel()
                                (withTimeoutOrNull(BrokerOperationalLimits.workerStartupJoin.value) {
                                    job.join()
                                    true
                                } == true)
                            } ?: true
                    ...
                    is StopSelection.Unreserved ->
                        withTimeoutOrNull(OperationExecutionBudget.LOCAL_QUALIFICATION.value) {
                            effects.retireUnreserved(canonical)
        ...
        return if (retired == InstalledWorkerRetirement.EXACT_RETIRED)
            InstalledWorkerStop.Stopped(canonical, selection.binding)
    ...
    suspend fun demand(
        ...
        consentAuthority: WorkerSeedConsentAuthority = WorkerSeedConsentAuthority.Unavailable,
        expectedConfiguration: WorkerConfigurationExpectation = WorkerConfigurationExpectation.ProtocolFixture,
    ): InstalledWorkerStart {
        if (!available()) return rejected(WorkerControlFailure.LIFECYCLE_TRANSITION)
        val selection = admission.withLock {
            if (!available()) return rejected(WorkerControlFailure.LIFECYCLE_TRANSITION)
            val registration =
                when (val registered = registry.enroll(root)) {
            ...
            if (rootRetirements.containsKey(workspace.root.path))
            ...
            if (
                expectedConfiguration is WorkerConfigurationExpectation.Selected &&
                    (expectedConfiguration.identity != selectedIdentity || selectedConfiguration.indexerHeap != heap)
            )
                return rejected(WorkerControlFailure.IDENTITY_REJECTED)
            val memory =
                when (
                    val admitted =
                        DeclaredWorkerMemory.admit(
                            heap,
                            configuration.workerCapacity.native.value,
                            configuration.workerCapacity.gradle.value,
                        )
                ) {
                    is Refinement.Refined -> admitted.value
            ...
            val existing =
                ledger.snapshot().workers.singleOrNull { it.reservation.identity.workspace.id == workspace.id }
            val previous = existing?.let { configurations[it.reservation.id] }
            if (
                existing != null &&
                    startup is InstalledWorkerStartup.Reuse &&
                    startup.ideHome != null &&
                    startups[workspace.id]?.startup?.ideHome != startup.ideHome
            )
                return rejected(WorkerControlFailure.IDENTITY_REJECTED)
            if (
                previous != null &&
                    workerConfigurationIdentity(previous) != selectedIdentity &&
                    startup is InstalledWorkerStartup.Reuse
            )
                return rejected(WorkerControlFailure.IDENTITY_REJECTED)
            if (existing?.phase == WorkerReservationPhase.READY && startup !is InstalledWorkerStartup.Reuse) {
                val endpoint =
                    routes[existing.reservation.id] ?: return rejected(WorkerControlFailure.RECOVERY_REQUIRED)
            ...
            if (
                selected is WorkerDemand.Awaiting &&
                    !(startup is InstalledWorkerStartup.Reuse && startup.ideHome == null) &&
                    startups[workspace.id]?.startup != startup
            )
                return rejected(WorkerControlFailure.IDENTITY_REJECTED)
            DemandSelection.Admitted(
                selected.also { demand ->
        ...
        return when (route) {
            ...
            is WorkerReadiness.Ready -> {
                val endpoint =
                    routes[route.route.reservation.id] ?: return rejected(WorkerControlFailure.RECOVERY_REQUIRED)
                if (effects.observe(endpoint) != InstalledWorkerObservation.EXACT_READY) {
                    return recoverLostWorker(
    ...
    private suspend fun retireAndDemand(
        ...
        admission.withLock {
            if (retired == InstalledWorkerRetirement.EXACT_RETIRED)
                rootRetirements.remove(reservation.identity.workspace.root.path)
    ...
    private suspend fun launch(reservation: WorkerReservation, request: InstalledWorkerStartRequest) {
        try {
            ...
            val started =
                withTimeoutOrNull(OperationExecutionBudget.WORKSPACE_READINESS.value) { effects.start(request) }
            ...
            when (started) {
                ...
                is InstalledWorkerStart.Ready -> {
                    ...
                    if (
                        endpoint.root != reservation.identity.workspace.root.path ||
                            effects.observe(endpoint) != InstalledWorkerObservation.EXACT_READY
                    ) {
    ...
    private fun quarantine(reservation: WorkerReservation, failure: WorkerAdmissionFailure) {
        ...
        if (!receipt(reservation, phase, routes[reservation.id])) {
            closed.set(true)
            ledger.drain()
        }
    }
    ...
    private suspend fun retire(
        ...
        val retired =
            ledger.retire(reservation) { exact ->
                if (
                    (if (endpoint == null) effects.retireUnpublished(reservation.identity.workspace.root.path)
                    else effects.stop(endpoint)) == InstalledWorkerRetirement.EXACT_RETIRED
                )
                    WorkerRetirementObservation.ExactRetired(exact)
        ...
        return try {
            ...
            startups.computeIfPresent(reservation.identity.workspace.id) { _, selected ->
                if (selected.reservation == reservation.id) null else selected
            }
    ...
    suspend fun handle(session: DefaultWebSocketServerSession, handshakeAllowed: Boolean = true) {
        ...
        val document =
            try {
                ...
                session.send(WorkerControlReply.encode(rejected(WorkerControlFailure.INVALID_REQUEST)))
                return
            }
                ...
                admission.withLock {
                    if (available())
                        buildJsonObject {
                            ...
                            put("serviceGeneration", generation.value.toString())
                            put("configurationIdentity", workerLaunchConfigurationIdentity(configuration))
                            val observed = ledger.snapshot()
                            put("reservedMiB", observed.reserved.value)
                            put("starting", observed.starting)
                            put("hostAttachment", hostObservation().name)
                            putJsonArray("workers") {
                                observed.workers.forEach { worker ->
                                    add(
                                        buildJsonObject {
                                            put("workspaceId", worker.reservation.identity.workspace.id.value)
                                            put("reservationId", worker.reservation.id.value.toString())
                                            put("phase", worker.phase.name)
                                            put("reservedMiB", worker.reservation.memory.total.value)
                                            put("requestedHeapMiB", worker.reservation.memory.heap.mebibytes)
                                            put("heapObservation", "UNOBSERVED")
                                            configurations[worker.reservation.id]?.let {
                                                put("configurationIdentity", workerConfigurationIdentity(it))
                                            }
                                        }
                                    )
                                }
                            ...
                            .toString()
                    else WorkerControlReply.encode(rejected(WorkerControlFailure.LIFECYCLE_TRANSITION))
                }
            )
        ...
        val result =
            try {
                ...
                when (document.action) {
                    ...
                    WorkerControlAction.DEMAND ->
                        when (val heap = IndexerHeapSize.parse(document.heap)) {
                            is Refinement.Refined ->
                                when (
                                    val expectation =
                                        WorkerConfigurationExpectation.Selected.admit(document.configurationIdentity)
                                ) {
                                    ...
                                    is Refinement.Refined ->
                                        withTimeoutOrNull(OperationExecutionBudget.WORKSPACE_READINESS.value) {
                                            demand(
                                                root,
                                                heap.value,
                                                document.startupRequest(),
                                                WorkerSeedConsentExchange(session),
                                                expectation.value,
                                            )
                                        } ?: rejected(WorkerControlFailure.DEADLINE_EXCEEDED)
                                }
                ...
                rejected(WorkerControlFailure.INVALID_REQUEST)
            }
    ...
    private fun binding(
        reservation: WorkerReservation
    ): Refinement<WorkerRouteBinding.Installation, WorkerControlFailure> {
        val revision = revisions[reservation.id] ?: return Refinement.Rejected(WorkerControlFailure.IDENTITY_REJECTED)
        return WorkerRouteBinding.Installation.admit(
            owner.installationId.value,
            owner.stateEpoch.value.toString(),
            generation.value.toString(),
            workerConfigurationIdentity(
                configurations[reservation.id] ?: return Refinement.Rejected(WorkerControlFailure.IDENTITY_REJECTED)
            ),
            revision.value,
        )
    }
    ...
    private fun available(): Boolean =
        !closed.get() &&
            ...
            when (val mode = lifecycle) {
                ...
                is WorkerControlLifecycle.Installed ->
                    try {
    ...
    private fun receipt(
        reservation: WorkerReservation,
        phase: WorkerReservationPhase,
        endpoint: InstalledWorkerEndpoint? = null,
    ): Boolean {
        return try {
            ...
            val document = buildJsonObject {
                put("schemaVersion", 1)
                put("reservationId", reservation.id.value.toString())
                put("installationId", owner.installationId.value)
                put("stateEpoch", owner.stateEpoch.value.toString())
                put("serviceGeneration", generation.value.toString())
                put("workspaceRoot", reservation.identity.workspace.root.path.toString())
                put("workspaceId", reservation.identity.workspace.id.value)
                put("phase", phase.name)
                put("coordinatorPid", process.pid())
                put("coordinatorStartedAt", started.toString())
                put(
                    "configurationIdentity",
                    workerConfigurationIdentity(configurations[reservation.id] ?: return false),
                )
                put("heapMiB", reservation.memory.heap.mebibytes)
                put("nativeMiB", reservation.memory.nativeAllowance.value)
                put("gradleMiB", reservation.memory.gradleAllowance.value)
                if (endpoint != null)
                    putJsonObject("endpoint") {
                        put("runtimeId", endpoint.runtimeId.value)
                        put("socket", endpoint.socket.toString())
                        put("bootstrapAttempt", endpoint.attempt.value)
                    }
            }
                .toString()
                .toByteArray()
            ...
            try {
                ...
                FileChannel.open(temporary, StandardOpenOption.WRITE).use { channel ->
                    val bytes = ByteBuffer.wrap(document)
                    while (bytes.hasRemaining()) channel.write(bytes)
                    channel.force(true)
                }
                Files.move(
                    temporary,
                    receipts.resolve("${reservation.id.value}.json"),
    ...
    companion object {
        fun create(
            ...
            lifecycle: WorkerControlLifecycle = WorkerControlLifecycle.ProtocolFixture,
            hostObservation: () -> BrokerFrontendObservation = { BrokerFrontendObservation.PENDING },
            sourceEnvironment: Map<String, String> = emptyMap(),
            workerDispatcher: CoroutineDispatcher = Dispatchers.IO,
        ): Refinement<WorkspaceRuntimeControl, WorkerControlFailure> {
            return try {
                if (
                    ...
                    return Refinement.Rejected(WorkerControlFailure.LIFECYCLE_TRANSITION)
                val receipts = installationRoot.resolve("state/workers")
                Files.createDirectories(receipts)
                if (receipts.toRealPath() != receipts || Files.list(receipts).use { it.findAny().isPresent })
                    return Refinement.Rejected(WorkerControlFailure.RECOVERY_REQUIRED)
                Files.setPosixFilePermissions(receipts, PosixFilePermissions.fromString("rwx------"))
                val capacity = configuration.workerCapacity
                val policy =
                    when (
                        val admitted =
                            WorkerAdmissionPolicy.admit(
                                capacity.resident.value,
                                capacity.startup.value,
                                capacity.aggregate.value,
                            )
                    ) {
                        is Refinement.Refined -> admitted.value
                ...
                Refinement.Refined(
                    WorkspaceRuntimeControl(
                        ...
                        configuration,
                        effects,
                        WorkerAdmissionCoordinator(
                            owner,
                            generation,
                            policy,
                            WorkerReservationIdSource { WorkerReservationId.fresh() },
                        ),
                        receipts,
                        lifecycle,
                        hostObservation,
                        sourceEnvironment.toMap(),
                        workerDispatcher,
                    )
                )
            } catch (_: Exception) {
...
private fun WorkerAdmissionFailure.controlFailure(): WorkerControlFailure =
    when (this) {
        WorkerAdmissionFailure.RESIDENT_CAPACITY_EXCEEDED,
        WorkerAdmissionFailure.STARTUP_CAPACITY_EXCEEDED,
