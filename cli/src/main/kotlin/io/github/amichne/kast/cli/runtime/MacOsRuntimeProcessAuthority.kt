...
internal fun interface RuntimeProcessSessionResolver {
     ...
     *
     * Establishes the deterministic operating-system session identity for the exact endpoint. Raw session labels remain
...
internal sealed interface RuntimeProcessSearchResult {
    ...
    data class Exact(val process: ProcessHandle) : RuntimeProcessSearchResult
...
internal fun interface RuntimeProcessSearch {
     ...
     *
     * [RuntimeProcessSearchResult.Exact] establishes one same-user process carrying every exact endpoint argument.
     * [RuntimeProcessSearchResult] closes absence, inaccessible state, and multiple matches. Raw process metadata
     * remains inside the platform search adapter.
     */
...
internal object JdkRuntimeProcessAuthority :
    RuntimeProcessAuthority by ExactRuntimeProcessAuthority(
        processSearch = JdkRuntimeProcessSearch,
        processSessions = RuntimeProcessSessionResolver { DirectRuntimeProcessSession },
    )

internal object LaunchdRuntimeProcessAuthority :
    RuntimeProcessAuthority by ExactRuntimeProcessAuthority(
        processSearch = JdkRuntimeProcessSearch,
...
internal class ExactRuntimeProcessAuthority(
    ...
    override fun observe(endpoint: RuntimeEndpoint): RuntimeProcessObservation {
        ...
        return when (val search = processSearch.find(endpoint)) {
            RuntimeProcessSearchResult.None ->
                when (processSession.observe()) {
                    ...
                    RuntimeSessionObservation.Rejected -> RuntimeProcessObservation.Ambiguous
                }
            is RuntimeProcessSearchResult.Exact ->
                RuntimeProcessObservation.Owned(
                    SessionRuntimeOwnedProcess(
...
internal class ExactRuntimeBootstrapProcessAuthority(
    ...
    override fun observe(query: RuntimeBootstrapProcessQuery): RuntimeBootstrapProcessObservation {
        ...
        return when (val search = processSearch.find(query)) {
            RuntimeBootstrapProcessSearchResult.None ->
                when (processSession.observe()) {
                    RuntimeSessionObservation.Absent -> RuntimeBootstrapProcessObservation.Absent
                    RuntimeSessionObservation.Present -> RuntimeBootstrapProcessObservation.Uncorrelated
                    RuntimeSessionObservation.Interrupted -> RuntimeBootstrapProcessObservation.Interrupted
                    RuntimeSessionObservation.Rejected -> RuntimeBootstrapProcessObservation.Ambiguous
                }
            is RuntimeBootstrapProcessSearchResult.Exact ->
                RuntimeBootstrapProcessObservation.Owned(
                    search.attemptId,
                    ProcessHandleRuntimeStartupSession(search.process),
                )
            RuntimeBootstrapProcessSearchResult.Ambiguous -> RuntimeBootstrapProcessObservation.Ambiguous
        }
    }
}
...
internal object JdkRuntimeBootstrapProcessSearch : RuntimeBootstrapProcessSearch {
    override fun find(query: RuntimeBootstrapProcessQuery): RuntimeBootstrapProcessSearchResult {
        ...
        val currentUser = current.info().user().orElse(null) ?: return RuntimeBootstrapProcessSearchResult.Ambiguous
            ...
            ProcessHandle.allProcesses().use { processes ->
                processes.forEach { process ->
                    ...
                    val isCandidate =
                        ...
                        } == true || commandLine.contains(INDEXER_MAIN_CLASS) || commandLine.contains(launcher)
                    ...
                    if (user == null || arguments == null) {
                        ...
                        val rawAttempt =
                            arguments
                                .singleOrNull { argument ->
                                    argument.startsWith(BOOTSTRAP_ATTEMPT_ARGUMENT_PREFIX)
                                }
                                ?.removePrefix(BOOTSTRAP_ATTEMPT_ARGUMENT_PREFIX)
                        when (val admitted = rawAttempt?.let(SemanticRuntimeBootstrapAttemptId::admit)) {
                            is Refinement.Refined ->
                                matches +=
                                    RuntimeBootstrapProcessSearchResult.Exact(
                                        process,
                                        admitted.value,
                                    )
                            is Refinement.Rejected,
                            null -> inaccessibleCandidate = true
                        }
                    }
                }
            }
...
internal object JdkRuntimeProcessSearch : RuntimeProcessSearch {
    override fun find(endpoint: RuntimeEndpoint): RuntimeProcessSearchResult {
        ...
        val current = ProcessHandle.current()
        val currentUser = current.info().user().orElse(null) ?: return RuntimeProcessSearchResult.Ambiguous
        ...
        try {
            ProcessHandle.allProcesses().use { processes ->
                processes.forEach { process ->
                    ...
                    val commandLine = info.commandLine().orElse("")
                    val isIndexer =
                        arguments?.contains(INDEXER_MAIN_CLASS) == true || commandLine.contains(INDEXER_MAIN_CLASS)
...
private class ProcessHandleRuntimeStartupSession(private val process: ProcessHandle) : AcceptedRuntimeStartupSession {
...
private sealed interface RuntimeProcessPresence {
    data class Live(val process: ProcessHandle) : RuntimeProcessPresence
...
private class SessionRuntimeOwnedProcess(
    ...
    override fun terminate(): RuntimeProcessTermination =
        when (processSession.observe()) {
            RuntimeSessionObservation.Present ->
                when (processSession.retire(RuntimeSessionObservation.Present)) {
    ...
    private fun afterSessionRetirement(): RuntimeProcessTermination =
        when (val observed = presence) {
            RuntimeProcessPresence.BetweenSessionAttempts -> RuntimeProcessTermination.Terminated
            is RuntimeProcessPresence.Live -> awaitExitAfterSessionRetirement(observed.process)
        }

    private fun awaitExitAfterSessionRetirement(process: ProcessHandle): RuntimeProcessTermination {
...
internal fun terminateDirectProcess(process: ProcessHandle): RuntimeProcessTermination =
    try {
