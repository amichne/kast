...
codex_plugin_ref="${docs_root}/reference/codex-plugin.md"
codex_use_doc="${docs_root}/use/codex.md"
privacy_doc="${docs_root}/privacy.md"
...
require_contains "$codex_plugin_ref" '`SessionStart`' "Codex plugin reference must document startup launch"
require_contains "$codex_plugin_ref" '`PostToolUse`' "Codex plugin reference must document advisory diagnostics"
require_contains "$codex_plugin_ref" "does not persist session state" "Codex plugin reference must document the stateless boundary"
require_contains "$codex_use_doc" "never blocks the edit or turn" "Codex usage must document advisory behavior"
require_contains "$privacy_doc" "stores no plugin session data" "Codex privacy notice must document the stateless boundary"
...
require_contains "$local_development_doc" 'machine status' "Development-machine docs must document the public machine status command"
require_contains "$local_development_doc" 'kast machine reconcile' "Development-machine docs must document synchronous reconciliation"
...
require_not_contains "$docs_root" '--output json' "Published examples must use TOON instead of deprecated JSON output"
require_file_not_contains "$readme" '--output json' "README examples must use TOON instead of deprecated JSON output"
...
require_contains "$runtime_ref" "Agent commands default to" "Runtime reference must document TOON-first agent output"
...
require_contains "$local_development_doc" "refreshDevelopmentMachine" "Development-machine docs must expose the one refresh entrypoint"
require_contains "$local_development_doc" "installs no LaunchAgent" "Development-machine docs must document the processless boundary"
require_contains "$local_development_doc" "HEADLESS_LOCAL_UNSUPPORTED" "Development-machine docs must document the macOS headless prohibition"
require_contains "$local_development_doc" "There is no backend" "Development-machine docs must document IDEA-only leases"
