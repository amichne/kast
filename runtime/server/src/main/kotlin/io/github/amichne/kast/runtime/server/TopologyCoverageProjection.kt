...
/**
 * Proof transition from complete domain coverage failure to its public protocol representation. Every exact path,
 * location-bearing node, and contradictory endpoint is retained.
 */
fun TopologyGenerationCoverageFailure.toProtocolCoverage():
    Refinement<
        TopologyCoverageFailure,
        TopologyCoverageProjectionRejection,
    > {
    val projectedMissing =
        when (val result = missing.project(WorkspaceSourcePath::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedUnexpected =
        when (val result = unexpected.project(WorkspaceSourcePath::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedDuplicateCandidates =
        when (val result = duplicateCandidates.project(WorkspaceSourcePath::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedDuplicateCompletions =
        when (val result = duplicateCompletions.project(WorkspaceSourcePath::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedWorkspaceMismatches =
        when (val result = workspaceMismatches.project(WorkspaceSourcePath::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedCandidateEvidenceMismatches =
        when (val result = candidateEvidenceMismatches.project(TopologyCandidateEvidenceMismatch::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedDuplicateSymbols =
        when (val result = duplicateSymbols.project(TopologyNodeIdentity::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedMissingEdgeTargets =
        when (val result = missingEdgeTargets.project(TopologyNodeIdentity::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedMismatchedEndpoints =
        when (val result = mismatchedEdgeEndpoints.project(TopologySymbol::toProtocol)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    return when (
        ...
        is Refinement.Rejected -> Refinement.Rejected(TopologyCoverageProjectionRejection.EMPTY_FAILURE)
    }
}

private fun WorkspaceSourcePath.toProtocol():
    Refinement<
        ProtocolText,
        TopologyCoverageProjectionRejection,
    > = value.toProtocolText()

private fun TopologyNodeIdentity.toProtocol():
    Refinement<
        TopologyCoverageNode,
        TopologyCoverageProjectionRejection,
    > {
    val compiler =
        when (val result = compilerIdentity.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val path =
        when (val result = file.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val start =
        when (val result = ProtocolOffset.parse(range.startInclusive)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected ->
                return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_RANGE)
        }
    val end =
        when (val result = ProtocolOffset.parse(range.endExclusive)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected ->
                return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_RANGE)
        }
    val projectedRange =
        when (val result = SourceRangeDocument.create(start, end)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected ->
                return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_RANGE)
        }
...
private fun TopologySymbol.toProtocol():
    Refinement<
        TopologyCoverageSymbol,
        TopologyCoverageProjectionRejection,
    > {
    val node =
        when (val result = nodeIdentity.toProtocol()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val fileEvidence =
        when (val result = file.toProtocol()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val name =
        when (val result = evidence.name.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val qualified =
        when (val identity = evidence.qualifiedIdentity) {
            is ExactDeclarationQualifiedIdentity.Available ->
                when (val result = identity.value.toProtocolText()) {
                    is Refinement.Refined -> TopologyCoverageQualifiedIdentity.Available(result.value)
                    is Refinement.Rejected -> return result
                }
            ExactDeclarationQualifiedIdentity.Unavailable -> TopologyCoverageQualifiedIdentity.Unavailable
        }
    val signature =
        evidence.signature.toProtocolDocument()
            ?: return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_COMPILER_EVIDENCE)
    val compilerIdentity =
        when (val result = evidence.compilerIdentity.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val compilerEvidence =
        when (val result = CompilerSymbolEvidenceDocument.restore(compilerIdentity, signature)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected ->
                return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_COMPILER_EVIDENCE)
        }
    return when (
        ...
        is Refinement.Rejected ->
            Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_COMPILER_EVIDENCE)
    }
}
    ...
    when (this) {
        is CanonicalCompilerSignature.Function ->
            CompilerSignatureDocument.Function(
                qualifiedIdentity = qualifiedIdentity.value.toProtocolText().valueOrNull() ?: return null,
                receiver =
                    when (val value = receiver) {
                        ...
                        is CanonicalCompilerReceiver.Present ->
                            CompilerReceiverDocument.Present(
                ...
                typeParameterCount =
                    CompilerTypeParameterCountDocument.parse(typeParameterCount.value).valueOrNull() ?: return null,
            )
        is CanonicalCompilerSignature.Property ->
            CompilerSignatureDocument.Property(
                ...
                when (val value = receiver) {
                    CanonicalCompilerReceiver.Absent -> CompilerReceiverDocument.Absent
                    is CanonicalCompilerReceiver.Present ->
                        CompilerReceiverDocument.Present(value.type.value.toProtocolText().valueOrNull() ?: return null)
                },
        ...
        is CanonicalCompilerSignature.TypeAlias ->
            CompilerSignatureDocument.TypeAlias(qualifiedIdentity.value.toProtocolText().valueOrNull() ?: return null)
        is CanonicalCompilerSignature.ClassLike ->
            CompilerSignatureDocument.ClassLike(qualifiedIdentity.value.toProtocolText().valueOrNull() ?: return null)
    }

private fun List<CanonicalCompilerType>.toProtocolTypes(): BoundedProtocolList<ProtocolText>? =
    BoundedProtocolList.create(map { type -> type.value.toProtocolText().valueOrNull() ?: return null }).valueOrNull()

private fun TopologyCandidateEvidenceMismatch.toProtocol():
    Refinement<
        TopologyCoverageCandidateEvidenceMismatch,
        TopologyCoverageProjectionRejection,
    > {
    val projectedCandidate =
        when (val result = candidate.toProtocol()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedCompleted =
        when (val result = completed.toProtocol()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    return Refinement.Refined(TopologyCoverageCandidateEvidenceMismatch(projectedCandidate, projectedCompleted))
}

private fun TopologySourceFile.toProtocol():
    Refinement<
        TopologyCoverageFileEvidence,
        TopologyCoverageProjectionRejection,
    > {
    val workspaceRoot =
        when (val result = workspace.lease.workspaceRoot.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val sourceState =
        when (val result = workspace.sourceState.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val module =
        when (val result = sourceRoot.owner.module.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val buildRoot =
        when (val result = sourceRoot.owner.project.buildRoot.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectPath =
        when (val result = sourceRoot.owner.project.projectPath.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val sourceSet =
        when (val result = sourceRoot.owner.sourceSet.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val location =
        when (val result = sourceRoot.location.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedPath =
        when (val result = path.value.toProtocolText()) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected -> return result
        }
    val projectedHash =
        when (val result = TopologyCoverageSourceHash.parse(contentHash.value)) {
            is Refinement.Refined -> result.value
            is Refinement.Rejected ->
                return Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_CONTENT_HASH)
        }
    val provenance =
        when (val value = sourceRoot.provenance) {
...
private fun String.toProtocolText(): Refinement<ProtocolText, TopologyCoverageProjectionRejection> =
    when (val parsed = ProtocolText.parse(this)) {
        ...
        is Refinement.Rejected -> Refinement.Rejected(TopologyCoverageProjectionRejection.UNREPRESENTABLE_TEXT)
    }
