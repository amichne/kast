...
internal sealed interface CleanupRetention {
    val reason: String
    ...
    data class RestorationFailed(
        ...
        val restorationFailure: Exception,
    ) : CleanupRetention {
        override val reason: String =
            "$primaryReason; cleanup restoration failed: ${restorationFailure.failureReason()}"
    }
}
...
internal sealed interface CleanupResult {
    ...
    data class Retained(
        ...
        val retention: CleanupRetention,
    ) : CleanupResult {
        constructor(recoveryFilePath: Path, reason: String) : this(
            recoveryFilePath,
            CleanupRetention.Primary(reason),
        )
        ...
        val restorationFailure: Exception?
            get() = (retention as? CleanupRetention.RestorationFailed)?.restorationFailure
        ...
        companion object {
            fun withRestorationFailure(
                recoveryFilePath: Path,
                primaryReason: String,
                restorationFailure: Exception,
            ): Retained = Retained(
                recoveryFilePath = recoveryFilePath,
                retention = CleanupRetention.RestorationFailed(primaryReason, restorationFailure),
            )
        }
    }
