...
class ProductInspectionCommandTest {
    ...
    fun `product inspection reports sidecar cache and per-socket telemetry without runtime demand`(
        ...
        val root = (FilesystemCanonicalRootDiscovery.discover(rootPath) as CanonicalRootDiscovery.Discovered).root
        val support =
        ...
        val exactEndpoint =
            (endpoint.forSidecarCache(cacheIdentity, identity.runtimeId, physicalCacheRoot)
        ...
        assertEquals(
            ...
            output.getValue("control").jsonObject.getValue("execution").jsonPrimitive.content,
        )
    ...
    private fun localMetadata(): CliLocalMetadata =
