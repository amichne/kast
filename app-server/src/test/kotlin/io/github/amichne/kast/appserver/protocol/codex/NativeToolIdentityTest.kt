...
class NativeToolIdentityTest {
    ...
    fun `started call exposes raw arguments in a native expandable tool item`() {
        val item =
            Json.parseToJsonElement(
                    """{"type":"dynamicToolCall","id":"call-1","namespace":"kast","tool":"query","arguments":{"selector":"exact:v2:opaque"},"status":"inProgress","futureField":"retained"}"""
                )
                .jsonObject
    ...
    fun `completed and failed calls expose every raw result without summary parsing`() {
        for (success in listOf(true, false)) {
            ...
            val item =
                Json.parseToJsonElement(
                        """{"type":"dynamicToolCall","id":"call-1","namespace":"kast","tool":"query","arguments":{},"status":"$status","success":$success,"durationMs":17,"contentItems":[{"type":"inputText","text":"query: qualified"},{"type":"inputText","text":"{\"status\":\"qualified\",\"items\":[],\"failures\":[\"budget\"]}"}]}"""
                    )
                    .jsonObject
            ...
            assertEquals(
                item.getValue("contentItems").jsonArray.map { it.jsonObject["text"] },
                content.map { it.jsonObject["text"] },
            )
            if (success) assertEquals(JsonNull, projected["error"])
            else
                assertEquals(
                    "Tool call failed",
                    projected.getValue("error").jsonObject.getValue("message").jsonPrimitive.content,
                )
        }
    }
    ...
    fun `contradictory completion is rejected before display`() {
        val item =
            Json.parseToJsonElement(
                    """{"type":"dynamicToolCall","id":"call-1","namespace":"kast","tool":"query","arguments":{},"status":"failed","success":true}"""
                )
                .jsonObject
        assertEquals(
            CodexToolCallProjection.Rejected(CodexToolCallProjectionFailure.COMPLETION_SUCCESS_CONFLICT),
            CodexToolCallProjector.projectCompleted(item),
        )
    }
...
internal fun assertRawToolDisplay(original: JsonObject, displayed: JsonObject) {
    ...
    if (original["status"] == JsonPrimitive("inProgress")) {
        ...
        raw.zip(visible).forEach { (source, rendered) ->
            ...
            val expected =
                if (source.jsonObject["type"] == JsonPrimitive("inputText")) source.jsonObject.getValue("text")
        ...
        if (original["status"] == JsonPrimitive("failed")) {
            assertEquals(
                "Tool call failed",
                displayed.getValue("error").jsonObject.getValue("message").jsonPrimitive.content,
            )
