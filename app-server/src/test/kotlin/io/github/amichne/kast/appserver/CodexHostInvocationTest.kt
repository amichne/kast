...
class CodexHostInvocationTest {
    ...
    fun `remote client cannot replace the host owned working directory`() {
        listOf(
            ...
            .forEach { arguments ->
                assertEquals(
                    Refinement.Rejected(
                        CodexHostInvocationFailure.ClientArguments(CodexArgumentFailure.WORKING_DIRECTORY_OVERRIDE)
                    ),
    ...
    fun `transport and subcommand text used as role option values are not reinterpreted`() {
        val invocation =
            assertInstanceOf(
                CodexHostInvocation.AppServer::class.java,
                refined(CodexHostInvocation.admit(listOf("app-server", "-c", "--listen", "-c", "proxy"))),
            )
    ...
    fun `global remote override and missing option values fail closed`() {
        assertEquals(
            Refinement.Rejected(
                CodexHostInvocationFailure.AppServerArguments(CodexAppServerArgumentFailure.REMOTE_OVERRIDE)
            ),
            CodexHostInvocation.admit(listOf("--remote", "wss://example.invalid", "app-server")),
        )
        assertEquals(
    ...
    @Test
    fun `stock desktop startup reaches installation admission while process overrides reject`() =
