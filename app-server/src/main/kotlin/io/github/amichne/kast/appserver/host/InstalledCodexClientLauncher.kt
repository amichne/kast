...
@JvmInline
internal value class CodexDesktopExecutable private constructor(private val value: BrokerExecutable) {
    val path: Path
        get() = value.path

    companion object {
        internal fun admit(candidate: Path): Refinement<CodexDesktopExecutable, CodexDesktopExecutableFailure> =
            when (val admission = BrokerExecutable.admit(candidate)) {
                is Refinement.Refined -> Refinement.Refined(CodexDesktopExecutable(admission.value))
                is Refinement.Rejected -> Refinement.Rejected(CodexDesktopExecutableFailure.UNAVAILABLE)
            }
    }
}
...
internal class InstalledCodexClientLauncher(
    ...
    override fun launch(client: CodexClientLaunch): CodexClientLaunchRun {
        if (
            client == CodexClientLaunch.Desktop &&
        ...
        val command =
            when (
                ...
                is BrokerServiceLaunchCommandResolution.Rejected ->
                    return CodexClientLaunchRun.Rejected(resolution.failure.launchFailure())
            }
        val codex =
        ...
        val workingDirectory =
            CanonicalBrokerDirectory.admit(Path.of(System.getProperty("user.dir")))
                ?: return CodexClientLaunchRun.Rejected(CodexClientLaunchFailure.WORKING_DIRECTORY_REJECTED)
        ...
        val request =
            when (client) {
                ...
                CodexClientLaunch.Desktop ->
                    when (val admission = desktopRequest(command, codex)) {
        ...
        return processLauncher.launch(request).also { result ->
            when (result) {
                is CodexClientLaunchRun.Completed ->
                    debug(command, client, "client-completed", result.exitCode.toString())
                is CodexClientLaunchRun.Rejected ->
                    debug(command, client, "client-rejected", result.failure.name.lowercase())
            }
        }
    }
    ...
    private fun desktopRequest(
        ...
        val facade =
            when (val admission = DesktopFacadeExecutable.admit(command.kast.parent.resolve("kast-codex"))) {
                is Refinement.Refined -> admission.value
                is Refinement.Rejected -> return Refinement.Rejected(CodexClientLaunchFailure.FACADE_UNAVAILABLE)
            }
        val executable =
            resolveDesktopExecutable(command.configuration)
                ?: return Refinement.Rejected(CodexClientLaunchFailure.DESKTOP_UNAVAILABLE)
        return Refinement.Refined(CodexClientProcessRequest.Desktop(executable, facade, codex, command.codexHome))
    }
...
private object JdkCodexClientProcessLauncher : CodexClientProcessLauncher {
    ...
    private fun launchDesktop(request: CodexClientProcessRequest.Desktop): CodexClientLaunchRun =
        try {
