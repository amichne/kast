...
internal sealed interface IntellijDiagnosticLeaseAdmission {
    ...
    data class Rejected(val reason: DiagnosticCompilerRejection) : IntellijDiagnosticLeaseAdmission
}

/**
 * Proof transition: `(SemanticReadAuthority, SemanticReadAuthority) -> IntellijDiagnosticLeaseAdmission`.
 *
 * Admitted proves exact canonical root and authority equality. Rejected preserves root mismatch or authority movement
 * as [DiagnosticCompilerRejection]. Raw identity extraction stays at the workspace publication boundary.
 */
internal fun admitDiagnosticLease(
    ...
    when {
        current.workspaceRoot != requested.workspaceRoot ->
            IntellijDiagnosticLeaseAdmission.Rejected(DiagnosticCompilerRejection.WORKSPACE_ROOT_MISMATCH)
        current != requested -> IntellijDiagnosticLeaseAdmission.Rejected(DiagnosticCompilerRejection.GENERATION_MOVED)
...
internal class IntellijDiagnosticCompilerQuery(private val fileAdmission: (Path) -> Boolean = { true }) {
     ...
     *
     * A non-rejected result establishes current lease equality, exact-file VFS and source-content admission,
     * request-local K2 analysis, detached facts, and complete or qualified per-file coverage.
     * [DiagnosticCompilerRejection] is the closed expected failure. Live Project, VFS, PSI, read-action, and K2 values
     * remain within this call; cancellation propagates.
     */
    suspend fun read(
        project: Project,
        ...
        when (val admission = admitDiagnosticLease(currentLease, scope.lease)) {
            ...
            is IntellijDiagnosticLeaseAdmission.Rejected -> return DiagnosticCompilation.Rejected(admission.reason)
        }
...
class IntellijDiagnosticCompilerAdapter
internal constructor(
     ...
     * Proof transition: `(Project, SemanticReadAuthority, DiagnosticScope) -> DiagnosticCompilation`.
     *
     * Complete or qualified output carries only detached exact-scope diagnostics and coverage for the requested
     * authority. [DiagnosticCompilerRejection] is the closed expected failure. Project, VFS, PSI, read-action, and K2
