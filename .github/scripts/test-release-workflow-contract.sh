...
ubuntu_debian_headless_packager="${repo_root}/scripts/package-ubuntu-debian-headless-bundle.sh"
ubuntu_debian_smoke="${repo_root}/scripts/smoke-ubuntu-debian-bundle.sh"
ubuntu_debian_headless_smoke="${repo_root}/scripts/smoke-ubuntu-debian-headless-bundle.sh"
ubuntu_debian_docker_validator="${repo_root}/scripts/validate-ubuntu-debian-bundle-in-docker.sh"
ubuntu_debian_headless_docker_validator="${repo_root}/scripts/validate-ubuntu-debian-headless-bundle-in-docker.sh"
...
[[ -x "$ubuntu_debian_headless_packager" ]] || die "Ubuntu/Debian headless bundle packager not found or not executable: $ubuntu_debian_headless_packager"
[[ -x "$ubuntu_debian_smoke" ]] || die "Ubuntu/Debian bundle smoke not found or not executable: $ubuntu_debian_smoke"
[[ -x "$ubuntu_debian_headless_smoke" ]] || die "Ubuntu/Debian headless bundle smoke not found or not executable: $ubuntu_debian_headless_smoke"
...
[[ -x "$ubuntu_debian_headless_docker_validator" ]] || die "Ubuntu/Debian headless Docker validator not found or not executable: $ubuntu_debian_headless_docker_validator"
...
require_contains "$ci_workflow" "./scripts/smoke-ubuntu-debian-headless-bundle.sh" "CI must smoke the Ubuntu/Debian headless bundle contract"
...
require_contains "$ci_workflow" "validate-ubuntu-debian-headless-bundle-in-docker.sh" "CI install-container validation must run the headless Docker validator"
...
require_contains "$ci_workflow" "id: headless-dist-cache" "CI must restore the headless backend distribution cache"
require_contains "$ci_workflow" "backend-headless/build/distributions" "CI must cache the headless backend distribution output"
require_contains "$ci_workflow" "steps.headless-dist-cache.outputs.cache-hit != 'true'" "CI must skip the headless distribution build on cache hits"
require_contains "$ci_workflow" "Download standalone daemon distribution" "CI install-container validation must use the standalone backend distribution"
require_contains "$ci_workflow" "backend-standalone-*-portable.zip" "CI install-container validation must package the standalone backend"
require_contains "$ci_workflow" "Download headless backend distribution" "CI headless install-container validation must use the headless backend distribution"
require_contains "$ci_workflow" "backend-headless-*-portable.zip" "CI headless install-container validation must package the headless backend"
...
require_contains "$release_provenance_assembler" '"ubuntu-debian-headless-x86_64"' "Release provenance must include the Ubuntu/Debian headless bundle"
...
require_contains "$release_workflow" "Download standalone backend artifact" "Ubuntu/Debian bundle packaging must consume the standalone backend artifact"
require_contains "$release_workflow" "standalone-artifact/dist/backend.zip" "Ubuntu/Debian bundle packaging must use the standalone backend zip"
require_contains "$release_workflow" "Download headless backend artifact" "Ubuntu/Debian headless bundle packaging must consume the headless backend artifact"
require_contains "$release_workflow" "headless-artifact/dist/headless.zip" "Ubuntu/Debian headless bundle packaging must use the headless backend zip"
...
require_contains "$release_workflow" '"kast-ubuntu-debian-headless-x86_64-${tag}.tar.gz"' "Release must require the Ubuntu/Debian headless bundle asset"
...
require_occurrences "$release_workflow" "if: always() && needs.prepare-release.result == 'success'" 6 "Tag-push release jobs must opt out of skipped bump-version poisoning"
...
require_contains "$release_workflow" "needs.build-ubuntu-debian-headless-bundle.result == 'success'" "Release publish job must require a successful Ubuntu/Debian headless bundle build"
require_contains "$release_workflow" "build-standalone-backend" "Ubuntu/Debian bundle job must depend on the standalone backend build"
require_contains "$release_workflow" "build-headless-backend" "Ubuntu/Debian headless bundle job must depend on the headless backend build"
...
require_contains "$ubuntu_debian_docker_validator" "kast up \\" "Ubuntu/Debian Docker validator must start the installed backend"
require_contains "$ubuntu_debian_docker_validator" "KAST_UBUNTU_DEBIAN_BUNDLE_KIND" "Ubuntu/Debian Docker validator must select standalone or headless bundle kind"
require_contains "$ubuntu_debian_docker_validator" "backend_args=()" "Ubuntu/Debian Docker validator must default to the standalone backend path"
require_contains "$ubuntu_debian_docker_validator" "backend_args=(--backend=headless)" "Ubuntu/Debian Docker validator must support the headless backend path"
...
require_contains "$ubuntu_debian_docker_validator" "kast capabilities \"\${backend_args[@]}\" --workspace-root=\"\${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}\"" "Ubuntu/Debian Docker validator must query the installed backend"
require_contains "$ubuntu_debian_headless_docker_validator" "validate-ubuntu-debian-bundle-in-docker.sh" "Headless Docker validator must use the same Docker validation implementation"
require_contains "$ubuntu_debian_headless_docker_validator" "KAST_UBUNTU_DEBIAN_BUNDLE_KIND=headless" "Headless Docker validator must select the headless bundle kind"
...
require_contains "$ubuntu_debian_packager" "backend-standalone" "Ubuntu/Debian packager must consume the standalone backend archive"
require_contains "$ubuntu_debian_packager" "standalone-backend" "Ubuntu/Debian bundle manifest must identify the standalone backend role"
require_contains "$ubuntu_debian_headless_packager" "package-ubuntu-debian-bundle.sh" "Ubuntu/Debian headless packager must be a sibling entry point"
require_contains "$ubuntu_debian_headless_packager" "KAST_UBUNTU_DEBIAN_BUNDLE_KIND=headless" "Ubuntu/Debian headless packager must select the headless backend bundle"
