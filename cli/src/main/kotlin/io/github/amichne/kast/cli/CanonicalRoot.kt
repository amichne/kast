...
class CanonicalRoot internal constructor(val path: Path) {
...
sealed interface CanonicalRootDiscovery {
    data class Discovered(val root: CanonicalRoot) : CanonicalRootDiscovery
...
fun interface CanonicalRootDiscoverer {
     ...
     *
     * Establishes the nearest canonical, settings-owned repository directory. [CanonicalRootFailure] is the closed
     * expected failure. The canonical path may be extracted only for exact-root process and socket admission.
     */
        ...
        while (true) {
            ...
            when (val parent = parentOf(candidate)) {
                ...
                CanonicalParent.FilesystemRoot ->
                    return CanonicalRootDiscovery.Rejected(CanonicalRootFailure.ROOT_MARKER_NOT_FOUND)
            }
        }
    }
}
...
private sealed interface CanonicalParent {
    data class Parent(val path: Path) : CanonicalParent

    data object FilesystemRoot : CanonicalParent
}

private fun parentOf(path: Path): CanonicalParent =
    path.parent?.let(CanonicalParent::Parent) ?: CanonicalParent.FilesystemRoot
