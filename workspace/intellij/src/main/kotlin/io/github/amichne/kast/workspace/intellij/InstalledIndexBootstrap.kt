...
import io.github.amichne.kast.workspace.intellij.read.IntellijIndexExclusionVerification
import io.github.amichne.kast.workspace.intellij.read.IntellijIndexExclusionVerificationFailure
import io.github.amichne.kast.workspace.intellij.read.IntellijIndexExclusionVerifier
...
private enum class InstalledGeneratedDirectoryKind(val directoryName: String) {
    ...
    companion object {
        ...
        fun identify(path: Path): InstalledGeneratedDirectoryKind? {
            ...
            return when (identified) {
                BUILD_OUTPUT ->
                    if (path.parent?.ownsConventionalGradleBuild() == true) {
                        identified
                    } else {
                        null
                    }
...
private val GRADLE_BUILD_OWNER_FILES =
    listOf(
        "build.gradle",
        "build.gradle.kts",
        "settings.gradle",
...
@JvmInline
internal value class InstalledIndexExcludedDirectory private constructor(val path: Path) {
    companion object {
...
internal class InstalledIndexExclusionPlan
private constructor(
    ...
    companion object {
        fun discover(workspaceRoot: Path): InstalledIndexExclusionPlanDiscovery {
            ...
            return try {
                Files.walkFileTree(
                    ...
                    object : SimpleFileVisitor<Path>() {
                        override fun preVisitDirectory(
                            ...
                            val admitted =
                                InstalledIndexExcludedDirectory.admit(
                                    canonicalRoot,
                                    directory,
                                )
                        ...
                        override fun visitFile(
                            ...
                            if (attributes.isSymbolicLink) {
                                InstalledIndexExcludedDirectory.admit(canonicalRoot, file)?.let(excluded::add)
                            }
                ...
                InstalledIndexExclusionPlanDiscovery.Rejected(InstalledIndexExclusionPlanFailure.DISCOVERY_FAILED)
...
internal sealed interface InstalledIndexExclusionPlanDiscovery {
    data class Discovered(val plan: InstalledIndexExclusionPlan) : InstalledIndexExclusionPlanDiscovery

    data class Rejected(val failure: InstalledIndexExclusionPlanFailure) : InstalledIndexExclusionPlanDiscovery
}

/** Runtime-owned generated project configuration installed before IntelliJ project open. */
internal class InstalledIndexBootstrap
private constructor(
                ...
                InstalledIndexBootstrapPreparation.Prepared(
                    InstalledIndexBootstrap(
                        ...
                        excludedDirectoryPaths =
                            plan.excludedDirectories.map { directory ->
                                directory.path
                            },
                        excludedDirectoryUrls =
                            plan.excludedDirectories.mapTo(linkedSetOf()) { directory ->
                                directory.path.intellijFileUrl()
                            },
                    )
                )
            } catch (_: IOException) {
                InstalledIndexBootstrapPreparation.Rejected(InstalledIndexBootstrapFailure.CONFIGURATION_WRITE_FAILED)
            } catch (_: SecurityException) {
                InstalledIndexBootstrapPreparation.Rejected(InstalledIndexBootstrapFailure.CONFIGURATION_WRITE_FAILED)
    ...
    override fun bind(project: Project): Boolean =
        try {
     ...
     *
     * Active proves the generated module, exact workspace content root, and every planned exclusion were loaded from
     * the runtime-owned XML before Gradle import. Rejected closes missing identity, weakened roots, and platform
     * observation failures.
     */
    fun activate(project: Project): InstalledIndexBootstrapActivation =
        try {
            ReadAction.computeBlocking<InstalledIndexBootstrapActivation, RuntimeException> {
                ...
                val installedPolicyCount =
                    DirectoryIndexExcludePolicy.EP_NAME.getExtensions(project).count { policy ->
                        policy is InstalledIndexExclusionPolicy
                    }
                if (
                    installedPolicyCount != 1 ||
                        project.getUserData(INSTALLED_INDEX_EXCLUSIONS)?.urls?.toSet() != excludedDirectoryUrls
                ) {
...
internal sealed interface InstalledIndexBootstrapPreparation {
    data class Prepared(val bootstrap: InstalledIndexBootstrap) : InstalledIndexBootstrapPreparation

    data class Rejected(val failure: InstalledIndexBootstrapFailure) : InstalledIndexBootstrapPreparation
}
...
internal sealed interface InstalledIndexBootstrapActivation {
    data class Active(val bootstrap: InstalledActiveIndexBootstrap) : InstalledIndexBootstrapActivation

    data class Rejected(val failure: InstalledIndexBootstrapActivationFailure) : InstalledIndexBootstrapActivation
}
...
internal class InstalledActiveIndexBootstrap(
    ...
    /**
     * Proof transition: `InstalledActiveIndexBootstrap + Project -> InstalledIndexBootstrapRetirement`.
     *
     * Retired proves the temporary module can no longer mask source roots from the imported Gradle model. The Gradle
     * import is permitted to replace it before explicit retirement.
     */
    fun retire(project: Project): InstalledIndexBootstrapRetirement =
        try {
            WriteAction.computeAndWait<InstalledIndexBootstrapRetirement, RuntimeException> {
                ...
                if (module.isDisposed) {
                    model.dispose()
                    InstalledIndexBootstrapRetirement.Retired(InstalledIndexBootstrapRetirementAuthority.GRADLE_IMPORT)
                    ...
                    try {
                        ...
                        model.commit()
                        InstalledIndexBootstrapRetirement.Retired(InstalledIndexBootstrapRetirementAuthority.RUNTIME)
    ...
    /**
     * Proves the persistent policy excludes every planned root after import, never masks a subtree of an imported
     * source root, and admits imported generated source roots below an exclusion.
     */
    fun verifyImportedModel(project: Project): InstalledIndexExclusionVerification =
        when (
            val verification =
                IntellijIndexExclusionVerifier.verify(
                    project,
                    module,
                    excludedDirectoryPaths,
                )
        ) {
            is IntellijIndexExclusionVerification.Verified ->
                InstalledIndexExclusionVerification.Verified(verification.generatedSourceRootCount)
            is IntellijIndexExclusionVerification.Rejected ->
                InstalledIndexExclusionVerification.Rejected(
                    when (verification.failure) {
                        ...
                        IntellijIndexExclusionVerificationFailure.PLATFORM_OBSERVATION_FAILED ->
...
class InstalledIndexExclusionPolicy(private val project: Project) : DirectoryIndexExcludePolicy {
...
private val INSTALLED_INDEX_EXCLUSIONS = Key.create<InstalledProjectIndexExclusions>("kast.installed.index.exclusions")
...
internal sealed interface InstalledIndexExclusionVerification {
    data class Verified(val generatedSourceRootCount: Int) : InstalledIndexExclusionVerification

    data class Rejected(val failure: InstalledIndexExclusionVerificationFailure) : InstalledIndexExclusionVerification
}
...
internal sealed interface InstalledIndexBootstrapRetirement {
    data class Retired(val authority: InstalledIndexBootstrapRetirementAuthority) : InstalledIndexBootstrapRetirement

    data class Rejected(val failure: InstalledIndexBootstrapRetirementFailure) : InstalledIndexBootstrapRetirement
}
