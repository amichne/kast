...
class WireQualificationTest {
    @Test fun `reused qualified session sends qualification only once`() = fixture { session, peer, identity ->
    ...
    @Test fun `exact peer qualification precedes semantic payload on the connected channel`() = fixture { session, peer, identity ->
        WireFrameCodec.write(peer, WireRuntimeQualification.response(identity))
    ...
    @Test fun `foreign qualification rejects before semantic payload is sent`() = fixture { session, peer, identity ->
        WireFrameCodec.write(peer, "{}")
        assertEquals(WireExchange.Rejected(WireTransportFailure.UNQUALIFIED_PEER), session.exchange("semantic-secret", identity))
        session.close()
        assertEquals(WireFrameRead.Received(WireRuntimeQualification.request(identity)), WireFrameCodec.read(peer))
    ...
    private fun fixture(test: (WireSession, SocketChannel, WireRuntimeIdentity) -> Unit) {
        ...
        val identity = (WireRuntimeIdentity.admit(root, "sha256:${"a".repeat(64)}",
            (SemanticRuntimeBootstrapAttemptId.admit("11111111-1111-4111-8111-111111111111") as Refinement.Refined).value) as Refinement.Refined).value
        try {
            ServerSocketChannel.open(StandardProtocolFamily.UNIX).use { server ->
                ...
                SocketChannel.open(StandardProtocolFamily.UNIX).use { client ->
                    ...
                    server.accept().use { peer ->
                        WireSession(client, (ElapsedTimeLimitMillis.parse(1_000) as Refinement.Refined).value).use { session ->
