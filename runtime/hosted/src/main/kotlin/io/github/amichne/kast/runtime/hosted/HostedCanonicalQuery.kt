    ...
    val encoded =
        when (request) {
            ...
            is HostedRequest.Discover ->
                CanonicalOperationWireBindings.symbolDiscover.encodeOutcome(
            ...
            is HostedRequest.Inspect ->
                CanonicalOperationWireBindings.symbolInspect.encodeOutcome(
        ...
        is WireEncoding.Encoded ->
            if (
                encoded.document.toByteArray(Charsets.UTF_8).size <=
                    context.limits[ReadLimitParameter.HOST_RESPONSE_BYTES].value
            )
                encoded.document
