...
private const val APPLICATION_STARTER_CLASS = "com/intellij/openapi/application/ApplicationStarter.class"
private val REQUIRED_GRADLE_PLUGIN_DIRECTORIES =
    listOf(
...
sealed interface IdeHomeSelection {
    ...
    companion object {
        ...
        fun standard(userHome: Path): Standard =
            Standard(
                listOf(
...
sealed interface InstalledIdeRuntimeDiscoveryResult {
    data class Discovered(val runtime: InstalledIdeRuntime) : InstalledIdeRuntimeDiscoveryResult

    data class Rejected(val failure: IndexSeedFailure) : InstalledIdeRuntimeDiscoveryResult
}
...
object InstalledIdeRuntimeDiscovery {
    /**
     * Proof transition: `supported pair + payload digest + IdeHomeSelection -> InstalledIdeRuntimeDiscoveryResult`.
     *
     * Establishes one physical IDEA home with a compatible platform release line while preserving the exact observed
     * build pair, bundled JBR identity and executable, Gradle integration, and required bootstrap classes. Explicit
     * selection retains its exact rejection. Standard selection admits exactly one matching candidate, rejects multiple
     * matches as [IndexSeedFailure.Ambiguity], and never guesses when none match.
     */
    fun discover(
    ...
    private fun discoverStandard(
        ...
        return when (discovered.size) {
            ...
            in 2..Int.MAX_VALUE -> InstalledIdeRuntimeDiscoveryResult.Rejected(IndexSeedFailure.Ambiguity)
            else ->
                InstalledIdeRuntimeDiscoveryResult.Rejected(
    ...
    private fun inspect(
        ...
        val home = canonicalDirectory(candidate) ?: return rejectedValidation()
        ...
        return InstalledIdeRuntimeDiscoveryResult.Discovered(InstalledIdeRuntime(home, jbr.javaExecutable, identity))
    }

    private fun readIdeaBuild(home: Path): String? =
        try {
            Files.readString(home.resolve(IDEA_BUILD_FILE)).trim().substringAfter('-', "").takeIf(String::isNotBlank)
        } catch (_: IOException) {
            null
    ...
    private fun readKotlinPluginBuild(home: Path): String? =
        try {
    ...
    private fun jarContains(path: Path, entry: String): Boolean =
        try {
