...
`bind` seals an ordered symbol snapshot under an earlier-only query-local name. `join` reads that binding or a retained symbol result on the right using canonical symbol identity. The join index stores row positions, so inner join emits every matching pair with its two named symbol or proven occurrence cells; it never collapses repeated rows. `project_binding` selects one named cell before symbol-only stages continue. Semi join merges matching evidence into the surviving left symbol, while anti join emits a left symbol only after complete right membership proves no match. Incomplete retained right input rejects anti join; incomplete named input retains `JOIN_INPUT_INCOMPLETE` qualification and cannot establish absence. Query checkpoint state retains partially built binding and join indexes. `QueryRetainedResult` has separate symbol and binding-row variants, and read-result pages either type without rediscovering the prefix.
