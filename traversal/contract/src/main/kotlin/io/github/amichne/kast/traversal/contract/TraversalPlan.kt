...
value class TraversalByteLimit private constructor(val value: Long) {
    companion object {
        /**
         * Proof transition: `Long -> Refinement<TraversalByteLimit, TraversalPositiveLimitFailure>`.
         *
         * Establishes a finite positive bound for returned detached relation-fact bytes.
         * [TraversalPositiveLimitFailure] is the closed expected failure. Raw byte limits may be extracted only at
         * request admission, traversal accounting, or transport.
         */
...
value class TraversalDepthLimit private constructor(val value: Int) {
    companion object {
        /**
         * Proof transition: `Int -> Refinement<TraversalDepthLimit, TraversalPositiveLimitFailure>`.
         *
         * Establishes a finite positive maximum semantic hop depth. [TraversalPositiveLimitFailure] is the closed
         * expected failure. Raw depth limits may be extracted only at request admission or transport.
         */
...
value class TraversalFrontierLimit private constructor(val value: Int) {
    companion object {
        /**
         * Proof transition: `Int -> Refinement<TraversalFrontierLimit, TraversalPositiveLimitFailure>`.
         *
         * Establishes a finite positive bound on one-hop frontier expansions per run. [TraversalPositiveLimitFailure]
         * is the closed expected failure. Raw frontier limits may be extracted only at request admission or transport.
         */
...
sealed interface TraversalPosition {
    ...
    data class Resume internal constructor(val continuation: TraversalContinuation) : TraversalPosition
}
...
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(SymbolSelector, RelationMeaning, TraversalBudget) -> Refinement<TraversalPlan,
         * TraversalPlanFailure>`.
         *
         * Establishes one exact root/generation/scope, one closed meaning, and a one-hop budget no stronger than every
         * aggregate traversal bound. [TraversalPlanFailure] is the closed expected failure. Raw selector and numeric
         * extraction may occur only before this public traversal boundary.
         */
        fun start(
        ...
        /**
         * Proof transition: `(SymbolSelector, RelationMeaning, TraversalBudget, TraversalContinuation) ->
         * Refinement<TraversalPlan, TraversalPlanResumeFailure>`.
         *
         * Establishes that explicit new bounds resume only the continuation's exact selector, meaning, root,
         * generation, and scope. [TraversalPlanResumeFailure] is the closed expected failure. Raw continuation decoding
         * may occur only before this public traversal boundary.
         */
        fun resume(
            selector: SymbolSelector,
            meaning: RelationMeaning,
            ...
            val resumeFailure =
                when {
                    ...
                    selector.scope != continuation.start.scope -> TraversalResumeFailure.SCOPE_MISMATCH
            ...
            if (resumeFailure != null) {
                return Refinement.Rejected(TraversalPlanResumeFailure.Resume(resumeFailure))
            }
            return when (
                val admitted =
                    admit(
                ...
                is Refinement.Rejected -> Refinement.Rejected(TraversalPlanResumeFailure.Plan(admitted.failure))
            }
        }

        /**
         * Proof transition: `(SymbolSelector, RelationMeaning, TraversalBudget, TraversalPosition) ->
         * Refinement<TraversalPlan, TraversalPlanFailure>`.
         *
         * Establishes that no one-hop limit exceeds its aggregate traversal authority. [TraversalPlanFailure] is the
         * closed expected failure. Raw values remain at the public start/resume boundary.
         */
        private fun admit(
            selector: SymbolSelector,
            meaning: RelationMeaning,
            ...
            when {
                budget.oneHop.resources.resultLimit.value > budget.records.value ->
                    Refinement.Rejected(TraversalPlanFailure.ONE_HOP_RECORD_LIMIT_EXCEEDS_TRAVERSAL)
                budget.oneHop.returnedBytes.value > budget.returnedBytes.value ->
                    Refinement.Rejected(TraversalPlanFailure.ONE_HOP_BYTE_LIMIT_EXCEEDS_TRAVERSAL)
                budget.oneHop.resources.workUnitLimit.value > budget.workUnits.value ->
                    Refinement.Rejected(TraversalPlanFailure.ONE_HOP_WORK_LIMIT_EXCEEDS_TRAVERSAL)
                budget.oneHop.resources.elapsedTimeLimit.value > budget.elapsedTime.value ->
                    Refinement.Rejected(TraversalPlanFailure.ONE_HOP_TIME_LIMIT_EXCEEDS_TRAVERSAL)
                else ->
                    Refinement.Refined(
                        TraversalPlan(
                            selector,
...
internal fun RelationMeaning.canonicalName(): String =
    when (this) {
