...
internal enum class GradleInitializationScriptObservation {
    UNAVAILABLE,
    UNCLASSIFIED,
}
...
internal fun observeGradleInitializationScript(failure: Throwable?): GradleInitializationScriptObservation {
    for (cause in generateSequence(failure) { it.cause }.take(16)) {
        ...
        if (
            message.length <= 4096 &&
                '\n' !in message &&
                message.startsWith("The specified initialization script '") &&
        ...
        fun observe(failure: Throwable?): GradlePayloadCompatibility {
            ...
            for (cause in generateSequence(failure) { it.cause }.take(16)) {
                ...
                for (pattern in patterns) {
                    ...
                    if (major in 45..100)
                        return GradlePayloadCompatibility.Unsupported(GradlePayloadClassFileMajor(major))
                }
            }
...
internal fun InstalledGradleImportExecutionIdentity.logFields(outcome: InstalledGradleImportOutcome) = buildJsonObject {
    ...
    when (outcome) {
        ...
        InstalledGradleImportOutcome.InitializationScriptUnavailable ->
            put("outcome", "initialization-script-unavailable")
