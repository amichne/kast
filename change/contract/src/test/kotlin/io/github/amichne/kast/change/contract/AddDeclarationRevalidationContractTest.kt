...
class AddDeclarationRevalidationContractTest {
    ...
    fun `every stale or unsafe predicate is a finite rejection before mutation begins`() {
        ...
        val cases =
            listOf(
                observation(plan, generation = 8L) to AddDeclarationRevalidationFailure.GENERATION_CHANGED,
                    ...
                    compilerContext =
                        ExpectedAddDeclarationCompilerContext.admit(
                                ...
                                projectModelFingerprint =
                                    AddDeclarationProjectModelFingerprint.parse("3".repeat(64)).refined(),
                                classpathFingerprint =
                                    AddDeclarationClasspathFingerprint.parse("4".repeat(64)).refined(),
                                contextFiles =
                                    listOf(AddDeclarationCompilerContextFile.admit(TARGET, hash(PREIMAGE)).refined()),
    ...
    private fun owner(ideaModuleName: String = "kast.indexer.main"): AddDeclarationSourceOwner =
