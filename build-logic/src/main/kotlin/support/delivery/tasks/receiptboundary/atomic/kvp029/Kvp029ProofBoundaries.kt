...
internal enum class Kvp029BoundaryFailure {
...
internal data class Kvp029ImplementationCommit(
    val revision: DeliveryGeneration,
    val changedPaths: List<String>,
)

internal class AdmittedKvp029ImplementationScope internal constructor(
    val commits: List<Kvp029ImplementationCommit>,
)

internal sealed interface Kvp029ImplementationScopeAdmission {
    data class Complete(val scope: AdmittedKvp029ImplementationScope) :
        Kvp029ImplementationScopeAdmission
    data class Rejected(val failure: Kvp029BoundaryFailure) :
        Kvp029ImplementationScopeAdmission
}

internal sealed interface Kvp029RelevantInputAdmission {
    data class Complete(val digest: RelevantInputDigest) : Kvp029RelevantInputAdmission
    data class Rejected(val failure: Kvp029BoundaryFailure) : Kvp029RelevantInputAdmission
}

/**
 * Proof transition: admitted KVP-028 baseline/current head and graph-declared write roots ->
 * `Kvp029ImplementationScopeAdmission`.
 *
 * Establishes a nonempty, ordered KVP-029 commit delta after observing every checkpoint without a
 * pathspec. Earlier completed-task checkpoints are skipped until the first KVP-029-exclusive path;
 ...
 */
internal fun admitKvp029ImplementationScope(
    exec: ExecOperations,
    repositoryRoot: Path,
    ...
    if (revisions.exitCode != 0) {
        return scopeRejected(Kvp029BoundaryFailure.GIT_COMMAND_REJECTED)
    }
    val commits = mutableListOf<Kvp029ImplementationCommit>()
    ...
    revisions.text.lineSequence().filter(String::isNotBlank).forEach { revision ->
        ...
        ) return scopeRejected(Kvp029BoundaryFailure.WRITE_OUTSIDE_DECLARED_SCOPE)
        commits += Kvp029ImplementationCommit(DeliveryGeneration(revision), taskPaths)
    }
    return if (commits.isEmpty()) {
        scopeRejected(Kvp029BoundaryFailure.NO_IMPLEMENTATION_COMMIT)
    } else {
        Kvp029ImplementationScopeAdmission.Complete(
            AdmittedKvp029ImplementationScope(commits),
        )
    }
}
...
internal fun admitPriorKvp029ImplementationScope(
    ...
    val implementationHead = candidate.commits.lastOrNull()?.revision ?: return scopeRejected(
        Kvp029BoundaryFailure.NO_IMPLEMENTATION_COMMIT,
    )
    if (git(exec, repositoryRoot, listOf(
 ...
 */
internal fun admitKvp029RelevantInputs(
    exec: ExecOperations,
    repositoryRoot: Path,
    ...
    if (status.exitCode != 0) return inputRejected(
        Kvp029BoundaryFailure.GIT_COMMAND_REJECTED,
    )
    if (status.bytes.isNotEmpty()) return inputRejected(
        Kvp029BoundaryFailure.DIRTY_RELEVANT_INPUT,
    )
    ...
    if (paths.isEmpty()) return inputRejected(Kvp029BoundaryFailure.EMPTY_RELEVANT_INPUT)
    val digests = linkedMapOf<String, String>()
    paths.forEach { path ->
        if (roots.none { path.inScope(it) }) return inputRejected(
            Kvp029BoundaryFailure.RELEVANT_INPUT_READ_REJECTED,
        )
    ...
    return Kvp029RelevantInputAdmission.Complete(
        RelevantInputDigest(sha256(canonicalJson(closure)).value),
    )
}

private data class Kvp029GitObservation(val exitCode: Int, val bytes: ByteArray) {
...
private fun git(
    ...
    return Kvp029GitObservation(result.exitValue, output.toByteArray())
}

private fun String.inScope(scope: String) = this == scope || startsWith("$scope/")

private fun scopeRejected(failure: Kvp029BoundaryFailure) =
    Kvp029ImplementationScopeAdmission.Rejected(failure)

private fun inputRejected(failure: Kvp029BoundaryFailure) =
    Kvp029RelevantInputAdmission.Rejected(failure)
