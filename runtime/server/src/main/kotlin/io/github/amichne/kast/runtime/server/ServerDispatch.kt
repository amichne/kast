...
sealed interface ServerDispatch {
    data class Responded(val document: String) : ServerDispatch

    data class Rejected(val failure: ServerDispatchFailure) : ServerDispatch
}

/** Closed server failures kept distinct from semantic operation rejection outcomes. */
sealed interface ServerDispatchFailure {
    data class RequestAdmissionFailed(val failure: WireFailure) : ServerDispatchFailure

    data class UnsupportedOperation(val operation: CanonicalOperation) : ServerDispatchFailure
