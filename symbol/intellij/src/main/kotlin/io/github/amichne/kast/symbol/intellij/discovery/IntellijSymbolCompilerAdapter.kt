...
class IntellijSymbolCompilerAdapter internal constructor(private val nativeDiscovery: IntellijNativeDiscoveryAdapter) {
    constructor() : this(IntellijNativeDiscoveryAdapter())

    /**
     * Proof transition: `(Project, SymbolDiscoveryRequest, WorkspaceSearchScopeModelCompilation) -> SymbolCompilation`.
     *
     * A compiled result establishes request-local scope compilation before bounded index access and detached
     * authority-bound candidate projection. [SymbolCompilerRejection] is the closed expected failure. The live
     * [Project], compiled scope, provider, VFS, and PSI values remain inside this call.
     */
    suspend fun compile(
        project: Project,
        ...
        when (val result = nativeDiscovery.discover(project, request, modelCompilation)) {
            is IntellijNativeDiscoveryResult.Discovered -> SymbolCompilation.Compiled(result.outcome)
            is IntellijNativeDiscoveryResult.ScopeRejected ->
                SymbolCompilation.Rejected(SymbolCompilerRejection.SCOPE_REJECTED)
            is IntellijNativeDiscoveryResult.Rejected ->
                SymbolCompilation.Rejected(
                    when (result.reason) {
