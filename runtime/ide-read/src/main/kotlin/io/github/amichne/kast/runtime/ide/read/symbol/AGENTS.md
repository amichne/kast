# Hosted symbol route owner

This package owns the retained-Project symbol routes introduced by KVP-029 through KVP-031.

- Start every semantic request from a just-admitted `VfsPassiveReadCapability` for the retained
  exact Project and enter the existing single-flight controller before native work.
- Invoke only the nominal native route authority supplied by hosted composition. Do not locate,
...
Run the graph-named symbol route tasks, then `:runtime:ide-read:check` and the architecture gates.
