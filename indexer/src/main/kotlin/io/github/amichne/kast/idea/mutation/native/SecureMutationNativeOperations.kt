...
internal fun SecureWorkspaceMutation.renameNoReplace(
        ...
        if (result == 0) {
            persistParentDirectory(
                parent = parent,
                target = target,
                transition = NamespaceTransition.Rename(
                    sourceName = sourceName,
                    destinationName = destinationName,
                    phase = phase,
                ),
            )
            return RenameNoReplaceOutcome.MOVED
        }
...
internal fun SecureWorkspaceMutation.requireWorkspaceTarget(target: Path, mutation: IdeaWorkspaceMutation): Path {
        ...
        val relativeTarget = normalizedWorkspaceRoot.relativize(normalizedTarget)
        if (WorkspacePathPolicy.isHardExcluded(relativeTarget)) {
            val excludedComponent = relativeTarget.first { component ->
                WorkspacePathPolicy.isHardExcluded(Path.of(component.toString()))
            }
            throw UnsafeWorkspaceMutationException(
                message = "Secure mutation target is inside a hard-excluded workspace directory",
                details = failureDetails(normalizedTarget, "hard-excluded-target") + mapOf(
                    "mutation" to mutation.wireName,
                    "hardExcludedComponent" to excludedComponent.toString(),
                ),
            )
        }
...
internal sealed interface NamespaceUnlinkResult {
    data object Removed : NamespaceUnlinkResult

    data class Failed(val errno: Int) : NamespaceUnlinkResult
}

internal fun SecureWorkspaceMutation.unlinkAndPersistParent(
    parent: NativeDescriptor,
    name: String,
    target: Path,
): NamespaceUnlinkResult {
    if (parent.api.unlinkat(parent.value, name, 0) < 0) {
        return NamespaceUnlinkResult.Failed(Native.getLastError())
    }
    persistParentDirectory(
        parent = parent,
        target = target,
        transition = NamespaceTransition.Unlink(name),
    )
    return NamespaceUnlinkResult.Removed
}
