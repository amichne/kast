...
class AddDeclarationPlanTest {
    ...
    fun `classlike target plans an insertion inside its body`() {
        val plan =
            PureAddDeclarationPlanningService()
                .plan(AddDeclarationPlanFixture(symbolKind = CompilerSymbolKind.CLASSLIKE).request())
                .planned()
    ...
    fun `hosted plan survives canonical encode and decode`() {
        ...
        val reopened =
            HostedAddDeclarationPlanCodec.decode(encoded).let { decoded ->
                when (decoded) {
    ...
    fun `required incomplete evidence fails closed before plan construction`() {
        ...
        val cases =
            listOf(
                complete.copy(evidence = complete.evidence.copy(relations = listOf(fixture.qualifiedRelation()))) to
                    AddDeclarationPlanningFailure.RELATION_EVIDENCE_INCOMPLETE,
                complete.copy(evidence = complete.evidence.copy(traversals = listOf(fixture.rejectedTraversal()))) to
                    AddDeclarationPlanningFailure.TRAVERSAL_EVIDENCE_INCOMPLETE,
                complete.copy(evidence = complete.evidence.copy(diagnostics = listOf(fixture.qualifiedDiagnostic()))) to
    ...
    private fun AddDeclarationPlanResult.planned(): AddDeclarationChangePlan =
        when (this) {
            is AddDeclarationPlanResult.Planned -> plan
            is AddDeclarationPlanResult.Rejected -> error(failure.toString())
        }

    private fun AddDeclarationPlanResult.rejected(): AddDeclarationPlanningFailure =
        when (this) {
