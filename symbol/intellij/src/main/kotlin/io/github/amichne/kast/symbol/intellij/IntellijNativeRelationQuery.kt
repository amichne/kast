...
internal fun interface IntellijRelationFactProjector {
    /**
     * Proof transition: NativeRelationRequest + request-local native event to Refinement<NativeRelationFact,
     * IntellijRelationProjectionFailure>.
     *
     * Establishes a detached exact endpoint and occurrence bound to the request selector, or one closed
     * unsupported/unresolved state. Live IntelliJ values remain inside this call.
     */
    fun project(
...
internal sealed interface IntellijNativeRelationSearchResult {
    data class Terminal(val limitations: Set<NativeRelationLimitation> = emptySet()) :
        IntellijNativeRelationSearchResult

    data class Halted(val limitations: Set<NativeRelationLimitation> = emptySet()) : IntellijNativeRelationSearchResult

    data class Rejected(val reason: IntellijNativeRelationSearchRejection) : IntellijNativeRelationSearchResult
}
...
internal fun interface IntellijNativeRelationSearch {
     ...
     * IntellijNativeRelationSearchResult.
     *
     * Establishes that the exact selector was revalidated and its requested one-hop family was streamed only through
     * the compiled native scope. Terminal completion is explicit; subject identity failures and incomplete provider
     * coverage are closed states. Live PSI and query objects remain inside this request-local call.
     */
    fun search(
...
internal sealed interface IntellijNativeRelationExecution {
    data class Produced(val outcome: NativeRelationOutcome) : IntellijNativeRelationExecution

    data class Rejected(val reason: IntellijNativeRelationRejection) : IntellijNativeRelationExecution
}
...
internal class IntellijNativeRelationQuery(
    ...
    /**
     * Proof transition: CompiledIntellijSearchScope + NativeRelationRequest + native one-hop providers to
     * IntellijNativeRelationExecution.
     *
     * Establishes exact selector/scope agreement, request-local revalidation, cancellation checks inside every event,
     * projection after record/work/time admission, byte bounds, deterministic facts, and exact cardinality only after
     * terminal enumeration. Expected rejection and partial coverage states are [IntellijNativeRelationRejection] and
     * [NativeRelationLimitation]. Platform cancellation propagates.
     */
    fun read(
        compiledScope: CompiledIntellijSearchScope,
        request: NativeRelationRequest,
    ): IntellijNativeRelationExecution {
        if (compiledScope.lease != request.selector.lease || compiledScope.scope != request.selector.scope) {
            return rejected(IntellijNativeRelationRejection.INTERNAL_INVARIANT)
        }
        when (environmentState()) {
            IntellijDiscoveryEnvironmentState.DUMB -> return rejected(IntellijNativeRelationRejection.DUMB_MODE)
        ...
        val collector =
            BoundedIntellijRelationCollector(
                request = request,
                projector = projector,
        ...
        return when (searchResult) {
            is IntellijNativeRelationSearchResult.Rejected -> rejected(searchResult.reason.publicRejection())
...
private class BoundedIntellijRelationCollector(
    ...
    fun accept(event: IntellijNativeRelationEvent): Boolean {
        ...
        val fact =
            when (val projected = projector.project(request, event)) {
            ...
            when (
                val created =
                    NativeRelationBatch.create(
                        ...
                        timings =
                            NativeRelationTimings(
                                nativeQuery =
                                    (totalNanoseconds - projectionNanoseconds).coerceAtLeast(0L).elapsedMeasure(),
                                projection = projectionNanoseconds.elapsedMeasure(),
                            ),
                    )
            ) {
                is Refinement.Refined -> created.value
                is Refinement.Rejected -> return rejected(IntellijNativeRelationRejection.INTERNAL_INVARIANT)
            }
        val outcome =
...
private fun IntellijRelationProjectionFailure.limitation(): NativeRelationLimitation =
    when (this) {
        IntellijRelationProjectionFailure.UNRESOLVED_TARGET -> NativeRelationLimitation.UNRESOLVED_TARGET
        IntellijRelationProjectionFailure.UNSUPPORTED_ITEM -> NativeRelationLimitation.UNSUPPORTED_ITEM
    }

private fun IntellijNativeRelationSearchRejection.publicRejection(): IntellijNativeRelationRejection =
    when (this) {
        IntellijNativeRelationSearchRejection.STALE_SELECTOR -> IntellijNativeRelationRejection.STALE_SELECTOR
        IntellijNativeRelationSearchRejection.OUTSIDE_SCOPE -> IntellijNativeRelationRejection.OUTSIDE_SCOPE
        IntellijNativeRelationSearchRejection.AMBIGUOUS_SUBJECT -> IntellijNativeRelationRejection.AMBIGUOUS_SUBJECT
        IntellijNativeRelationSearchRejection.UNSUPPORTED_SUBJECT -> IntellijNativeRelationRejection.UNSUPPORTED_SUBJECT
        IntellijNativeRelationSearchRejection.SELECTOR_CHANGED -> IntellijNativeRelationRejection.SELECTOR_CHANGED
    }

private fun rejected(reason: IntellijNativeRelationRejection): IntellijNativeRelationExecution.Rejected =
    IntellijNativeRelationExecution.Rejected(reason)
