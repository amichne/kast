...
class GradleJvmSelectionEvidenceTest {
    ...
    fun `selection report carries wrapper range selected authority and rejected candidates`() {
        ...
        val selected =
            GradleJvmCandidateSelector.select(GradleVersion.version("7.6"), candidates)
                as GradleJvmCandidateSelection.Selected
    ...
    fun `failure report retains available candidates and corrective action`() {
        val report =
            gradleJvmRejectionReport(
                GradleVersion.version("7.6"),
                listOf(candidate(25)),
                GradleJvmSelectionFailure.NO_COMPATIBLE_RUNTIME,
            )
    ...
    fun `report candidate evidence is bounded and deterministic`() {
        ...
        val first =
            gradleJvmRejectionReport(
                GradleVersion.version("7.6"),
                candidates,
                GradleJvmSelectionFailure.SDK_REGISTRATION_FAILED,
            )
        val reversed =
            gradleJvmRejectionReport(
                GradleVersion.version("7.6"),
    ...
    private fun candidate(feature: Int, suffix: Int = feature) =
        GradleJvmCandidate(
            Path.of("/private/jdk/$suffix"),
            JavaFeature.of(feature),
            "$feature.0.1",
            GradleJvmSelectionSource.PLATFORM_RESOLVER,
        )
}
