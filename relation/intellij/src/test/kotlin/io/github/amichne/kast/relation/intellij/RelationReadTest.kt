...
class RelationReadTest {
    ...
    fun `diagnostics distinguish complete coverage from unsupported items and candidate caps`() {
        ...
        assertInstanceOf(
            ...
            complete.finish(IntellijRelationTermination.Terminal),
        )
        ...
        assertInstanceOf(
            ...
            incomplete.finish(IntellijRelationTermination.Terminal),
        )
        assertEquals(
            listOf(io.github.amichne.kast.workspace.intellij.read.IntellijReadTermination.RELATION_UNSUPPORTED_ITEM),
            reasons,
        )
        reasons.clear()
        ...
        assertEquals(
            listOf(io.github.amichne.kast.workspace.intellij.read.IntellijReadTermination.CANDIDATE_CAP),
            reasons,
        )
    }
    ...
    fun `relation package admission uses detached post-collection evidence`() {
        ...
        assertEquals(
            IntellijRelationPackageAdmission.ADMITTED,
        ...
        assertEquals(
            ...
            SymbolDiscoveryConstraints.None.packageName.admitPackage {
                error("unrestricted relation must not inspect package PSI")
            },
        )
    }
        ...
        RelationMeaning.all.forEach { meaning ->
            ...
            while (!terminal && pageCount < 8) {
                ...
                when (val page = compileStableProviderPage(current)) {
                    ...
                    is RelationCompilation.Qualified -> {
                        ...
                        val coverage =
                            assertInstanceOf(
                                io.github.amichne.kast.relation.contract.RelationIncompleteCoverage.Resumable::class
                                    .java,
                                page.coverage,
                            )
                        current =
            ...
            val expected =
                assertInstanceOf(
                    ...
                    .batch
                    .facts
                    .map(RelationFact::canonicalProjection)
    ...
    fun `provider exhausted incomplete evidence is terminal and has no continuation`() {
        ...
        val result =
            collector.finish(
                IntellijRelationTermination.TerminalIncomplete(setOf(RelationLimitation.UNRESOLVED_TARGET))
            )
    ...
    private fun providerFixtures(): List<ProviderFixture> =
        listOf(
    ...
    private fun evidenceForSelection(selection: SymbolDiscoverySelection): CompilerGroundedSymbolEvidence {
