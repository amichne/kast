...
internal sealed interface TopologyGenerationReuse {
    data class Rebound(val generation: CompleteTopologyGeneration) : TopologyGenerationReuse
...
internal sealed interface VerifiedTopologyGenerationReuse {
    data class Rebound(val generation: CompleteTopologyGeneration) : VerifiedTopologyGenerationReuse

    data class Rejected(val failure: VerifiedTopologyGenerationReuseFailure) : VerifiedTopologyGenerationReuse
}

/**
 * Rebinds a complete prior generation after one verified source write. Only the changed file's current compiler
 * extraction is admitted; every other file must retain byte-exact candidate evidence. Incoming edges are rebound
 * through compiler-grounded declaration meaning so source offsets may move without weakening exact current topology
 * identities.
 */
internal fun rebindVerifiedSingletonChange(
    ...
    val currentChanged =
        currentByPath[changed.file.path]
            ?: return rejected(VerifiedTopologyGenerationReuseFailure.CHANGED_FILE_MISMATCH)
    if (currentChanged != changed.file) {
        return rejected(VerifiedTopologyGenerationReuseFailure.CHANGED_FILE_MISMATCH)
    }
    val changedPaths =
        currentByPath.keys.filterTo(linkedSetOf()) { path ->
            currentByPath.getValue(path).canonicalProjection() != priorByPath.getValue(path).file.canonicalProjection()
        }
    ...
    for (priorSymbol in prior.symbols) {
        val current =
            if (priorSymbol.file.path == changed.file.path) {
                ...
                when (val admitted = TopologySymbol.admit(file, priorSymbol.evidence)) {
                    ...
                    is Refinement.Rejected -> return rejected(VerifiedTopologyGenerationReuseFailure.FACT_REJECTED)
                }
            }
    ...
    for (priorFile in prior.files) {
        ...
        for (edge in priorFile.edges) {
            val source =
                reboundSymbols[edge.source]
                    ?: return rejected(VerifiedTopologyGenerationReuseFailure.PRIOR_SYMBOL_MISSING)
            val target =
                reboundSymbols[edge.target]
                    ?: return rejected(VerifiedTopologyGenerationReuseFailure.PRIOR_SYMBOL_MISSING)
            when (
                val rebound =
    ...
    return when (
        ...
        is Refinement.Rejected -> rejected(VerifiedTopologyGenerationReuseFailure.COVERAGE_REJECTED)
    }
}
...
private fun TopologySymbol.meaning(): TopologySymbolMeaning =
    TopologySymbolMeaning(
...
private fun rejected(failure: VerifiedTopologyGenerationReuseFailure): VerifiedTopologyGenerationReuse =
    VerifiedTopologyGenerationReuse.Rejected(failure)

/**
 * Proof transition: `(PublishedWorkspace, TopologyCandidateSet, TopologySnapshotContent) -> TopologyGenerationReuse`.
 *
 * Rebound establishes that every current candidate has byte- and source-root-identical terminal facts in the prior
 * admitted snapshot, with all symbols and edges rebound to the current lease. SourceChanged closes candidate-set or
 * content staleness. The source-state token may differ: an exact canonical candidate projection is the stronger proof
 * that permits rebinding a snapshot after a conservative cold-start generation advance.
 * [TopologyGenerationReuse.Rejected] closes a malformed proof transition without manufacturing detail that the public
 * snapshot contract cannot consume. Raw compiler facts are never accepted by this transition.
 */
internal fun rebindUnchangedTopologyGeneration(
    ...
    for (symbol in prior.symbols) {
        ...
        val rebound =
            when (val admitted = TopologySymbol.admit(currentFile, symbol.evidence)) {
                is Refinement.Refined -> admitted.value
                is Refinement.Rejected -> return TopologyGenerationReuse.Rejected
            }
        symbols[symbol] = rebound
    }
    val edges = mutableListOf<TopologyEdge>()
    for (edge in prior.edges) {
        val source = symbols[edge.source] ?: return TopologyGenerationReuse.Rejected
        val target = symbols[edge.target] ?: return TopologyGenerationReuse.Rejected
        when (
            val rebound =
                TopologyEdge.fromBoundary(
                    edge.kind,
    ...
    return when (
        ...
        is Refinement.Rejected -> TopologyGenerationReuse.Rejected
    }
}
