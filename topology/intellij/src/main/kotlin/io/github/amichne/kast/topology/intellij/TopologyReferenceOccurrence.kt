...
internal sealed interface EnclosingSuperTypeCallRange {
    data class Observed internal constructor(val range: TextRange) : EnclosingSuperTypeCallRange
...
internal sealed interface TopologyReferenceOccurrence {
    data class Admitted internal constructor(val range: ExactDeclarationTextRange) : TopologyReferenceOccurrence

    data object Rejected : TopologyReferenceOccurrence

    companion object {
        /**
         * Proof transition: `(TextRange, EnclosingSuperTypeCallRange) -> TopologyReferenceOccurrence`.
         *
         * Admitted establishes a non-empty source range. A direct reference range has authority; a synthetic zero-width
         * reference may refine to its enclosing super-type call range. Rejected is the closed expected failure when
         * neither range carries source evidence. Raw PSI ranges may enter only from the request-local IntelliJ
         * extraction boundary.
         */
        fun refine(
            direct: TextRange,
