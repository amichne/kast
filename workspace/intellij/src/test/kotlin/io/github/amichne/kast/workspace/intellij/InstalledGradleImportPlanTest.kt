...
class InstalledGradleImportPlanTest {
    ...
    fun `linked exact workspace refreshes explicitly after project open`() {
        val linkedSettings =
            GradleProjectSettings("/workspace").apply {
                gradleJvm = "prior-jvm"
            }
    ...
    fun `unlinked exact workspace is linked`() {
        val application =
            assertInstanceOf(
                InstalledGradleImportApplication.Applied::class.java,
                InstalledGradleLinkPresence.Unlinked(GradleProjectSettings("/workspace"))
    ...
    fun `invalid Gradle JVM callback retains its exact terminal cause`() {
        ...
        callback.completeExceptionally(ExternalSystemJdkException("invalid fixture JDK", null))
