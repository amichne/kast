...
internal object RepoIngestion {
    ...
    /** Validates the URL and clones into a temp dir. Returns workspace root. */
    fun clone(repoUrl: String, processRunner: GitRunner = SystemGitRunner): Path {
        ...
        if (!GITHUB_URL_REGEX.matches(repoUrl)) {
            throw CliFailure(
                code = "DEMO_GEN_INVALID_URL",
                message = "Only GitHub HTTPS or SSH URLs are supported: $repoUrl",
                details = mapOf("url" to repoUrl),
            )
        }
        val tempDir = Files.createTempDirectory("kast-demo-gen-")
    ...
    suspend fun bootstrap(workspaceRoot: Path, cliService: CliService): WorkspaceEnsureResult {
