...
object InstalledConfigurationSchema {
            ...
            ConfigurationSchemaDocument(
                ...
                operationalLimits = buildList {
                        ...
                        for (budget in OperationExecutionBudget.entries) {
                            add(
                                limit(
                                    "operation.${budget.name.lowercase()}.dispatch",
                                    ":protocol:registry",
                                    budget.operation.value,
                                    ConfigurationUnit.MILLISECONDS,
                                    "OperationExecutionBudget.${budget.name}.operation",
                                )
                            )
                            add(
                                limit(
                                    "operation.${budget.name.lowercase()}.invocation",
                                    ":protocol:registry",
                                    budget.invocation.value,
                                    ConfigurationUnit.MILLISECONDS,
                                    "OperationExecutionBudget.${budget.name}.invocation",
                                )
                            )
                        }
                        add(
                            limit(
                                "operation.local_qualification",
                                ":protocol:registry",
                                OperationExecutionBudget.LOCAL_QUALIFICATION.value,
                                ConfigurationUnit.MILLISECONDS,
                                "OperationExecutionBudget.LOCAL_QUALIFICATION",
                            )
                        )
                        add(
                            limit(
                                "operation.workspace_readiness",
                                ":protocol:registry",
                                OperationExecutionBudget.WORKSPACE_READINESS.value,
                                ConfigurationUnit.MILLISECONDS,
                                "OperationExecutionBudget.WORKSPACE_READINESS",
                            )
                        )
                        add(
                            limit(
                                ...
                                ":protocol:registry",
                                OperationExecutionBudget.PROVIDER_QUALIFICATION.value,
                                ConfigurationUnit.MILLISECONDS,
                                "OperationExecutionBudget.PROVIDER_QUALIFICATION",
                            )
                        )
                        add(
                            limit(
                                "transport.qualification.maximum_bytes",
                        ...
                        add(
                            limit(
                                "coordinator.status.maximum_bytes",
                                ":app-server",
                                CoordinatorStatusProtocol.maximumMessageBytes.toLong(),
                                ConfigurationUnit.BYTES,
                                "CoordinatorStatusProtocol.maximumMessageBytes; generated maximum scalar widths and admitted worker count",
                            )
                        )
                        add(
                            limit(
                                "coordinator.command.maximum_bytes",
                                ":app-server",
                                CoordinatorStatusProtocol.maximumCommandBytes.toLong(),
                                ConfigurationUnit.BYTES,
                                "CoordinatorStatusProtocol.maximumCommandBytes",
                            )
                        )
                    }
                        .sortedBy { it.key },
                sourceCoverage =
                    "declared-process-saved-workspace-saved-installation-derived-jvm-and-operational-owner-contracts",
            )
    ...
    private fun limit(
        ...
        authority: String,
        scope: ConfigurationScope = ConfigurationScope.REQUEST,
