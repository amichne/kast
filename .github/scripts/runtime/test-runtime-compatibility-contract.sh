...
required_paths=(
  ...
  ".github/scripts/runtime/render-runtime-compatibility.py"
  "cli-rs/tests/runtime/installed_idea_sidecar_smoke/main.rs"
)
...
renderer=".github/scripts/runtime/render-runtime-compatibility.py"
source_file="packaging/indexer/runtime-compatibility.json"
release_tag="v0.13.0"
...
[[ "$("$renderer" runtime-build-range --source "$source_file" --field sinceBuild)" == "261" ]]
[[ -z "$("$renderer" runtime-build-range --source "$source_file" --field untilBuild)" ]]
"$renderer" render \
  --source "$source_file" \
...
write("bad-build-range", lambda value: value["runtimeBuildRange"].__setitem__("untilBuild", "252"))
...
if manifest.get("schemaVersion") != 2:
    raise SystemExit("rendered compatibility manifest schemaVersion drifted")
...
if any(pair.get("indexerVersion") != "0.13.0" for pair in pairs):
...
write("unknown-capability", lambda value: value["supportedPairs"][0]["requiredCapabilities"][0].__setitem__("name", "UNKNOWN"))
write("missing-capability", lambda value: value["supportedPairs"][0]["optionalCapabilities"].pop())
write("invalid-build-range", lambda value: value["runtimeBuildRange"].__setitem__("untilBuild", "252"))
