...
class AnalysisBackendFindReferencesTest {
    @TempDir
    ...
    fun `find references returns cross-file usages and declaration metadata`(): TestResult = runTest {
        ...
        session.use { session ->
            ...
            assertEquals(normalizedString(declarationFile), result.declaration?.location?.filePath)
            assertEquals(
                listOf(normalizedString(secondUsageFile), normalizedString(firstUsageFile)),
    ...
    fun `find references keeps cross-module public callers when dependent map is incomplete`(): TestResult = runTest {
        ...
        val phasedDiscoveryResult = PhasedDiscoveryResult(
            initialLayout = WorkspaceLayout(
                sourceModules = listOf(
                    SourceModuleSpec(
                        name = libModuleName,
                        sourceRoots = listOf(normalizePath(workspaceRoot.resolve("lib/src/main/kotlin"))),
                        binaryRoots = emptyList(),
                        dependencyModuleNames = emptyList(),
                    ),
                    SourceModuleSpec(
                        name = appModuleName,
                        sourceRoots = listOf(normalizePath(workspaceRoot.resolve("app/src/main/kotlin"))),
                        binaryRoots = emptyList(),
        ...
        session.use { session ->
            ...
            assertEquals(listOf(normalizedString(callerFile)), result.references.map { reference -> reference.filePath }.distinct())
    ...
    fun `find references for private function returns only same-file references`(): TestResult = runTest {
        ...
        session.use { session ->
            ...
            assertEquals(listOf(normalizedString(declarationFile)), referenceFiles)
            assertFalse(session.isFullKtFileMapLoaded())
        }
    }
