...
internal enum class BrokerFrontendObservation { PENDING, PREPARED, REJECTED, CLOSED }
...
internal class DeferredBrokerFrontend(private val prepare: suspend () -> BrokerFrontendAdmission) : BrokerFrontend {
    ...
    internal fun observe(): BrokerFrontendObservation = when (state) {
    ...
    override suspend fun connect(session: DefaultWebSocketServerSession) {
        val selected = transition.withLock {
            if (state == State.Pending) state = when (val admission = prepare()) {
        ...
        when (selected) {
            ...
            State.Pending, State.Rejected, State.Closed -> session.close(
                CloseReason(CloseReason.Codes.TRY_AGAIN_LATER, "Codex host unavailable"),
            )
        }
    }
