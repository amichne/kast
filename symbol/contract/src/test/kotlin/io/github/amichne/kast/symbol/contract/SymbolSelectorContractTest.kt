...
class SymbolSelectorContractTest {
    ...
    fun `canonical signature collections cannot mutate after identity derivation`() {
        ...
        assertThrows(UnsupportedOperationException::class.java) {
            @Suppress("UNCHECKED_CAST") (signature.contextReceivers as MutableList<CanonicalCompilerType>).clear()
        }
        assertThrows(UnsupportedOperationException::class.java) {
            @Suppress("UNCHECKED_CAST") (signature.valueParameters as MutableList<CanonicalCompilerType>).clear()
        }
    ...
    fun `property identities retain extension and context receivers`() {
        ...
        assertEquals(
            stringReceiver,
            CanonicalCompilerSignature.restoreCanonicalEncoding(stringReceiver.canonicalEncoding().value).refined(),
        )
        assertThrows(UnsupportedOperationException::class.java) {
            @Suppress("UNCHECKED_CAST") (stringReceiver.contextReceivers as MutableList<CanonicalCompilerType>).clear()
        }
    }
