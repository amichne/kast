...
internal constructor(
    ...
    companion object {
         ...
         *
         * Establishes complete platform, architecture, compatibility, content digest, safe layout, and derived runtime
         * identity invariants. [SemanticRuntimeFailure.MANIFEST_INVALID] is the closed expected failure. Raw JSON is
         * permitted only at this embedded-resource boundary.
         */
        fun admit(rawJson: String): SemanticRuntimeManifestAdmission {
            val document =
                try {
                    ...
                    return SemanticRuntimeManifestAdmission.Rejected(SemanticRuntimeFailure.MANIFEST_INVALID)
                }
...
sealed interface SemanticRuntimeManifestAdmission {
    ...
    data class Rejected(val failure: SemanticRuntimeFailure) : SemanticRuntimeManifestAdmission
}
...
sealed interface SemanticRuntimeSource {
    ...
    companion object {
         ...
         * [SemanticRuntimeFailure.SOURCE_INVALID] is the closed expected failure. Nullable raw text is permitted only
         * for extraction of the optional `KAST_RUNTIME_ARCHIVE` environment value at this boundary.
         */
        fun select(rawArchive: String?): SemanticRuntimeSourceSelection {
            ...
            if (rawArchive.isBlank()) {
                return SemanticRuntimeSourceSelection.Rejected(SemanticRuntimeFailure.SOURCE_INVALID)
            }
            val path =
                try {
                    Path.of(rawArchive)
            ...
            return if (path.isAbsolute) {
                SemanticRuntimeSourceSelection.Preseeded(PreseededArchive(path.normalize()))
            } else {
...
sealed interface SemanticRuntimeSourceSelection {
    ...
    data class Preseeded(val source: SemanticRuntimeSource.PreseededArchive) : SemanticRuntimeSourceSelection
...
private fun ManifestDocument.admit(json: Json): SemanticRuntimeManifestAdmission {
    if (
        schemaVersion != MANIFEST_SCHEMA_VERSION ||
            ...
            layout.requiredEntries.isEmpty() ||
            layout.executableEntries.isEmpty()
    )
        return invalidManifest()

    val runtime =
        when (val refined = SemanticRuntimeId.parse(runtimeId)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val product =
        when (val refined = RuntimeProductVersion.parse(productVersion)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val idea =
        when (val refined = IntellijBuildIdentity.parse(ideaBuild)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val kotlinPlugin =
        when (val refined = KotlinPluginBuildIdentity.parse(kotlinPluginBuild)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val pluginDigest =
        when (val refined = RuntimeDigest.parse(kastPluginSha256)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val schema =
        when (val refined = RuntimeWireSchemaIdentity.parse(wireSchemaId)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val archiveDigest =
        when (val refined = RuntimeDigest.parse(archive.sha256)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val archiveSize =
        when (val refined = RuntimeArchiveSize.parse(archive.bytes)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val archiveName =
        when (val refined = RuntimeLayoutEntry.parse(archive.fileName)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    if (archiveName.value.contains('/')) return invalidManifest()
    val executable =
        when (val refined = RuntimeLayoutEntry.parse(layout.executable)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return invalidManifest()
        }
    val requiredEntries =
        when (val admitted = layout.requiredEntries.admitLayoutEntries()) {
    ...
    val executableEntries =
        when (val admitted = layout.executableEntries.admitLayoutEntries()) {
            is LayoutEntriesAdmission.Admitted -> admitted.entries
            LayoutEntriesAdmission.Rejected -> return invalidManifest()
        }
 ...
 *
 * Establishes distinct, safe relative runtime-layout entries or the closed rejected state. Raw entry text is permitted
 * only at manifest admission.
 */
...
private fun invalidManifest() = SemanticRuntimeManifestAdmission.Rejected(SemanticRuntimeFailure.MANIFEST_INVALID)
