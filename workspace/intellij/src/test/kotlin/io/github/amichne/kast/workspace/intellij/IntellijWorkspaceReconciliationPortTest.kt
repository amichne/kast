    ...
    fun `adapter physically canonicalizes root and retains complete Gradle evidence`() {
        ...
        val adapter =
            IntellijWorkspaceReconciliationPort(
                ...
                gradleModel =
                    GradleWorkspaceModelPort { _, signals ->
                        observedSignals += signals
                        GradleWorkspaceModelCapture.Captured(WorkspaceStateIdentity("gradle-model"))
                    },
                reconcile =
                    IntellijWorkspaceReconciliation { _ ->
                        IntellijWorkspaceReconciliationResult.Reconciled(WorkspaceEvidenceKind.entries.toSet())
                    },
            )

        val captured = adapter.capture(setOf(WorkspaceSignal.BuildSemantic)) as WorkspaceCandidateCapture.Captured
        val reconciled = adapter.reconcile(captured.candidate) as WorkspaceCandidateReconciliation.Reconciled
