...
val topologyBuildCliProjector =
    ...
    > { outcome ->
        projectClosedOutcome(
            outcome,
            complete = { result, live -> topologyCompleteFactory.create(result.completeDocument(live)) },
...
private fun TopologyBuildResult.completeDocument(live: LiveReadCliEvidence?): TopologyBuildCompleteCliDocument =
...
private fun TopologyBuildRejection.rejectedDocument(): CanonicalJsonDocument =
    when (this) {
        ...
        is TopologyBuildRejection.ExtractionFailed ->
            extractionRejectedFactory.create(
        ...
        TopologyBuildRejection.ExtractionContractViolation -> rejectedDocument("extraction-contract-violation")
...
private fun rejectedDocument(reason: String): CanonicalJsonDocument =
    topologyRejectedFactory.create(
...
private val topologyCompleteFactory = CanonicalJsonDocument.generated(TopologyBuildCompleteCliDocument.serializer())
private val topologyQualifiedFactory = CanonicalJsonDocument.generated(TopologyBuildQualifiedCliDocument.serializer())
private val topologyRejectedFactory = CanonicalJsonDocument.generated(TopologyBuildRejectedCliDocument.serializer())
private val failureRejectedFactory =
    CanonicalJsonDocument.generated(TopologyBuildFailureRejectedCliDocument.serializer())
private val extractionRejectedFactory =
    CanonicalJsonDocument.generated(TopologyBuildExtractionRejectedCliDocument.serializer())

private fun TopologyBuildStatus.wireName(): String =
    when (this) {
        TopologyBuildStatus.PUBLISHED -> "published"
        TopologyBuildStatus.REUSED -> "reused"
    }

private fun TopologyBuildQualification.wireName(): String =
    when (this) {
        TopologyBuildQualification.PROGRESS_UNAVAILABLE -> "progress-unavailable"
    }

private fun TopologySnapshotRejection.wireName(): String =
    when (this) {
...
private fun TopologyEnumerationRejection.wireName(): String =
    when (this) {
...
private fun TopologyExtractionRejection.wireName(): String =
    when (this) {
        ...
        TopologyExtractionRejection.SOURCE_CONTENT_CHANGED_DURING_BUILD -> "source-content-changed-during-build"
        ...
        TopologyExtractionRejection.DECLARATION_EVIDENCE_REJECTED -> "declaration-evidence-rejected"
        TopologyExtractionRejection.PROJECTION_REGISTRY_REJECTED -> "projection-registry-rejected"
        TopologyExtractionRejection.COMPILER_IDENTITY_MISMATCH -> "compiler-identity-mismatch"
        TopologyExtractionRejection.REFERENCE_TARGET_REJECTED -> "reference-target-rejected"
...
private fun TopologyPublicationRejection.wireName(): String =
    when (this) {
...
private fun TopologyCoverageProjectionRejection.wireName(): String =
    when (this) {
        ...
        TopologyCoverageProjectionRejection.UNREPRESENTABLE_CONTENT_HASH -> "unrepresentable-content-hash"
        TopologyCoverageProjectionRejection.UNREPRESENTABLE_COMPILER_EVIDENCE -> "unrepresentable-compiler-evidence"
