...
internal fun lifecycleCommands(): List<LifecycleKastCommand> =
    listOf(
        StartCommand,
        StopCommand,
        StatusCommand,
...
private data object StartCommand : LifecycleKastCommand("start", CliLifecycleCommand.START) {
    ...
    private val sourceIdeaSystem by
        absolutePathOption(
                "--source-idea-system",
                "Source IntelliJ system directory used only for explicit seeding.",
            )
            .optionalOnce()
    private val acceptGlobalIndexCopy by
        option(
    ...
    override fun help(context: Context): String = "Start the isolated exact-root IntelliJ sidecar."
    ...
    override fun resolveAction(): CliActionResolution {
        val startup =
            when (
                ...
                is StartInputRefinement.Rejected -> return CliActionResolution.UsageRejected(refinement.failure)
            }
        return CliActionResolution.Selected(CliAction.Lifecycle.Start(startup))
    }
}

private enum class StartCacheMode {
    REUSE,
    SEED,
    REBUILD,
}
...
private fun refineStartup(
    ...
    if (cacheMode != StartCacheMode.SEED && (sourceIdeaSystem is CliOptionValue.Present || acceptGlobalIndexCopy)) {
        return StartInputRefinement.Rejected(CliUsageFailure.Start.OPTIONS_REQUIRE_SEED)
    }
    if (
        ideaHome is CliOptionValue.Absent &&
            cacheMode == StartCacheMode.REUSE &&
            sourceIdeaSystem is CliOptionValue.Absent &&
            !acceptGlobalIndexCopy
    ) {
...
private data object StopCommand : LifecycleKastCommand("stop", CliLifecycleCommand.STOP) {
    override fun help(context: Context): String = "Stop only the process proven to own this exact workspace endpoint."
...
private data object StatusCommand : LifecycleKastCommand("status", CliLifecycleCommand.STATUS) {
    override fun help(context: Context): String = "Report exact-root runtime and private cache identity and state."
