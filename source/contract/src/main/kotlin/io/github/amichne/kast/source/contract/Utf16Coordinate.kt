...
@JvmInline
value class Utf16CodeUnitOffset private constructor(val value: Int) : Comparable<Utf16CodeUnitOffset> {
    companion object {
        /** Proof transition: `Int -> Refinement<Utf16CodeUnitOffset, Utf16CodeUnitOffsetFailure>`. */
        fun parse(raw: Int): Refinement<Utf16CodeUnitOffset, Utf16CodeUnitOffsetFailure> =
            if (raw < 0) {
...
@JvmInline
value class Utf16CodeUnitCount private constructor(val value: Int) {
    companion object {
        /** Proof transition: `Int -> Refinement<Utf16CodeUnitCount, Utf16CodeUnitCountFailure>`. */
        fun parse(raw: Int): Refinement<Utf16CodeUnitCount, Utf16CodeUnitCountFailure> =
            if (raw < 0) {
