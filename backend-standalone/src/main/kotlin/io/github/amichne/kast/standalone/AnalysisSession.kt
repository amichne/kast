...
internal class AnalysisSession(
    ...
    internal val telemetry: Telemetry = Telemetry.disabled(),
    ...
    @Volatile
    private var sourceModuleSpecs: List<SourceModuleSpec> = emptyList()
    ...
    init {
        val workspaceLayout = phasedDiscoveryResult?.initialLayout ?: discoverWorkspaceLayout(
            workspaceRoot = normalizedWorkspaceRoot,
    ...
    fun moduleSpecs(): List<SourceModuleSpec> = sourceModuleSpecs
    ...
    fun refreshFiles(paths: Set<String>): RefreshResult {
        ...
        telemetry.inSpan(
            scope = TelemetryScope.SESSION_LIFECYCLE,
    ...
    private fun applyWorkspaceLayout(workspaceLayout: WorkspaceLayout) {
    ...
    private fun beginEnrichment(enrichmentFuture: CompletableFuture<WorkspaceLayout>?) {
    ...
    private fun rebuildWorkspaceLayout(workspaceLayout: WorkspaceLayout) {
        telemetry.inSpan(
            scope = TelemetryScope.SESSION_LIFECYCLE,
            ...
            telemetry.inSpan(
                scope = TelemetryScope.SESSION_LOCK,
     ...
     */
    private fun buildAnalysisStateForSpecs(specs: List<SourceModuleSpec>): AnalysisState {
