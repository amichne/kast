...
internal sealed interface IntellijExactDeclarationLookupResult {
    data class Found(val evidence: ExactDeclarationEvidence) : IntellijExactDeclarationLookupResult

    data class Rejected(val reason: IntellijExactDeclarationLookupRejection) : IntellijExactDeclarationLookupResult
}
...
internal sealed interface IntellijLiveExactDeclarationLookupResult {
    ...
    data class Rejected(val reason: IntellijExactDeclarationLookupRejection) : IntellijLiveExactDeclarationLookupResult
}

internal fun interface IntellijExactDeclarationLookup {
    /**
     * Proof transition: CompiledIntellijSearchScope + IntellijExactDeclarationLookupKey to
     * IntellijExactDeclarationLookupResult.
     *
     * Establishes one scope-contained live declaration with detached exact native evidence, or a closed stale,
     * out-of-scope, ambiguous, or unsupported state. Live files and PSI must remain inside this request-local call.
     */
    fun find(
...
internal class IntellijPsiExactDeclarationLookup(private val project: Project) : IntellijExactDeclarationLookup {
    /**
     * Proof transition: CompiledIntellijSearchScope + IntellijExactDeclarationLookupKey to
     * IntellijExactDeclarationLookupResult.
     *
     * Establishes exactly one valid, scope-contained [PsiNamedElement] at the retained source offset whose name also
     * matches, and detaches its complete evidence. Stale files/elements, scope exclusion, multiple matching declaration
     * ancestors, and unsupported PSI are closed [IntellijExactDeclarationLookupRejection] values. Live IntelliJ objects
     * remain local.
     */
    override fun find(
        compiledScope: CompiledIntellijSearchScope,
        ...
        when (val live = findLive(compiledScope, key)) {
            ...
            is IntellijLiveExactDeclarationLookupResult.Rejected -> rejected(live.reason)
        }
     ...
     * IntellijLiveExactDeclarationLookupResult.
     *
     * Establishes exactly one request-local live declaration plus its detached evidence, or the same closed lookup
     * rejection as [find]. The live declaration must be consumed within the current IntelliJ read and may never cross
     * an adapter boundary.
     */
    internal fun findLive(
        ...
        val file =
            when (val resolution = key.file.resolveVirtualFile()) {
...
private sealed interface IntellijExactVirtualFileResolution {
    data class Found(val file: VirtualFile) : IntellijExactVirtualFileResolution

    data object Stale : IntellijExactVirtualFileResolution
}

/**
 * Proof transition: SymbolDiscoveryFileIdentity to IntellijExactVirtualFileResolution.
 *
 * Establishes a current valid IntelliJ virtual file for the detached workspace path or external URL, or the closed
 * stale state. Raw path/URL extraction and the live file remain inside this request-local IntelliJ lookup boundary.
 */
private fun SymbolDiscoveryFileIdentity.resolveVirtualFile(): IntellijExactVirtualFileResolution {
    val manager = VirtualFileManager.getInstance()
    val file =
        when (this) {
            is SymbolDiscoveryFileIdentity.Workspace -> manager.findFileByNioPath(Path.of(path.value))
            is SymbolDiscoveryFileIdentity.External -> manager.findFileByUrl(url.value)
        }
...
/**
 * Proof transition: SymbolDiscoverySelection to IntellijExactDeclarationLookupKey.
 *
 * Establishes a lookup key containing only the declaration file, refined source offset, and name retained by the
 * batch-owned selection. Raw values may be extracted only by [IntellijExactDeclarationLookup.find].
 */
...
/**
 * Proof transition: ExactDeclarationSelector to IntellijExactDeclarationLookupKey.
 *
 * Establishes a lookup key from the selector's exact file, invariant-preserving range start, and name. The range
 * invariant makes offset refinement total; raw values may be extracted only by [IntellijExactDeclarationLookup.find].
 */
internal fun ExactDeclarationSelector.lookupKey(): IntellijExactDeclarationLookupKey {
    val offset =
        when (val parsed = SymbolDiscoverySourceOffset.parse(range.startInclusive)) {
 ...
 *
 * Establishes a request-local lookup key from the exact selector's retained file, invariant-safe range start, and name.
 * Raw offset extraction is permitted only at this native lookup boundary.
 */
...
private fun rejected(reason: IntellijExactDeclarationLookupRejection): IntellijExactDeclarationLookupResult.Rejected =
...
): IntellijLiveExactDeclarationLookupResult.Rejected = IntellijLiveExactDeclarationLookupResult.Rejected(reason)

internal fun IntellijExactDeclarationLookupRejection.toPublicRejection(): IntellijExactSelectorRejection =
    when (this) {
        IntellijExactDeclarationLookupRejection.STALE_LOCATION -> IntellijExactSelectorRejection.STALE_LOCATION
        IntellijExactDeclarationLookupRejection.OUTSIDE_SCOPE -> IntellijExactSelectorRejection.OUTSIDE_SCOPE
