...
class CanonicalDiagnosticCheckProtocol(
    ...
    > {
        ...
        val scope =
            when (val resolved = scopes.resolve(query)) {
        ...
        return when (val result = operations.check(DomainDiagnosticRequest(scope))) {
            ...
            is DomainDiagnosticResult.Complete ->
                project(
                    result.batch.facts,
                    scope,
            ...
            is DomainDiagnosticResult.Qualified ->
                project(
                    result.batch.facts,
                    scope,
                    minOf(request.limit.value, maximumResults.value),
    ...
    > {
        ...
        val knownCount =
            DiagnosticKnownCountDocument.parse(facts.size).refinedOrNull()
                ?: return OperationOutcome.Rejected(DiagnosticCheckRejection.SCOPE_REJECTED)
        val analyzedFiles =
            projection.analyzedFiles.map { file ->
        ...
        val limitations =
            when (projection) {
                ...
                is DiagnosticProjection.Incomplete ->
                    projection.coverage.limitations
...
private sealed interface DiagnosticProjection {
    ...
    data class Incomplete(val coverage: DiagnosticIncompleteCoverage) : DiagnosticProjection {
...
private fun DiagnosticReadRejection.protocol(): DiagnosticCheckRejection =
    when (this) {
        ...
        DiagnosticReadRejection.STALE_GENERATION -> DiagnosticCheckRejection.WORKSPACE_NOT_READY
        DiagnosticReadRejection.WORKSPACE_INDEX_UNAVAILABLE,
        DiagnosticReadRejection.SCOPE_REJECTED,
        DiagnosticReadRejection.COMPILER_CONTRACT_VIOLATION -> DiagnosticCheckRejection.SCOPE_REJECTED
    }
