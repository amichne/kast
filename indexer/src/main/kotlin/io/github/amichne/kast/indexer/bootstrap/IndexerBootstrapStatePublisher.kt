...
internal sealed interface IndexerBootstrapStatePublisherAdmission {
    data class Admitted(val publisher: AdmittedIndexerBootstrapStatePublisher) : IndexerBootstrapStatePublisherAdmission

    data class Rejected(val failure: IndexerBootstrapStatePublisherFailure) : IndexerBootstrapStatePublisherAdmission
}
...
private enum class IndexerBootstrapPublicationPhase {
    ADMITTED,
    ACTIVE,
    TERMINAL,
}
...
internal class AdmittedIndexerBootstrapStatePublisher
private constructor(
    ...
    fun publishProgress(next: InstalledRuntimeBootstrapPhase): IndexerBootstrapStatePublication {
        ...
        val outcome =
            when {
                ...
                next.ordinal !in bootstrapPhase.ordinal..bootstrapPhase.ordinal + 1 ->
                    IndexerBootstrapProgressOutcome.OUT_OF_ORDER
                next == bootstrapPhase -> IndexerBootstrapProgressOutcome.UNCHANGED
                else ->
                    when (publish(attempt.startingDocument(next))) {
        ...
        return when (outcome) {
            IndexerBootstrapProgressOutcome.ADVANCED,
            IndexerBootstrapProgressOutcome.UNCHANGED -> IndexerBootstrapStatePublication.PUBLISHED
            IndexerBootstrapProgressOutcome.NOT_ACTIVE,
            IndexerBootstrapProgressOutcome.OUT_OF_ORDER,
    ...
    fun observeGradleJvm(report: InstalledGradleJvmSelectionReport): IndexerBootstrapStatePublication {
        if (
            phase != IndexerBootstrapPublicationPhase.ACTIVE ||
                bootstrapPhase != InstalledRuntimeBootstrapPhase.GRADLE_JVM_SELECTION
        )
            return IndexerBootstrapStatePublication.REJECTED
        val refined =
            when (val refinement = attempt.withGradleJvm(report)) {
    ...
    ): IndexerBootstrapStatePublication = publishTerminal(attempt.terminalFailureDocument(bootstrapPhase, failure))
    ...
    fun publishRejection(failures: Set<InstalledKastRuntimeFailure>): IndexerBootstrapRejectionPublication {
        ...
        return when (val projection = attempt.rejectionDocument(failures, bootstrapPhase)) {
            is InstalledSemanticRuntimeBootstrapRejection.Projected ->
                when (publish(projection.document)) {
            ...
            InstalledSemanticRuntimeBootstrapRejection.Ambiguous ->
                when (
                    publish(
                        attempt.terminalFailureDocument(
                            bootstrapPhase,
    ...
    private fun publishTerminal(document: InstalledSemanticRuntimeBootstrapDocument): IndexerBootstrapStatePublication {
        if (phase != IndexerBootstrapPublicationPhase.ACTIVE) {
            return IndexerBootstrapStatePublication.REJECTED
        }
        phase = IndexerBootstrapPublicationPhase.TERMINAL
    ...
    private fun publish(document: InstalledSemanticRuntimeBootstrapDocument): IndexerBootstrapStatePublication {
        val staging =
            try {
    ...
    companion object {
        ...
        internal fun admit(
            documentSink: IndexerBootstrapDocumentSink,
            progressSink: (IndexerBootstrapProgressEvidence) -> Unit,
            ...
            val rawAttempt =
                System.getProperty(ATTEMPT_PROPERTY)
                    ?: return rejected(IndexerBootstrapStatePublisherFailure.ATTEMPT_UNAVAILABLE)
            val attempt =
                when (val admission = InstalledSemanticRuntimeBootstrapAttempt.admit(rawAttempt)) {
                    is InstalledSemanticRuntimeBootstrapAttemptAdmission.Admitted -> admission.attempt
                    is InstalledSemanticRuntimeBootstrapAttemptAdmission.Rejected ->
                        return rejected(IndexerBootstrapStatePublisherFailure.ATTEMPT_REJECTED)
                }
        ...
        ): IndexerBootstrapStatePublisherAdmission.Rejected = IndexerBootstrapStatePublisherAdmission.Rejected(failure)
    }
}
...
internal object IndexerBootstrapStatePublisher {
    fun admit(
        ...
        progressSink: (IndexerBootstrapProgressEvidence) -> Unit = { event ->
            System.err.println(
                "kast-indexer: bootstrap-transition: current=${event.current} requested=${event.requested} outcome=${event.outcome}"
            )
        },
    ): IndexerBootstrapStatePublisherAdmission =
...
internal enum class IndexerBootstrapProgressOutcome {
    ADVANCED,
    UNCHANGED,
    NOT_ACTIVE,
    OUT_OF_ORDER,
