...
data class IdeHostCapability private constructor(val operation: CanonicalOperation) {
    ...
    companion object {
        /**
         * Proof transition: `OperationId -> Refinement<IdeHostCapability, IdeHostCompatibilityFailure>`.
         *
         * Establishes membership in the canonical operation set. Hosted exposure is deliberately not decided here: the
         * canonical operation definitions are the sole authority and generated compatibility metadata supplies its
         * projection at the outer boundary.
         */
        internal fun resolve(operationId: OperationId): Refinement<IdeHostCapability, IdeHostCompatibilityFailure> =
            when (val resolution = CanonicalOperation.resolve(operationId)) {
                is CanonicalOperationResolution.Unknown ->
                    Refinement.Rejected(IdeHostCompatibilityFailure.UnknownCapability(resolution.id))
                is CanonicalOperationResolution.Known -> Refinement.Refined(byOperation.getValue(resolution.operation))
            }
...
class IdeHostCapabilitySet private constructor(val capabilities: List<IdeHostCapability>) {
    override fun equals(other: Any?): Boolean = other is IdeHostCapabilitySet && capabilities == other.capabilities
    ...
    companion object {
         ...
         *
         * Establishes an ordered, unique set of canonical operation identities. The exact public set is supplied by
         * generated canonical metadata and retained by [IdeHostCompatibilityPolicy], rather than duplicated in this
         * contract.
         */
        fun parse(raw: List<String>): Refinement<IdeHostCapabilitySet, IdeHostCompatibilityFailure> {
            ...
            raw.forEach { identity ->
                val operationId =
                    when (val parsed = OperationId.parse(identity)) {
                ...
                val capability =
                    when (val resolved = IdeHostCapability.resolve(operationId)) {
                        is Refinement.Refined -> resolved.value
                        is Refinement.Rejected -> return resolved
                    }
                if (!observed.add(capability)) {
                    return Refinement.Rejected(IdeHostCompatibilityFailure.DuplicateCapability(capability))
                }
                admitted += capability
            }
...
private fun PermanentIdentityFailure.toCompatibilitySyntaxFailure(): IdeHostCompatibilitySyntaxFailure =
    when (this) {
