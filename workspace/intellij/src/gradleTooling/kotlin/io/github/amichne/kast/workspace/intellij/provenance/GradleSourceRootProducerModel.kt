...
data class DefaultGradleSourceRootProducerModel(override val entries: List<GradleSourceRootProducerModelEntry>) :
 ...
 *
 * Source roots, roles, and producer dependencies come from [SourceSetContainer]. A root is generated only when it is
 * owned by an output of a task that Gradle records as a dependency of the source-directory collection. Resource
 * membership is retained as a closed role so it cannot authorize a code root. The builder never scans the project's
 * task container and never inspects a path segment or directory name.
 */
class GradleSourceRootProducerModelBuilder : ModelBuilderService {
    override fun canBuild(modelName: String): Boolean = modelName == GradleSourceRootProducerModel::class.java.name

    /**
     * Proof transition: `(String, Project) -> GradleSourceRootProducerModel`.
     *
     * Establishes an exact absolute normalized project directory and source roots with Gradle project, source-set,
     * code/resource role, and producer-backed generated classification. The output retains the owner required for exact
     * installed lookup. Gradle-owned [File] values are normalized at this outer boundary; raw source sets, producer
     * tasks, outputs, and paths do not leave it.
     */
    override fun buildAll(
        ...
        for (sourceSet in sourceSets) {
            ...
            val resourceRoots =
                sourceSet.resources.sourceDirectories.files
                    .asSequence()
                    .map { resource -> resource.toPath().toAbsolutePath().normalize() }
                    .toSet()
            val producerOutputRoots =
                sourceDirectories.buildDependencies
