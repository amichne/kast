...
idea_plugin_xml="${repo_root}/backend-idea/src/main/resources/META-INF/plugin.xml"
...
idea_plugin_update_feed="${repo_root}/packaging/jetbrains/updatePlugins.xml"
...
require_not_contains "${repo_root}/backend-idea/build.gradle.kts" "SignPluginTask" "IDEA plugin build must not configure signing"
require_not_contains "${repo_root}/backend-idea/build.gradle.kts" "VerifyPluginSignatureTask" "IDEA plugin build must not configure signature verification"
require_not_contains "${repo_root}/backend-idea/build.gradle.kts" "kast.idea.signing" "IDEA plugin build must not require signing properties"
require_block_contains "$idea_plugin_xml" '<extensions defaultExtensionNs="org.jetbrains.kotlin">' '</extensions>' "supportsKotlinPluginMode" "Kotlin plugin-mode support must use the Kotlin extension namespace"
require_block_not_contains "$idea_plugin_xml" '<extensions defaultExtensionNs="com.intellij">' '</extensions>' "supportsKotlinPluginMode" "The invalid IntelliJ extension registration must not block dynamic unload"
...
require_block_not_contains "$ci_workflow" "  workflow-contracts:" "  local-authority-contracts:" "test-local-development-clean-break-contract.sh" "CI static fanout contracts must not execute the local clean-break contract"
...
require_block_contains "$release_workflow" "  publish-release:" "  verify-release-state:" "installed-semantic-canary" "GitHub release publication must depend on the installed semantic canary"
require_block_contains "$release_workflow" "  publish-release:" "  verify-release-state:" "needs.installed-semantic-canary.result == 'success'" "GitHub release publication must require a successful installed semantic canary"
...
require_block_contains "$ci_workflow" "  local-authority-contracts:" "  runtime-contracts:" "test-local-development-clean-break-contract.sh" "CI local-authority contracts must prove the retired partial installer stays absent"
require_block_not_contains "$ci_workflow" "  local-authority-contracts:" "  runtime-contracts:" "if: github.event_name == 'push' && github.ref == 'refs/heads/main'" "CI local-authority contracts must not hide the clean-break proof on pull requests"
...
require_block_contains "$ci_workflow" "  source-bound-headless-backend:" "  install-ubuntu-debian-container:" "ci-artifact-ledger-headless-ubuntu-latest" "The sole pull-request headless producer must upload its artifact ledger"
...
require_not_contains "$ci_build_and_test_workflow" "macOS" "Linux-only JVM validation must not retain retired macOS cache branches"
...
require_contains "$ci_build_and_test_workflow" 'intellij-runtime-Linux-' "CI JVM validation must use the Linux runtime cache namespace"
...
require_block_contains "$ci_workflow" "  prepared-ubuntu-debian-bundle:" "  source-bound-headless-backend:" "compression-level: 0" "The prepared bundle upload must not recompress its gzip archive"
require_block_not_contains "$ci_workflow" "  prepared-ubuntu-debian-bundle:" "  source-bound-headless-backend:" "Free disk for" "Prepared bundle derivation must not spend its critical path deleting unrelated runner SDKs"
...
require_contains "$root_build_file" "entryCompression = org.gradle.api.tasks.bundling.ZipEntryCompression.STORED" "The ephemeral source-bound backend archive must store bytes that the prepared-generation consumer immediately extracts"
require_contains "$root_build_file" 'dependsOn(stageDevelopmentBackend, ":backend-headless:verifyHeadlessPortableDistLayout")' "Source-bound backend packaging must reuse the owning Gradle layout verifier"
...
require_not_contains "$prepared_generation_assembler" "developer local verify" "Prepared-generation assembly must not duplicate the packaging boundary's verification"
require_contains "$prepared_generation_packager" "developer local verify" "Prepared-generation packaging must verify the exact generation before archiving it"
...
require_contains "$ubuntu_debian_validator" "kast ready --for machine" "Ubuntu/Debian container validation must prove machine installation readiness before workspace startup"
...
require_block_not_contains "$ci_workflow" "  kast-action-runtime-contract:" "  installed-semantic-fixture:" "Free disk for" "kast-action validation must not spend its critical path deleting unrelated runner SDKs"
require_contains "$ci_workflow" "scripts/verify-setup-kast-install.sh" "CI must run the shared setup-kast install verifier"
require_contains "$setup_kast_verifier" '"$kast_bin" ready --for machine' "setup-kast validation must prove machine installation readiness before workspace startup"
...
require_block_not_contains "$ci_workflow" "  source-bound-headless-backend:" "  install-ubuntu-debian-container:" "Assert source-bound distribution excludes fat jar" "The source-bound producer must not duplicate the Gradle layout verifier"
require_not_contains "$ci_build_and_test_workflow" "portableDistZip" "JVM validation must not duplicate portable distribution production"
require_not_contains "$ci_build_and_test_workflow" "Upload portable headless backend distribution" "JVM validation must not publish an unshipped platform duplicate"
...
require_contains "$ci_build_and_test_workflow" "workflow_call:" "CI build-and-test implementation must remain a focused reusable boundary"
require_contains "$ci_build_and_test_workflow" "runs-on: ubuntu-latest" "Linux must exclusively own JVM backend test execution"
require_not_contains "$ci_build_and_test_workflow" "inputs:" "The single-platform JVM owner must not retain a speculative runner parameter"
require_not_contains "$ci_build_and_test_workflow" "if: runner.os" "The single-platform JVM owner must not retain dead platform branches"
require_block_contains "$ci_workflow" "  build-and-test-linux:" "  install-ubuntu-debian-container:" "    uses: ./.github/workflows/ci-build-and-test.yml" "CI must call the reusable JVM validation workflow for Linux"
require_block_contains "$ci_workflow" "  build-and-test-linux:" "  install-ubuntu-debian-container:" "    needs: workflow-contracts" "CI Linux build-and-test must wait for the static workflow preflight"
require_not_contains "$ci_workflow" "  build-and-test-macos:" "Ordinary pull-request CI must not compile an unshipped macOS portable backend"
...
require_block_not_contains "$release_workflow" "  release-preflight:" "  bump-version:" "IDEA_PLUGIN_" "Release preflight must not require IDEA signing inputs"
require_block_not_contains "$release_workflow" "  release-preflight:" "  bump-version:" "plugin-repository.json" "Release preflight must not validate Marketplace publication state"
require_block_not_contains "$release_workflow" "  release-preflight:" "  bump-version:" "immutable-releases" "Release preflight must not require repository immutability"
require_block_contains "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" ":backend-idea:buildPlugin" "Release must build the unsigned IDEA plugin ZIP"
require_block_contains "$release_workflow" "  build-cli:" "  build-codex-plugin:" "KAST_RELEASE_REVISION" "Release CLI artifacts must embed the exact checked-out release revision"
require_block_contains "$release_workflow" "  build-codex-plugin:" "  build-idea-plugin:" "KAST_RELEASE_REVISION" "Release Codex generation must use the exact checked-out release revision"
require_block_contains "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" "KAST_RELEASE_REVISION" "Release IDEA plugin artifacts must embed the exact checked-out release revision"
require_block_contains "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" 'asset_name="kast-idea-${tag}.zip"' "Release must publish a versioned IDEA plugin ZIP"
require_block_contains "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" "packaging/jetbrains/updatePlugins.xml" "Release must render the GitHub-hosted update feed"
require_block_contains "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" 'gh release upload "$tag" "$asset_path" "$feed_path" --clobber' "Release reruns must replace the unsigned plugin and update feed together"
require_block_not_matches "$release_workflow" "  build-idea-plugin:" "  build-headless-backend:" 'signPlugin|verifyPlugin|verify-idea-plugin-artifact|certificate|PRIVATE_KEY' "IDEA release must not sign or run Marketplace publication verification"
require_contains "$idea_plugin_update_feed" 'id="io.github.amichne.kast"' "IDEA update feed must retain the plugin identity"
...
require_not_contains "$release_workflow" "build-jetbrains-plugin-repository-pages" "Release must not build a JetBrains Pages repository"
require_not_contains "$release_workflow" "deploy-jetbrains-plugin-repository-pages" "Release must not deploy a JetBrains Pages repository"
require_not_contains "$ci_workflow" "test-jetbrains-plugin-repository-contract.sh" "CI must not run retired Pages repository checks"
require_not_contains "$ci_workflow" "test-idea-plugin-signing-contract.sh" "CI must not run retired signing checks"
...
require_order "$release_workflow" "Render and push Homebrew tap" "Publish draft release with provenance annotation" "Stable plugin discovery must not precede the matching Homebrew CLI"
