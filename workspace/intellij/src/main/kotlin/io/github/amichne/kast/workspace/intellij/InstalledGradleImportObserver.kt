...
internal class InstalledGradleImportObserver(private val workspaceRoot: Path) : ExternalSystemTaskNotificationListener {
     ...
     *
     * A terminal observation may publish immediately only after this transition and only when no exact task remains
     * active. Before closure, an empty cancelled generation remains provisional so a later exact replacement can refine
     * it.
     */
     ...
     *
     * [GradleTaskIdentity.EXACT_WORKSPACE] establishes one Gradle project-resolution task whose contextual path is the
     * exact canonical workspace. Invalid or inaccessible raw paths fail closed as [GradleTaskIdentity.OTHER]. Raw
     * callback data remains inside this observer.
     */
     ...
     *
     * [GradleTaskKind.PROJECT_RESOLUTION] establishes a Gradle `RESOLVE_PROJECT` task. Every other external-system task
     * fails closed as [GradleTaskKind.OTHER]. Raw task fields remain inside this observer.
     */
    private fun ExternalSystemTaskId.projectResolutionKind(): GradleTaskKind =
        if (projectSystemId == GradleConstants.SYSTEM_ID && type == ExternalSystemTaskType.RESOLVE_PROJECT) {
            GradleTaskKind.PROJECT_RESOLUTION
        } else {
 ...
 *
 * Refines IntelliJ's callback future into closed completed, failed, cancelled, or invalid-JVM import data. The platform
 * `Void` future and exceptional completion remain confined to the External System callback boundary.
 */
internal fun CompletableFuture<Void>.closedImportOutcome(
    ...
    when {
        ...
        failure is java.util.concurrent.CancellationException -> InstalledGradleImportOutcome.Cancelled
        failure is CompletionException && failure.cause is java.util.concurrent.CancellationException ->
            InstalledGradleImportOutcome.Cancelled
        failure.hasCause<ExternalSystemJdkException>() -> InstalledGradleImportOutcome.InvalidJvmConfiguration
        observeGradleInitializationScript(failure) == GradleInitializationScriptObservation.UNAVAILABLE ->
            ...
            when (val payload = GradlePayloadClassFileMajor.observe(failure)) {
                is GradlePayloadCompatibility.Unsupported ->
                    InstalledGradleImportOutcome.IncompatiblePayload(payload.major)
...
private inline fun <reified Failure : Throwable> Throwable?.hasCause(): Boolean =
    generateSequence(this) { current -> current.cause }.take(16).any { current -> current is Failure }

private enum class GradleTaskIdentity {
    EXACT_WORKSPACE,
    OTHER,
}

private enum class GradleTaskKind {
    PROJECT_RESOLUTION,
    OTHER,
}
...
private sealed interface GradleImportCohortTransition {
    ...
    data class Retained(override val cohort: GradleImportCohort) : GradleImportCohortTransition

    data class Published(override val cohort: GradleImportCohort.Published) : GradleImportCohortTransition {
...
private sealed interface GradleImportCohortMemory {
    ...
    fun remember(terminal: GradleImportTerminal): Terminal =
        when {
    ...
    fun beginReplacement(): GradleImportCohortMemory =
        when (this) {
            Failed -> Failed
            NoTerminal,
            Completed,
            Cancelled -> NoTerminal
        }
}

private enum class GradleImportAdmission {
    OPEN,
    CLOSED,
}

private enum class GradleTerminalAuthority {
    EXACT_PATH,
    ADMITTED_ID,
}

private fun GradleImportCohort.Collecting.admit(id: ExternalSystemTaskId): GradleImportCohort.Collecting =
    when {
        id in admitted -> this
        admitted.isEmpty() && admission == GradleImportAdmission.OPEN ->
            copy(
                admitted = setOf(id),
...
private fun GradleImportCohort.closeAdmission(): GradleImportCohortTransition =
    when (this) {
...
/**
 * Proof transition: `GradleImportCohort + ExternalSystemTaskId + GradleImportTerminal + GradleTerminalAuthority ->
 * GradleImportCohortTransition`.
 *
 * Retained establishes that exact project-open task admission is still open or at least one exact task remains active.
 * Published establishes closed admission, an empty admitted set, and one terminal outcome. A new exact generation
 * admitted after a provisional empty generation replaces completion or cancellation memory before closure while
 * retaining any proven failure. An unadmitted ID-only terminal remains no evidence. Raw IntelliJ callback identity is
 * permitted only at [InstalledGradleImportObserver].
 */
            ...
            when {
                id in admitted -> {
                    ...
                    if (remaining.isEmpty() && admission == GradleImportAdmission.CLOSED) {
                        ...
                        GradleImportCohortTransition.Retained(
                            copy(
                                admitted = remaining,
