 ...
 *
 * Establishes a non-blank detached identity for the exact inputs observed around one reconciliation. Raw extraction is
 * permitted only at evidence persistence and physical identity capture boundaries.
 */
@JvmInline
value class WorkspaceStateIdentity(val value: String) {
    init {
    ...
    companion object {
        /**
         * Proof transition: `String -> Refinement<WorkspaceStateIdentity, WorkspaceStateIdentityFailure>`.
         *
         * Establishes a non-blank workspace-state identity. [WorkspaceStateIdentityFailure] is the closed expected
         * failure. Raw identity text may enter only from a physical identity resolver or persistent evidence boundary.
         */
        fun parse(raw: String): Refinement<WorkspaceStateIdentity, WorkspaceStateIdentityFailure> =
            if (raw.isBlank()) {
...
@JvmInline
value class WorkspaceSourcePath private constructor(val value: String) : Comparable<WorkspaceSourcePath> {
    companion object {
        /**
         * Proof transition: `String -> Refinement<WorkspaceSourcePath, WorkspaceSourcePathFailure>`.
         *
         * Establishes a non-blank, normalized, relative path with no parent traversal. [WorkspaceSourcePathFailure] is
         * the closed expected failure. Raw path extraction is permitted only at a physical workspace adapter boundary.
         */
        fun parse(raw: String): Refinement<WorkspaceSourcePath, WorkspaceSourcePathFailure> {
            ...
            return Refinement.Refined(
                WorkspaceSourcePath(normalized.joinToString(separator = "/") { segment -> segment.toString() })
            )
        }
    }
...
@JvmInline
value class WorkspaceSourceContentHash private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<WorkspaceSourceContentHash, WorkspaceSourceContentHashFailure>`.
         *
         * Establishes a canonical lowercase SHA-256 digest. [WorkspaceSourceContentHashFailure] is the closed expected
         * failure. Raw digest text may enter only from a physical content hashing adapter.
         */
        fun parse(raw: String): Refinement<WorkspaceSourceContentHash, WorkspaceSourceContentHashFailure> {
            val canonical = raw.lowercase()
            return if (
                canonical.length == SHA256_HEX_LENGTH &&
...
sealed interface WorkspaceSourceContentIdentity {
    data class Present(val hash: WorkspaceSourceContentHash) : WorkspaceSourceContentIdentity
...
sealed interface WorkspaceSourceFreshnessClaimsFailure {
    ...
    data class DuplicatePath(val path: WorkspaceSourcePath) : WorkspaceSourceFreshnessClaimsFailure
}

/** Non-empty exact path-and-content claims for one source request. */
class WorkspaceSourceFreshnessClaims
private constructor(private val claimsByPath: Map<WorkspaceSourcePath, WorkspaceSourceContentIdentity>) {
    companion object {
        /**
         * Proof transition: `List<WorkspaceSourceFreshnessClaim> -> Refinement<WorkspaceSourceFreshnessClaims,
         * WorkspaceSourceFreshnessClaimsFailure>`.
         *
         * Establishes a non-empty, uniquely keyed, deterministic source-freshness claim set.
         * [WorkspaceSourceFreshnessClaimsFailure] is the closed expected failure. Individual physical path and content
         * observations may enter only after their own refinement.
         */
        fun refine(
            ...
            claims.forEach { claim ->
                if (retained.putIfAbsent(claim.path, claim.content) != null) {
                    return Refinement.Rejected(WorkspaceSourceFreshnessClaimsFailure.DuplicatePath(claim.path))
                }
            }
    ...
    fun followedBy(later: WorkspaceSourceFreshnessClaims): WorkspaceSourceFreshnessClaims =
        WorkspaceSourceFreshnessClaims((claimsByPath + later.claimsByPath).toSortedMap())
...
sealed interface WorkspaceTransitionRequest {
    ...
    data class SourceFiles(val claims: WorkspaceSourceFreshnessClaims) : WorkspaceTransitionRequest {
...
sealed interface WorkspaceSourceFreshness {
    ...
    fun followedBy(later: WorkspaceSourceFreshness): WorkspaceSourceFreshness =
        when (this) {
            Absent -> later
            Unkeyed -> Unkeyed
            is Claimed ->
    ...
    companion object {
        fun from(request: WorkspaceTransitionRequest): WorkspaceSourceFreshness =
            when (request) {
