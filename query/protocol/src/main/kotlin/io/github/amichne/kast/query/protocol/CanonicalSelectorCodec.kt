...
sealed interface CanonicalSelectorEncoding {
    ...
    data class Rejected(val failure: CanonicalSelectorEncodingFailure) : CanonicalSelectorEncoding
}
...
sealed interface CanonicalSelectorDecoding<out Value> {
    ...
    data class Rejected(val failure: CanonicalSelectorDecodingFailure) : CanonicalSelectorDecoding<Nothing>
}
...
object CanonicalSelectorCodec {
     ...
     *
     * [CanonicalSelectorEncoding.Encoded] establishes one digest-bound, discriminated candidate document.
     * [CanonicalSelectorEncodingFailure] closes unsupported declaration scopes and public-text admission failure. Raw
     * document text leaves only at the protocol-token edge.
     */
    fun encodeCandidate(selector: CandidateSelector): CanonicalSelectorEncoding {
        ...
        val retainedScope =
            if (retainedScopeNeeded) {
                when (val projected = selector.scope.selectorDocumentProjection()) {
                    is SelectorScopeDocumentProjection.Projected ->
                        SelectorReadScopeDocument(
                            selector.scope.sourceKinds.name,
                            selector.scope.generatedSources.name,
                            projected.kind,
                            projected.file,
                            projected.libraries,
                            selector.constraints.selectorDocument(),
                        )
                    SelectorScopeDocumentProjection.Rejected ->
                        return CanonicalSelectorEncoding.Rejected(CanonicalSelectorEncodingFailure.UNSUPPORTED_SCOPE)
                }
            } else null
        val document =
            when (selector) {
                is CandidateSelector.Declaration -> {
                    ...
                    val scope =
                        when (val projection = selection.scope.selectorDocumentProjection()) {
        ...
        return encodeToken(
            CANDIDATE_PREFIX,
            if (
                selector.lease is LiveSemanticReadAuthority ||
                    selector.constraints != SymbolDiscoveryConstraints.None ||
                    retainedScope != null
            )
                LIVE_TOKEN_VERSION
            else CANDIDATE_TOKEN_VERSION,
     ...
     *
     * A decoded value establishes token structure, digest, generated-schema decoding, and all lease, scope, file,
     * candidate, and selection invariants. [CanonicalSelectorDecodingFailure] is the closed expected failure. Raw token
     * text is extracted only by this decoder boundary.
     */
    fun decodeCandidate(
        token: ProtocolText,
        ...
        val payload =
            when (val admission = parseToken(token, CANDIDATE_PREFIX, tokenVersion(token, CANDIDATE_TOKEN_VERSION))) {
        ...
        val document =
            try {
                ...
                return CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.MALFORMED_DOCUMENT)
            } catch (_: IllegalArgumentException) {
                return CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.MALFORMED_DOCUMENT)
            }
        if (
            token.value.split(':')[1] == CANDIDATE_TOKEN_VERSION &&
                (document.live != null ||
                    when (document) {
     ...
     *
     * An encoded token establishes a digest-bound exact-selector document produced by the generated
     * `ExactSelectorDocument.serializer()` factory. [CanonicalSelectorEncodingFailure] closes unsupported scopes and
     * public-text admission failure. Raw document text leaves only at the token edge.
     */
    fun encodeExact(selector: SymbolSelector): CanonicalSelectorEncoding {
        val scope =
            when (val projection = selector.scope.selectorDocumentProjection()) {
                is SelectorScopeDocumentProjection.Projected -> projection
                SelectorScopeDocumentProjection.Rejected ->
                    return CanonicalSelectorEncoding.Rejected(CanonicalSelectorEncodingFailure.UNSUPPORTED_SCOPE)
            }
     ...
     *
     * A decoded value establishes token structure, digest, generated-schema decoding, and all exact compiler-evidence
     * and fingerprint invariants. [CanonicalSelectorDecodingFailure] is the closed expected failure. Raw token text is
     * extracted only by this decoder boundary.
     */
    fun decodeExact(
        token: ProtocolText,
        current: SemanticReadAuthority? = null,
        ...
        val payload =
            when (val admission = parseToken(token, EXACT_PREFIX, tokenVersion(token, EXACT_TOKEN_VERSION))) {
                is SelectorTokenPayloadAdmission.Admitted -> admission.payload
                is SelectorTokenPayloadAdmission.Rejected -> return admission.failure.rejected()
            }
        val document =
        ...
        if (
            token.value.split(':')[1] == EXACT_TOKEN_VERSION && (document.live != null || document.constraints != null)
        ) {
            return CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.UNSUPPORTED_REFERENCE_VERSION)
        }
        when (val identity = admitSelectorReference(document.root, document.generation, document.live, current)) {
...
private sealed interface SelectorTokenPayloadAdmission {
    ...
    data class Rejected(val failure: CanonicalSelectorDecodingFailure) : SelectorTokenPayloadAdmission
}
...
private fun encodeToken(
    ...
    return when (val admitted = ProtocolText.parse(raw)) {
        ...
        is Refinement.Rejected -> CanonicalSelectorEncoding.Rejected(CanonicalSelectorEncodingFailure.TOKEN_REJECTED)
    }
}
 ...
 *
 * Admission proves the token family, version, base64url encoding, SHA-256 digest, and strict UTF-8 payload.
 * [CanonicalSelectorDecodingFailure] closes each expected rejection. Raw bytes leave only at strict UTF-8 decoding for
 * the generated serializer boundary.
 */
private fun parseToken(
    ...
    if (parts.size != TOKEN_PART_COUNT || parts[0] != prefix || parts[1] != version) {
        return SelectorTokenPayloadAdmission.Rejected(CanonicalSelectorDecodingFailure.INVALID_TOKEN_STRUCTURE)
    }
    val payload =
        try {
            Base64.getUrlDecoder().decode(parts[2])
        } catch (_: IllegalArgumentException) {
            return SelectorTokenPayloadAdmission.Rejected(CanonicalSelectorDecodingFailure.INVALID_PAYLOAD_ENCODING)
        }
    if (sha256(payload) != parts[3]) {
        return SelectorTokenPayloadAdmission.Rejected(CanonicalSelectorDecodingFailure.PAYLOAD_DIGEST_MISMATCH)
    }
    val decoded =
        try {
...
private fun <Value> SelectorDocumentAdmission<Value>.asDecoding(): CanonicalSelectorDecoding<Value> =
    when (this) {
...
private fun tokenVersion(token: ProtocolText, published: String): String =
    if (token.value.split(':').getOrNull(1) == LIVE_TOKEN_VERSION) LIVE_TOKEN_VERSION else published

private fun LiveSemanticReadReference.selectorDocument() =
    LiveSelectorAuthorityDocument(
        host.value.toString(),
        epoch.value,
        contentView.name,
        version,
