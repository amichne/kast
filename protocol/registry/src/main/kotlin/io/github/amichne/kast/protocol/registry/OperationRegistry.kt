 ...
 *
 * Implementations other than [OperationDefinition] remain boundary candidates and cannot enter a successfully
 * constructed [OperationRegistry].
 */
...
private constructor(definitionsByOperation: Map<CanonicalOperation, OperationDefinition<*, *, *, *, *>>) {
    ...
    fun lookup(id: OperationId): OperationLookup =
        when (CanonicalOperation.resolve(id)) {
            is CanonicalOperationResolution.Unknown -> OperationLookup.Unknown(id)
            is CanonicalOperationResolution.Known -> OperationLookup.Found(definitionsById.getValue(id))
        }
    ...
    companion object {
         ...
         *
         * Establishes exactly one fully typed definition for every canonical operation, unique operation and schema
         * identities, and canonical ordering. [OperationRegistryFailure] is the closed expected failure. Weak metadata
         * iteration is permitted only at runtime composition.
         */
        fun create(metadata: Iterable<OperationMetadata>): OperationRegistryConstruction {
            ...
            val failures = buildSet {
                ...
                CanonicalOperation.entries.filterNot(typedOperations::contains).forEach {
                    add(OperationRegistryFailure.MissingOperationId(it.id))
                }
            }

            return if (failures.isEmpty()) {
                OperationRegistryConstruction.Created(OperationRegistry(typed.associateBy { it.operation }))
            } else {
...
sealed interface OperationRegistryConstruction {
    data class Created(val registry: OperationRegistry) : OperationRegistryConstruction

    data class Rejected(val failures: Set<OperationRegistryFailure>) : OperationRegistryConstruction
}

sealed interface OperationRegistryFailure {
    data class DuplicateOperationId(val id: OperationId) : OperationRegistryFailure

    data class DuplicateSchemaIdentity(val schema: SchemaIdentity) : OperationRegistryFailure

    data class MissingOperationId(val id: OperationId) : OperationRegistryFailure

    data class UnknownOperationId(val id: OperationId) : OperationRegistryFailure

    data class UntypedOperationMetadata(val id: OperationId) : OperationRegistryFailure
}

sealed interface OperationLookup {
    data class Found(val definition: OperationDefinition<*, *, *, *, *>) : OperationLookup

    data class Unknown(val id: OperationId) : OperationLookup
}
