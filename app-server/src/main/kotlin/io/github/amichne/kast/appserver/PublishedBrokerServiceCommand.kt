...
class PublishedBrokerServiceEnvironment internal constructor(val values: Map<String, String>)

private enum class PublishedLaunchForm {
    PRIVATE_DAEMON,
    LEGACY_CLI,
}

private data class PublishedLaunchReceipt(
    val environment: Map<String, String>,
    val form: PublishedLaunchForm,
    val bytes: ByteArray,
)
...
object PublishedBrokerServiceCommand {
    ...
    internal fun recover(observed: BrokerServiceLaunchCommand): BrokerServiceLaunchCommand? =
        try {
            val receipt = readEnvironment(observed) ?: return null
            recoverFromReceipt(observed, receipt)
        } catch (_: Exception) {
            null
        }

    private fun recoverFromReceipt(
        observed: BrokerServiceLaunchCommand,
        receipt: PublishedLaunchReceipt,
    ): BrokerServiceLaunchCommand? {
        val environment = receipt.environment
        ...
        return recovered.takeIf {
            it.identity.value == publishedIdentity &&
                ...
                (receipt.form == PublishedLaunchForm.LEGACY_CLI ||
                    receipt.bytes.contentEquals(
                        BrokerLaunchdServiceDocument.render(it).toByteArray(StandardCharsets.UTF_8)
                    ))
        }
    }
    ...
    fun retirementEnvironment(
        ...
        try {
            ...
            val receipt = readEnvironment(observed) ?: return null
            if (receipt.form == PublishedLaunchForm.PRIVATE_DAEMON && recoverFromReceipt(observed, receipt) == null)
                return null
            val environment = receipt.environment
            if (
                environment["HOME"] != userHome.toString() ||
                    ...
                    environment["KAST_CONFIGURATION_FILE"] !=
                        installationRoot.resolve("config/environment").toString() ||
    ...
    private fun readEnvironment(observed: BrokerServiceLaunchCommand): PublishedLaunchReceipt? =
        try {
            ...
            val arguments = readArguments(bytes, observed.serviceLabel.value) ?: return null
            parseReceipt(arguments, observed, bytes)
        } catch (_: Exception) {
            null
        }

    private fun readArguments(bytes: ByteArray, label: String): List<String>? {
        val document =
        ...
        val dictionary = document.documentElement.directElements().singleOrNull { it.tagName == "dict" } ?: return null
        val entries = dictionary.directElements()
        if (entries.size % 2 != 0) return null
        val values =
            entries.chunked(2).associate { pair ->
                val key = pair[0].takeIf { it.tagName == "key" }?.textContent ?: return null
                if (key.isBlank()) return null
                key to pair[1]
            }
        if (values.size != entries.size / 2) return null
        if (values["Label"]?.takeIf { it.tagName == "string" }?.textContent != label) return null
        return values["ProgramArguments"]
            ?.takeIf { it.tagName == "array" }
            ?.directElements()
    ...
    private fun parseReceipt(
        ...
        observed: BrokerServiceLaunchCommand,
        bytes: ByteArray,
    ): PublishedLaunchReceipt? {
        if (arguments.size !in 5..MAXIMUM_ARGUMENTS || arguments[0] != "/usr/bin/env" || arguments[1] != "-i")
            return null
        val form =
            when {
                arguments.last() == observed.daemonExecutable.toString() -> PublishedLaunchForm.PRIVATE_DAEMON
                arguments.takeLast(3) == listOf(observed.kast.toString(), "broker", "serve") ->
                    PublishedLaunchForm.LEGACY_CLI
                else -> return null
            }
        val suffix = if (form == PublishedLaunchForm.PRIVATE_DAEMON) 1 else 3
        ...
        return PublishedLaunchReceipt(environment, form, bytes)
    }
