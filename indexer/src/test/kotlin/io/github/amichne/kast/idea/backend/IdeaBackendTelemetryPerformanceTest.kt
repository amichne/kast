...
/**
 * Performance baselines for indexer telemetry and read-action timing.
 *
 * Tests are tagged `performance` so they can be excluded from the default CI run:
 *     ./gradlew :indexer:test -PexcludeTags=performance
 * or run in isolation:
 *     ./gradlew :indexer:test -PincludeTags=performance
 */
@Tag("performance")
