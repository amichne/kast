...
class LiveReadOutputSchemaTest {
    private val live = EvidenceBasis.Live(LiveReadEvidence.create("/workspace",
    ...
    fun `source and traversal schemas reject mixed or missing snapshot bases`() {
        ...
        for ((operation, document) in completeDocuments(live).filter { (operation) ->
            ...
            assertRejects(operation, document.withSnapshot(operation, snapshot.with("sourceState", JsonPrimitive("fake"))))
            ...
            assertRejects(operation, document.withSnapshot(operation, snapshot(operation, publishedDocuments.getValue(operation))))
        }
    }
    ...
    fun `every read admits only packaged pre-authority rejection shapes`() {
        ...
        val hosted = Json.parseToJsonElement("""{"schemaVersion":1,"outcome":"rejected","failure":"DIRTY_DOCUMENTS","detail":"saved content required","stage":"EPOCH_OBSERVATION"}""").jsonObject
    ...
    private fun completeDocuments(basis: EvidenceBasis): List<Pair<CanonicalOperation, JsonObject>> = listOf(
        CanonicalOperation.QUERY_RUN to CanonicalQueryCliDocuments.project(
            complete(CanonicalOperation.QUERY_RUN, basis, QueryRunResult(empty(), empty()))).document(),
        CanonicalOperation.SYMBOL_DISCOVER to CanonicalSymbolCliDocuments.projectDiscovery(
            complete(CanonicalOperation.SYMBOL_DISCOVER, basis, SymbolDiscoverResult(empty()))).document(),
        CanonicalOperation.SYMBOL_INSPECT to CanonicalSymbolCliDocuments.projectInspection(
            complete(CanonicalOperation.SYMBOL_INSPECT, basis, SymbolInspectResult(symbol()))).document(),
        CanonicalOperation.SOURCE_READ to CanonicalSourceReadCliDocuments.project(
            complete(CanonicalOperation.SOURCE_READ, basis, sourceResult(basis))).document(),
        CanonicalOperation.RELATION_READ to CanonicalReadCliDocuments.projectRelation(
            complete(CanonicalOperation.RELATION_READ, basis, RelationReadResult(empty()))).document(),
        CanonicalOperation.TRAVERSAL_RUN to CanonicalReadCliDocuments.projectTraversal(
            complete(CanonicalOperation.TRAVERSAL_RUN, basis, TraversalRunResult(text("/workspace"), empty()))).document(),
        CanonicalOperation.DIAGNOSTIC_CHECK to CanonicalReadCliDocuments.projectDiagnostics(
            complete(CanonicalOperation.DIAGNOSTIC_CHECK, basis, DiagnosticCheckResult(empty()))).document(),
    )

    private fun qualifiedDocuments(basis: EvidenceBasis): List<Pair<CanonicalOperation, JsonObject>> = listOf(
        CanonicalOperation.QUERY_RUN to CanonicalQueryCliDocuments.project(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.QUERY_RUN.id, basis, QueryRunResult(empty(), empty())),
            QueryRunQualification.create(QueryKnownMinimum.parse(0).refined(),
                listOf(QueryLimitationDocument.DISCOVERY_INCOMPLETE)).refined())).document(),
        CanonicalOperation.SYMBOL_DISCOVER to CanonicalSymbolCliDocuments.projectDiscovery(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.SYMBOL_DISCOVER.id, basis, SymbolDiscoverResult(empty())),
            SymbolDiscoverQualification.from(setOf(SymbolDiscoverLimitation.WORK_LIMIT)).refined())).document(),
        CanonicalOperation.SYMBOL_INSPECT to CanonicalSymbolCliDocuments.projectInspection(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.SYMBOL_INSPECT.id, basis, SymbolInspectResult(symbol())),
            SymbolInspectQualification.EVIDENCE_INCOMPLETE)).document(),
        CanonicalOperation.SOURCE_READ to CanonicalSourceReadCliDocuments.project(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.SOURCE_READ.id, basis, sourceResult(basis)),
            SourceReadQualification.create(SourceEntityCountDocument.parse(0).refined(),
                listOf(SourceReadLimitationDocument.WORK_LIMIT_REACHED),
                SourceReadContinuationStateDocument.Unavailable).refined())).document(),
        CanonicalOperation.RELATION_READ to CanonicalReadCliDocuments.projectRelation(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.RELATION_READ.id, basis, RelationReadResult(empty())),
            RelationReadQualification.terminalIncomplete(RelationKnownMinimumDocument.parse(0).refined(),
                listOf(RelationLimitationDocument.WORK_LIMIT_REACHED)).refined())).document(),
        CanonicalOperation.TRAVERSAL_RUN to CanonicalReadCliDocuments.projectTraversal(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.TRAVERSAL_RUN.id, basis, TraversalRunResult(text("/workspace"), empty())),
            TraversalRunQualification.terminalIncomplete(listOf(TraversalLimitationDocument.DEPTH_LIMIT_REACHED),
                emptyList()).refined())).document(),
        CanonicalOperation.DIAGNOSTIC_CHECK to CanonicalReadCliDocuments.projectDiagnostics(OperationOutcome.Qualified(
            EvidenceEnvelope(CanonicalOperation.DIAGNOSTIC_CHECK.id, basis, DiagnosticCheckResult(empty())),
            DiagnosticCheckQualification.create(DiagnosticKnownCountDocument.parse(0).refined(), false, emptyList(),
                listOf(DiagnosticLimitationDocument(text("src/Example.kt"), DiagnosticLimitationReasonDocument.INDEXING)))
                .refined())).document(),
    )
    ...
    private fun sourceResult(basis: EvidenceBasis): SourceReadResult {
        ...
        val selection = SourceSelectionDocument(text("source-selector-v1:payload:digest"),
    ...
    private fun symbol(): SymbolDocument = SymbolDocument.create(text("exact:v2:payload:digest"),
        SymbolKindDocument.CLASSLIKE, text("Example"), SymbolQualifiedIdentityDocument.Available(text("sample.Example")),
        text("src/Example.kt"), SourceRangeDocument.create(offset(0), offset(1)).refined(),
        CompilerSymbolEvidenceDocument.fromSignature(CompilerSignatureDocument.ClassLike(text("sample.Example"))).refined()).refined()
    ...
    private fun validate(operation: CanonicalOperation, document: JsonObject) =
        schemas.getSchema(installedServerOutputSchema(operation).toString()).validate(
            buildJsonObject { put("status", "completed"); put("document", document) }.toString(), InputFormat.JSON)

    private fun ProjectedCliOutcome.document(): JsonObject = when (this) {
