...
KVP-016's detached existing-Project model closure.
...
- `state/` owns KVP-008's generated state report and exact-head receipt progression; `firewall/`
  ...
  receipt owner plus the KVP-012 through KVP-016 closures; `registration/` owns compiled task
  registration through KVP-010 and for KVP-012 through KVP-016.
- Registration must replace generic placeholders for KVP-001 through KVP-010 and KVP-012 through
  KVP-016, and preserve one registered task for every program gate without executing later
  placeholders.
