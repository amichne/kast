...
fn no_backend_error(workspace_root: &Path, backend_name: Option<BackendName>) -> CliError {
    ...
    let mut error = match backend_name {
        ...
        BackendName::Idea => CliError::new(
            "NO_BACKEND_AVAILABLE",
            format!(
                "No idea backend is installed or running for {}. Install or update the signed Kast plugin through JetBrains, open the project in IDEA or Android Studio, then acquire with: kast agent lease acquire --workspace-root <root> --backend=idea",
                workspace_root.display()
            ),
        ),
    };
