...
internal sealed interface DiscoveryRequestAdmission {
    data class Admitted(val request: io.github.amichne.kast.symbol.contract.SymbolDiscoveryRequest) :
...
/**
 * Proof transition: `(SemanticReadAuthority, SymbolDiscoverRequest, SymbolDiscoveryBudget) ->
 * DiscoveryRequestAdmission`.
 *
 * Establishes one closed discovery target, exact current lease, compiled-scope request, and resource limits. Boundary
 * primitives do not survive this transition.
 */
    ...
    val admitted =
        when (val target = request.target) {
            is SymbolDiscoverTargetDocument.Name -> {
                val pattern =
                    SymbolDiscoveryPattern.parse(target.query.value).refinedOrNull()
                        ?: return DiscoveryRequestAdmission.Rejected
                val kind =
                    when (target.kind) {
            ...
            is SymbolDiscoverTargetDocument.Location -> {
                val file = workspaceFile(current, target.file.value) ?: return DiscoveryRequestAdmission.Rejected
                val offset =
                    SymbolDiscoverySourceOffset.parse(target.offset.value).refinedOrNull()
                        ?: return DiscoveryRequestAdmission.Rejected
                io.github.amichne.kast.symbol.contract.SymbolDiscoveryRequest(
