...
class HostedQualifiedClassSelection private constructor(
    override val root: CanonicalWorkspaceRoot,
    val signature: CanonicalCompilerSignature.ClassLike,
) : HostedSupertypeSelection {
    companion object {
        fun parse(root: CanonicalWorkspaceRoot, qualifiedName: String): Refinement<HostedQualifiedClassSelection, HostedQueryFailure> {
            if (qualifiedName.toByteArray(Charsets.UTF_8).size !in 1..4096 ||
            ...
            return when (val signature = CanonicalCompilerSignature.classLike(qualifiedName)) {
                is Refinement.Refined -> when (val classLike = signature.value) {
                    is CanonicalCompilerSignature.ClassLike -> Refinement.Refined(HostedQualifiedClassSelection(root, classLike))
                    is CanonicalCompilerSignature.Function, is CanonicalCompilerSignature.Property,
                    is CanonicalCompilerSignature.TypeAlias -> Refinement.Rejected(HostedQueryFailure.INVALID_SELECTION)
                }
...
internal class HostedUniqueDeclaration<Value> private constructor(val value: Value) {
    companion object {
        fun <Value> select(candidates: List<Value>): Refinement<HostedUniqueDeclaration<Value>, HostedQueryFailure> = when (candidates.size) {
