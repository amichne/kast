...
| Ubuntu/Debian hosted agent | `kast.sh install` from the Linux headless tarball, with `scripts/install-ubuntu-debian.sh install` as the bundled compatibility entrypoint | Contained CLI and headless backend under `KAST_UBUNTU_DEBIAN_ROOT` | `kast` on `PATH` plus `KAST_CONFIG_HOME` when a custom config root is used |
