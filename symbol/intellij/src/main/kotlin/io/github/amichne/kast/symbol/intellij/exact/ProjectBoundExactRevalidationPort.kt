            ...
            val result =
                scope.execute(
                                ...
                                return@execute rejected(
                                    when (found.reason) {
                                        IntellijExactDeclarationLookupRejection.STALE_LOCATION ->
                                            ExactRevalidationRejection.DECLARATION_MISSING
                                        IntellijExactDeclarationLookupRejection.OUTSIDE_SCOPE ->
                                            ExactRevalidationRejection.SCOPE_REJECTED
                                        IntellijExactDeclarationLookupRejection.AMBIGUOUS_DECLARATION ->
                                            ExactRevalidationRejection.AMBIGUOUS
                    ...
                    when (val checked = capture.check(locator, live.declaration.containingFile)) {
                        ...
                        is Refinement.Rejected -> return@execute rejected(checked.failure)
                    }
                    when (
                        val found = IntellijKotlinCompilerSymbolLookup(psi, observation, capture).find(compiled, key)
                    ) {
                            ...
                            rejected(
                                when (found.reason) {
                                    IntellijSymbolSelectorRejection.WORKSPACE_ROOT_MISMATCH ->
                                        ExactRevalidationRejection.WORKSPACE_MISMATCH
                                    IntellijSymbolSelectorRejection.GENERATION_MOVED ->
                                        ExactRevalidationRejection.BASIS_MOVED
                                    ...
                                    IntellijSymbolSelectorRejection.DUMB_MODE ->
                                        ExactRevalidationRejection.WORKSPACE_NOT_READY
                                    IntellijSymbolSelectorRejection.PROJECT_DISPOSED ->
                                        ExactRevalidationRejection.RETIRED
                                    ...
                                    IntellijSymbolSelectorRejection.INTERNAL_INVARIANT ->
                                        ExactRevalidationRejection.COMPILER_UNAVAILABLE
                                }
                            )
                    }
                }
