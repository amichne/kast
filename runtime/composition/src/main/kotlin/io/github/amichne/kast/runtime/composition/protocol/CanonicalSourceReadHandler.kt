...
internal class CanonicalSourceReadHandler(
    ...
    override suspend fun execute(
        ...
        val current =
            when (val anchor = request.anchor) {
                is SourceReadAnchorDocument.Candidate ->
                    (authority.candidate(anchor.selector) as? CandidateSelectorLookup.Found)?.selector?.lease
                is SourceReadAnchorDocument.Symbol ->
                    (authority.exact(anchor.selector) as? ExactSelectorLookup.Found)?.selector?.lease
                is SourceReadAnchorDocument.Source ->
                    SourceSelectorToken.parse(anchor.selector.value).valueOrNull()?.let {
        ...
        val entities =
            SourceEntityLimit.parse(request.entityLimit.value).valueOrNull()
                ?: return OperationOutcome.Rejected(SourceReadRejection.CONTRACT_VIOLATION)
        val bytes =
            SourceTextByteLimit.parse(request.textByteLimit.value).valueOrNull()
                ?: return OperationOutcome.Rejected(SourceReadRejection.CONTRACT_VIOLATION)
