...
internal class OwnedHostedEndpoint private constructor(
    ...
    override fun close() {
        try {
            ...
            try {
                try {
                    if (Files.exists(descriptor, NOFOLLOW_LINKS) && key(descriptor) == descriptorKey) Files.delete(descriptor)
                } finally {
    ...
    companion object {
        fun directory(home: Path, root: CanonicalWorkspaceRoot): Path {
            val digest = MessageDigest.getInstance("SHA-256").digest(root.value.toByteArray(Charsets.UTF_8))
        ...
        fun open(directory: Path, root: CanonicalWorkspaceRoot): Refinement<OwnedHostedEndpoint, HostedEndpointFailure> {
            ...
            try {
                if (!directory.isAbsolute || directory.normalize() != directory || socket.toString().toByteArray().size > 100) {
                    return Refinement.Rejected(HostedEndpointFailure.DIRECTORY_REJECTED)
                }
                var ancestor: Path? = directory
                while (ancestor != null) {
                    if (Files.isSymbolicLink(ancestor)) return Refinement.Rejected(HostedEndpointFailure.DIRECTORY_REJECTED)
                    ancestor = ancestor.parent
                }
                Files.createDirectories(directory)
                if (directory.toRealPath() != directory) return Refinement.Rejected(HostedEndpointFailure.DIRECTORY_REJECTED)
                Files.setPosixFilePermissions(directory, PosixFilePermissions.fromString("rwx------"))
                val channel = FileChannel.open(directory.resolve("owner.lock"), StandardOpenOption.CREATE, StandardOpenOption.WRITE, NOFOLLOW_LINKS)
                val lock = try { channel.tryLock() } catch (_: java.nio.channels.OverlappingFileLockException) { null }
                catch (failure: java.io.IOException) { channel.close(); throw failure }
                if (lock == null) { channel.close(); return Refinement.Rejected(HostedEndpointFailure.OWNERSHIP_CONFLICT) }
                ...
                val server = try { ServerSocketChannel.open(StandardProtocolFamily.UNIX) } catch (_: Exception) {
                    lock.release(); channel.close()
                ...
                try {
                    ...
                    server.close(); lock.release(); channel.close()
                    return Refinement.Rejected(HostedEndpointFailure.SOCKET_UNAVAILABLE)
                }
            } catch (_: java.io.IOException) { return Refinement.Rejected(HostedEndpointFailure.IO_UNAVAILABLE) }
            catch (_: SecurityException) { return Refinement.Rejected(HostedEndpointFailure.DIRECTORY_REJECTED) }
        }
