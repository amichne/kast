...
tasks.named<WriteWrapperScriptTask>("writeWrapperScript") {
    ...
    scriptContent.set(
        listOf(
            ...
            "export KAST_LAUNCHER_PATH=\"${dollar}{script_dir}/$launcherName\"",
            "runtime_candidates=(",
...
tasks.named<Zip>("portableDistZip").configure {
    val archiveRoot = project.name
    eachFile {
        if (relativePath.pathString == "$archiveRoot/smoke.sh") {
