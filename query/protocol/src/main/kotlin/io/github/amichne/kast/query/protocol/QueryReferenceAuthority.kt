...
interface QueryReferenceAuthority {
    fun restoreSource(token: SourceSelectorToken, current: SemanticReadAuthority): Refinement<SourceSelector, SourceSelectorTokenFailure>
    fun issueCandidates(batch: SymbolDiscoveryBatch): CandidateSelectorIssuance
    fun restoreCandidate(token: ProtocolText, current: SemanticReadAuthority): CanonicalSelectorDecoding<CandidateSelector>
    ...
    fun issueRangeCandidate(lease: SemanticReadAuthority, file: SymbolDiscoveryFileIdentity, rawStartInclusive: Int, rawEndExclusive: Int): CandidateSelectorTokenIssuance
}

internal fun QueryReferenceAuthority.candidate(token: ProtocolText, current: SemanticReadAuthority): CandidateSelectorLookup =
    when (val restored = restoreCandidate(token, current)) {
        ...
        is CanonicalSelectorDecoding.Rejected -> CandidateSelectorLookup.Rejected(restored.failure.lookupRejection(token, false))
    }
...
internal fun QueryReferenceAuthority.exact(token: ProtocolText, current: SemanticReadAuthority): ExactSelectorLookup =
    when (val restored = restoreExact(token, current)) {
        ...
        is CanonicalSelectorDecoding.Rejected -> ExactSelectorLookup.Rejected(restored.failure.lookupRejection(token, true))
    }

private fun CanonicalSelectorDecodingFailure.lookupRejection(token: ProtocolText, exact: Boolean): SelectorLookupRejection = when (this) {
    ...
    else -> if (token.value.startsWith(if (exact) "candidate:" else "exact:") || token.value.startsWith("source-selector-"))
        SelectorLookupRejection.WRONG_KIND else SelectorLookupRejection.MALFORMED
}
