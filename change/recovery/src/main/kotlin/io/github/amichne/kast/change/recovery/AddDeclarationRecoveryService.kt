...
sealed interface AddDeclarationRecoveryOperationFailure {
    ...
    data class Evidence(val failure: MutationRecoveryEvidenceFailure) : AddDeclarationRecoveryOperationFailure
}

class PreparedAddDeclarationRecovery
internal constructor(
    val input: AddDeclarationRecoveryPreparation,
    val record: MutationRecoveryRecord.PreWriteDurable,
)

class AppliedAddDeclarationRecovery
internal constructor(
...
sealed interface PrepareAddDeclarationRecoveryResult {
    data class Prepared(val recovery: PreparedAddDeclarationRecovery) : PrepareAddDeclarationRecoveryResult

    data class Rejected(val failure: AddDeclarationRecoveryOperationFailure) : PrepareAddDeclarationRecoveryResult
}

sealed interface RecordAppliedAddDeclarationResult {
    data class Recorded(val recovery: AppliedAddDeclarationRecovery) : RecordAppliedAddDeclarationResult

    data class Rejected(val failure: AddDeclarationRecoveryOperationFailure) : RecordAppliedAddDeclarationResult
}
...
sealed interface AddDeclarationRollbackResult {
    ...
    data class Rejected(val failure: AddDeclarationRollbackFailure) : AddDeclarationRollbackResult
}
...
fun interface AddDeclarationRollbackPort {
     ...
     *
     * A rolled-back result establishes restoration of every exact persisted preimage. [AddDeclarationRollbackFailure]
     * is the closed expected failure. Raw bytes may be extracted only inside the physical recovery adapter implementing
     * this boundary.
     */
...
sealed interface AddDeclarationRecoveryOutcome {
    data class PriorState(val evidence: PriorStateEvidence) : AddDeclarationRecoveryOutcome

    data class RolledBack(val record: MutationRecoveryRecord.RolledBack) : AddDeclarationRecoveryOutcome

    data class RecoveryRequired(val evidence: RecoveryRequiredEvidence) : AddDeclarationRecoveryOutcome
}
...
class AddDeclarationRecoveryService(
     ...
     *
     * A prepared result establishes durable exact pre-write evidence before any applied write can be represented.
     * Expected failures are closed by [AddDeclarationRecoveryOperationFailure]. Raw bytes and persistence handles
    ...
    fun prepare(input: AddDeclarationRecoveryPreparation): PrepareAddDeclarationRecoveryResult {
        val preparation =
            when (
                ...
                is Refinement.Rejected ->
                    return PrepareAddDeclarationRecoveryResult.Rejected(
        ...
        return when (val persisted = evidence.prepare(record)) {
            is MutationRecoveryPersistResult.Durable ->
                PrepareAddDeclarationRecoveryResult.Prepared(PreparedAddDeclarationRecovery(input, persisted.record))
     ...
     *
     * A recorded result establishes the exact planned singleton write set durably chained to its pre-write record.
     * Expected failures are closed by [AddDeclarationRecoveryOperationFailure]. This transition performs no source
     * write and extracts no raw content.
     */
    fun recordApplied(prepared: PreparedAddDeclarationRecovery): RecordAppliedAddDeclarationResult {
        val writeSet =
            when (
                val admitted =
                ...
                is Refinement.Refined -> admitted.value
                is Refinement.Rejected ->
                    return RecordAppliedAddDeclarationResult.Rejected(
                        AddDeclarationRecoveryOperationFailure.ContractViolation
                    )
            }
        val record =
        ...
        return when (val persisted = evidence.recordApplied(prepared.record, record)) {
            is MutationRecoveryPersistResult.Durable ->
                RecordAppliedAddDeclarationResult.Recorded(AppliedAddDeclarationRecovery(prepared, persisted.record))
            is MutationRecoveryPersistResult.Rejected ->
    ...
    /**
     * Proof transition: `(MutationPlanBinding, AddDeclarationRollbackPort) -> AddDeclarationRecoveryOutcome`.
     *
     * Resolves only to prior state, durably `RolledBack`, or explicitly `RecoveryRequired`. Evidence absence,
     * corruption, and unavailability never become mutation success. Raw preimage extraction is delegated only to the
     * supplied rollback capability.
     */
    fun recover(
    ...
    private fun recoverFound(
        ...
        when (record) {
            ...
            is MutationRecoveryRecord.RecoveryRequired ->
                AddDeclarationRecoveryOutcome.RecoveryRequired(RecoveryRequiredEvidence.Durable(record))
        }
    ...
    private fun recoverApplied(
        ...
        when (rollback.rollback(applied)) {
    ...
    private fun undurableTerminal(binding: MutationPlanBinding): AddDeclarationRecoveryOutcome.RecoveryRequired =
        AddDeclarationRecoveryOutcome.RecoveryRequired(
            RecoveryRequiredEvidence.Undurable(
                binding,
    ...
    private fun MutationRecoveryEvidenceFailure.toRequirement(): UndurableRecoveryRequirement =
        when (this) {
            MutationRecoveryEvidenceFailure.CORRUPT_RECORD -> UndurableRecoveryRequirement.EVIDENCE_CORRUPT
            MutationRecoveryEvidenceFailure.STORAGE_UNAVAILABLE,
            MutationRecoveryEvidenceFailure.PLAN_BINDING_COLLISION,
            MutationRecoveryEvidenceFailure.PRIOR_STATE_MISMATCH -> UndurableRecoveryRequirement.EVIDENCE_UNAVAILABLE
        }
}
