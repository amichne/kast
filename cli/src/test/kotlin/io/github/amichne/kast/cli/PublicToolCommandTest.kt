...
class PublicToolCommandTest {
    ...
    fun `all public tools share CLI admission and canonical wire lowering`() {
        val examples = mapOf(
            ...
            PublicToolIdentity.SEARCH_DECLARATIONS to """{"declaration_name":"order","name_match":null,"scope":null,"declaration_kinds":["property","type_alias"]}""",
            PublicToolIdentity.CHECK_DIAGNOSTICS to """{"relative_path":".","max_diagnostics":null}""",
            PublicToolIdentity.QUERY_SYMBOLS to """{"source":{"type":"all_declarations","declaration_kinds":null,"scope":null},"steps":null,"return_fields":[]}""",
        )
        ...
        examples.forEach { (identity, raw) ->
            val expected = (PublicToolContract.admit(identity, Json.parseToJsonElement(raw)) as Refinement.Refined).value.canonical
            ...
            when (expected) {
                is PublicToolCanonical.Query -> assertEquals(WireDecoding.Decoded(expected.request), CanonicalOperationWireBindings.queryRun.decodeRequest(request))
                is PublicToolCanonical.Diagnostics -> assertEquals(WireDecoding.Decoded(expected.request), CanonicalOperationWireBindings.diagnosticCheck.decodeRequest(request))
            }
        }
        assertTrue(graph.parse(listOf("tool", "search_classes"), CliRequestDocumentInput.Provided(examples.getValue(PublicToolIdentity.SEARCH_FUNCTIONS))) is CliCommandParsing.Rejected)
    }
}
