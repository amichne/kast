...
internal sealed interface InstalledObservedDeclarationIdentityObservation {
    data class Observed(val identity: InstalledObservedDeclarationIdentity) :
...
private sealed interface InstalledQualifiedContainerObservation {
    data class Observed(val value: String) : InstalledQualifiedContainerObservation
 ...
 * InstalledObservedDeclarationIdentityObservation`.
 *
 * Observed establishes that the unique added declaration shares the exact compiler-qualified container with the
 * re-resolved insertion anchor and retains the package proven by the planning compiler. Rejected closes missing
 * qualified identities, container movement, and unsupported declaration kinds. Raw qualified names are extracted only
 * inside this result-observation boundary.
 */
internal fun SymbolSelector.observeAddedIdentity(
    ...
    val addedContainer =
        when (val observed = qualifiedContainer()) {
    ...
    val anchorContainer =
        when (val observed = anchor.expectedAddedContainer()) {
            is InstalledQualifiedContainerObservation.Observed -> observed.value
            InstalledQualifiedContainerObservation.Rejected ->
                return InstalledObservedDeclarationIdentityObservation.Rejected
        }
    ...
    val declarationKind =
        when (kind) {
            ...
            CompilerSymbolKind.CONSTRUCTOR -> return InstalledObservedDeclarationIdentityObservation.Rejected
        }
 ...
 *
 * Observed establishes the exact compiler-qualified container where AddDeclaration places its result: inside a
 * classlike target, or beside any other declaration. Rejected closes unavailable compiler-qualified identity. Raw
 * qualified-name text remains inside result observation.
 */
private fun SymbolSelector.expectedAddedContainer(): InstalledQualifiedContainerObservation =
    if (kind == CompilerSymbolKind.CLASSLIKE) {
        when (val identity = qualifiedIdentity) {
            ...
            ExactDeclarationQualifiedIdentity.Unavailable -> InstalledQualifiedContainerObservation.Rejected
        }
    } else {
        qualifiedContainer()
    }
 ...
 *
 * Observed establishes the exact compiler-qualified container prefix after removing the selector's own canonical name.
 * Rejected closes unavailable or structurally inconsistent qualified identity. Raw qualified-name text is extracted
 * only inside this result-observation boundary.
 */
private fun SymbolSelector.qualifiedContainer(): InstalledQualifiedContainerObservation {
    val qualified =
        (qualifiedIdentity as? ExactDeclarationQualifiedIdentity.Available)?.value
    ...
    return when {
        ...
        qualified.endsWith(suffix) -> InstalledQualifiedContainerObservation.Observed(qualified.removeSuffix(suffix))
 ...
 *
 * Matched establishes the same file, declaration start, name, qualified identity, and kind across generations while
 * permitting the planned AddDeclaration to extend the target's end range. Discovery scope is request provenance rather
 * than declaration identity and remains subject to the verification service's relation-target proof.
 * [InstalledPriorDeclarationMatch.MovedOrChanged] closes every identity mismatch. Raw selector fields remain inside the
 * resulting-generation observation boundary.
 */
internal fun SymbolSelector.matchDeclarationAcrossGeneration(prior: SymbolSelector): InstalledPriorDeclarationMatch =
    if (
        file == prior.file &&
