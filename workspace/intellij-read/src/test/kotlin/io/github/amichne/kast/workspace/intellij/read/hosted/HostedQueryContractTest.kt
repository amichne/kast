...
class HostedQueryContractTest {
    ...
    fun `published wire preserves both compiler identities and request-local scope evidence`() {
        val model = (captureDetachedFixture(DetachedModelObservation.Observed(detachedModelBoundary())) as DetachedModelCapture.Captured).model
        val selection = (HostedKotlinSelection.parse(FIXTURE_ROOT, "module0/src/main/kotlin/Child.kt", 0) as Refinement.Refined).value
        ...
        fun declaration(name: String): HostedCompilerDeclaration {
            ...
            val evidence = (CompilerGroundedSymbolEvidence.fromBoundary(
                selection.file, 0, 50, name, qualified, CompilerSymbolKind.CLASSLIKE, signature,
        ...
        val publication = HostedQueryPublication(HostedQueryEndpoint(), RecordingFreshnessEpochSource().observeEpoch(), model, relation)
        ...
        for ((field, declaration) in listOf("supertype" to relation.supertype, "inheritor" to relation.inheritor)) {
            ...
            assertEquals(declaration.symbol.signature.canonicalEncoding().value, endpoint["canonicalSignature"].asString)
    ...
    fun `selection preserves the exact root and excludes external or malformed paths`() {
        val good = HostedKotlinSelection.parse(FIXTURE_ROOT, "module0/src/main/kotlin/Child.kt", 12) as Refinement.Refined
        ...
        for (path in listOf("../Child.kt", "/outside/Child.kt", "module/../Child.kt", "module//Child.kt", "module\\Child.kt", "Child.java", "./Child.kt", "x".repeat(4096) + ".kt")) {
            assertEquals(Refinement.Rejected(HostedQueryFailure.INVALID_SELECTION), HostedKotlinSelection.parse(FIXTURE_ROOT, path, 0), path)
        }
        assertEquals(Refinement.Rejected(HostedQueryFailure.INVALID_SELECTION), HostedKotlinSelection.parse(FIXTURE_ROOT, "Child.kt", -1))
    }

    @Test
    fun `scope requires a unique supported cached source-folder owner`() {
        val selection = (HostedKotlinSelection.parse(FIXTURE_ROOT, "module0/src/main/kotlin/Child.kt", 0) as Refinement.Refined).value
        fun model(roots: List<DetachedSourceRootBoundary>) = (captureDetachedFixture(DetachedModelObservation.Observed(
            detachedModelBoundary(modules = listOf(detachedModuleBoundary(sourceRoots = roots))),
        )) as DetachedModelCapture.Captured).model
        ...
        assertEquals(Refinement.Rejected(HostedQueryFailure.OUTSIDE_SCOPE), HostedSourceScope.admit(model(listOf(detachedSourceRootBoundary("other"))), selection.file))
        assertEquals(Refinement.Rejected(HostedQueryFailure.AMBIGUOUS_SCOPE), HostedSourceScope.admit(model(listOf(root, detachedSourceRootBoundary("module0/src"))), selection.file))
        assertEquals(Refinement.Rejected(HostedQueryFailure.UNSUPPORTED_MODEL), HostedSourceScope.admit(model(listOf(root.copy(provenance = DetachedSourceRootProvenance.GENERATED))), selection.file))
        assertEquals(Refinement.Rejected(HostedQueryFailure.UNSUPPORTED_MODEL), HostedSourceScope.admit(model(listOf(root.copy(kind = DetachedSourceRootKind.RESOURCE))), selection.file))
    }

    @Test
    fun `rejection wire retains the bounded failing stage and model causes`() {
        val json = JsonParser.parseString(HostedQueryWire.encode(HostedQueryResult.Rejected(
            HostedQueryFailure.ModelCapture(DetachedModelCapture.Rejected(DetachedModelCaptureFailure.SDK_UNAVAILABLE)),
