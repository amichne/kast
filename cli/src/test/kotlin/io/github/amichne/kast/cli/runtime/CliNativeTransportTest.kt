...
class CliNativeTransportTest {
    @Test
    fun `semantic runtime admission rejection prevents wire exchange`(@TempDir temporary: Path) {
        ...
        assertEquals(
            "{\"status\":\"rejected\",\"boundary\":\"runtime\"," + "\"reason\":\"endpoint-unavailable\"}",
        ...
        try {
                        ...
                        WireDecoding.Decoded(
                            SymbolDiscoverRequest(
                                SymbolDiscoverTargetDocument.Name(
                                    ProtocolText.parse("Example").refinedValue(),
                                ...
                                payload =
                                    SymbolDiscoverResult(
                                        BoundedProtocolList.create(
                                                emptyList<
                                                    io.github.amichne.kast.protocol.contract.SymbolDiscoveryDocument
                                                >()
                                            )
                                            .refinedValue()
                                    ),
            ...
            val cli =
                KastCli(
                    ...
                    rootDiscovery = FilesystemCanonicalRootDiscovery,
                    endpointLocator =
                        RuntimeEndpointLocator { discovered ->
                    ...
                    runtimeDemander =
                        RuntimeDemander { discovered, requestedEndpoint ->
    ...
    private fun testLocalMetadata(): CliLocalMetadata =
        when (
            val admitted =
    ...
    private fun WireRequestAdmission.admittedRequest() =
        when (this) {
    ...
    private fun RuntimeEndpointResolution.resolvedEndpoint(): RuntimeEndpoint =
        when (this) {
            is RuntimeEndpointResolution.Resolved -> endpoint
            is RuntimeEndpointResolution.Rejected -> error("Expected endpoint, got $failure")
        }

    private fun WireFrameRead.receivedDocument(): String =
        when (this) {
