...
class InstallSkillServiceTest {
    ...
    fun `install copies bundled skill tree and writes version marker`() {
        ...
        assertTrue(Files.isRegularFile(installedSkillDir.resolve("scripts/kast-workspace-files.sh")))
        assertTrue(Files.isRegularFile(installedSkillDir.resolve("scripts/kast-write-and-validate.sh")))
