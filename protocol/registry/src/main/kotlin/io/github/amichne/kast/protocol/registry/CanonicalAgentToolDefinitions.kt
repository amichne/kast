...
object CanonicalAgentToolDefinitions {
    ...
    private fun facade(identity: PublicToolIdentity): AgentToolDefinition {
        val operation = when (identity) {
            PublicToolIdentity.SEARCH_CLASSES, PublicToolIdentity.SEARCH_FUNCTIONS,
            PublicToolIdentity.SEARCH_DECLARATIONS, PublicToolIdentity.QUERY_SYMBOLS -> CanonicalOperationDefinitions.queryRun
            PublicToolIdentity.CHECK_DIAGNOSTICS -> CanonicalOperationDefinitions.diagnosticCheck
        }
        return AgentToolDefinition(operation, refined(AgentToolName.parse(identity.toolName)),
            refined(ProtocolText.parse(identity.description)), HostedApprovalPolicy.NONE, identity.loading,
