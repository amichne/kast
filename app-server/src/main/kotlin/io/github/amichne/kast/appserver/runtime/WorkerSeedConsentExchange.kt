...
@Serializable internal enum class WorkerSeedMessageKind { SEED_CONSENT_REQUEST, SEED_CONSENT_REPLY }
@Serializable internal data class WorkerSeedPromptDocument(
    val kind: WorkerSeedMessageKind,
    val challenge: String,
    val categories: List<WorkerSeedCategory>,
    val estimatedBytes: Long,
)
@Serializable internal data class WorkerSeedReplyDocument(
    val kind: WorkerSeedMessageKind,
    val challenge: String,
...
internal class WorkerSeedPrompt private constructor(val challenge: UUID, val disclosure: WorkerSeedDisclosure) {
    fun encode(): String = Json.encodeToString(WorkerSeedPromptDocument(WorkerSeedMessageKind.SEED_CONSENT_REQUEST, challenge.toString(), disclosure.categories.sortedBy { it.name }, disclosure.estimatedBytes))
    fun reply(consent: WorkerSeedConsent): String = Json.encodeToString(WorkerSeedReplyDocument(WorkerSeedMessageKind.SEED_CONSENT_REPLY, challenge.toString(), consent))
    fun decision(raw: String): WorkerSeedConsent = try {
        val reply = Json.decodeFromString<WorkerSeedReplyDocument>(raw)
        if (reply.kind == WorkerSeedMessageKind.SEED_CONSENT_REPLY && reply.challenge == challenge.toString()) reply.consent else WorkerSeedConsent.ABSENT
    ...
    companion object {
        ...
        fun isPrompt(raw: String): Boolean = try { Json.parseToJsonElement(raw).jsonObject["kind"] == JsonPrimitive(WorkerSeedMessageKind.SEED_CONSENT_REQUEST.name) } catch (_: Exception) { false }
        fun decode(raw: String): Refinement<WorkerSeedPrompt,WorkerControlFailure> = try {
            val value = Json.decodeFromString<WorkerSeedPromptDocument>(raw)
            val challenge = UUID.fromString(value.challenge)
            if (value.kind != WorkerSeedMessageKind.SEED_CONSENT_REQUEST || challenge.toString() != value.challenge || value.categories.distinct().size != value.categories.size)
                Refinement.Rejected(WorkerControlFailure.INVALID_REQUEST)
            else when (val disclosure = WorkerSeedDisclosure.admit(value.categories.toSet(), value.estimatedBytes)) {
...
internal class WorkerSeedConsentExchange(private val session: DefaultWebSocketServerSession) : WorkerSeedConsentAuthority {
