...
internal sealed interface IndexerTransportExecution<out Value> {
    data class Completed<Value>(val value: Value) : IndexerTransportExecution<Value>
 ...
 *
 * The platform invokes [KastIndexerApplicationStarter] with startup cancellation state installed on its worker thread.
 * A long-lived request must not inherit that completed job: synchronous IntelliJ effects would otherwise observe
 * cancellation after startup and abort valid requests.
 */
internal data object DetachedIndexerTransportExecutor {
    /**
     * Proof transition: `operation -> IndexerTransportExecution<Value>`.
     *
     * Establishes execution on one named platform thread with inherited thread-local state disabled. Expected thread
     * creation, execution, and interruption failures remain finite data.
     */
    fun <Value> execute(operation: () -> Value): IndexerTransportExecution<Value> {
        val task = FutureTask(operation)
        val thread =
            try {
                Thread.ofPlatform().name(TRANSPORT_THREAD_NAME).inheritInheritableThreadLocals(false).unstarted(task)
