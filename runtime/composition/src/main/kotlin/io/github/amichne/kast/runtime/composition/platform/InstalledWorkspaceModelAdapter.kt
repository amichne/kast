...
internal class InstalledGradleWorkspaceModel
private constructor(
    ...
    companion object {
         ...
         * WorkspaceSearchScopeModelCompilation.Compiled) -> Refinement< InstalledGradleWorkspaceModel,
         * InstalledGradleWorkspaceModelFailure>`.
         *
         * Establishes one exact root and identical Gradle owner/source-root sets across workspace publication and
         * semantic search-scope authority. [InstalledGradleWorkspaceModelFailure] closes root or source-root
         * disagreement. Raw path comparison is confined to this model adapter boundary.
         */
        fun admit(
            ...
            if (searchScope.model.workspaceRoot != root) {
                return Refinement.Rejected(InstalledGradleWorkspaceModelFailure.MODEL_ROOT_MISMATCH)
            }
            val published =
                sourceRoots.mapTo(linkedSetOf()) { sourceRoot ->
            ...
            val semantic =
                searchScope.model.sourceRoots.mapTo(linkedSetOf()) { sourceRoot ->
                    InstalledSourceRootIdentity(
            ...
            if (published != semantic) {
                return Refinement.Rejected(InstalledGradleWorkspaceModelFailure.MODEL_SOURCE_ROOT_MISMATCH)
            }
            return Refinement.Refined(InstalledGradleWorkspaceModel(root, state, sourceRoots.toList(), searchScope))
        }
    }
}
...
internal sealed interface InstalledGradleModelRead {
    ...
    data class Unavailable(val failure: InstalledGradleModelFailure) : InstalledGradleModelRead
}
...
internal class InstalledWorkspaceModelAdapter(private val models: InstalledGradleModelReadOperations) :
    ...
    override fun capture(
        ...
        when (val read = refresh()) {
            is InstalledGradleModelRead.Captured ->
                if (read.model.root == root) {
    ...
    override fun reconcile(candidate: WorkspaceCandidate): IntellijWorkspaceReconciliationResult =
        when (val read = refresh()) {
            is InstalledGradleModelRead.Captured ->
                if (read.model.root == candidate.root && read.model.state == candidate.sourceState) {
            ...
            is InstalledGradleModelRead.Unavailable -> IntellijWorkspaceReconciliationResult.Unavailable
        }

    fun searchScope(lease: SemanticReadLease): WorkspaceSearchScopeModelCompilation =
        when (val current = synchronized(lock) { state }) {
            is State.Ready ->
                if (current.model.root == lease.workspaceRoot) {
    ...
    private fun refresh(): InstalledGradleModelRead =
        models.read().also { read ->
            synchronized(lock) {
                state =
                    when (read) {
    ...
    private sealed interface State {
        ...
        data class Ready(val model: InstalledGradleWorkspaceModel) : State
    }
}
