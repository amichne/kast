...
object InstalledWorkspaceConfigurationIngress {
    ...
    ): Refinement<ResolvedKastConfiguration, WorkspaceConfigurationRejection> = when (
        val loaded = load(installationRoot, workspaceRoot, environment)
    ) {
        is SavedConfigurationIngress.Rejected -> Refinement.Rejected(WorkspaceConfigurationRejection.Source(loaded.rejection))
        is SavedConfigurationIngress.Loaded -> when (val resolved = ResolvedKastConfiguration.resolve(loaded.sources)) {
            is Refinement.Refined -> resolved
            is Refinement.Rejected -> Refinement.Rejected(WorkspaceConfigurationRejection.Configuration(resolved.failure))
        }
    }
    ...
    fun load(installationRoot: Path, workspaceRoot: Path, environment: Map<String, String>): SavedConfigurationIngress {
        ...
        if (explicit != null && explicit != installationFile.toString()) return rejected(SavedConfigurationIngressFailure.INVALID_SELECTOR)
        val installation = when (val read = optional(installationRoot, installationFile, environment)) {
            is SavedConfigurationIngress.Rejected -> return read
            is SavedConfigurationIngress.Loaded -> read
        }
        if (explicit != null && installation.observation == SavedConfigurationObservation.ABSENT) return rejected(SavedConfigurationIngressFailure.UNAVAILABLE)
        val identity = MessageDigest.getInstance("SHA-256").digest(workspaceRoot.toString().toByteArray(Charsets.UTF_8))
            .joinToString("") { "%02x".format(it) }
        val overlay = when (val read = optional(installationRoot, installationRoot.resolve("config/workspaces/$identity/environment"), emptyMap())) {
            is SavedConfigurationIngress.Rejected -> return read
            is SavedConfigurationIngress.Loaded -> read
        }
        return SavedConfigurationIngress.Loaded(
            installation.sources.copy(savedWorkspace = overlay.sources.savedInstallation),
            installation.observation, overlay.observation,
        )
    }
    ...
    private fun optional(root: Path, file: Path, environment: Map<String, String>): SavedConfigurationIngress {
        ...
        for (component in root.relativize(file)) {
            ...
            val attributes = try { Files.readAttributes(current, BasicFileAttributes::class.java, NOFOLLOW_LINKS) }
            catch (_: NoSuchFileException) {
                return SavedConfigurationIngress.Loaded(ConfigurationSources(environment = environment), SavedConfigurationObservation.ABSENT)
            } catch (_: IOException) { return rejected(SavedConfigurationIngressFailure.UNAVAILABLE) }
            catch (_: SecurityException) { return rejected(SavedConfigurationIngressFailure.UNAVAILABLE) }
            if (attributes.isSymbolicLink || (current != file && !attributes.isDirectory)) return rejected(SavedConfigurationIngressFailure.NOT_REGULAR)
        }
