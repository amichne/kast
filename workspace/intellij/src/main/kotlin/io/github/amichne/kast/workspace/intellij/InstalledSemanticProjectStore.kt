...
internal class InstalledSemanticProjectStore
private constructor(
    val path: Path,
    ...
    companion object {
        /**
         * Proof transition: `(CanonicalWorkspaceRoot, Path) -> InstalledSemanticProjectStorePreparation`.
         *
         * Prepared proves a newly created physical directory under runtime state that is disjoint from the Gradle
         * workspace and contains only runtime-generated pre-open index exclusions. Rejected closes overlap, filesystem,
         * exclusion-discovery, configuration, and identity failures. No existing project configuration is admitted.
         */
        fun prepare(
            ...
            val created =
                try {
                    Files.createTempDirectory(runtimeStateDirectory, SEMANTIC_PROJECT_STORE_PREFIX).toRealPath()
                } catch (_: IOException) {
            ...
            val exclusionPlan =
                when (val discovery = InstalledIndexExclusionPlan.discover(workspacePath)) {
                    is InstalledIndexExclusionPlanDiscovery.Discovered -> discovery.plan
                    is InstalledIndexExclusionPlanDiscovery.Rejected ->
                        return rejected(
            ...
            val indexBootstrap =
                when (val preparation = InstalledIndexBootstrap.prepare(created, exclusionPlan)) {
                    is InstalledIndexBootstrapPreparation.Prepared -> preparation.bootstrap
                    is InstalledIndexBootstrapPreparation.Rejected ->
                        return rejected(
            ...
            val root =
                when (
                    ...
                    is Refinement.Rejected -> return rejected(InstalledSemanticProjectStoreFailure.IDENTITY_REJECTED)
                }
...
internal sealed interface InstalledSemanticProjectStorePreparation {
    data class Prepared(val store: InstalledSemanticProjectStore) : InstalledSemanticProjectStorePreparation

    data class Rejected(val failure: InstalledSemanticProjectStoreFailure) : InstalledSemanticProjectStorePreparation
}

private fun rejected(failure: InstalledSemanticProjectStoreFailure): InstalledSemanticProjectStorePreparation.Rejected =
