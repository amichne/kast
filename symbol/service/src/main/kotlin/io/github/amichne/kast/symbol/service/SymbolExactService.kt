...
class SymbolExactService(
    ...
    constructor(
        ...
        compiler: SymbolExactCompilerPort,
    ) : this(workspaces.semanticReadValidation(), compiler)

    /**
     * Proof transition: `(WorkspaceRuntimeState, SymbolResolutionRequest, SymbolResolutionCompilation) ->
     * SymbolResolutionResult`.
     *
     * A resolved result establishes that the batch-owned selection remained current before and after compiler
     * resolution and that the returned selector retains its exact lease, scope, file, name, and offset.
     * [SymbolExactRejection] is the closed expected failure. Workspace observation and compiler execution are the only
     * effect boundaries.
     */
    override suspend fun resolve(request: SymbolResolutionRequest): SymbolResolutionResult {
        when (
            ...
            is SymbolExactLeaseAdmission.Rejected -> return SymbolResolutionResult.Rejected(admission.reason)
        }
        val selector =
            when (val compilation = compiler.resolve(request)) {
        ...
        return when (admitSelector(request.selection, selector)) {
            SymbolSelectorAdmission.Admitted -> SymbolResolutionResult.Resolved(ResolvedSymbol(selector))
            SymbolSelectorAdmission.Rejected ->
                SymbolResolutionResult.Rejected(SymbolExactRejection.COMPILER_CONTRACT_VIOLATION)
        }
    }

    /**
     * Proof transition: `(WorkspaceRuntimeState, ExactSymbolRequest, SymbolDescriptionCompilation) ->
     * SymbolDescriptionResult`.
     *
     * A described result establishes that the selector remained current before and after native compiler revalidation
     * and that the detached description retains the same exact selector. [SymbolExactRejection] is the closed expected
     * failure. Workspace observation and compiler execution are the only effect boundaries.
     */
    override suspend fun describe(request: ExactSymbolRequest): SymbolDescriptionResult {
        when (
            ...
            SymbolExactLeaseAdmission.Admitted -> Unit
            is SymbolExactLeaseAdmission.Rejected -> return SymbolDescriptionResult.Rejected(admission.reason)
        }
        val description =
            when (val compilation = compiler.describe(request)) {
        ...
        return if (description.selector === request.selector) {
            ...
            SymbolDescriptionResult.Rejected(SymbolExactRejection.COMPILER_CONTRACT_VIOLATION)
        }
    }
     ...
     *
     * [SymbolExactLeaseAdmission.Admitted] establishes that a ready publication has the exact expected canonical root
     * and generation. [SymbolExactLeaseAdmission.Rejected] preserves unavailable, root-mismatch, or stale-generation
     * failure as closed data. Raw root and generation extraction remains inside workspace publication.
     */
    private suspend fun admitCurrentLease(
        ...
        when (authorities.validate(expected)) {
            ...
            SemanticReadValidation.MOVED -> SymbolExactLeaseAdmission.Rejected(SymbolExactRejection.STALE_GENERATION)
        }
}
...
private sealed interface SymbolExactLeaseAdmission {
    ...
    data class Rejected(val reason: SymbolExactRejection) : SymbolExactLeaseAdmission
}
 ...
 *
 * Admission establishes exact preservation of the selected lease, scope, file, name, and source offset. Rejection is
 * closed and becomes a compiler-contract violation at the public boundary.
 */
...
private fun SymbolExactCompilerRejection.toPublicRejection(): SymbolExactRejection =
    when (this) {
        SymbolExactCompilerRejection.WORKSPACE_ROOT_MISMATCH -> SymbolExactRejection.WORKSPACE_ROOT_MISMATCH
        ...
        SymbolExactCompilerRejection.UNSUPPORTED_DECLARATION -> SymbolExactRejection.UNSUPPORTED_DECLARATION
        SymbolExactCompilerRejection.COMPILER_IDENTITY_UNAVAILABLE -> SymbolExactRejection.COMPILER_IDENTITY_UNAVAILABLE
        SymbolExactCompilerRejection.DECLARATION_MOVED_OR_CHANGED -> SymbolExactRejection.DECLARATION_MOVED_OR_CHANGED
        SymbolExactCompilerRejection.INTERNAL_INVARIANT -> SymbolExactRejection.COMPILER_CONTRACT_VIOLATION
    }
