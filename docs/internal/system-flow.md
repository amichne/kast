...
```mermaid
flowchart TD
    caller["User or agent"] --> cli["kast CLI"]
    installer["install.sh bootstrap"] --> setup["kast setup"]
    hooks["Codex SessionStart or PostToolUse"] --> cli

    cli --> parse["Clap parsing and typed command dispatch"]
    parse --> local["Local control path"]
    parse --> agent["Agent envelope"]
    parse --> lifecycle["Runtime lifecycle path"]

    local --> setup
    local --> localState["Receipt, configuration, filesystem, and contract sources"]
    setup --> authority["Validated bundle, active receipt, current release"]

    lifecycle --> admission["Exact-root admission and backend selection"]
    agent --> localIndex["Local repository and source-index handlers"]
    agent --> admission
    admission --> host["Reuse or start one IDEA or headless runtime"]
    host --> transport["Descriptor and local transport"]
    transport --> server["AnalysisServer and RpcAnalysisDispatcher"]
    server --> router["Capability gate and typed method router"]
    router --> skill["SkillRpcOrchestrator"]
    skill --> backend["AnalysisBackend"]

    backend --> idea["IDEA host: PSI, K2, read actions, write actions"]
    backend --> headless["Headless host: the same IDEA backend runtime"]
    idea --> index["Reference index and SQLite semantic graph"]
    headless --> index

    authority --> result["Typed state or result"]
    localState --> result
    localIndex --> result
    index --> result
    result --> projection["CLI projection: human, TOON, or JSON compatibility"]
    projection --> answer["Caller receives data, coverage, limitations, and next actions"]
```
...
kast agent symbol \
  --workspace-root "$PWD" \
  --query io.github.amichne.kast.api.contract.AnalysisBackend \
  --kind interface \
  --explain
```
