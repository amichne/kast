...
internal suspend fun ensureWorkerService(
    ...
    when (context) {
        ...
        BrokerServiceDemandContext.Rejected ->
...
class InstalledWorkerClient(
        ...
        exchange(WorkerControlDocument.demand(root, heap, startup), startService = true, ::rejected) { raw ->
            when (val result = WorkerControlReply.decode(raw)) {
                ...
                is InstalledWorkerStart.Ready ->
                    when (val binding = result.binding) {
    ...
    override suspend fun retired(root: Path): InstalledWorkerStop =
        exchange(WorkerControlDocument.retired(root), startService = false, InstalledWorkerStop::Rejected) { raw ->
            when (val result = WorkerStopReply.decode(raw)) {
                ...
                is InstalledWorkerStop.Stopped ->
                    if (result.root == root) Refinement.Refined(Decoded(result, result.binding))
    ...
    internal suspend fun status(command: BrokerServiceLaunchCommand): CoordinatorStatusRead {
        val publication =
            when (val read = published(command, requireDesired = false)) {
        ...
        return withTimeoutOrNull(OperationExecutionBudget.LOCAL_QUALIFICATION.value) {
            ...
            try {
                if (
                    connection.send(Json.encodeToString(WorkerControlDocument(WorkerControlAction.STATUS, ""))) !=
                ...
                val document =
                    when (val frame = connection.receive()) {
                        is BrokerUpstreamFrame.Text ->
                            try {
                                Json.parseToJsonElement(frame.message).jsonObject
                            } catch (_: Exception) {
                                return@withTimeoutOrNull CoordinatorStatusRead.Rejected(
                ...
                if (
                    document["status"] != JsonPrimitive("READY") ||
                        document["serviceGeneration"] != JsonPrimitive(publication.generation) ||
                        document["installationId"] != JsonPrimitive(publication.installation) ||
                        document["stateEpoch"] != JsonPrimitive(publication.epoch)
                )
    ...
    private suspend fun <T> exchange(
        request: WorkerControlDocument,
        startService: Boolean,
        reject: (WorkerControlFailure) -> T,
        decode: (String) -> Refinement<Decoded<T>, WorkerControlFailure>,
    ): T {
        ...
        if (startService) {
            ...
            val service =
                ensureWorkerService(demandContext) {
        ...
        val selectedRequest =
            if (request.action == WorkerControlAction.DEMAND) {
                ...
                request.copy(
                    heap = "${selected.indexerHeap.mebibytes}m",
                    configurationIdentity = workerConfigurationIdentity(selected),
                )
            } else request
        val published =
            when (val observed = published(command, requireDesired = startService)) {
            ...
            when (
                val connected =
                    connectCodexUnixWebSocket(
                        command.publicSocket,
                        CoordinatorStatusProtocol.maximumMessageBytes,
                        BrokerOperationalLimits.clientConnect.value,
                        BrokerControlRoute.RUNTIME,
                    )
            ) {
                is BrokerUpstreamConnectionAdmission.Connected -> connected.connection
        ...
        return try {
            withTimeoutOrNull(OperationExecutionBudget.WORKSPACE_READINESS.value) {
                if (
                    ...
                    return@withTimeoutOrNull reject(WorkerControlFailure.UNAVAILABLE)
                val status =
                    when (val frame = connection.receive()) {
                        is BrokerUpstreamFrame.Text ->
                            try {
                                ...
                                return@withTimeoutOrNull reject(WorkerControlFailure.COORDINATOR_IDENTITY_REJECTED)
                            }
                        else -> return@withTimeoutOrNull reject(WorkerControlFailure.UNAVAILABLE)
                    }
                if (
                    status["status"] != JsonPrimitive("READY") ||
                        status["serviceGeneration"] != JsonPrimitive(published.generation) ||
                        status["installationId"] != JsonPrimitive(published.installation) ||
                        status["stateEpoch"] != JsonPrimitive(published.epoch) ||
                        (startService && status["configurationIdentity"] != JsonPrimitive(published.configuration))
                )
                    return@withTimeoutOrNull reject(WorkerControlFailure.COORDINATOR_IDENTITY_REJECTED)
                if (connection.send(Json.encodeToString(selectedRequest)) != BrokerUpstreamSend.SENT)
                    return@withTimeoutOrNull reject(WorkerControlFailure.UNAVAILABLE)
    ...
    private suspend fun <T> receiveResult(
        ...
        while (true) {
            ...
            if (WorkerSeedPrompt.isPrompt(frame.message)) {
                if (
                    prompted ||
                        request.action != WorkerControlAction.DEMAND ||
                        request.startup != WorkerStartupAction.SEED ||
                        request.seedConsent != WorkerSeedConsentSelection.INTERACTIVE
                )
                    return reject(WorkerControlFailure.INVALID_REQUEST)
                prompted = true
                val prompt =
                    when (val admitted = WorkerSeedPrompt.decode(frame.message)) {
                        is Refinement.Refined -> admitted.value
                        is Refinement.Rejected -> return reject(admitted.failure)
                    }
                val decision =
                    withTimeoutOrNull(BrokerOperationalLimits.seedConsent.value) {
                        seedConsent.request(prompt.disclosure)
                    } ?: WorkerSeedConsent.ABSENT
                if (connection.send(prompt.reply(decision)) != BrokerUpstreamSend.SENT)
                    return reject(WorkerControlFailure.UNAVAILABLE)
            } else
                return when (val decoded = decode(frame.message)) {
                    is Refinement.Rejected -> reject(decoded.failure)
                    is Refinement.Refined -> {
                        val binding = decoded.value.binding
                        if (
                            binding.installationId != published.installation ||
                                binding.stateEpoch.toString() != published.epoch ||
                                binding.serviceGeneration.toString() != published.generation ||
                                (request.action == WorkerControlAction.DEMAND &&
                                    binding.configurationIdentity != request.configurationIdentity)
                        )
    ...
    private fun published(
        command: BrokerServiceLaunchCommand,
        requireDesired: Boolean = true,
    ): Refinement<Published, WorkerControlFailure> {
        return try {
            ...
            if (
                state !is BrokerServiceStateDocument.Ready ||
                    state.schemaVersion != BROKER_SERVICE_STATE_SCHEMA_VERSION ||
                    (requireDesired && state.serviceIdentity != command.identity.value)
            )
...
@Serializable
internal enum class WorkerControlAction {
    STATUS,
    DEMAND,
    RETIRED,
}
...
@Serializable
internal enum class WorkerStartupAction {
    REUSE,
    REBUILD,
    SEED,
}
...
@Serializable
internal data class WorkerControlDocument(
    ...
    fun startupRequest(): InstalledWorkerStartup =
        when (startup) {
            ...
            WorkerStartupAction.SEED ->
                InstalledWorkerStartup.Seed(ideHome?.let(Path::of), sourceSystem?.let(Path::of), seedConsent)
        }

    companion object {
        fun demand(root: Path, heap: IndexerHeapSize, startup: InstalledWorkerStartup) =
            WorkerControlDocument(
                ...
                startup.ideHome?.toString(),
                (startup as? InstalledWorkerStartup.Seed)?.sourceSystem?.toString(),
                seedConsent =
                    (startup as? InstalledWorkerStartup.Seed)?.consent ?: WorkerSeedConsentSelection.PREGRANTED,
            )
...
@Serializable
internal data class WorkerControlReply(
    ...
    companion object {
        fun encode(result: InstalledWorkerStart): String =
            Json.encodeToString(
                when (result) {
                    is InstalledWorkerStart.Rejected -> WorkerControlReply(failure = result.failure)
                    is InstalledWorkerStart.Ready ->
                        when (val binding = result.binding) {
                            WorkerRouteBinding.EffectBoundary ->
                                WorkerControlReply(failure = WorkerControlFailure.IDENTITY_REJECTED)
                            is WorkerRouteBinding.Installation ->
                                WorkerControlReply(
                                    root = result.endpoint.root.toString(),
                                    runtimeId = result.endpoint.runtimeId.value,
                                    socket = result.endpoint.socket.toString(),
                                    attempt = result.endpoint.attempt.value,
                                    installationId = binding.installationId,
                                    stateEpoch = binding.stateEpoch.toString(),
                                    serviceGeneration = binding.serviceGeneration.toString(),
        ...
        fun decode(raw: String): InstalledWorkerStart =
            try {
                ...
                when {
                    value.failure != null &&
                        listOf(
                            ...
                            .all { it == null } -> rejected(value.failure)
                    value.failure == null &&
                        value.root != null &&
                        ...
                        value.stateEpoch != null &&
                        value.serviceGeneration != null &&
                        value.configurationIdentity != null &&
                        ...
                        if (
                            runtime !is Refinement.Refined ||
                                attempt !is Refinement.Refined ||
                                binding !is Refinement.Refined
                        )
                            rejected(WorkerControlFailure.IDENTITY_REJECTED)
                        else
                            when (
                                val endpoint =
...
internal fun workerLaunchConfigurationIdentity(
    ...
    val material =
        kotlinx.serialization.json
            .JsonObject(values.toSortedMap().mapValues { kotlinx.serialization.json.JsonPrimitive(it.value) })
            .toString()
    return java.util.HexFormat.of()
        .formatHex(java.security.MessageDigest.getInstance("SHA-256").digest(material.toByteArray(Charsets.UTF_8)))
}

internal fun workerConfigurationIdentity(
    configuration: io.github.amichne.kast.distribution.contract.configuration.ResolvedKastConfiguration
): String {
    val values =
        configuration
