...
require_contains "$release_workflow" "Build native Kast CLI release asset" "Release must build a native CLI asset when GraalVM is available"
require_contains "$release_workflow" ":kast-cli:nativeCompile" "Release must compile the native CLI before packaging the release asset"
...
require_contains "$release_workflow" "--native-binary \"kast-cli/build/native/nativeCompile/kast\"" "Release must package the GraalVM native binary into the CLI asset"
...
require_not_contains "$release_workflow" "--shrink" "Release workflow must not publish ProGuard/R8-shrunk runtime libs"
require_not_contains "$release_workflow" "kast.shrinkRuntime=true" "Release workflow must not enable the ProGuard/R8 shrink property"
