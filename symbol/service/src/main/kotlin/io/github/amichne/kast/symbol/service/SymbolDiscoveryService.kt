...
class SymbolDiscoveryService(
    ...
    /**
     * Proof transition: `(WorkspaceRuntimeState, SymbolDiscoveryRequest, SymbolCompilation) -> SymbolDiscoveryResult`.
     *
     * A discovered result establishes that only the current published lease reached compiler work and that the returned
     * scope, lease, candidate kind, and measured work remain within the request. [SymbolDiscoveryRejection] is the
     * closed expected failure. Workspace observation and compiler execution are the only outer effect boundaries.
     */
    ...
    private suspend fun discoverObserved(request: SymbolDiscoveryRequest): SymbolDiscoveryResult {
        when (authorities.validate(request.scope.lease)) {
            ...
            SemanticReadValidation.UNAVAILABLE ->
                return SymbolDiscoveryResult.Rejected(SymbolDiscoveryRejection.WORKSPACE_NOT_READY)
        ...
        return when (val compilation = compiler.compile(request)) {
            is SymbolCompilation.Rejected ->
                SymbolDiscoveryResult.Rejected(
                    when (compilation.reason) {
                        SymbolCompilerRejection.SCOPE_REJECTED -> SymbolDiscoveryRejection.SCOPE_REJECTED
                        SymbolCompilerRejection.WORKSPACE_INDEX_UNAVAILABLE ->
                            SymbolDiscoveryRejection.WORKSPACE_INDEX_UNAVAILABLE
                        SymbolCompilerRejection.PROVIDER_UNAVAILABLE -> SymbolDiscoveryRejection.PROVIDER_UNAVAILABLE
    ...
    /**
     * Proof transition: `(SymbolDiscoveryRequest, SymbolDiscoveryOutcome, WorkspaceRuntimeState) ->
     * SymbolDiscoveryResult`.
     *
     * A discovered result establishes that readiness remained current through compilation and that lease, scope, kind,
     * and work measures match the request. [SymbolDiscoveryRejection] is the closed expected failure. The final
     * workspace observation is the only effect boundary.
     */
...
private fun SymbolDiscoveryResult.traceObservation(): KastSpanObservation =
    when (this) {
        is SymbolDiscoveryResult.Discovered ->
            when (val discovered = outcome) {
        ...
        is SymbolDiscoveryResult.Rejected ->
            KastSpanObservation(
                KastSpanCompletion.Rejected(
                    when (reason) {
                        SymbolDiscoveryRejection.WORKSPACE_NOT_READY -> KastSpanFailure.SYMBOL_WORKSPACE_NOT_READY
                        SymbolDiscoveryRejection.STALE_GENERATION -> KastSpanFailure.SYMBOL_STALE_GENERATION
                        SymbolDiscoveryRejection.SCOPE_REJECTED,
                        SymbolDiscoveryRejection.WORKSPACE_INDEX_UNAVAILABLE,
                        SymbolDiscoveryRejection.PROVIDER_UNAVAILABLE,
                        SymbolDiscoveryRejection.COMPILER_CONTRACT_VIOLATION -> KastSpanFailure.SYMBOL_QUERY_REJECTED
                    }
                )
            )
    }
...
private fun SymbolDiscoveryBatch.measurements(): Set<KastSpanMeasurement> =
    setOf(
