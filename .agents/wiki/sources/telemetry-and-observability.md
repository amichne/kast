...
This page summarizes the raw note [[Telemetry-and-Observability]]. It is the main source for Kast's local observability
story.
...
This note explains telemetry configuration, scopes, detail levels, span instrumentation, and JSONL export. Its main
contribution is showing that the standalone backend can be inspected through plain local files without depending on an
external collector.

It also shows that observability is scoped and opt-in, which limits overhead and keeps the common path simple.
...
- Instrumentation is designed around selective inspection rather than permanent always-on tracing.
