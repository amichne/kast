...
internal class HostedToolCatalog private constructor(val definitions: List<HostedToolDefinition>) {
    companion object {
        internal fun qualify(
            ...
            if (definitions.isEmpty()) {
                return HostedToolCatalogQualification.Rejected(HostedToolCatalogFailure.EMPTY)
            }
            if (
                definitions.groupBy(HostedToolDefinition::operation).values.any { presentations ->
                    val owner = presentations.first()
                    presentations.any {
                        it.effect != owner.effect ||
                            it.approval != owner.approval ||
                            it.executionBudget != owner.executionBudget ||
                            it.outputSchema.digest != owner.outputSchema.digest
                    }
                }
            ) {
            ...
            if (definitions.map { it.name.value }.toSet().size != definitions.size) {
                return HostedToolCatalogQualification.Rejected(HostedToolCatalogFailure.DUPLICATE_NAME)
            }
            val advertised = definitions.mapTo(linkedSetOf()) { it.name.value }
            val executable = executableRoutes.mapTo(linkedSetOf()) { it.value }
            return when {
                advertised.any { it !in executable } ->
                    HostedToolCatalogQualification.Rejected(HostedToolCatalogFailure.ADVERTISED_ROUTE_MISSING)
                executable.any { it !in advertised } ->
                    HostedToolCatalogQualification.Rejected(HostedToolCatalogFailure.UNADVERTISED_ROUTE_PRESENT)
...
internal class AgentSessionBootstrap
private constructor(
    ...
    companion object {
        internal fun qualify(
            ...
            executableRoutes: Set<ToolName>,
        ): AgentSessionBootstrapQualification =
            when (val qualification = HostedToolCatalog.qualify(definitions, executableRoutes)) {
                is HostedToolCatalogQualification.Qualified ->
                    AgentSessionBootstrapQualification.Qualified(AgentSessionBootstrap(qualification.catalog, policy))
