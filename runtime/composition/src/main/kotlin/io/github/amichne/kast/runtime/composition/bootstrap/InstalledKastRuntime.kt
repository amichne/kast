...
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `Path -> Refinement<InstalledWorkspaceRoot, InstalledWorkspaceRootFailure>`.
         *
         * Establishes an absolute, normalized, physically canonical, non-symlinked directory with one regular
         * `settings.gradle.kts` marker. [InstalledWorkspaceRootFailure] is the closed expected failure. Raw path
         * extraction is permitted only at installed IntelliJ project bootstrap and physical workspace adapter
         * boundaries.
         */
        fun admit(raw: Path): Refinement<InstalledWorkspaceRoot, InstalledWorkspaceRootFailure> {
            val normalized =
                when {
                    !raw.isAbsolute -> return Refinement.Rejected(InstalledWorkspaceRootFailure.NOT_ABSOLUTE)
                    raw.normalize() != raw -> return Refinement.Rejected(InstalledWorkspaceRootFailure.NOT_NORMALIZED)
                    else -> raw
                }
            ...
            if (Files.isSymbolicLink(settings) || !Files.isRegularFile(settings, LinkOption.NOFOLLOW_LINKS)) {
                return Refinement.Rejected(InstalledWorkspaceRootFailure.SETTINGS_MARKER_UNAVAILABLE)
            }
            val canonical =
                when (val admitted = CanonicalWorkspaceRoot.fromCanonicalPath(physical)) {
                    is Refinement.Refined -> admitted.value
                    is Refinement.Rejected -> return Refinement.Rejected(InstalledWorkspaceRootFailure.UNAVAILABLE)
                }
...
class InstalledRuntimeStateDirectory private constructor(internal val path: Path) {
    companion object {
        /**
         ...
         * [InstalledRuntimeStateDirectoryFailure] is the closed expected failure. Raw extraction is permitted only
         * while opening composition-owned persistence.
         */
        fun admit(raw: Path): Refinement<InstalledRuntimeStateDirectory, InstalledRuntimeStateDirectoryFailure> {
            val normalized =
                when {
                    !raw.isAbsolute -> return Refinement.Rejected(InstalledRuntimeStateDirectoryFailure.NOT_ABSOLUTE)
                    raw.normalize() != raw ->
                        return Refinement.Rejected(InstalledRuntimeStateDirectoryFailure.NOT_NORMALIZED)
                    else -> raw
                }
            if (Files.isSymbolicLink(normalized)) {
...
class InstalledKastRuntimeRequest
...
internal sealed interface InstalledRuntimeObservabilityRequest {
    ...
    data class Sidecar(val socketPath: Path) : InstalledRuntimeObservabilityRequest
}
...
sealed interface InstalledRuntimeTelemetryFailure {
    data class Output(val failure: SidecarTelemetryOutputFailure) : InstalledRuntimeTelemetryFailure

    data class Forwarding(val failure: OpenTelemetryFileForwardingFailure) : InstalledRuntimeTelemetryFailure
}
...
class InstalledRuntimeIndexScope
internal constructor(
    ...
    fun processDiagnostic(): String = "workspaceRoot=${workspaceRoot.value}, sourceRoots=$sourceRoots"
}

/** Finite installed construction failures visible to the isolated host. */
sealed interface InstalledKastRuntimeFailure {
    data class WorkspaceRoot(val failure: InstalledWorkspaceRootFailure) : InstalledKastRuntimeFailure

    data class StateDirectory(val failure: InstalledRuntimeStateDirectoryFailure) : InstalledKastRuntimeFailure

    data class Telemetry(val failure: InstalledRuntimeTelemetryFailure) : InstalledKastRuntimeFailure

    data class Assembly(val failure: InstalledRuntimeAssemblyFailure) : InstalledKastRuntimeFailure
}

/** Closed production-graph assembly failures. */
sealed interface InstalledRuntimeAssemblyFailure {
    data class Persistence(val failure: InstalledRuntimePersistenceFailure) : InstalledRuntimeAssemblyFailure

    data class WorkspacePublication(val failure: InstalledRuntimeWorkspaceFailure) : InstalledRuntimeAssemblyFailure

    data class Composition(val failures: Set<KastRuntimeCompositionFailure>) : InstalledRuntimeAssemblyFailure
}
...
sealed interface InstalledRuntimeWorkspaceFailure {
    ...
    data class IntellijBootstrap(val failure: InstalledIntellijWorkspaceFailure) : InstalledRuntimeWorkspaceFailure

    data class ModelRefinementUnavailable(val failure: InstalledGradleModelFailure) : InstalledRuntimeWorkspaceFailure
...
sealed interface InstalledKastRuntimeConstruction {
    data class Created(val dispatch: KastRuntimeDispatchOperations) : InstalledKastRuntimeConstruction

    data class Rejected(val failures: Set<InstalledKastRuntimeFailure>) : InstalledKastRuntimeConstruction
}

internal sealed interface InstalledRuntimeAssembly {
    data class Assembled(val dispatch: KastRuntimeDispatchOperations) : InstalledRuntimeAssembly

    data class Rejected(val failure: InstalledRuntimeAssemblyFailure) : InstalledRuntimeAssembly
}
...
object InstalledKastRuntime {
     ...
     *
     * Establishes exact physical workspace/state admission followed by the sole production IntelliJ/K2 assembly.
     * [InstalledKastRuntimeFailure] closes every admission, bootstrap, persistence, publication, handler, and
     * composition failure. Raw paths leave only for those named outer boundaries; callers receive only dispatch
     * authority.
     */
    fun create(
        workspaceRoot: Path,
    ...
    /**
     * Proof transition: `(Path, Path, InstalledRuntimeBootstrapObserver) -> InstalledKastRuntimeConstruction`.
     *
     * Preserves the same exact-root runtime proof while making every production bootstrap phase an explicit effect
     * owned by the caller.
     */
    fun create(
        workspaceRoot: Path,
    ...
    /**
     * Proof transition: `(Path, Path, Path, InstalledRuntimeBootstrapObserver) -> InstalledKastRuntimeConstruction`.
     *
     * Preserves exact-root construction while deriving and opening the default telemetry destination from the
     * already-admitted exact sidecar socket. Telemetry failures remain closed construction data and the process owner
     * receives no telemetry implementation type.
     */
    fun create(
        workspaceRoot: Path,
        stateDirectory: Path,
    ...
    /**
     * Proof transition: `(Path, Path, InstalledRuntimeAssembler) -> InstalledKastRuntimeConstruction`.
     *
     * Establishes exact physical workspace and state ownership before granting either value to the composition-owned
     * assembler. [InstalledKastRuntimeFailure] is the closed expected failure. Raw paths remain confined to this
     * installed admission boundary.
     */
    internal fun create(
        workspaceRoot: Path,
        stateDirectory: Path,
    ...
    internal fun create(
        ...
        val root =
            when (val admitted = InstalledWorkspaceRoot.admit(workspaceRoot)) {
                is Refinement.Refined -> admitted.value
        ...
        val state =
            when (val admitted = InstalledRuntimeStateDirectory.admit(stateDirectory)) {
                is Refinement.Refined -> admitted.value
                is Refinement.Rejected -> {
        ...
        val observability =
            when (observabilityRequest) {
                InstalledRuntimeObservabilityRequest.Disabled -> KastObservability.Disabled
                is InstalledRuntimeObservabilityRequest.Sidecar ->
                    when (
                        val output = SidecarTelemetryOutput.fromSocketPath(observabilityRequest.socketPath.toString())
                    ) {
                        is Refinement.Rejected ->
                        ...
                        is Refinement.Refined ->
                            when (val opened = OpenTelemetryFileForwarding.open(output.value)) {
        ...
        return when (val assembly = assembler.assemble(request)) {
            is InstalledRuntimeAssembly.Assembled -> InstalledKastRuntimeConstruction.Created(assembly.dispatch)
            is InstalledRuntimeAssembly.Rejected ->
                InstalledKastRuntimeConstruction.Rejected(setOf(InstalledKastRuntimeFailure.Assembly(assembly.failure)))
        }
    }
}
