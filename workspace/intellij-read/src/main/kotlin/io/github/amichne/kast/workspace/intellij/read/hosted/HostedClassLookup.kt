...
value class HostedClassName private constructor(val value: String) {
    companion object {
        internal fun parse(raw: String): Refinement<HostedClassName, HostedQueryFailure> =
            if (raw.isNotEmpty() && raw.toByteArray(Charsets.UTF_8).size <= 512 &&
...
internal class HostedIndexCandidates<Value>(private val limits: ReadLimits = ReadLimits.Default) {
    ...
    fun finish(): Refinement<List<Value>, HostedQueryFailure> = when (val current = state) {
...
class HostedIndexedClasses internal constructor(val lookup: HostedClassLookup, declarations: List<HostedCompilerDeclaration>) {
    val declarations: List<HostedCompilerDeclaration> = Collections.unmodifiableList(ArrayList(declarations))
}

class HostedIndexPublication internal constructor(
...
sealed interface HostedIndexResult {
    ...
    data class Rejected(val failure: HostedQueryFailure, val stage: HostedQueryStage = HostedQueryStage.REQUEST_ADMISSION) : HostedIndexResult
}
