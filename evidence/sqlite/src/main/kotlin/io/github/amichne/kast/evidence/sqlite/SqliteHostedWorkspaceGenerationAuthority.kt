...
sealed interface HostedWorkspaceGenerationIssuance {
    ...
    data class Rejected(val failure: HostedWorkspaceGenerationFailure) : HostedWorkspaceGenerationIssuance
}
...
sealed interface HostedWorkspaceGenerationResumption {
    ...
    data class Rejected(val failure: HostedWorkspaceGenerationFailure) : HostedWorkspaceGenerationResumption
}
...
object SqliteHostedWorkspaceGenerationAuthority {
    fun issue(
        ...
        val database =
            when (val admitted = SqliteMutationRecoveryDatabase.admit(path)) {
                ...
                is Refinement.Rejected -> return rejected(HostedWorkspaceGenerationFailure.STORAGE_UNAVAILABLE)
            }
        return try {
            ...
            connections.use { connection ->
                ...
                try {
                    ...
                    val generation =
                        when (val issued = issueInside(connection, state)) {
    ...
    /**
     * Resumes the latest state in the exact bounded-source lineage, creating its initial durable publication when no
     * lineage exists. Repository content is never observed at this boundary.
     */
    fun resume(
        location: MutationDatabaseLocation,
        ...
        return try {
            ...
            connections.use { connection ->
                ...
                try {
                    initializeTables(connection)
                    val publication =
                        when (val lineage = readLineageHead(connection)) {
                            LineageRow.Empty -> {
                                val generation =
                                    when (val issued = issueInside(connection, basis)) {
                                        is GenerationRow.Issued -> issued.generation
                                        is GenerationRow.Rejected -> {
                                            connection.rollback()
                            ...
                            is LineageRow.Current -> {
                                ...
                                if (head.basis == basis) {
                                    ...
                                    val successor =
                                        transitionBasis(head.current, basis)
                                            ?: run {
                                                connection.rollback()
                                    ...
                                    val generation =
                                        when (val issued = issueInside(connection, successor)) {
                                            is GenerationRow.Issued -> issued.generation
                                            is GenerationRow.Rejected -> {
                                                connection.rollback()
                                                return@use resumptionRejected(issued.failure)
                                    ...
                                    if (generation.value <= head.generation.value) {
                                        connection.rollback()
                                        return@use resumptionRejected(HostedWorkspaceGenerationFailure.CORRUPT_STATE)
                                    }
                                    val updated =
                                        connection
                                            .prepareStatement(
        ...
        return try {
            ...
            connections.use { connection ->
                ...
                try {
                    ...
                    val generation =
                        when (val issued = issueInside(connection, next)) {
                            is GenerationRow.Issued -> issued.generation
                            is GenerationRow.Rejected -> {
                                connection.rollback()
                                return@use rejected(issued.failure)
        ...
        connection.createStatement().use { statement ->
            statement
                ...
                .use { rows ->
                    ...
                    if (rows.next()) {
                        return@use LineageRow.Rejected(HostedWorkspaceGenerationFailure.CORRUPT_STATE)
                    }
                    val basis =
                        when (val parsed = WorkspaceStateIdentity.parse(rawBasis)) {
                            is Refinement.Refined -> parsed.value
                            is Refinement.Rejected ->
                                return@use LineageRow.Rejected(HostedWorkspaceGenerationFailure.CORRUPT_STATE)
                        }
                    val current =
                        when (val parsed = WorkspaceStateIdentity.parse(rawCurrent)) {
                            is Refinement.Refined -> parsed.value
                            is Refinement.Rejected ->
                                return@use LineageRow.Rejected(HostedWorkspaceGenerationFailure.CORRUPT_STATE)
                        }
    ...
    private fun issueInside(
            ...
            existing
                ?: run {
                    val maximum =
                        connection.createStatement().use { statement ->
                            statement
                                .executeQuery("SELECT MAX(generation) AS maximum FROM hosted_workspace_generation")
                                .use { rows ->
        ...
        return when (val parsed = EvidenceGeneration.parse(raw)) {
            ...
            is Refinement.Rejected -> GenerationRow.Rejected(HostedWorkspaceGenerationFailure.CORRUPT_STATE)
        }
    }
    ...
    private fun generationFor(
        ...
        connection.prepareStatement("SELECT generation FROM hosted_workspace_generation WHERE source_state = ?").use {
            statement ->
            statement.setString(1, state.value)
