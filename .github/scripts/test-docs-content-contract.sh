...
require_contains "$macos_install_doc" "Homebrew owns the Kast CLI; JetBrains owns the installed plugin" "macOS install guide must separate distribution authorities"
require_contains "$macos_install_doc" "idea installPlugins io.github.amichne.kast" "macOS install guide must document IDE-owned programmatic installation"
require_contains "$macos_install_doc" "releases/latest/download/updatePlugins.xml" "macOS install guide must document the stable GitHub update feed"
require_contains "$macos_install_doc" "JetBrains can still require a restart" "macOS install guide must retain the truthful dynamic-update fallback"
require_contains "$macos_install_doc" "does not write workspace state or plugin directories" "macOS install guide must reject direct IDE mutation by the installer"
