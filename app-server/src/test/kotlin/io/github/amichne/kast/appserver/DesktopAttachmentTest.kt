...
class DesktopAttachmentTest {
    @Test fun `empty injected filesystem cannot discover applications on the host`() {
    ...
    @Test fun `discovery and all command outcomes use the injected filesystem`() {
        for (bundleName in listOf("/users/test/Applications/Codex.app", "/Applications/ChatGPT.app", "/Applications/Codex.app")) {
            Jimfs.newFileSystem(Configuration.unix()).use { filesystem ->
                ...
                assertEquals(DesktopAttachmentAdmission.Eligible, InstalledDesktopAttachmentProbe.inspect(emptyMap(), home, commands, reporter))
                ...
                for (failed in listOf(
                    ...
                    val failing = DesktopInspectionCommands { if (it == failed) DesktopInspectionRead.Rejected else commands.read(it) }
                    assertEquals(
                ...
                for (override in DesktopOverride.entries) {
                    val conflicting = DesktopInspectionCommands {
                        if (it == DesktopInspectionCommand.ReadOverride(override)) DesktopInspectionRead.Value("1") else commands.read(it)
                    }
                    assertEquals(conflict, InstalledDesktopAttachmentProbe.inspect(emptyMap(), home, conflicting, reporter))
                }
            }
        }
    }

    @Test fun `global discovery order is preserved without falling back from rejection`() {
        Jimfs.newFileSystem(Configuration.unix()).use { filesystem ->
            val home = filesystem.getPath("/users/test")
    ...
    @Test fun `explicit desktop selection inspects only its admitted bundle`() {
        Jimfs.newFileSystem(Configuration.unix()).use { filesystem ->
            val home = filesystem.getPath("/users/test")
            ...
            assertEquals(DesktopAttachmentAdmission.Eligible, InstalledDesktopAttachmentProbe.inspect(environment, home, commands))
            Files.delete(plist)
            assertEquals(
                DesktopAttachmentAdmission.Rejected(DesktopAttachmentFailure.DESKTOP_UNAVAILABLE),
                InstalledDesktopAttachmentProbe.inspect(environment, home, DesktopInspectionCommands { error("missing metadata must not execute commands") }),
            )
            for (invalid in listOf("relative", "/users/../test", "bad\u0000path")) {
                assertEquals(
                    DesktopAttachmentAdmission.Rejected(DesktopAttachmentFailure.INSPECTION_REJECTED),
                    InstalledDesktopAttachmentProbe.inspect(mapOf("KAST_CODEX_DESKTOP_EXECUTABLE" to invalid), home, commands),
                )
            }
        }
    }

    @Test fun `closed filesystem reports inspection rejection without invoking commands`() {
        val filesystem = Jimfs.newFileSystem(Configuration.unix())
        val home = filesystem.getPath("/users/test")
        filesystem.close()
        val observations = mutableListOf<DesktopAttachmentAdmission>()
        val expected = DesktopAttachmentAdmission.Rejected(DesktopAttachmentFailure.INSPECTION_REJECTED)
        assertEquals(expected, InstalledDesktopAttachmentProbe.inspect(
            emptyMap(), home, DesktopInspectionCommands { error("closed filesystem must not execute commands") },
