...
run_kast_cli() {
  ...
  cargo run --quiet --manifest-path "${repo_root}/cli-rs/Cargo.toml" -- "$@"
}
...
[[ ! -e "${bundle_root}/scripts/setup-kast-devin-runtime.sh" ]] || die "Bundle must not include setup shell script"
[[ ! -e "${bundle_root}/scripts/verify-kast-devin-runtime.sh" ]] || die "Bundle must not include verify shell script"
...
run_kast_cli devin-runtime setup --prefix "$bundle_root"
...
run_kast_cli devin-runtime verify --prefix "$bundle_root"
