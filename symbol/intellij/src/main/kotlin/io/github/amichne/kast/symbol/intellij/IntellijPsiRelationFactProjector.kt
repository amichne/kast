...
internal object IntellijPsiRelationFactProjector : IntellijRelationFactProjector {
    /**
     * Proof transition: NativeRelationRequest + IntellijPsiRelationEvent to Refinement<NativeRelationFact,
     * IntellijRelationProjectionFailure>.
     *
     * Establishes exact related-declaration evidence and an exact occurrence range bound to the request selector's
     * root, authority, scope, and relation family. Projection failures are closed as unsupported items; all live
     * IntelliJ values remain inside this call.
     */
    override fun project(
        ...
        val psiEvent =
            event as? IntellijPsiRelationEvent
                ?: return Refinement.Rejected(IntellijRelationProjectionFailure.UNSUPPORTED_ITEM)
        val relatedFile =
            PsiUtilCore.getVirtualFile(psiEvent.related)
                ?: return Refinement.Rejected(IntellijRelationProjectionFailure.UNSUPPORTED_ITEM)
        val detachedRelatedFile =
            when (val detached = relatedFile.detach(request)) {
            ...
            when (
                val refined =
                    ExactDeclarationEvidence.fromBoundary(
                        ...
                        rawQualifiedIdentity = (psiEvent.related as? PsiQualifiedNamedElement)?.qualifiedName,
                        rawRuntimeType = psiEvent.related.javaClass.name,
                    )
            ) {
                is Refinement.Refined -> refined.value
                is Refinement.Rejected -> return Refinement.Rejected(IntellijRelationProjectionFailure.UNSUPPORTED_ITEM)
            }
        val occurrenceFile =
            when (val detached = psiEvent.occurrenceFile.detach(request)) {
                is IntellijDetachedRelationFile.Found -> detached.identity
                IntellijDetachedRelationFile.Unsupported ->
                    return Refinement.Rejected(IntellijRelationProjectionFailure.UNSUPPORTED_ITEM)
            }
        ...
        return when (
            ...
            is Refinement.Rejected -> Refinement.Rejected(IntellijRelationProjectionFailure.UNSUPPORTED_ITEM)
        }
    }
     ...
     *
     * Establishes a detached file identity rooted in the selector's canonical workspace. Relative, unavailable, or
     * non-refinable native paths are represented by the closed unsupported state.
     */
    private fun VirtualFile.detach(request: NativeRelationRequest): IntellijDetachedRelationFile {
        val path =
            when (val classified = nativePath(this)) {
