...
class HostedReadDiagnosticsTest {
    @Test fun `unexpected failures retain bounded adapter frames without throwable payloads`() {
        val limits = (io.github.amichne.kast.kernel.ReadLimits.resolve(mapOf(
            "KAST_READ_DIAGNOSTIC_FRAMES" to "1", "KAST_READ_DIAGNOSTIC_FAILURES" to "1",
        )) as io.github.amichne.kast.kernel.Refinement.Refined).value
        ...
        diagnostic.finish(HostedDiagnosticOutcome.Rejected(HostedQueryFailure.Platform(HostedPlatformFailureCause.RUNTIME)))
    ...
    @Test fun `configured outer deadline is enforced and rejection is always observed`() = runTest {
        val limits = (io.github.amichne.kast.kernel.ReadLimits.resolve(mapOf(
            "KAST_READ_DIAGNOSTIC_SCOPE_MILLIS" to "10", "KAST_READ_SEMANTIC_MILLIS" to "10", "KAST_READ_HOST_QUERY_MILLIS" to "10",
        )) as io.github.amichne.kast.kernel.Refinement.Refined).value
        val receipts = mutableListOf<HostedReadDiagnosticReceipt>()
        val executor = HostedQueryExecutor(backgroundScope) { policy -> HostedReadDiagnostics({ testScheduler.currentTime * 1_000_000 }, policy, receipts::add) }
        val result = executor.execute(executor.endpoint, limits) { delay(11); 1 }
        ...
        executor.retire(); executor.drain()
    ...
    fun `outer deadline can reject before a later inner budget expires and owner recovers`() = runTest {
        ...
        val result = executor.execute(executor.endpoint) { progress ->
        ...
        assertEquals(HostedExecution.Rejected(HostedQueryFailure.BUDGET_EXCEEDED, HostedQueryStage.SEMANTIC_READ), result)
    ...
    fun `admission rejection has no invented semantic entry or successful outcome`() = runTest {
        ...
        executor.execute(executor.endpoint) { progress ->
            ...
            HostedSemanticRead.Rejected(HostedQueryFailure.NamedSourceScope(NamedGradleSourceScopeFailure.CAPTURE_LIMIT))
        }
        assertEquals(HostedSemanticEntry.NotEntered, receipts.single().semanticEntry)
        assertEquals(HostedDiagnosticOutcome.Rejected(HostedQueryFailure.NamedSourceScope(NamedGradleSourceScopeFailure.CAPTURE_LIMIT)), receipts.single().outcome)
    ...
    fun `diagnostics saturate counters deduplicate reasons and publish once`() {
        ...
        assertEquals(listOf(HostedNativeTermination(IntellijReadTermination.NAME_CAP, IntellijReadContributor.KOTLIN_CLASS)), receipts.single().terminations)
