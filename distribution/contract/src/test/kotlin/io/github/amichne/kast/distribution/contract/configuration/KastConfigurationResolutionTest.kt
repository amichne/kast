...
class KastConfigurationResolutionTest {
    @Test fun `heap keeps canonical authority and explicit source precedence`() {
        val resolved = admitted(ConfigurationSources(
        ...
        assertEquals(listOf(ConfigurationSource.PROCESS_ENVIRONMENT, ConfigurationSource.SAVED_WORKSPACE, ConfigurationSource.SAVED_INSTALLATION), entry.overriddenSources)
        assertEquals(IndexerHeapSize.Default, admitted(ConfigurationSources()).indexerHeap)
    }

    @Test fun `explicit malformed assignments never fall through to valid lower precedence`() {
        for (value in listOf("", "255m", "8G", "2147483648m", "1g\n")) {
            assertInstanceOf(Refinement.Rejected::class.java, ResolvedKastConfiguration.resolve(ConfigurationSources(
    ...
    @Test fun `unknown duplicate unsupported and test-only assignments reject without values`() {
        val cases = listOf(
            ConfigurationSources(environment = mapOf("KAST_UNKNOWN" to "private-value")) to ConfigurationFailure.UNKNOWN_KEY,
            ConfigurationSources(savedInstallation = listOf("KAST_INDEXER_MAX_HEAP" to "1g", "KAST_INDEXER_MAX_HEAP" to "2g")) to ConfigurationFailure.DUPLICATE_ASSIGNMENT,
            ConfigurationSources(savedWorkspace = listOf("KAST_INSTALL_ROOT" to "/fixture")) to ConfigurationFailure.UNSUPPORTED_SOURCE,
            ConfigurationSources(environment = mapOf("KAST_INSTALL_PROCESS_KILL_COMMAND" to "/fixture/kill")) to ConfigurationFailure.TEST_ONLY_INPUT,
        )
    ...
    @Test fun `child projection preserves heap network and explicitly delegated environment only`() {
        val resolved = admitted(ConfigurationSources(environment = mapOf(
            "KAST_INDEXER_MAX_HEAP" to "8g", "KAST_IDE_CONFIG_HOME" to "/fixture/idea-config",
            "KAST_GRADLE_IMPORT_VARIABLES" to "REPOSITORY_TOKEN", "REPOSITORY_TOKEN" to "private-value",
            "UNSELECTED_TOKEN" to "unselected-value", "GRADLE_USER_HOME" to "/fixture/gradle",
        )))
    ...
    @Test fun `missing or reserved delegated variable fails before child launch`() {
        for (names in listOf("MISSING_TOKEN", "JAVA_TOOL_OPTIONS", "KAST_INDEXER_MAX_HEAP", "A,A")) {
            assertEquals(ConfigurationFailure.DELEGATED_ENVIRONMENT_REJECTED,
                rejected(ConfigurationSources(environment = mapOf("KAST_GRADLE_IMPORT_VARIABLES" to names))).reason)
        }
    }

    @Test fun `catalogue is deterministic unique and projects heap default from its owner`() {
    ...
    @Test fun `resolved snapshot cannot be changed by editing caller environment`() {
    ...
    @Test fun `version owned runtime directories reject workspace overrides at source admission`() {
        for (key in listOf("KAST_RUNTIME_STORE", "KAST_RUNTIME_DIRECTORY", "KAST_CACHE_ROOT")) {
            val resolution = ResolvedKastConfiguration.resolve(ConfigurationSources(savedWorkspace = listOf(key to "/another/workspace-owned-directory")))
            ...
            assertEquals(ConfigurationScope.INSTALLATION, KastConfigurationCatalogue.declarations.single { it.key == key }.scope)
        }
    }

    private fun admitted(sources: ConfigurationSources): ResolvedKastConfiguration = when (val result = ResolvedKastConfiguration.resolve(sources)) {
        is Refinement.Refined -> result.value
        is Refinement.Rejected -> error("Unexpected ${result.failure}")
    }
    private fun rejected(sources: ConfigurationSources): ConfigurationRejection = when (val result = ResolvedKastConfiguration.resolve(sources)) {
