...
usage() {
  cat >&2 <<'USAGE'
Usage: scripts/packaging/assemble-prepared-local-generation.sh \
...
source_bound_control="${cli_extract}/kastctl"
source_bound_agent="${cli_extract}/kast"
source_bound_backend="${backend_extract}/backend-headless"
[[ -x "$source_bound_control" ]] || die "CLI archive must contain executable kastctl at its root"
[[ -x "$source_bound_agent" ]] || die "CLI archive must contain executable kast at its root"
cmp -s "$source_bound_control" "$source_bound_agent" \
...
mkdir -p "${prepared_generation}/bin" "${prepared_generation}/libexec"
cp "$source_bound_control" "${prepared_generation}/libexec/kastctl"
cp "$source_bound_agent" "${prepared_generation}/bin/kast"
chmod 755 "${prepared_generation}/libexec/kastctl" "${prepared_generation}/bin/kast"
...
"${repo_root}/scripts/packaging/package-prepared-local-generation.sh" \
