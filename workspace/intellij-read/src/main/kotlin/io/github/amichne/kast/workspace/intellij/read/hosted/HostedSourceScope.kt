...
internal class HostedSourceScope private constructor(val module: DetachedIdeModule, val root: DetachedIdeSourceRoot) {
    companion object {
        fun admit(model: DetachedIdeWorkspaceModel, file: SymbolDiscoveryFileIdentity.Workspace): Refinement<HostedSourceScope, HostedQueryFailure> {
            val owners = model.modules.flatMap { module ->
                module.sourceRoots.filter { root ->
                    Path.of(file.path.value).startsWith(Path.of(model.canonicalRoot.value).resolve(root.location.value))
                }.map { module to it }
            }
            val (module, root) = owners.singleOrNull()
                ?: return Refinement.Rejected(if (owners.isEmpty()) HostedQueryFailure.OUTSIDE_SCOPE else HostedQueryFailure.AMBIGUOUS_SCOPE)
            if (root.kind !in setOf(DetachedSourceRootKind.PRODUCTION, DetachedSourceRootKind.TEST) ||
