...
class InstalledSemanticRuntimeBootstrapDocument private constructor(private val value: String) {
    fun boundaryValue(): String = value

    internal companion object {
        fun encode(state: SemanticRuntimeBootstrapState): InstalledSemanticRuntimeBootstrapDocument =
            InstalledSemanticRuntimeBootstrapDocument(SemanticRuntimeBootstrapCodec.encode(state))
    }
}

enum class InstalledSemanticRuntimeBootstrapAttemptFailure {
    INVALID_IDENTITY
}
...
sealed interface InstalledSemanticRuntimeBootstrapAttemptAdmission {
    data class Admitted(val attempt: InstalledSemanticRuntimeBootstrapAttempt) :
        InstalledSemanticRuntimeBootstrapAttemptAdmission

    data class Rejected(val failure: InstalledSemanticRuntimeBootstrapAttemptFailure) :
        InstalledSemanticRuntimeBootstrapAttemptAdmission
}

sealed interface InstalledSemanticRuntimeBootstrapRejection {
    data class Projected(val document: InstalledSemanticRuntimeBootstrapDocument) :
...
sealed interface InstalledSemanticRuntimeGradleJvmRefinement {
    data class Refined(val attempt: InstalledSemanticRuntimeBootstrapAttempt) :
        InstalledSemanticRuntimeGradleJvmRefinement
...
class InstalledSemanticRuntimeBootstrapAttempt
private constructor(
    ...
    fun withGradleJvm(report: InstalledGradleJvmSelectionReport): InstalledSemanticRuntimeGradleJvmRefinement =
        when (gradleJvm) {
            GradleJvmSelectionObservation.Unobserved ->
                InstalledSemanticRuntimeGradleJvmRefinement.Refined(
                    InstalledSemanticRuntimeBootstrapAttempt(
                        attemptId,
                        GradleJvmSelectionObservation.Observed(report.report),
                    )
                )
            is GradleJvmSelectionObservation.Observed ->
                if (gradleJvm.report == report.report) {
    ...
    fun startingDocument(
        ...
        document(SemanticRuntimeBootstrapState.Starting(attemptId, phase.contractPhase(), gradleJvm))

    fun readyDocument(): InstalledSemanticRuntimeBootstrapDocument =
        document(SemanticRuntimeBootstrapState.Ready(attemptId, gradleJvm))
    ...
    fun rejectionDocument(
        ...
        return when {
            ...
            projections.all { it is IntellijBootstrapFailureProjection.Projected } && projected.size == 1 ->
    ...
    private fun document(state: SemanticRuntimeBootstrapState): InstalledSemanticRuntimeBootstrapDocument =
        ...
        fun admit(raw: String): InstalledSemanticRuntimeBootstrapAttemptAdmission =
            when (val refinement = SemanticRuntimeBootstrapAttemptId.admit(raw)) {
                ...
                is Refinement.Rejected ->
                    InstalledSemanticRuntimeBootstrapAttemptAdmission.Rejected(
...
private sealed interface IntellijBootstrapFailureProjection {
    data class Projected(val failure: SemanticRuntimeBootstrapCause) : IntellijBootstrapFailureProjection

    data object NotIntellijBootstrap : IntellijBootstrapFailureProjection
}

private fun InstalledKastRuntimeFailure.intellijBootstrapFailure(): IntellijBootstrapFailureProjection =
    when (this) {
        ...
        is InstalledKastRuntimeFailure.WorkspaceRoot -> IntellijBootstrapFailureProjection.NotIntellijBootstrap
    }

private fun InstalledRuntimeAssemblyFailure.intellijBootstrapFailure(): IntellijBootstrapFailureProjection =
    when (this) {
        ...
        is InstalledRuntimeAssemblyFailure.Persistence -> IntellijBootstrapFailureProjection.NotIntellijBootstrap
    }

private fun InstalledRuntimeWorkspaceFailure.intellijBootstrapFailure(): IntellijBootstrapFailureProjection =
    when (this) {
        ...
        is InstalledRuntimeWorkspaceFailure.IntellijBootstrap ->
            IntellijBootstrapFailureProjection.Projected(
                SemanticRuntimeBootstrapCause.Standard(failure.bootstrapFailure())
            )
        ...
        InstalledRuntimeWorkspaceFailure.RootMismatch -> IntellijBootstrapFailureProjection.NotIntellijBootstrap
    }

private fun InstalledIntellijWorkspaceFailure.bootstrapFailure(): SemanticRuntimeBootstrapFailure =
    when (this) {
        ...
        InstalledIntellijWorkspaceFailure.PROJECT_OPEN_FAILED -> SemanticRuntimeBootstrapFailure.PROJECT_OPEN_FAILED
        InstalledIntellijWorkspaceFailure.STARTUP_FAILED -> SemanticRuntimeBootstrapFailure.STARTUP_FAILED
        ...
        InstalledIntellijWorkspaceFailure.GRADLE_IMPORT_FAILED -> SemanticRuntimeBootstrapFailure.GRADLE_IMPORT_FAILED
        ...
        InstalledIntellijWorkspaceFailure.INDEXING_INTERRUPTED -> SemanticRuntimeBootstrapFailure.INDEXING_INTERRUPTED
        InstalledIntellijWorkspaceFailure.MODEL_UNAVAILABLE -> SemanticRuntimeBootstrapFailure.MODEL_UNAVAILABLE
...
private fun InstalledRuntimeBootstrapPhase.contractPhase(): SemanticRuntimeBootstrapPhase =
    when (this) {
