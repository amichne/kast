...
class AdmittedAppServerConfiguration private constructor(
    ...
    companion object {
        fun admit(configuration: ResolvedKastConfiguration): Refinement<AdmittedAppServerConfiguration, AppServerConfigurationFailure> {
            val admittedTools = configuration.ownerCandidates(ConfigurationOwner.APP_SERVER)
                ...
                .map { candidate ->
                    when (val admitted = KastToolSelection.admit(candidate.valueAtOwnerBoundary())) {
                        ...
                        is Refinement.Rejected -> return Refinement.Rejected(AppServerConfigurationFailure.INVALID_TOOL_SELECTION)
                    }
                }
            val inputs = configuration.ownerInputs(ConfigurationOwner.APP_SERVER)
            val mode = when (val result = AppServerToolingMode.admit(inputs[APP_SERVER_ENABLE_ENVIRONMENT])) {
                is Refinement.Refined -> result.value
                is Refinement.Rejected -> return Refinement.Rejected(AppServerConfigurationFailure.INVALID_TOOLING_MODE)
            }
...
enum class AppServerConfigurationFailure { INVALID_TOOLING_MODE, INVALID_TOOL_SELECTION }
...
internal object InstalledBrokerConfigurationIngress {
    fun admit(environment: Map<String, String>): Refinement<AdmittedAppServerConfiguration, BrokerConfigurationIngressRejection> {
        val sources = when (val read = InstalledSavedConfigurationIngress.load(environment)) {
            is SavedConfigurationIngress.Loaded -> read.sources
            is SavedConfigurationIngress.Rejected -> return Refinement.Rejected(BrokerConfigurationIngressRejection.Source(read.rejection))
        }
        val configuration = when (val resolution = ResolvedKastConfiguration.resolve(sources)) {
            is Refinement.Refined -> resolution.value
            is Refinement.Rejected -> return Refinement.Rejected(BrokerConfigurationIngressRejection.Configuration(resolution.failure))
        }
