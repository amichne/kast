...
sealed interface WorkspacePublicationOpening {
    data class Opened(val publication: OpenCanonicalWorkspacePublication) : WorkspacePublicationOpening

    data class Rejected(val failure: WorkspacePublicationFailure) : WorkspacePublicationOpening
}

/** Closed result of preparing reconciled evidence for atomic commit. */
sealed interface WorkspacePublicationPreparation {
    data class Prepared(val publication: PreparedCanonicalWorkspacePublication) : WorkspacePublicationPreparation

    data class Rejected(val failure: WorkspacePublicationFailure) : WorkspacePublicationPreparation
}
...
sealed interface WorkspacePublicationResult {
    data class Advanced(val workspace: PublishedWorkspace) : WorkspacePublicationResult

    data class Unchanged(val workspace: PublishedWorkspace) : WorkspacePublicationResult

    data class Rejected(val failure: WorkspacePublicationFailure) : WorkspacePublicationResult
}
...
sealed interface WorkspacePublicationDiscard {
    ...
    data class Rejected(val failure: WorkspacePublicationFailure) : WorkspacePublicationDiscard
}
...
interface WorkspacePublicationTransaction {
     ...
     *
     * Establishes exclusive ownership of the transaction that will receive reconciliation evidence, or the closed
     * [WorkspacePublicationFailure].
     */
    fun begin(): WorkspacePublicationOpening

    /**
     * Proof transition: `(OpenCanonicalWorkspacePublication, ReconciledWorkspace) -> WorkspacePublicationPreparation`.
     *
     * Establishes that complete candidate evidence and its generation manifest share the open atomic transaction, or
     * the closed [WorkspacePublicationFailure].
     */
    fun prepare(
     ...
     *
     * An advanced result atomically commits one new evidence generation. An unchanged result proves the candidate is
     * canonically identical and retains the current generation. Both return the only [PublishedWorkspace] assembled
     * from that exact prepared candidate. The closed expected failure is [WorkspacePublicationFailure]. Persistence
     * primitives may be extracted only by the evidence adapter implementing this transaction.
     */
