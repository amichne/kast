...
internal class WorkspaceExecution(
        ...
        val accepted =
            synchronized(this) {
                when (val lane = lanes[identity.workspace] ?: Lane.Idle) {
                    ...
                    is Lane.Busy ->
                        if (lane.pending.size >= policy.maximumQueued) {
        ...
        val matching =
            synchronized(this) {
                when (val lane = lanes[workspace]) {
                    is Lane.Busy ->
                        (listOf(lane.active) + lane.pending).filter {
    ...
    private suspend fun perform(request: Request, operation: suspend () -> ProtocolRouting) {
        try {
            ...
            if (queueRemaining <= 0) {
                finish(request, WorkspaceExecutionResult.Rejected(WorkspaceExecutionFailure.WORKSPACE_QUEUE_TIMED_OUT))
                return
            }
            try {
    ...
    @Synchronized
    private fun begin(request: Request): Boolean {
    ...
    @Synchronized
    private fun interrupted(request: Request): WorkspaceExecutionResult.Rejected =
    ...
    private fun publish(
        ...
        failure: WorkspaceExecutionFailure? = null,
    ) {
