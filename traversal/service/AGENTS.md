...
- Exact endpoint fingerprints terminate cycles; a node is expanded at most once per continuation
  lineage.
- The reader receives only the plan's exact scope, meaning, generation, continuation, and bounded
  one-hop budget. Divergent output is a closed contract rejection.
