 ...
 *
 * Construction remains compiler-confined to the hosted adapter. Consumers may retain the strong root and epoch evidence
 * for later read execution and revalidation, but cannot mint, parse, copy, or alter the capability.
 */
class VfsPassiveReadCapability
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(CanonicalWorkspaceRoot, ProjectReadEpoch<*>) -> VfsPassiveReadCapability`.
         *
         * Preserves the exact root and current same-source epoch established by freshness admission freshness
         * admission. Construction is permitted only from the friend hosted adapter after its sole live epoch
         * observation; raw IDE state is never accepted or retained here.
         */
        @JvmSynthetic
        internal fun issue(
...
sealed interface VfsPassiveReadAdmission {
    data class Admitted(val capability: VfsPassiveReadCapability) : VfsPassiveReadAdmission

    data class Rejected(val failure: VfsPassiveReadAdmissionFailure) : VfsPassiveReadAdmission
}
...
sealed interface VfsPassiveReadAdmissionFailure {
    ...
    data class Unavailable(val cause: VfsPassiveReadUnavailableCause) : VfsPassiveReadAdmissionFailure
}
...
sealed interface VfsPassiveReadUnavailableCause {
    ...
    data class ObservationFailed(val stage: ProjectReadEpochObservationStage) : VfsPassiveReadUnavailableCause
}
