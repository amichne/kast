...
internal fun Connection.insertPreparation(record: MutationRecoveryRecord.PreWriteDurable): Boolean {
    val inserted =
        prepareStatement(
 ...
 *
 * Reconstructs every state transition from typed planned and applied write evidence, then accepts only the exact stored
 * digest. Corruption is closed by [MutationRecoveryEvidenceFailure]. Raw columns are extracted only inside this SQLite
 * decoder.
 */
internal fun Connection.loadMutationRecovery(binding: MutationPlanBinding): MutationRecoveryLoadResult {
    val raw =
        prepareStatement(
    ...
    val preparation =
        when (val loaded = loadPreparation(binding)) {
    ...
    val reconstructed =
        when (raw.stage) {
            MutationRecoveryStage.PRE_WRITE_DURABLE.name ->
                when {
            ...
            MutationRecoveryStage.RECOVERY_REQUIRED.name -> {
                ...
                val requirement =
                    when (val stored = raw.requirement) {
                        is SqlRecoveryRequirement.Present -> stored.value
                        SqlRecoveryRequirement.Absent,
                        SqlRecoveryRequirement.Invalid -> return corrupt()
                    }
 ...
 *
 * Establishes contiguous ordering, canonical paths, byte-exact preimages, and one matching plan binding.
 * [SqlMutationRecoveryDecodeFailure] is the closed expected failure. Raw columns remain inside this SQLite decoder.
 */
...
/**
 * Proof transition: stored applied-write rows plus `PreWriteDurable` -> `Refinement< AppliedWritesDurable,
 * SqlMutationRecoveryDecodeFailure>`.
 *
 * Establishes a contiguous deterministic subset of the planned writes and re-derives its digest chain.
 * [SqlMutationRecoveryDecodeFailure] is the closed expected failure. Raw columns remain inside this SQLite decoder.
 */
...
private fun String?.toSqlRequirement(): SqlRecoveryRequirement =
    when (this) {
...
private fun corrupt(): MutationRecoveryLoadResult.Rejected =
    MutationRecoveryLoadResult.Rejected(MutationRecoveryEvidenceFailure.CORRUPT_RECORD)
