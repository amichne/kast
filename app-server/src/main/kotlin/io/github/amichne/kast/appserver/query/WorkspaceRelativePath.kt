...
internal value class WorkspaceRelativePath private constructor(val value: String) {
    companion object {
        /**
         * Refines String to an exact workspace-relative path, or [WorkspaceRelativePathFailure]. Preserves spelling and
         * excludes traversal and absolute paths without filesystem effects. Raw extraction belongs only to query
         * serialization and canonical protocol lowering.
         */
        fun parse(raw: String): Refinement<WorkspaceRelativePath, WorkspaceRelativePathFailure> =
            when {
...
internal enum class WorkspaceRelativePathFailure {
    BLANK,
    TOO_LONG,
    ABSOLUTE,
    CONTROL_CHARACTER,
