...
class NativeToolIdentityTest {
    ...
    fun `started call retains its native identity and arguments`() {
        val item = Json.parseToJsonElement("""{"type":"dynamicToolCall","id":"call-1","namespace":"kast","tool":"query","arguments":{"from":"example"},"status":"inProgress"}""").jsonObject
    ...
    fun `completed call retains the complete structured result`() {
        val item = Json.parseToJsonElement("""{"type":"dynamicToolCall","id":"call-1","namespace":"kast","tool":"query","arguments":{},"status":"completed","success":true,"contentItems":[{"type":"inputText","text":"{\"status\":\"qualified\",\"items\":[],\"failures\":[\"budget\"]}"}]}""").jsonObject
