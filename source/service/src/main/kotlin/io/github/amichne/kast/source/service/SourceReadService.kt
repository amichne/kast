...
class SourceReadService(
    ...
    override suspend fun read(request: SourceReadRequest): SourceReadResult {
        ...
        if (initial.lease != expected) return SourceReadResult.Rejected(SourceReadRejection.INTERNAL_CONTEXT_LEASE_MISMATCH)
        ...
        return when {
            snapshot.context != initial -> SourceReadResult.Rejected(SourceReadRejection.INTERNAL_SNAPSHOT_CONTEXT_MISMATCH)
            snapshot.readScope != anchor.readScope() -> SourceReadResult.Rejected(SourceReadRejection.INTERNAL_SCOPE_MISMATCH)
