...
@JvmInline
value class ExistingDeclarationSourceText private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<ExistingDeclarationSourceText, DeclarationSourceTextFailure>`.
         *
         * Establishes non-blank LF-normalized current declaration source without a terminal line break or unsupported
         * control characters. [DeclarationSourceTextFailure] is the closed expected failure. Raw text may enter only
         * from compiler-backed declaration extraction and leave only at exact preimage admission.
         */
        fun parse(raw: String): Refinement<ExistingDeclarationSourceText, DeclarationSourceTextFailure> =
            when {
                raw.isBlank() -> Refinement.Rejected(DeclarationSourceTextFailure.BLANK)
                '\r' in raw -> Refinement.Rejected(DeclarationSourceTextFailure.NON_CANONICAL_LINE_ENDING)
                raw.endsWith('\n') -> Refinement.Rejected(DeclarationSourceTextFailure.TERMINAL_LINE_BREAK)
                raw.any { character ->
...
@JvmInline
value class ReplacementDeclarationSourceText private constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<ReplacementDeclarationSourceText, DeclarationSourceTextFailure>`.
         *
         * Establishes non-blank LF-normalized replacement declaration source without a terminal line break or
         * unsupported control characters. [DeclarationSourceTextFailure] is the closed expected failure. Raw text may
         * enter only at the change-intent boundary and leave only at the authority-bound IntelliJ declaration parser.
         */
        fun parse(raw: String): Refinement<ReplacementDeclarationSourceText, DeclarationSourceTextFailure> =
            when {
                raw.isBlank() -> Refinement.Rejected(DeclarationSourceTextFailure.BLANK)
                '\r' in raw -> Refinement.Rejected(DeclarationSourceTextFailure.NON_CANONICAL_LINE_ENDING)
                raw.endsWith('\n') -> Refinement.Rejected(DeclarationSourceTextFailure.TERMINAL_LINE_BREAK)
...
class ReplaceDeclarationTarget
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes that the compiler-grounded declaration range has exactly the UTF-16 length of the extracted
         * current declaration. [ReplaceDeclarationTargetFailure] closes range mismatch. Raw offsets or source may leave
         * only at mutation admission.
         */
        fun admit(
            target: EditableMutationTarget,
            expected: ExistingDeclarationSourceText,
        ): Refinement<ReplaceDeclarationTarget, ReplaceDeclarationTargetFailure> =
            if (target.range.endExclusive - target.range.startInclusive != expected.value.length) {
...
sealed interface ReplaceDeclarationPlanningFailure {
    ...
    data class Evidence(val failure: ChangePlanningFailure) : ReplaceDeclarationPlanningFailure
}

/** Strong replacement input with exact preimage and complete target-bound semantic evidence. */
class AdmittedReplaceDeclarationPlanInput
private constructor(
    val target: ReplaceDeclarationTarget,
    val replacement: ReplacementDeclarationSourceText,
    ...
    companion object {
        /**
         * Proof transition: `ReplaceDeclarationPlanRequest -> Refinement< AdmittedReplaceDeclarationPlanInput,
         * ReplaceDeclarationPlanningFailure>`.
         *
         * Establishes a changed whole declaration plus complete normalized semantic evidence for its exact
         * compiler-grounded target. [ReplaceDeclarationPlanningFailure] is the closed expected failure. Raw source and
         * compiler observations must cross their typed boundaries before this pure transition.
         */
        fun admit(
            ...
            if (request.replacement.value == request.target.expected.value) {
                return Refinement.Rejected(ReplaceDeclarationPlanningFailure.REPLACEMENT_UNCHANGED)
            }
            return when (
                val evidence =
                ...
                is Refinement.Rejected ->
                    Refinement.Rejected(ReplaceDeclarationPlanningFailure.Evidence(evidence.failure))
            }
        }
    }
}
...
class ReplaceDeclarationChangePlan
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `AdmittedReplaceDeclarationPlanInput -> ReplaceDeclarationChangePlan`.
         *
         * Establishes a detached singleton plan binding exact current and replacement declaration source, the selected
         * range, source preimage, complete evidence, and exhaustive obligations. There is no expected failure because
         * the admitted input carries every invariant. Raw declaration source may leave only after mutation-authority
         * admission.
         */
...
sealed interface ReplaceDeclarationPlanResult {
    data class Planned(val plan: ReplaceDeclarationChangePlan) : ReplaceDeclarationPlanResult

    data class Rejected(val failure: ReplaceDeclarationPlanningFailure) : ReplaceDeclarationPlanResult
}
...
fun interface ReplaceDeclarationPlanOperations {
     ...
     *
     * Planned carries one deterministic exact declaration replacement plan; rejection is closed by
     * [ReplaceDeclarationPlanningFailure]. No source-write or platform capability crosses this pure boundary.
     */
