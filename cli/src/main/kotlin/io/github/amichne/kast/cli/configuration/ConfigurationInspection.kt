...
internal object ConfigurationCliInspection {
    fun inspect(arguments: List<String>, environment: Map<String, String>): ConfigurationInspectionHandling {
        ...
        if (workspaceOption >= 0 && (workspaceText == null || supplied.count { it == "--workspace" } != 1)) {
            return complete(
                failureFactory.create(
                    ConfigurationInspectionFailureDocument(operation = "config", reason = "INVALID_COMMAND")
                )
            )
        }
        val tail =
            if (workspaceOption < 0) supplied
            else supplied.filterIndexed { index, _ -> index != workspaceOption && index != workspaceOption + 1 }
        ...
        if (!show && explainKey == null && validationFile == null)
            return finish(
                failureFactory.create(
                    ConfigurationInspectionFailureDocument(
                        operation = "config",
        ...
        if (explainKey != null && KastConfigurationCatalogue.declarations.none { it.key == explainKey }) {
            return complete(
                failureFactory.create(
                    ConfigurationInspectionFailureDocument(operation = "config-explain", reason = "UNKNOWN_KEY")
                )
            )
        }
        val operation =
        ...
        val ingress =
            if (workspaceText != null) {
                ...
                val paths =
                    try {
                        Path.of(selector) to Path.of(workspaceText)
        ...
        val resolved =
            when (val result = ResolvedKastConfiguration.resolve(source.sources)) {
        ...
        if (validationFile != null) {
            ...
            return complete(
                validationFactory.create(
                    ConfigurationValidationDocument(
        ...
        if (show)
            return complete(
                showFactory.create(
                    ConfigurationShowDocument(
                        resolvedNextLaunch = resolved.inspection(),
                        desiredSavedConfiguration = source.observation,
        ...
        return complete(
            explainFactory.create(
                ConfigurationExplainDocument(
                    key = declaration.key,
    ...
    private fun observeApplied(
        ...
        val selector =
            environment[ConfigurationParameter.CONFIGURATION_FILE.key]
                ?: return AppliedConfigurationInspection.Unobserved(
                    AppliedConfigurationUnavailable.INSTALLATION_UNSELECTED
                )
        val home =
            environment["HOME"]
                ?: return AppliedConfigurationInspection.Unobserved(
        ...
        return try {
            ...
            if (selected != root.resolve("config/environment"))
                return AppliedConfigurationInspection.Unobserved(
                    AppliedConfigurationUnavailable.INSTALLATION_UNSELECTED
                )
            runBlocking {
                InstalledConfigurationAppliedInspection.read(
                    root.resolve("bin/kast"),
                    Path.of(home),
                    environment,
                    configuration,
                    workspace?.let(Path::of),
                )
            }
        } catch (_: InvalidPathException) {
    ...
    private fun AppliedConfigurationInspection.state(): String =
        when (this) {
    ...
    private fun finish(
        ...
        if (rejectedValidation)
            ConfigurationInspectionHandling.Handled(CliExit.BoundaryRejected(CliBoundaryExitStatus.USAGE, document))
...
private data class ConfigurationValidationDocument(
    ...
    val validationBoundary: String =
        "configuration-syntax-source-policy-and-app-server-tools; physical-path-admission-remains-explicit",
    val applied: String = "unobserved",
)
