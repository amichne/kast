...
class ObservedReplaceDeclarationDelta
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(String, ExactDeclarationTextRange, Int) -> Refinement< ObservedReplaceDeclarationDelta,
         * ObservedReplaceDeclarationDeltaFailure>`.
         *
         * Establishes exactly one compiler-visible canonical replacement declaration source whose exact range has the
         * same UTF-16 length. [ObservedReplaceDeclarationDeltaFailure] closes invalid, absent, ambiguous,
         * invalid-source, or range-mismatch observations. Raw compiler source and counts may enter only at the
        ...
        > =
            when {
                matchingDeclarationCount < 0 ->
                    Refinement.Rejected(ObservedReplaceDeclarationDeltaFailure.DECLARATION_COUNT_INVALID)
                matchingDeclarationCount == 0 ->
                    Refinement.Rejected(ObservedReplaceDeclarationDeltaFailure.DECLARATION_MISSING)
                matchingDeclarationCount > 1 ->
                    Refinement.Rejected(ObservedReplaceDeclarationDeltaFailure.DECLARATION_AMBIGUOUS)
                range.endExclusive - range.startInclusive != rawDeclaration.length ->
                    Refinement.Rejected(ObservedReplaceDeclarationDeltaFailure.RANGE_LENGTH_MISMATCH)
                else ->
                    when (val source = ReplacementDeclarationSourceText.parse(rawDeclaration)) {
                        is Refinement.Refined ->
                            Refinement.Refined(ObservedReplaceDeclarationDelta(source.value, range))
                        is Refinement.Rejected ->
                            Refinement.Rejected(ObservedReplaceDeclarationDeltaFailure.DECLARATION_SOURCE_INVALID)
                    }
            }
    }
}
...
class CompleteReplaceDeclarationVerification
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(ReplaceDeclarationChangePlan, AppliedUnverified, DistinctResultingWorkspace,
         * ReplaceDeclarationVerificationEvidence) -> Refinement< CompleteReplaceDeclarationVerification,
         * Set<ReplaceDeclarationProofFailure>>`.
         *
         * Establishes the exact authority-derived postimage, complete source-scoped clean diagnostics, one
         * compiler-observed declaration equal to the planned replacement, and the exhaustive obligation set.
         * [ReplaceDeclarationProofFailure] is the closed expected failure. Raw compiler extraction is confined to
         * [ObservedReplaceDeclarationDelta].
         */
        fun admit(
        ...
        > {
            ...
            val expectedRange =
                plan.target.target.range.let { range ->
            ...
            when (expectedRange) {
                is Refinement.Refined ->
                    if (evidence.observedDelta.range != expectedRange.value) {
                        failures += ReplaceDeclarationProofFailure.REPLACEMENT_RANGE_MISMATCH
                    }
                is Refinement.Rejected -> failures += ReplaceDeclarationProofFailure.REPLACEMENT_RANGE_MISMATCH
            }
            ...
            return if (failures.isEmpty()) {
                Refinement.Refined(CompleteReplaceDeclarationVerification(plan, applied, resulting, evidence))
            } else {
