...
@Serializable
enum class WorkerSeedConsentSelection {
    PREGRANTED,
    INTERACTIVE,
}

@Serializable
enum class WorkerSeedConsent {
    GRANTED,
    ABSENT,
}

@Serializable
enum class WorkerSeedCategory {
    GLOBAL_VFS,
    GLOBAL_INDEXES,
    PROJECT_MODEL,
    CLASSPATH_METADATA,
...
class WorkerSeedDisclosure private constructor(val categories: Set<WorkerSeedCategory>, val estimatedBytes: Long) {
    companion object {
        fun admit(
            categories: Set<WorkerSeedCategory>,
            estimatedBytes: Long,
        ): Refinement<WorkerSeedDisclosure, WorkerControlFailure> =
            if (categories.isNotEmpty() && estimatedBytes >= 0)
                Refinement.Refined(WorkerSeedDisclosure(categories.toSet(), estimatedBytes))
