...
internal class CoordinatedRootRuntimeDemander(
    private val client: WorkerControlClient,
    private val heap: IndexerHeapSize,
    private val budgetPolicy: RuntimeInvocationBudgetPolicy = RuntimeInvocationBudgetPolicy(),
) : RootRuntimeDemander {
    override fun demand(
        root: CanonicalRoot,
        ...
        val selected =
            when (val cache = startup.cacheIntent) {
                ...
                is StartupCacheIntent.Seed -> {
                    InstalledWorkerStartup.Seed(
                        ideHome,
                        (cache.sourceSystem as? StartupIdeaSystem.Explicit)?.path,
        ...
        val initial =
            when (val allowance = deadline.remaining()) {
                is Refinement.Refined -> allowance.value
                is Refinement.Rejected -> return rejected(WorkerControlFailure.DEADLINE_EXCEEDED)
            }
        return when (
            val admitted =
                runBlocking { withTimeoutOrNull(initial.value) { client.demand(root.path, heap, selected) } }
                    ?: return rejected(WorkerControlFailure.DEADLINE_EXCEEDED)
        ) {
            is InstalledWorkerStart.Rejected -> rejected(admitted.failure)
            is InstalledWorkerStart.Ready -> {
                if (admitted.endpoint.root != root.path || admitted.binding !is WorkerRouteBinding.Installation)
                ...
                val identity =
                    when (
                        val admittedIdentity =
                            WireRuntimeIdentity.admit(
                                root.path,
                                admitted.endpoint.runtimeId.value,
                                admitted.endpoint.attempt,
                            )
                    ) {
                ...
                when (val endpoint = RuntimeEndpoint.at(root, admitted.endpoint.runtimeId, admitted.endpoint.socket)) {
                    is RuntimeEndpointResolution.Resolved ->
                        RuntimeAdmission.Ready(
                            endpoint.endpoint,
                            workerBinding = admitted.binding,
                            deadline = deadline,
                            wireAuthority = RuntimeWireAuthority.Qualified(identity),
                        )
...
internal class CoordinatedRuntimeLifecycle(
    private val client: WorkerControlClient,
    private val local: RuntimeLifecycleController,
    ...
    override fun stop(endpoint: RuntimeEndpoint): RuntimeStopResult {
        ...
        return when (notified) {
            ...
            is InstalledWorkerStop.Rejected ->
                when (notified.failure) {
                    WorkerControlFailure.UNAVAILABLE,
                    WorkerControlFailure.LIFECYCLE_TRANSITION -> local.stop(endpoint)
...
private fun rejected(failure: WorkerControlFailure) =
    RuntimeAdmission.Rejected(RuntimeAdmissionFailure.WorkerControlRejected(failure))
