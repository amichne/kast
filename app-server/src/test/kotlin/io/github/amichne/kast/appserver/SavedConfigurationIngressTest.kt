...
class SavedConfigurationIngressTest {
    ...
    fun `unselected home configuration is never read`(@TempDir temporary: Path) {
        ...
        val loaded = InstalledSavedConfigurationIngress.load(mapOf("HOME" to home.toString())) as SavedConfigurationIngress.Loaded
        ...
        assertEquals(ConfigurationSource.DEFAULT, resolved.value.inspection().single { it.key == "KAST_INDEXER_MAX_HEAP" }.source)
    }
    ...
    fun `explicit saved selector rejects symlink and missing files`(@TempDir temporary: Path) {
        ...
        val symbolic = InstalledSavedConfigurationIngress.read(alias.toString(), emptyMap()) as SavedConfigurationIngress.Rejected
        assertEquals("NOT_REGULAR", symbolic.rejection.reason())
        val missing = InstalledSavedConfigurationIngress.read(directory.resolve("missing").toString(), emptyMap()) as SavedConfigurationIngress.Rejected
    ...
    fun `workspace overlay cannot follow a symlinked configuration ancestor`(@TempDir temporary: Path) {
        ...
        val result = InstalledWorkspaceConfigurationIngress.load(root, workspace, emptyMap()) as SavedConfigurationIngress.Rejected
    ...
    fun `literal shell expressions are data and never executed`(@TempDir temporary: Path) {
        ...
        val loaded = InstalledSavedConfigurationIngress.read(target.toString(), emptyMap()) as SavedConfigurationIngress.Loaded
