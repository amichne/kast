...
internal class LiveIntellijSourceRegionAccess(
    ...
    private fun selectInReadAction(
        ...
        val document = when (val admission = liveDocument(context, fileIdentity, request.anchor.readScope())) {
    ...
    private fun liveDocument(
        ...
        if (readScope is SourceReadScope.Constrained) {
            ...
            constraints.directory?.let { restriction ->
                val requested = Path.of(context.lease.workspaceRoot.value).resolve(restriction.directory.value).normalize()
                val actual = Path.of(file.path)
                val allowed = when (restriction.containment) {
                ...
                if (!allowed) return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.OUTSIDE_SOURCE_SCOPE)
            }
            constraints.packageName?.let { restriction ->
                ...
                val allowed = when (restriction.containment) {
                    ...
                    SymbolDiscoveryContainment.DESCENDANTS -> actual == restriction.packageName.value ||
...
private fun KtNamedDeclaration.candidateSelector(
    ...
    val candidateFile = (candidate.location as? SymbolDiscoveryCandidateLocation.Declaration)
        ...
        ?: return null
    val scope = when (val sourceScope = snapshot.readScope) {
        SourceReadScope.ExactFile -> SymbolSearchScope.ExactFile(
            candidateFile.path,
    ...
    val retainedConstraints = when (val sourceScope = snapshot.readScope) {
    ...
    val selection = when (
        val restored = SymbolDiscoverySelection.restore(snapshot.lease, scope, candidate, constraints)
    ) {
