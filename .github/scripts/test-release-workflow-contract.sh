...
require_contains "$ubuntu_debian_validator" 'kast runtime capabilities "${backend_args[@]}" --workspace-root="${KAST_UBUNTU_DEBIAN_SMOKE_WORKSPACE}" --accept-indexing=true --no-auto-start=true' "Ubuntu/Debian validator capabilities smoke must accept servable indexing state"
...
require_contains "$runtime_artifact_contract" "kast release package ubuntu-debian-bundle" "Runtime artifact docs must document bundle packaging"
