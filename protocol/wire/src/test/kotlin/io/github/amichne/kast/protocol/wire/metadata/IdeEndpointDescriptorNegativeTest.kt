...
class IdeEndpointDescriptorNegativeTest {
    ...
    fun `invalid root socket process and epoch values remain field-specific failures`() {
        listOf(
                candidate.copy(canonicalRoot = " ") to
                    IdeEndpointDescriptorFailure.InvalidCanonicalRoot(IdeEndpointPathFailure.BLANK),
                candidate.copy(canonicalRoot = "workspace/kast") to
                    IdeEndpointDescriptorFailure.InvalidCanonicalRoot(IdeEndpointPathFailure.NOT_ABSOLUTE),
                candidate.copy(canonicalRoot = "/workspace/../kast") to
                    IdeEndpointDescriptorFailure.InvalidCanonicalRoot(IdeEndpointPathFailure.NOT_NORMALIZED),
                candidate.copy(canonicalRoot = "/workspace/\u0000kast") to
                    IdeEndpointDescriptorFailure.InvalidCanonicalRoot(IdeEndpointPathFailure.CONTAINS_NUL),
                candidate.copy(canonicalRoot = "/" + "é".repeat(2_048)) to
                    IdeEndpointDescriptorFailure.InvalidCanonicalRoot(IdeEndpointPathFailure.TOO_LONG),
                candidate.copy(socketPath = "") to
                    IdeEndpointDescriptorFailure.InvalidSocketPath(IdeEndpointPathFailure.BLANK),
                candidate.copy(socketPath = "/tmp//kast.sock") to
                    IdeEndpointDescriptorFailure.InvalidSocketPath(IdeEndpointPathFailure.NOT_NORMALIZED),
                candidate.copy(socketPath = "/tmp/\u0000kast.sock") to
                    IdeEndpointDescriptorFailure.InvalidSocketPath(IdeEndpointPathFailure.CONTAINS_NUL),
                candidate.copy(socketPath = "/" + "a".repeat(103)) to
                    IdeEndpointDescriptorFailure.InvalidSocketPath(IdeEndpointPathFailure.TOO_LONG),
                candidate.copy(socketPath = "/" + "é".repeat(52)) to
                    IdeEndpointDescriptorFailure.InvalidSocketPath(IdeEndpointPathFailure.TOO_LONG),
                candidate.copy(processId = 0) to
                    IdeEndpointDescriptorFailure.InvalidProcessId(IdeProcessIdFailure.NOT_POSITIVE),
                candidate.copy(runtimeEpoch = -1) to
                    IdeEndpointDescriptorFailure.InvalidRuntimeEpoch(IdeRuntimeEpochFailure.NEGATIVE),
            )
    ...
    fun `unknown unsupported duplicate missing and reordered capabilities fail closed`() {
        assertRejected(
            candidate.copy(
                capabilities = candidate.capabilities + HostedCapabilityCandidate("other.read", emptyList())
            ),
            hostedCapabilityFailure(HostedCapabilitySetFailure.UnknownOperation(operationId("other.read"))),
        )
        assertRejected(
            candidate.copy(
                capabilities = candidate.capabilities + HostedCapabilityCandidate("a".repeat(97), emptyList())
            ),
        ...
        assertRejected(
            ...
            hostedCapabilityFailure(
                HostedCapabilitySetFailure.DuplicateOperation(
                    io.github.amichne.kast.protocol.contract.CanonicalOperation.WORKSPACE_LIFECYCLE
                )
            ),
        )
        listOf("index.sync", "topology.build").forEach { internalOperation ->
            assertRejected(
                candidate.copy(
                    capabilities = candidate.capabilities + HostedCapabilityCandidate(internalOperation, emptyList())
                ),
    ...
    private fun candidateDocument(): String =
        IdeEndpointDescriptorV2.create(
                candidate,
                policy,
            )
            .admittedDescriptor()
    ...
    private fun compatibilityFailure(failure: IdeHostCompatibilityFailure) =
        IdeEndpointDescriptorFailure.CompatibilityRejected(failure)

    private fun rejectedCompatibilityMismatch(invalid: IdeEndpointDescriptorCandidate): IdeHostCompatibilityMismatch {
        val descriptorFailure =
            when (val admission = IdeEndpointDescriptorV2.create(invalid, policy)) {
                is IdeEndpointDescriptorAdmission.Admitted -> fail<Nothing>("endpoint unexpectedly admitted")
                is IdeEndpointDescriptorAdmission.Rejected -> admission.failure
            }
        val compatibilityFailure =
            when (descriptorFailure) {
    ...
    private fun IdeHostCompatibilityMismatch.identityValues(): Pair<String, String> =
        when (this) {
    ...
    private fun hostedCapabilityFailure(failure: HostedCapabilitySetFailure) =
    ...
    private fun operationId(raw: String): OperationId =
        when (val parsed = OperationId.parse(raw)) {
