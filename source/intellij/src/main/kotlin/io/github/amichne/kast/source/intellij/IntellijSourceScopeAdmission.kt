...
internal fun SymbolDiscoveryConstraints.forSourceDeclaration(kind: DeclarationKind): SymbolDiscoveryConstraints {
    ...
    val selection =
        when (val refined = SymbolDiscoveryDeclarationKinds.from(setOf(compilerKind))) {
                ...
                val selected =
                    when (scope) {
                        ...
                        is SymbolSearchScope.SourceSet ->
                            owner.project == scope.project && owner.sourceSet == scope.sourceSet
                ...
                val sourceKind =
                    when (scope.sourceKinds) {
                        ...
                        SymbolSourceKindPolicy.PRODUCTION_AND_TEST ->
                            owner.sourceKind == WorkspaceSourceRootKind.PRODUCTION ||
                                owner.sourceKind == WorkspaceSourceRootKind.TEST
                    }
                val provenance =
                    scope.generatedSources == SymbolGeneratedSourcePolicy.INCLUDE ||
                        owner.provenance == WorkspaceSourceRootProvenance.AUTHORED
                val sourceSet =
                    when (val selection = readScope.constraints.sourceSets) {
