...
internal class LiveIntellijSourceRegionAccess(
        ...
        return try {
            ...
            observation.unexpected(
                io.github.amichne.kast.workspace.intellij.read.IntellijReadUnexpectedFailure.capture(
                    io.github.amichne.kast.workspace.intellij.read.IntellijReadStage.SOURCE,
                    failure,
                    limits,
                )
            )
    ...
    private fun selectInReadAction(
        ...
        val fileIdentity =
            when (val anchor = request.anchor) {
                ...
                is io.github.amichne.kast.source.contract.SourceReadAnchor.Source -> anchor.selector.snapshot.file
                is io.github.amichne.kast.source.contract.SourceReadAnchor.Candidate -> anchor.selector.workspaceFile()
            } ?: return regionRejected(IntellijSourceReadRejection.OUTSIDE_SOURCE_SCOPE)
        val document =
            when (val admission = liveDocument(context, fileIdentity, request.anchor.readScope())) {
        ...
        val anchor =
            when (val requested = request.anchor) {
                is io.github.amichne.kast.source.contract.SourceReadAnchor.Symbol ->
        ...
        val selected =
            deriveRegion(admittedAnchor, request.region)
                ?: return regionRejected(IntellijSourceReadRejection.REGION_ABSENT)
        val selectedRange =
            document.snapshot.sourceRange(selected.range)
    ...
    private fun liveDocument(
        ...
        val file =
            VirtualFileManager.getInstance().findFileByNioPath(Path.of(fileIdentity.path.value))?.takeIf { it.isValid }
                ?: return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.SOURCE_UNAVAILABLE)
        if (!fileAdmission(Path.of(file.path)) || !IntellijProjectSourceMembership.contains(project, file)) {
            return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.OUTSIDE_SOURCE_SCOPE)
        }
        ...
        if (!PsiDocumentManager.getInstance(project).isCommitted(document)) {
            return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.PSI_DOCUMENT_UNCOMMITTED)
        }
        val psiFile =
            PsiManager.getInstance(project).findFile(file) as? KtFile
                ?: return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.SOURCE_UNAVAILABLE)
        if (readScope is SourceReadScope.Constrained) {
            val constraints = readScope.constraints
            constraints.directory?.let { restriction ->
                val requested =
                    Path.of(context.lease.workspaceRoot.value).resolve(restriction.directory.value).normalize()
                val actual = Path.of(file.path)
                val allowed =
                    when (restriction.containment) {
                        SymbolDiscoveryContainment.DIRECT -> actual.parent == requested
                        SymbolDiscoveryContainment.DESCENDANTS -> actual.startsWith(requested)
                    }
                if (!allowed)
                    return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.OUTSIDE_SOURCE_SCOPE)
            }
            constraints.packageName?.let { restriction ->
                val actual = psiFile.packageFqName.asString()
                val allowed =
                    when (restriction.containment) {
                        SymbolDiscoveryContainment.DIRECT -> actual == restriction.packageName.value
                        SymbolDiscoveryContainment.DESCENDANTS ->
                            actual == restriction.packageName.value ||
                                actual.startsWith("${restriction.packageName.value}.")
                    }
                if (!allowed)
                    return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.OUTSIDE_SOURCE_SCOPE)
            }
        ...
        val length =
            when (val parsed = Utf16CodeUnitCount.parse(text.length)) {
                is Refinement.Refined -> parsed.value
                is Refinement.Rejected ->
                    return LiveSourceDocumentAdmission.Rejected(IntellijSourceReadRejection.CONTRACT_VIOLATION)
            }
    ...
    private fun admitCandidateAnchor(
        ...
        return when (selector) {
            ...
            is CandidateSelector.Range -> {
                ...
                val element =
                    if (selector.startInclusive.value < document.text.length) {
            ...
            is CandidateSelector.Declaration -> {
                val declaration =
                    when (val lookup = exactCandidateDeclaration(document.psiFile, selector)) {
                        ...
                        CandidateDeclarationLookup.Ambiguous ->
                            return LiveSourceAnchorResult.Rejected(IntellijSourceReadRejection.AMBIGUOUS_ANCHOR)
                    }
                val range =
    ...
    private fun admitSymbolAnchor(
        ...
        when (RevalidatedSymbolSelector.validate(selector, evidence)) {
            ...
            is Refinement.Rejected ->
                return LiveSourceAnchorResult.Rejected(IntellijSourceReadRejection.DECLARATION_MOVED_OR_CHANGED)
        }
        val range =
            document.snapshot.sourceRange(declaration.textRange)
                ?: return LiveSourceAnchorResult.Rejected(IntellijSourceReadRejection.CONTRACT_VIOLATION)
    ...
    private fun admitSourceAnchor(
        ...
        val element =
            if (selector.range.startInclusive.value < document.text.length) {
        ...
        val regionSelector =
            when (selector) {
                ...
                is SourceSelector.NestedRegion -> selector
...
private fun deriveRegion(
    ...
    when (selection) {
        ...
        RegionSelection.File ->
            anchor.element?.containingFile?.let { file ->
                NativeSourceRegion(
        ...
        is RegionSelection.Body ->
            when (selection.kind) {
                BodyKind.CALLABLE ->
                    anchor.element.nearestCallable()?.bodyExpression?.let {
                        NativeSourceRegion(it.textRange, SourceRegionKind.CALLABLE_BODY, it)
                    }
                BodyKind.CLASS ->
                    anchor.element.nearestClass()?.body?.let {
                        NativeSourceRegion(it.textRange, SourceRegionKind.CLASS_BODY, it)
                    }
            }
        is RegionSelection.Enclosing ->
            when (selection.kind) {
                EnclosingRegionKind.DECLARATION ->
                    anchor.element
                        .strictAncestors(anchor.selector)
                        .filterIsInstance<KtDeclaration>()
                        .firstOrNull()
                        ?.let { NativeSourceRegion(it.textRange, SourceRegionKind.DECLARATION, it) }
                EnclosingRegionKind.CALLABLE_BODY ->
                    anchor.element
                        .strictAncestors(anchor.selector)
                ...
                EnclosingRegionKind.CLASS_BODY ->
                    anchor.element.strictAncestors(anchor.selector).filterIsInstance<KtClassBody>().firstOrNull()?.let {
                        NativeSourceRegion(it.textRange, SourceRegionKind.CLASS_BODY, it)
                    }
            }
    }
...
private fun PsiElement.exactRegionElement(range: TextRange): PsiElement =
    generateSequence(this) { it.parent }.firstOrNull { element -> element.textRange == range } ?: this
...
private class NativeSourceEntityEnumerator(
    ...
    private fun visitReference(
        ...
        val name = reference.referencedName() ?: return qualify(SourceReadLimitation.UNSUPPORTED_ENTITY)
        val selector =
            issueEntitySelector(
                reference.textRange,
    ...
    private fun visitParameter(
        ...
        val entity =
            when (
                val created =
                    SourceEntity.ValueParameter.create(
                        selector,
                        sourceNestingDepth(parent.depth),
                    )
            ) {
    ...
    private fun visitAnonymousCallable(
        ...
        val range =
            document.snapshot.sourceRange(body.textRange)
                ?: return reject(IntellijSourceReadRejection.CONTRACT_VIOLATION)
        val selector =
            when (
                val issued =
    ...
    private fun issueEntitySelector(
        ...
        when (val admitted = SourceEntityName.present(name)) {
    ...
    private fun issueEntitySelector(
        ...
        val nonEmpty =
            when (val admitted = NonEmptySourceRange.create(range)) {
                is Refinement.Refined -> admitted.value
                is Refinement.Rejected -> return null
            }
...
private fun KtNamedDeclaration.sourceDeclarationKind(): DeclarationKind? =
    when (this) {
...
private fun KtNamedDeclaration.sourceEntityName(kind: DeclarationKind): String? =
    name
        ?: if (kind == DeclarationKind.CONSTRUCTOR) {
...
private fun KtNamedDeclaration.candidateSelector(
    ...
    val candidateFile =
        (candidate.location as? SymbolDiscoveryCandidateLocation.Declaration)?.file
            as? SymbolDiscoveryFileIdentity.Workspace ?: return null
    val scope =
        when (val sourceScope = snapshot.readScope) {
            SourceReadScope.ExactFile ->
                SymbolSearchScope.ExactFile(
                    candidateFile.path,
                    SymbolSourceKindPolicy.PRODUCTION_AND_TEST,
                    when (snapshot.context) {
                        is SourceReadContext.Published -> SymbolGeneratedSourcePolicy.INCLUDE
                        is SourceReadContext.Live -> SymbolGeneratedSourcePolicy.EXCLUDE
                    },
                )
            is SourceReadScope.Constrained -> sourceScope.scope
        }
    val retainedConstraints =
        when (val sourceScope = snapshot.readScope) {
            SourceReadScope.ExactFile -> SymbolDiscoveryConstraints.None
            is SourceReadScope.Constrained -> sourceScope.constraints
        }
    val constraints = retainedConstraints.forSourceDeclaration(kind)
    val selection =
        when (val restored = SymbolDiscoverySelection.restore(snapshot.lease, scope, candidate, constraints)) {
...
private fun KaSymbolVisibility.sourceVisibility(): DeclarationVisibility? =
    when (this) {
...
private fun sourceNestingDepth(raw: Int): SourceNestingDepth =
    when (val parsed = SourceNestingDepth.parse(raw)) {
        is Refinement.Refined -> parsed.value
...
private fun exactCandidateDeclaration(
    ...
    val location =
        candidate.location as? SymbolDiscoveryCandidateLocation.Declaration ?: return CandidateDeclarationLookup.Missing
...
private fun KtNamedDeclaration.matches(kind: io.github.amichne.kast.symbol.contract.SymbolDiscoveryKind): Boolean =
    when (kind) {
        io.github.amichne.kast.symbol.contract.SymbolDiscoveryKind.CLASS -> this is KtClassOrObject
        io.github.amichne.kast.symbol.contract.SymbolDiscoveryKind.SYMBOL -> sourceDeclarationKind() != null
        io.github.amichne.kast.symbol.contract.SymbolDiscoveryKind.FILE,
        io.github.amichne.kast.symbol.contract.SymbolDiscoveryKind.TEXT -> false
    }

private fun KtNamedDeclaration.matchesCompilerKinds(constraints: SymbolDiscoveryConstraints): Boolean {
    val selection = constraints.declarationKinds ?: return true
    val kind =
        when (this) {
            ...
            is KtParameter -> if (hasValOrVar()) CompilerSymbolKind.PROPERTY else return false
            is KtTypeAlias -> CompilerSymbolKind.TYPE_ALIAS
            else -> return false
        }
    return kind in selection.values
}
...
private fun KaSymbol.sourceEntityTarget(document: LiveSourceDocument): SourceEntityTarget {
    val declaration =
        psi as? KtNamedDeclaration ?: return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
    val kind =
        declaration.sourceDeclarationKind()
            ?: return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
    val name =
        declaration.sourceEntityName(kind)
            ?: return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
    val candidate =
        declaration.candidateSelector(document.snapshot, kind, name)
            ?: return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
...
private fun KtNamedDeclaration.localSourceTarget(document: LiveSourceDocument): SourceEntityTarget {
    if (containingFile?.virtualFile?.url != document.psiFile.virtualFile?.url) {
        return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
    }
    val range =
        document.snapshot.sourceRange(textRange)
            ?: return SourceEntityTarget.Unresolved(CompilerUnresolvedReason.UNSUPPORTED_TARGET)
