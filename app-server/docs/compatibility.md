...
| --- | --- |
| Module direction | Compiler and architecture verification: CLI depends on app-server; app-server has no CLI imports or project dependency. Semantic/runtime modules are unchanged. |
...
| Native representation | Regression tests initially failed against MCP relabeling; now native started/completed and history items retain their complete original documents. Kast summaries and structured results share the native dynamic-tool content channel. |
...
| Former presentation relabeling | Live/history no longer convert dynamic calls to MCP or file changes. Native identity tests cover exact preservation. | App-server protocol adapters | Keep real desktop rendering/history acceptance alongside these contract tests. |
