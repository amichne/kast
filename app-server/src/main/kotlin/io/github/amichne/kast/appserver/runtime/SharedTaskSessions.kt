...
internal value class ClientConnectionId private constructor(val value: String) {
    companion object {
        ...
        fun admit(raw: String): ClientConnectionId? =
            raw.takeIf { it.length == 36 && runCatching { UUID.fromString(it).toString() == it }.getOrDefault(false) }
...
internal value class ControllerLeaseId private constructor(val value: String) {
    companion object {
        fun fresh() = ControllerLeaseId(UUID.randomUUID().toString())
    }
}
...
internal data class TaskSessionView(
    val thread: BrokerThreadId,
    val controller: ClientConnectionId?,
    val observers: Set<ClientConnectionId>,
...
internal class SharedTaskSessions(private val capacity: Int = BrokerOperationalLimits.maximumTaskSessions) {
    private class Session(val thread: BrokerThreadId, initial: ClientConnectionId?) {
        private var control: TaskController =
            initial?.let { TaskController.Leased(it, ControllerLeaseId.fresh()) } ?: TaskController.Unclaimed
        var controller: ClientConnectionId?
            get() = (control as? TaskController.Leased)?.owner
            set(owner) {
                control =
                    owner?.let { TaskController.Leased(it, ControllerLeaseId.fresh()) } ?: TaskController.Unclaimed
            }

        val lease: ControllerLeaseId?
            get() = (control as? TaskController.Leased)?.lease
    ...
    @Synchronized
    fun connect(id: ClientConnectionId) {
        connections += id
    }
    ...
    @Synchronized
    fun attach(thread: BrokerThreadId, client: ClientConnectionId): ControlResult {
    ...
    fun claim(thread: BrokerThreadId, client: ClientConnectionId): ControlResult {
        ...
        if (client !in connections || client !in current.observers)
            return ControlResult.Rejected(ControlFailure.CONNECTION_UNKNOWN)
        if (current.activity is TaskActivity.ReconciliationRequired)
            return ControlResult.Rejected(ControlFailure.RECONCILIATION_REQUIRED)
        if (current.controller == client) return ControlResult.Accepted
        if (current.activity != TaskActivity.Idle || current.pending.isNotEmpty())
            return ControlResult.Rejected(ControlFailure.TASK_BUSY)
    ...
    @Synchronized
    fun release(thread: BrokerThreadId, client: ClientConnectionId): ControlResult {
        val current = sessions[thread] ?: return ControlResult.Rejected(ControlFailure.TASK_UNKNOWN)
    ...
    @Synchronized
    fun authorize(thread: BrokerThreadId, client: ClientConnectionId): ControlResult =
        when (val current = sessions[thread]) {
            null -> ControlResult.Rejected(ControlFailure.TASK_UNKNOWN)
                ...
                else if (current.controller == client) ControlResult.Accepted
                else ControlResult.Rejected(ControlFailure.NOT_CONTROLLER)
        }

    @Synchronized
    fun begin(thread: BrokerThreadId, turn: String) {
        sessions[thread]?.activity = TaskActivity.Active(turn)
    }

    @Synchronized
    fun finish(thread: BrokerThreadId, turn: String? = null) {
    ...
    @Synchronized
    fun pending(thread: BrokerThreadId, request: String) {
        sessions[thread]?.pending?.add(request)
    }

    @Synchronized
    fun resolved(thread: BrokerThreadId, request: String) {
        sessions[thread]?.let {
    ...
    @Synchronized
    fun unsubscribe(thread: BrokerThreadId, client: ClientConnectionId) {
        sessions[thread]?.observers?.remove(client)
    }
    ...
    fun disconnect(client: ClientConnectionId) {
        ...
        sessions.values.forEach {
            it.observers -= client
            if (it.controller == client && it.activity == TaskActivity.Idle && it.pending.isEmpty())
                it.controller = null
        }
    }

    @Synchronized
    fun reconcileIdle(thread: BrokerThreadId) {
        sessions[thread]?.let {
    ...
    @Synchronized
    fun requireReconciliation(thread: BrokerThreadId) {
        sessions[thread]?.let {
    ...
    @Synchronized
    fun upstreamLost(client: ClientConnectionId) {
        disconnect(client)
        sessions.values
            .filter { it.controller == client }
            .forEach {
                it.activity = TaskActivity.ReconciliationRequired((it.activity as? TaskActivity.Active)?.turnId)
                it.controller = null
            }
    ...
    @Synchronized
    fun reconcile(thread: BrokerThreadId, turn: String, terminal: Boolean) {
    ...
    @Synchronized
    fun observers(thread: BrokerThreadId): Set<ClientConnectionId> = sessions[thread]?.observers?.toSet().orEmpty()

    @Synchronized fun controller(thread: BrokerThreadId): ClientConnectionId? = sessions[thread]?.controller

    @Synchronized
    fun views(): List<TaskSessionView> =
        sessions.values.map {
            TaskSessionView(it.thread, it.controller, it.observers.toSet(), it.activity, it.pending.size, it.lease)
        }
}
