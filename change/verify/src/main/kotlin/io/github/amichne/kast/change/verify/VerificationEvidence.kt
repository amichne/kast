...
class ObservedAddDeclarationDelta private constructor(private val identity: ExpectedAddDeclarationDelta) {
    ...
    companion object {
        /**
         * Proof transition: `(String, String, AddDeclarationKind, Int) -> Refinement< ObservedAddDeclarationDelta,
         * ObservedAddDeclarationDeltaFailure>`.
         *
         * Establishes one unique compiler-projected canonical package and declaration identity.
         * [ObservedAddDeclarationDeltaFailure] is the closed expected failure. Raw compiler names and match count may
         * enter only at the result-generation semantic observation boundary.
         */
        ...
        ): Refinement<ObservedAddDeclarationDelta, ObservedAddDeclarationDeltaFailure> =
            when (matchingDeclarationCount) {
                ...
                is Refinement.Rejected ->
                    Refinement.Rejected(
                        when (admitted.failure) {
                            io.github.amichne.kast.change.contract.ExpectedAddDeclarationDeltaFailure
                                .PACKAGE_NAME_INVALID -> ObservedAddDeclarationDeltaFailure.PACKAGE_NAME_INVALID
                            io.github.amichne.kast.change.contract.ExpectedAddDeclarationDeltaFailure
                                .DECLARATION_NAME_INVALID -> ObservedAddDeclarationDeltaFailure.DECLARATION_NAME_INVALID
                        }
                    )
            }
    }
...
class AcceptedAddDeclarationSemanticDelta
private constructor(
    ...
    companion object {
         ...
         * [AddDeclarationSemanticDeltaFailure] is the closed expected failure. No raw extraction is permitted beyond
         * this pure comparator.
         */
        fun compare(
...
sealed interface ChangeVerificationObservation {
    data class Observed(val evidence: ChangeVerificationEvidence) : ChangeVerificationObservation

    data class Rejected(val reason: ChangeVerificationObservationRejection) : ChangeVerificationObservation
}

/** Result-generation compiler observation boundary. */
fun interface ChangeVerificationObserver {
    /**
     * Proof transition: `AddDeclarationVerificationObservationRequest -> AddDeclarationVerificationObservation`.
     *
     * Observed output carries detached source, relation, diagnostic, and declaration identity evidence for G1. Expected
     * compiler-boundary failure is closed by [AddDeclarationVerificationObservationRejection]. Live platform values
     * never escape.
     */
    fun observe(request: ChangeVerificationObservationRequest): ChangeVerificationObservation
}
...
class CompleteAddDeclarationVerification
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(AddDeclarationChangePlan, AppliedUnverified, DistinctResultingWorkspace,
         * AddDeclarationVerificationEvidence) -> Refinement< CompleteAddDeclarationVerification,
         * Set<AddDeclarationProofFailure>>`.
         *
         * Establishes exact resulting source content, complete generation-bound relation and diagnostic coverage,
         * unchanged existing semantic relations, error-free diagnostics, accepted declaration delta, and the exhaustive
         * planned obligation set. [AddDeclarationProofFailure] is the closed expected failure. Raw platform extraction
         * is prohibited; only detached contract evidence enters this pure evaluator.
         */
        fun admit(
            ...
            return when (semanticDelta) {
                is Refinement.Rejected -> Refinement.Rejected(setOf(AddDeclarationProofFailure.SEMANTIC_DELTA_REJECTED))
