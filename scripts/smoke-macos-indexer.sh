...
identity_file="${KAST_SMOKE_IDENTITY_FILE:-${workspace}/build/kast-indexer-smoke-identity.json}"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/kast-macos-indexer-smoke.XXXXXX")"
cleanup() {
...
grep -Eq '^backend: indexer$' "$scratch/up.toon" || {
  printf 'error: public Kast did not select the indexer\n' >&2
  exit 1
}
...
"$kastctl_binary" --output json status \
  --workspace-root "$workspace" >"$scratch/status.json"
...
if descriptor.get("backendName") != "indexer":
    raise SystemExit("status selected a non-indexer runtime")
if runtime.get("state") != "READY" or runtime.get("healthy") is not True:
    raise SystemExit("indexer is not healthy and READY")
...
if missing:
    raise SystemExit(f"indexer descriptor has incomplete identity: {', '.join(missing)}")

if identity_path.exists():
    baseline = json.loads(identity_path.read_text())
    if baseline.get("identity") != identity:
        raise SystemExit("foreground state changed the exact indexer identity")
...
printf 'PASS %s: exact-root indexer stayed READY\n' "$scenario"
