...
class EpochSignalCharacterizationTest {
    ...
    fun `two samples detect every supported stable and moving fixture`() {
        ...
        assertEquals(
            1_000,
            observed.single { case -> case.caseId == EpochCaseId.VFS_EVENT_STORM }.vfsEventCount,
        )
        assertEquals(
            listOf(EpochProjectModelTransition.GRADLE_IMPORT_COMPLETED),
            observed.single { case -> case.caseId == EpochCaseId.GRADLE_IMPORT_COMPLETED }.projectModelTransitions,
        )
        assertEquals(
            listOf(EpochProjectModelTransition.GRADLE_ROOT_CHANGED),
            observed.single { case -> case.caseId == EpochCaseId.GRADLE_ROOT_MOVEMENT }.projectModelTransitions,
        )
        assertEquals(
            listOf(EpochDumbModeState.SMART, EpochDumbModeState.SMART),
            observed.single { case -> case.caseId == EpochCaseId.SMART_DUMB_SMART }.dumbModeSamples,
        )
        assertEquals(
            listOf(
            ...
            observed.single { case -> case.caseId == EpochCaseId.SMART_DUMB_SMART }.dumbModeTransitions,
        )
    }
    ...
    fun `root-filtered VFS events detect exact inbound outbound and storm movement`() {
        assertEquals(
            0,
            rootFilteredBatchCount(listOf(EpochVfsObservedEvent.Change(Path.of("/workspace/kast-other/A.kt")))),
        )
        assertEquals(
            1,
            rootFilteredBatchCount(listOf(EpochVfsObservedEvent.Change(Path.of("/workspace/kast/src/A.kt")))),
        )
        assertEquals(
            1,
        ...
        assertEquals(
            ...
            rootFilteredBatchCount(
                List(1_000) { index ->
                    EpochVfsObservedEvent.Change(Path.of("/workspace/kast/src/Event$index.kt"))
                }
            ),
        )
    }
...
class IdeProjectReadEpochTest {
    ...
    fun `live source maps lifecycle dumb root Gradle stage and preemption failures`() {
        ...
        val cases =
            listOf(
                ...
                failure(
                    { it.dumbStates = ArrayDeque(listOf(true)) },
                ...
                failure(
                    { it.gradle = Refinement.Rejected(ProjectReadEpochObservationFailure.GradleModelUnavailable) },
                    ProjectReadEpochObservationFailure.GradleModelUnavailable,
                ),
                failure(
                    { it.throwAt = ProjectReadEpochObservationStage.ROOT_MODEL },
                    ProjectReadEpochObservationFailure.ObservationFailed(ProjectReadEpochObservationStage.ROOT_MODEL),
                ),
                failure(
                    { it.dumbStates = ArrayDeque(listOf(false, true)) },
                    ProjectReadEpochObservationFailure.DumbMode,
                ),
            )
        ...
        assertEquals(
            ...
            assertRejected(liveObservation(execution = RecordingProjectReadEpochExecution(true))).failure,
        )
        assertEquals(
            ProjectReadEpochObservationFailure.ObservationFailed(ProjectReadEpochObservationStage.THREAD),
            assertRejected(
                    liveObservation(
                        execution =
        ...
        assertEquals(
            ...
            assertRejected(
                    liveObservation(
                        execution = RecordingProjectReadEpochExecution(failure = ReadAction.CannotReadException())
                    )
                )
                .failure,
        )
        assertThrows(ProcessCanceledException::class.java) {
            liveObservation(
                platform =
                    RecordingProjectReadEpochPlatform().apply {
    ...
    fun `dumb malformed unavailable incoherent and exhausted observations reject exactly`() {
        ...
        val cases =
            listOf(
                ...
                stable.copy(psiModificationCount = signal(-1)) to ProjectReadEpochObservationFailure.SignalExhausted,
                stable.copy(
                    rootFilteredVfsBatchCount =
    ...
    fun `root-filtered production counter advances once per bounded VFS batch`() {
        assertEquals(
            signal(0),
            observeVfsBatch(listOf(ProjectReadEpochVfsEvent.Change("/workspace/kast-other/A.kt"))),
        )
        assertEquals(
            signal(1),
            observeVfsBatch(
        ...
        listOf("relative/A.kt", "/" + "a".repeat(4_096), "/" + "€".repeat(2_731)).forEach { raw ->
            assertEquals(
                ProjectReadEpochSignalSample.Rejected(ProjectReadEpochObservationFailure.VfsPathMalformed),
    ...
    fun `admitted project exposes only the typed epoch observation`() {
        val method =
            AdmittedIdeProject::class.java.methods.single { candidate ->
    ...
    private fun observeVfsBatch(events: List<ProjectReadEpochVfsEvent>): ProjectReadEpochSignalSample {
...
private class AdapterEpochSource(var boundary: ProjectReadEpochBoundary) {
...
private fun stableProjectReadEpochBoundary() =
...
private fun assertRejected(observation: ProjectReadEpochObservation): ProjectReadEpochObservation.Rejected =
    observation as? ProjectReadEpochObservation.Rejected
