...
internal fun VerifiedMutationFixture.observedPrecondition(plan: ChangePlan = this.plan): ObservedMutationPrecondition =
    when (plan) {
        ...
        is ReplaceDeclarationChangePlan -> observedSource(plan)
    }
...
internal fun VerifiedMutationFixture.addFilePlan(): AddFileChangePlan {
    ...
    val target =
        CreatableKotlinFileTarget.admit(AddFileTargetObservation(workspace, file, plan.target.sourceRoot.owner))
            .refined()
    val result =
...
internal fun VerifiedMutationFixture.addFileEvidence(applied: AppliedUnverified): AddFileVerificationEvidence {
    val scope =
