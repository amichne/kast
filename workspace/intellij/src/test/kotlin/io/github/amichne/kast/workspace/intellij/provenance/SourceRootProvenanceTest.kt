...
class SourceRootProvenanceTest {
    ...
    fun `producer evidence wins over source-root path names`() {
        val authoredLookingGenerated = workspaceDirectory.resolve("src/main/kotlin").toAbsolutePath().normalize()
        val generatedLookingAuthored =
            workspaceDirectory.resolve("build/generated/authored").toAbsolutePath().normalize()
        val authority =
            GradleSourceRootProvenanceAuthority.compile(
                listOf(
                    captured(
    ...
    fun `conflicting exact entries are rejected before import`() {
        ...
        val imported =
            combineGradleSourceRootProducerEvidence(
                ideaEntries = listOf(idea(root, GradleSourceRootProducerProvenance.AUTHORED)),
                producerEntries =
                    listOf(
    ...
    fun `conflicting exact IDEA entries are rejected before import`() {
        ...
        val imported =
            combineGradleSourceRootProducerEvidence(
                ...
                producerEntries = listOf(producer(root, GradleSourceRootProducerProvenance.GENERATED)),
            )
    ...
    fun `producer entries projected outside the standard IDEA model remain authoritative`() {
        ...
        val testFixturesRoot = workspaceDirectory.resolve("src/testFixtures/kotlin").toAbsolutePath().normalize()

        val imported =
            combineGradleSourceRootProducerEvidence(
                ideaEntries = listOf(idea(mainRoot, GradleSourceRootProducerProvenance.AUTHORED)),
                producerEntries =
                    listOf(
