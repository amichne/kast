...
class InstalledWorkerClientOwnerTest {
    @Test
    fun `current broker descendant reuses its publication without ensuring its own service`(): Unit = runBlocking {
        ...
        try {
            ...
            val configurationEnvironment =
                BrokerServiceDemandContext.configurationEnvironment(environment + ("KAST_ENABLE_APP_SERVER" to "1"))
            var ensures = 0
    ...
    @Test
    fun `partial or mismatched broker descendant authority fails closed`(): Unit = runBlocking {
        val root = Files.createTempDirectory(Path.of("/private/tmp"), "kast-descendant-").toRealPath()
        try {
            listOf("bin", "lib", "share").forEach { Files.createDirectory(root.resolve(it)) }
            val kast = Files.writeString(root.resolve("bin/kast"), "#!/bin/sh\nexit 0\n")
    ...
    @Test
    fun `self consistent forged epoch and live status cannot replace physical installation authority`(): Unit =
        runBlocking {
            ...
            try {
                ...
                val options =
                    (InstalledCoordinatorConfiguration.admit(kast, root, environment) as Refinement.Refined).value
                val actual = (BrokerInstallationState.admit(root) as Refinement.Refined).value
                val forged =
                    (ThreadBindingOwner.admit("forged-installation", actual.stateEpoch.value.toString())
                ...
                val control =
                    (WorkspaceRuntimeControl.create(
                            ...
                            InstalledWorkerEffects.Unavailable,
                        ) as Refinement.Refined)
                        .value
                val frontend = DeferredBrokerFrontend { BrokerFrontendAdmission.Rejected }
                val server =
                    (KtorBrokerServer.startCoordinator(options.socket, control, frontend)
                            as KtorBrokerServerStart.Started)
                        .server
                try {
                    readiness.ready()
                    Files.writeString(
                        root.resolve("state/epoch.json"),
                        buildJsonObject {
                            put("schemaVersion", 1)
                            put("installation", forged.installationId.value)
                            put("epoch", forged.stateEpoch.value.toString())
                        }
                            .toString(),
                    )
                    val connection =
                    ...
                    try {
                        ...
                        val response =
                            Json.parseToJsonElement((connection.receive() as BrokerUpstreamFrame.Text).message)
                                .jsonObject
                        assertEquals(
                            forged.installationId.value,
                            response.getValue("installationId").jsonPrimitive.content,
                        )
                        assertEquals(
                            forged.stateEpoch.value.toString(),
                            response.getValue("stateEpoch").jsonPrimitive.content,
                        )
                    } finally {
                        connection.close()
                    }
                    assertEquals(
                        CoordinatorStatusRead.Rejected(WorkerControlFailure.IDENTITY_REJECTED),
                        InstalledWorkerClient(kast, root, emptyMap()).status(command),
                    )
                } finally {
                    server.close()
                    readiness.retire()
