...
private constructor(
    ...
    companion object {
         ...
         *
         * Establishes deterministic unique compiler facts whose declarations and outgoing edges belong to the exact
         * candidate file. [CompleteTopologyFileFailure] is the closed expected failure. Only the terminal K2 extraction
         * boundary may submit raw fact collections.
         */
        fun admit(
...
data class TopologyCandidateEvidenceMismatch
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(TopologySourceFile, TopologySourceFile) -> TopologyCandidateEvidenceComparison`.
         *
         * A mismatched result establishes one same-path, non-equal candidate/completion pair while retaining both exact
         * workspace, source-root, path, and content-hash values. Exact and different-path inputs remain distinct closed
         * states. Completed compiler evidence may enter only at complete-generation admission.
         */
        internal fun compare(
            ...
            when {
                ...
                candidate == completed -> TopologyCandidateEvidenceComparison.Exact
                    ...
                    TopologyCandidateEvidenceComparison.Mismatched(
...
internal sealed interface TopologyCandidateEvidenceComparison {
    ...
    data class Mismatched(val evidence: TopologyCandidateEvidenceMismatch) : TopologyCandidateEvidenceComparison
}

/** Lowercase SHA-256 over the canonical complete topology projection. */
@JvmInline
value class TopologyGenerationDigest internal constructor(val value: String) {
    companion object {
        /**
         * Proof transition: `String -> Refinement<TopologyGenerationDigest, TopologyGenerationDigestFailure>`.
         *
         * Establishes the canonical lowercase SHA-256 form used by a persisted topology manifest.
         * [TopologyGenerationDigestFailure] is the closed expected failure. Raw digest text may enter only at the
         * SQLite result-set boundary.
         */
        fun parse(raw: String): Refinement<TopologyGenerationDigest, TopologyGenerationDigestFailure> =
...
class CompleteTopologyGeneration
private constructor(
    ...
    companion object {
        /**
         * Proof transition: `(PublishedWorkspace, List<TopologySourceFile>, List<CompleteTopologyFile>) ->
         * Refinement<CompleteTopologyGeneration, TopologyGenerationCoverageFailure>`.
         *
         * Establishes exact candidate-to-terminal coverage, one workspace identity, location-distinct topology symbols,
         * closed exact edge targets, canonical ordering, and a deterministic generation digest. Compiler identity may
         * repeat at distinct exact locations. [TopologyGenerationCoverageFailure] retains every finite coverage
        ...
        fun admit(
            ...
            val candidateEvidenceMismatches = buildSet {
                candidatePaths.intersect(completedPaths).forEach { path ->
                    val candidate = candidateGroups.getValue(path).singleOrNull() ?: return@forEach
                    val completedFile = completedGroups.getValue(path).singleOrNull()?.file ?: return@forEach
                    when (
                        val comparison =
                            TopologyCandidateEvidenceMismatch.compare(
                                candidate,
                                completedFile,
                            )
                    ) {
                        TopologyCandidateEvidenceComparison.DifferentPath,
                        TopologyCandidateEvidenceComparison.Exact -> Unit
                        is TopologyCandidateEvidenceComparison.Mismatched -> add(comparison.evidence)
                    }
                }
            }
            ...
            val duplicateSymbols =
                symbols
                    .groupBy(TopologySymbol::nodeIdentity)
            ...
            val edgeEndpoints =
                ordered.flatMap(CompleteTopologyFile::edges).flatMap { edge -> listOf(edge.source, edge.target) }
            val missingEdgeTargets =
                edgeEndpoints
                    .map(TopologySymbol::nodeIdentity)
                    .filterNot(knownIdentities::contains)
                    .toCollection(linkedSetOf())
            val mismatchedEdgeEndpoints =
                edgeEndpoints
                    .filter { endpoint ->
            ...
            if (
                failure.missing.isNotEmpty() ||
                    failure.unexpected.isNotEmpty() ||
                    ...
                    failure.duplicateSymbols.isNotEmpty() ||
                    failure.missingEdgeTargets.isNotEmpty() ||
            ...
            return Refinement.Refined(CompleteTopologyGeneration(identity, ordered, TopologyGenerationDigest(digest)))
        }
    }
}
...
private fun <Value> duplicatePaths(groups: Map<WorkspaceSourcePath, List<Value>>): Set<WorkspaceSourcePath> =
