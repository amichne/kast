...
enum class WireRuntimeIdentityFailure { ROOT_REJECTED, RUNTIME_REJECTED }

/** One installed process incarnation. Raw fields leave only at transport qualification. */
class WireRuntimeIdentity private constructor(
    val root: Path,
    ...
    companion object {
        ...
        fun admit(root: Path, runtimeId: String, bootstrapAttempt: SemanticRuntimeBootstrapAttemptId):
            Refinement<WireRuntimeIdentity, WireRuntimeIdentityFailure> = when {
            !root.isAbsolute || root.normalize() != root -> Refinement.Rejected(WireRuntimeIdentityFailure.ROOT_REJECTED)
            !runtimeId.matches(Regex("sha256:[a-f0-9]{64}")) -> Refinement.Rejected(WireRuntimeIdentityFailure.RUNTIME_REJECTED)
