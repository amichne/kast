...
internal fun SymbolSearchScope.selectorDocumentProjection(): SelectorScopeDocumentProjection =
    when (this) {
        is SymbolSearchScope.ExactFile ->
            SelectorScopeDocumentProjection.Projected(
        ...
        is SymbolSearchScope.SourceSet -> SelectorScopeDocumentProjection.Rejected
    }

/** Projects one closed symbol-file identity into its fixed document discriminator and value. */
internal fun SymbolDiscoveryFileIdentity.selectorDocumentProjection(): SelectorFileDocumentProjection =
    when (this) {
        is SymbolDiscoveryFileIdentity.Workspace ->
            SelectorFileDocumentProjection(
                WORKSPACE_FILE,
                path.value,
            )
 ...
 *
 * Admission dispatches the closed token discriminator, establishes its lease and workspace-file identity, and restores
 * exactly the facts carried by that variant. Primitive fields leave only while calling their owning contract
 * refinements.
 */
internal fun CandidateSelectorDocument.admitCandidateSelector(
    ...
    return when (this) {
        is CandidateSelectorDocument.Declaration ->
            when (val admitted = admitSelection(current)) {
                is SelectorDocumentAdmission.Admitted ->
                    when (val restored = CandidateSelector.declaration(admitted.value)) {
...
private fun CandidateSelectorDocument.Declaration.admitSelection(
    ...
    val lease =
        when (val admission = admitAuthority(root, generation, live, current)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val admittedScope =
        when (val admission = admitScope(this, lease.workspaceRoot)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val candidateKind =
        when (val admission = enumAdmission<SymbolDiscoveryKind>(kind)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    ...
    val fileIdentity =
        when (val admission = admitFile(fileType, file, lease.workspaceRoot)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val nativePath =
    ...
    return when (
        val restored =
            SymbolDiscoverySelection.restore(
                ...
                candidate,
                constraints =
                    when (val admitted = constraints.admitConstraints()) {
                        is SelectorDocumentAdmission.Admitted -> admitted.value
                        SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
                    },
            )
 ...
 *
 * Admission establishes every domain type and deterministic fingerprint carried by an exact selector. Rejection closes
 * malformed enum, path, scope, evidence, and restore state. Primitive fields leave only while calling their owning
...
internal fun ExactSelectorDocument.admitExactSelector(
    ...
    val symbolKind =
        when (val admission = enumAdmission<CompilerSymbolKind>(kind)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val compiler =
        when (val refined = CompilerSymbolIdentity.parse(compilerIdentity)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val signature =
        when (val refined = CanonicalCompilerSignature.restoreCanonicalEncoding(compilerSignature)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val evidence =
    ...
    val admittedFingerprint =
        when (val refined = SymbolSelectorFingerprint.parse(fingerprint)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    return when (
        val restored =
            SymbolSelector.restore(
                lease,
                admittedScope,
                evidence,
                admittedFingerprint,
 ...
 *
 * Admission establishes a canonical workspace root and positive evidence generation. Rejection closes malformed paths
 * and failed contract refinement. Raw values leave only at those contract boundaries.
 */
private fun admitAuthority(
    ...
    if (live != null) {
        ...
        return if (
            root == reference.workspaceRoot.value &&
                live.host == reference.host.value.toString() &&
                live.epoch == reference.epoch.value &&
                live.contentView == reference.contentView.name &&
                live.version == reference.version &&
                live.version == LiveSemanticReadReference.VERSION
        )
            SelectorDocumentAdmission.Admitted(current)
    ...
    val workspace =
        when (val refined = CanonicalWorkspaceRoot.fromCanonicalPath(path)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val admittedGeneration =
        when (val refined = EvidenceGeneration.parse(generation)) {
            is Refinement.Refined -> refined.value
            is Refinement.Rejected -> return SelectorDocumentAdmission.Rejected
        }
 ...
 *
 * Admission establishes one supported scope variant and its closed policies. Rejection closes contradictory optional
 * fields, unknown variants, enum values, and invalid exact-file paths.
 */
private fun admitScope(
    ...
    val sourceKinds =
        when (val admission = enumAdmission<SymbolSourceKindPolicy>(fields.sourceKinds)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    val generated =
        when (val admission = enumAdmission<SymbolGeneratedSourcePolicy>(fields.generatedSources)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
...
private fun admitWorkspaceScope(
    ...
    val libraries =
        when (
            val admission =
                fields.libraries?.let { enumAdmission<SymbolLibraryPolicy>(it) } ?: SelectorDocumentAdmission.Rejected
        ) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    return SelectorDocumentAdmission.Admitted(SymbolSearchScope.Workspace(sourceKinds, generated, libraries))
}
...
private fun admitExactFileScope(
    ...
    val path =
        when (val admission = admitPath(fields.scopeFile)) {
            is SelectorDocumentAdmission.Admitted -> admission.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
    return when (val refined = CanonicalWorkspaceFilePath.fromCanonicalPath(root, path)) {
        is Refinement.Refined ->
            SelectorDocumentAdmission.Admitted(SymbolSearchScope.ExactFile(refined.value, sourceKinds, generated))
        is Refinement.Rejected -> SelectorDocumentAdmission.Rejected
    }
}
 ...
 *
 * Admission establishes exactly one workspace or external discovery identity. Rejection closes unknown variants and all
 * path or URL refinement failures.
 */
private fun admitFile(
    kind: String,
    value: String,
    root: CanonicalWorkspaceRoot,
): SelectorDocumentAdmission<SymbolDiscoveryFileIdentity> =
    when (kind) {
        WORKSPACE_FILE ->
            when (val path = admitPath(value)) {
                is SelectorDocumentAdmission.Admitted ->
                    when (val refined = CanonicalWorkspaceFilePath.fromCanonicalPath(root, path.value)) {
                        is Refinement.Refined ->
                            SelectorDocumentAdmission.Admitted(SymbolDiscoveryFileIdentity.Workspace(refined.value))
                        is Refinement.Rejected -> SelectorDocumentAdmission.Rejected
                    }
                SelectorDocumentAdmission.Rejected -> SelectorDocumentAdmission.Rejected
            }
        EXTERNAL_FILE ->
            when (val refined = DetachedVirtualFileUrl.parse(value)) {
                is Refinement.Refined ->
                    SelectorDocumentAdmission.Admitted(SymbolDiscoveryFileIdentity.External(refined.value))
                is Refinement.Rejected -> SelectorDocumentAdmission.Rejected
            }
        else -> SelectorDocumentAdmission.Rejected
    }
...
private inline fun <reified Value : Enum<Value>> enumAdmission(value: String): SelectorDocumentAdmission<Value> =
    try {
...
): Refinement<Unit, CanonicalSelectorDecodingFailure> =
    when {
        ...
        root != current.workspaceRoot.value ->
            Refinement.Rejected(CanonicalSelectorDecodingFailure.INCOMPATIBLE_WORKSPACE)
        host != current.host.value.toString() ->
            Refinement.Rejected(CanonicalSelectorDecodingFailure.INCOMPATIBLE_AUTHORITY)
        epoch != current.epoch.value || contentView != current.contentView.name ->
            Refinement.Rejected(CanonicalSelectorDecodingFailure.STALE_AUTHORITY)
...
internal fun admitSelectorReference(
    ...
    if (live != null) {
        if (generation != null)
            return CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.INVALID_DOCUMENT)
        if (current !is LiveSemanticReadAuthority)
            return CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.LIVE_AUTHORITY_REQUIRED)
    ...
    return when (val published = admitAuthority(root, generation, null, null)) {
        is SelectorDocumentAdmission.Admitted ->
            when {
                current != null && published.value.workspaceRoot != current.workspaceRoot ->
                    CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.INCOMPATIBLE_WORKSPACE)
                current != null && published.value != current ->
        ...
        SelectorDocumentAdmission.Rejected ->
            CanonicalSelectorDecoding.Rejected(CanonicalSelectorDecodingFailure.INVALID_DOCUMENT)
    }
}

internal fun SymbolDiscoveryConstraints.selectorDocument(): SelectorConstraintsDocument? =
    if (this == SymbolDiscoveryConstraints.None) null
    else
        SelectorConstraintsDocument(
...
private fun SelectorConstraintsDocument?.admitConstraints(): SelectorDocumentAdmission<SymbolDiscoveryConstraints> {
    ...
    val directoryConstraint = directory?.let {
        ...
        val containment =
            SymbolDiscoveryContainment.entries.singleOrNull { entry -> entry.name == it.containment }
                ?: return SelectorDocumentAdmission.Rejected
    ...
    val kinds = declarationKinds?.let { names ->
        ...
        val values =
            names.mapTo(linkedSetOf()) { name ->
                CompilerSymbolKind.entries.singleOrNull { it.name == name } ?: return SelectorDocumentAdmission.Rejected
            }
    ...
    val sets =
        sourceSets?.let { names ->
            if (names.distinct().size != names.size) return SelectorDocumentAdmission.Rejected
            val values =
                names.mapTo(linkedSetOf()) {
                    WorkspaceSourceSetName.parse(it).valueOrNull() ?: return SelectorDocumentAdmission.Rejected
                }
    ...
    return SelectorDocumentAdmission.Admitted(
        SymbolDiscoveryConstraints(directoryConstraint, packageConstraint, kinds, sets)
    )
}
...
private fun SelectorReadScopeDocument?.admitReadScope(
    ...
    val scope =
        when (val admitted = admitScope(this, root)) {
            is SelectorDocumentAdmission.Admitted -> admitted.value
            SelectorDocumentAdmission.Rejected -> return SelectorDocumentAdmission.Rejected
        }
