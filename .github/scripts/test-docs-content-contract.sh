...
require_contains "$index_doc" "kast install copilot" "Overview must show repository Copilot install"
...
require_contains "$install_doc" "kast install copilot" "Install docs must make repository Copilot install primary"
require_contains "$install_doc" "kast doctor --repair" "Install docs must document repair"
...
require_contains "$headless_doc" 'kast package ubuntu-debian-bundle' "Headless docs must document the release bundle packager"

require_contains "$quickstart_doc" "kast up --backend=headless" "Quickstart must start a backend"
...
require_contains "$commands_index" "Lifecycle" "Command overview must list lifecycle commands"
require_contains "$commands_index" "Install and repair" "Command overview must list install commands"
require_contains "$commands_index" "Agent automation" "Command overview must list agent commands"
require_contains "$lifecycle_doc" "kast up" "Lifecycle docs must cover up"
require_contains "$lifecycle_doc" "kast status" "Lifecycle docs must cover status"
require_contains "$lifecycle_doc" "kast restart" "Lifecycle docs must cover restart"
require_contains "$lifecycle_doc" "kast stop" "Lifecycle docs must cover stop"
require_contains "$install_repair_doc" "kast install copilot" "Install command docs must cover Copilot install"
require_contains "$install_repair_doc" "kast doctor --repair" "Install command docs must cover repair"
...
require_contains "$metrics_doc" "kast metrics fan-in" "Metrics docs must cover direct metrics"
require_contains "$metrics_doc" "kast agent metrics" "Metrics docs must cover agent metrics"
require_contains "$lsp_doc" "kast lsp --stdio" "LSP docs must cover the stdio command"
...
require_contains "$troubleshooting_doc" "kast --output json doctor" "Troubleshooting must start from doctor"
require_contains "$troubleshooting_doc" "kast agent health" "Troubleshooting must include agent health"
require_contains "$distribution_doc" "kast package ubuntu-debian-bundle" "Distribution docs must cover bundle packaging"
require_contains "$distribution_doc" "kast install activate-bundle" "Distribution docs must cover bundle activation"
