...
require_block_not_contains "$ci_workflow" "  prepared-generation:" "  prepared-linux-packages:" "package-headless-runtime.sh" "Prepared-generation publication must not wait for derived Linux package assembly"
require_block_not_contains "$ci_workflow" "  prepared-generation:" "  prepared-linux-packages:" "package-gradle-ro-cache.sh" "Prepared-generation publication must not wait for the derived Gradle cache"
require_block_contains "$ci_workflow" "  prepared-linux-packages:" "  install-ubuntu-debian-container:" "      - prepared-generation" "Derived Linux packages must consume the immutable prepared generation"
require_block_contains "$ci_workflow" "  prepared-linux-packages:" "  install-ubuntu-debian-container:" "package-prepared-local-generation-derivatives.sh" "Derived Linux packages must be packaged from verified prepared bytes"
require_block_contains "$ci_workflow" "  prepared-linux-packages:" "  install-ubuntu-debian-container:" "dist/kast-ubuntu-debian-headless-x86_64-v0.7.11-ci.tar.gz.sha256" "The derived Ubuntu and Debian bundle upload must preserve its required digest sidecar"
require_block_contains "$ci_workflow" "  install-ubuntu-debian-container:" "  kast-action-runtime-contract:" "      - prepared-linux-packages" "Ubuntu and Debian validation must consume the derived-package producer"
...
require_block_contains "$ci_workflow" "  kast-action-runtime-contract:" "  installed-semantic-fixture:" "      - prepared-linux-packages" "kast-action validation must consume the derived-package producer"
