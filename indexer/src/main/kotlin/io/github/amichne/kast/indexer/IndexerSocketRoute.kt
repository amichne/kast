...
internal sealed class IndexerSocketRoute(val address: Path, val physical: Path) {
    ...
    private class Direct(address: Path, physical: Path, private val parentKey: Any) :
        ...
        override fun validate(): Validation<IndexerSocketRoute, IndexerTransportFailure> = try {
            if (address.parent.toRealPath() == physical.parent &&
                Files.readAttributes(physical.parent, BasicFileAttributes::class.java, NOFOLLOW_LINKS).fileKey() == parentKey
            ) Validation.validated(this)
            else Validation.rejected(IndexerTransportFailure.SOCKET_PARENT_UNAVAILABLE)
        } catch (_: Exception) { Validation.rejected(IndexerTransportFailure.SOCKET_PARENT_UNAVAILABLE) }
    }

    companion object {
        fun prepare(address: Path): Validation<IndexerSocketRoute, IndexerTransportFailure> = try {
            if (InstalledEndpointAliases.isReservedSocket(address)) {
                ...
                val admitted = if (prefix == null) absolute else prefix.toRealPath().resolve(prefix.relativize(absolute))
                var cursor = admitted.root
                for (part in admitted) {
                    cursor = cursor.resolve(part)
                    if (Files.isSymbolicLink(cursor)) return Validation.rejected(IndexerTransportFailure.SOCKET_PARENT_UNAVAILABLE)
                }
                Files.createDirectories(admitted)
                val canonical = admitted.toRealPath()
                val key = Files.readAttributes(canonical, BasicFileAttributes::class.java, NOFOLLOW_LINKS).fileKey()
...
internal class IndexerOwnedFile private constructor(private val path: Path, private val key: Any) {
    fun retire(): IndexerArtifactRetirement = try {
    ...
    } catch (_: Exception) { IndexerArtifactRetirement.UNPROVEN }

    companion object {
        fun capture(path: Path): Validation<IndexerOwnedFile, IndexerTransportFailure> = try {
            val attributes = Files.readAttributes(path, BasicFileAttributes::class.java, NOFOLLOW_LINKS)
            val key = attributes.fileKey()
            if (attributes.isSymbolicLink || key == null) Validation.rejected(IndexerTransportFailure.SOCKET_BIND_FAILED)
            else Validation.validated(IndexerOwnedFile(path, key))
        } catch (_: Exception) { Validation.rejected(IndexerTransportFailure.SOCKET_BIND_FAILED) }
    }
}

internal enum class IndexerArtifactRetirement { PROVEN, UNPROVEN }
