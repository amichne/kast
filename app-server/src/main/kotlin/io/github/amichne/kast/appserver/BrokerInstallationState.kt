...
internal enum class InstallationStateFailure { PATH_REJECTED, PAYLOAD_REJECTED, EPOCH_ABSENT, EPOCH_REJECTED, WRITE_REJECTED }
...
internal object BrokerInstallationState {
    fun admit(root: Path): Refinement<ThreadBindingOwner.Installation, InstallationStateFailure> {
        val identity = when (val payload = identity(root)) {
        ...
        return try {
            ...
            if (!Files.exists(epoch, LinkOption.NOFOLLOW_LINKS)) {
                ...
                try {
                    ...
                    try { Files.createLink(epoch, temporary) }
                    catch (_: java.nio.file.FileAlreadyExistsException) { /* Read and validate the winning owner. */ }
                    java.nio.channels.FileChannel.open(state, java.nio.file.StandardOpenOption.READ).use { it.force(true) }
                } finally { Files.deleteIfExists(temporary) }
            }
            readEpoch(state, identity)
        } catch (_: Exception) { Refinement.Rejected(InstallationStateFailure.WRITE_REJECTED) }
    }
    ...
    private fun readEpoch(state: Path, identity: String): Refinement<ThreadBindingOwner.Installation, InstallationStateFailure> {
      return try {
        if (!Files.readAttributes(state, java.nio.file.attribute.BasicFileAttributes::class.java, LinkOption.NOFOLLOW_LINKS).isDirectory || state.toRealPath() != state)
        ...
        val epoch = state.resolve("epoch.json")
            ...
            if (document.keys != setOf("schemaVersion", "installation", "epoch") ||
                document["schemaVersion"] != JsonPrimitive(1) || document["installation"] != JsonPrimitive(identity)) {
                return Refinement.Rejected(InstallationStateFailure.EPOCH_REJECTED)
            }
            val raw = (document["epoch"] as? JsonPrimitive)?.contentOrNull
                ?: return Refinement.Rejected(InstallationStateFailure.EPOCH_REJECTED)
        ...
        } catch (_: java.nio.file.NoSuchFileException) { Refinement.Rejected(InstallationStateFailure.EPOCH_ABSENT) }
        catch (_: Exception) { Refinement.Rejected(InstallationStateFailure.EPOCH_REJECTED) }
    }
    ...
    private fun identity(root: Path): Refinement<String, InstallationStateFailure> {
      ...
      } catch (_: Exception) { Refinement.Rejected(InstallationStateFailure.PAYLOAD_REJECTED) }
    }
}
