...
): HostedReadPreparation<HostedInheritorEvidence> =
    prepareHostedRead(
        selection.root,
        progress,
        checkpoint,
...
internal suspend fun <Evidence : Any> AdmittedIdeProject.prepareHostedRead(
    ...
    val epoch =
        when (val observation = observeReadEpoch()) {
            is ProjectReadEpochObservation.Observed -> observation.epoch
            is ProjectReadEpochObservation.Rejected ->
                return rejected(HostedQueryFailure.ReadEpoch(observation.failure))
        }
    progress.advance(HostedQueryStage.MODEL_CAPTURE)
    val model =
        when (val captured = captureDetachedModelAsync(progress.limits)) {
    ...
    val freshness =
        when (val admitted = admitVfsPassiveRead(epoch)) {
    ...
    val execution =
        when (val admitted = cancellableReadExecution(freshness)) {
                ...
                return rejected(
                    when (val failure = admitted.failure) {
                        ...
                        is AdmittedProjectReadExecutionAdmissionFailure.FreshnessRejected ->
                            HostedQueryFailure.Freshness(failure.cause)
                    }
                )
        }
    ...
    val evidence =
        when (
            val read = execution.executeAsync { project ->
                when (val current = admitVfsPassiveRead(epoch)) {
                    ...
                    is VfsPassiveReadAdmission.Rejected ->
                        HostedSemanticRead.Rejected(HostedQueryFailure.Freshness(current.failure))
                }
            }
        ) {
            is AdmittedProjectReadExecutionResult.Completed ->
                when (val value = read.value) {
...
private fun AdmittedProjectReadExecutionFailure.hostedFailure(): HostedQueryFailure =
    when (this) {
