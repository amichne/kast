...
class HostedClientTest(unittest.TestCase):
    ...
    def reply(self, raw, request=None):
        ...
        metadata = dict(type="KAST_IDE_ENDPOINT", protocol=1, root=str(self.root), socket=str(path), hostPid=123,
                        operations=["DESCRIBE", "CLASS_LOOKUP", "DIRECT_SUPERTYPE"])
    ...
    def test_host_identity_must_match_descriptor(self):
        result = dict(type="KAST_IDE_HOST", protocol=1, root=str(self.root), hostPid=124,
                      operations=["DESCRIBE", "CLASS_LOOKUP", "DIRECT_SUPERTYPE"], indexAuthority="existing_ide_kotlin_stub_index")
    ...
    def test_schema_closes_host_capabilities(self):
        document = dict(type="KAST_IDE_HOST", protocol=1, root=str(self.root), hostPid=os.getpid(),
