...
enum class NetworkProperty(val key: String) {
    ...
    HTTP_HOST("http.proxyHost"), HTTP_PORT("http.proxyPort"),
    BYPASS("http.nonProxyHosts"),
    HTTPS_HOST("https.proxyHost"), HTTPS_PORT("https.proxyPort");

    val group: NetworkPropertyGroup get() = when (this) {
        TRUST_STORE, TRUST_STORE_TYPE, TRUST_STORE_PROVIDER, TRUST_STORE_PASSWORD -> NetworkPropertyGroup.TRUST
        HTTP_HOST, HTTP_PORT -> NetworkPropertyGroup.HTTP_PROXY
        HTTPS_HOST, HTTPS_PORT -> NetworkPropertyGroup.HTTPS_PROXY
        BYPASS -> NetworkPropertyGroup.BYPASS
    }
    val isTrust: Boolean get() = group == NetworkPropertyGroup.TRUST
}

enum class NetworkPropertyGroup { TRUST, HTTP_PROXY, HTTPS_PROXY, BYPASS }

enum class NetworkConfigurationFailure {
    UNKNOWN_PROPERTY, INVALID_VALUE, INVALID_PATH, INVALID_PORT, INCOMPLETE_TRUST, INCOMPLETE_PROXY,
    CONFIGURATION_UNAVAILABLE, MALFORMED_JVM_ARGUMENTS,
}
...
class TrustStorePassword private constructor(private val characters: CharArray?) {
    fun <T> useAtJsseBoundary(operation: (CharArray?) -> T): T {
        ...
        return try { operation(copy) } finally { copy?.fill('\u0000') }
    }
...
class NetworkConfiguration private constructor(private val values: Map<NetworkProperty, String>) {
    val trust: TrustSelection = values[NetworkProperty.TRUST_STORE]?.let { raw ->
        TrustSelection.Explicit(
            Path.of(raw),
            values[NetworkProperty.TRUST_STORE_TYPE] ?: "JKS",
            values[NetworkProperty.TRUST_STORE_PROVIDER],
            values[NetworkProperty.TRUST_STORE_PASSWORD]?.let(TrustStorePassword::fromBoundary) ?: TrustStorePassword.Unspecified,
        )
    ...
    fun missingFrom(owner: NetworkConfiguration): NetworkConfiguration = NetworkConfiguration(
        values.filterKeys { key -> owner.values.keys.none { it.group == key.group } },
    )

    fun withDerivedTrust(path: Path): Refinement<NetworkConfiguration, NetworkConfigurationFailure> = parse(
        values.filterKeys { !it.isTrust }.mapKeys { it.key.key } + mapOf(
    ...
    companion object {
        ...
        fun parse(raw: Map<String, String>): Refinement<NetworkConfiguration, NetworkConfigurationFailure> {
            ...
            for ((key, value) in raw) {
                val property = NetworkProperty.entries.singleOrNull { it.key == key }
                ...
                when (property) {
                    NetworkProperty.TRUST_STORE -> {
                        val path = try { Path.of(value) } catch (_: java.nio.file.InvalidPathException) {
                            return Refinement.Rejected(NetworkConfigurationFailure.INVALID_PATH)
                        }
                        if (!path.isAbsolute || path.normalize() != path) return Refinement.Rejected(NetworkConfigurationFailure.INVALID_PATH)
                    }
                    NetworkProperty.TRUST_STORE_TYPE -> if (value !in setOf("JKS", "PKCS12")) {
                        return Refinement.Rejected(NetworkConfigurationFailure.INVALID_VALUE)
                    }
                    NetworkProperty.HTTP_PORT, NetworkProperty.HTTPS_PORT -> if (value.toIntOrNull() !in 1..65535) {
                        return Refinement.Rejected(NetworkConfigurationFailure.INVALID_PORT)
                    }
                    NetworkProperty.HTTP_HOST, NetworkProperty.HTTPS_HOST -> if (!value.matches(Regex("[a-zA-Z0-9._:\\[\\]-]+"))) {
                        return Refinement.Rejected(NetworkConfigurationFailure.INVALID_VALUE)
                    }
                    NetworkProperty.BYPASS -> if (!value.matches(Regex("[a-zA-Z0-9.*_|:\\[\\]-]+"))) {
                        return Refinement.Rejected(NetworkConfigurationFailure.INVALID_VALUE)
                    }
                    NetworkProperty.TRUST_STORE_PROVIDER -> if (!value.matches(Regex("[a-zA-Z0-9._-]+"))) {
                        return Refinement.Rejected(NetworkConfigurationFailure.INVALID_VALUE)
                    }
            ...
            if ((NetworkProperty.HTTP_PORT in values && NetworkProperty.HTTP_HOST !in values) ||
...
sealed interface TrustSelection {
    ...
    class Explicit internal constructor(
        val path: Path,
        val type: String,
        val provider: String?,
...
enum class NetworkConsumer { SIDECAR_TOOLING_CLIENT, GRADLE_DAEMON }
enum class TrustProvenance { EXPLICIT_KAST, EXPLICIT_GRADLE, DONOR_JVM, INTELLIJ_ACCEPTED, TARGET_JVM }
