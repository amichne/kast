...
enum class InstalledEndpointAliasFailure {
    PHYSICAL_DIRECTORY_REJECTED, ALIAS_REJECTED, RECEIPT_REJECTED, FILESYSTEM_REJECTED,
}

@Serializable
internal data class EndpointFileIdentity(val device: Long, val inode: Long, val owner: Long)
...
class InstalledEndpointAliasReceipt private constructor(
    val alias: Path,
    val physicalDirectory: Path,
    private val aliasIdentity: EndpointFileIdentity,
    private val targetIdentity: EndpointFileIdentity,
) {
    fun validate(): Validation<InstalledEndpointAliasReceipt, InstalledEndpointAliasFailure> = try {
        if (!InstalledEndpointAliases.matches(alias, physicalDirectory) ||
            physicalDirectory.fileName.toString() != "run" || physicalDirectory.parent.fileName.toString() != "state" ||
            InstalledEndpointAliases.physicalDirectoryIdentity(physicalDirectory) != targetIdentity ||
            !Files.isSymbolicLink(alias) || Files.readSymbolicLink(alias) != physicalDirectory ||
            InstalledEndpointAliases.identity(alias) != aliasIdentity || aliasIdentity.owner != targetIdentity.owner
    ...
    companion object {
        internal fun capture(alias: Path, physicalDirectory: Path):
            Validation<InstalledEndpointAliasReceipt, InstalledEndpointAliasFailure> = try {
            val physical = InstalledEndpointAliases.physicalDirectoryIdentity(physicalDirectory)
                ?: return Validation.rejected(InstalledEndpointAliasFailure.PHYSICAL_DIRECTORY_REJECTED)
            val link = InstalledEndpointAliases.identity(alias)
        ...
        internal fun read(document: EndpointAliasDocument): Validation<InstalledEndpointAliasReceipt, InstalledEndpointAliasFailure> =
            InstalledEndpointAliasReceipt(Path.of(document.alias), Path.of(document.target),
                document.aliasIdentity, document.targetIdentity).validate()
    }

    internal fun document(): String = Json.encodeToString(EndpointAliasDocument(
        1, alias.toString(), physicalDirectory.toString(), aliasIdentity, targetIdentity,
    ))
}
...
object InstalledEndpointAliases {
    ...
    private fun aliasFor(directory: Path): Path = root.resolve(PREFIX + HexFormat.of().formatHex(
        MessageDigest.getInstance("SHA-256").digest(directory.toString().toByteArray(StandardCharsets.UTF_8)),
    ).take(32))

    internal fun matches(alias: Path, directory: Path): Boolean = alias == aliasFor(directory)

    fun isReservedSocket(path: Path): Boolean = path.parent?.parent == root &&
    ...
    fun prepare(directory: Path): Validation<InstalledEndpointAliasReceipt, InstalledEndpointAliasFailure> = try {
        if (physicalDirectoryIdentity(directory) == null ||
            directory.fileName.toString() != "run" || directory.parent.fileName.toString() != "state"
        ) return Validation.rejected(InstalledEndpointAliasFailure.PHYSICAL_DIRECTORY_REJECTED)
        ...
        if (Files.exists(receipt, NOFOLLOW_LINKS)) return Validation.rejected(InstalledEndpointAliasFailure.RECEIPT_REJECTED)
        try {
            Files.createSymbolicLink(alias, directory)
        } catch (_: FileAlreadyExistsException) {
            return observe(alias.resolve("c.sock"))
        }
        val admitted = when (val observed = InstalledEndpointAliasReceipt.capture(alias, directory)) {
    ...
    fun observe(socket: Path): Validation<InstalledEndpointAliasReceipt, InstalledEndpointAliasFailure> = try {
        ...
        if (!target.isAbsolute || target.normalize() != target || aliasFor(target) != alias ||
            target.fileName.toString() != "run" || target.parent.fileName.toString() != "state"
        ) return Validation.rejected(InstalledEndpointAliasFailure.ALIAS_REJECTED)
        val physicalIdentity = physicalDirectoryIdentity(target)
            ?: return Validation.rejected(InstalledEndpointAliasFailure.PHYSICAL_DIRECTORY_REJECTED)
        val receipt = target.resolve(RECEIPT)
        if (!Files.isRegularFile(receipt, NOFOLLOW_LINKS) || Files.size(receipt) > 4096 ||
        ...
        if (document.schemaVersion != 1 || document.alias != alias.toString() || document.target != target.toString())
            return Validation.rejected(InstalledEndpointAliasFailure.RECEIPT_REJECTED)
    ...
    internal fun physicalDirectoryIdentity(path: Path): EndpointFileIdentity? =
        if (!path.isAbsolute || path.normalize() != path || path.toRealPath() != path ||
            !Files.isDirectory(path, NOFOLLOW_LINKS) || Files.getPosixFilePermissions(path, NOFOLLOW_LINKS) != privateDirectory
        ) null else identity(path)?.takeIf { it.owner == UnixSystem().uid }
}
