...
internal object JdkRuntimeProcessStarter : RuntimeProcessStarter {
     ...
     *
     * Starts the exact admitted command with only Kast's allowlisted runtime environment. The returned session retains
     * the concrete child identity until endpoint readiness or terminal startup, while process and environment failures
     * remain finite [RuntimeProcessStart] data.
     */
    override fun start(command: IndexerLaunchCommand): RuntimeProcessStart {
        ...
        val launcher =
            try {
                ...
                return RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.ProcessCreationRejected)
            } catch (_: SecurityException) {
                return RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.ProcessCreationRejected)
            }
        return when (val detachment = launcher.awaitDetachedChild()) {
            is DirectRuntimeChildStart.Started ->
            ...
            DirectRuntimeChildStart.Rejected ->
                RuntimeProcessStart.Rejected(RuntimeProcessStartFailure.ChildStartRejected)
        }
    }
}
 ...
 *
 * Preserves the exact admitted indexer arguments while establishing a separate process group and terminal-hangup
 * immunity for the default direct child. The fixed shell program receives every admitted argument positionally, so no
 * workspace or runtime value is evaluated as shell source. Raw arguments leave only at the [ProcessBuilder] boundary.
 */
...
private sealed interface DirectRuntimeChildStart {
    data class Started(val process: ProcessHandle) : DirectRuntimeChildStart
...
private sealed interface DirectRuntimeProcessIdAdmission {
    data class Admitted(val processId: DirectRuntimeProcessId) : DirectRuntimeProcessIdAdmission
...
@JvmInline
private value class DirectRuntimeProcessId private constructor(val value: Long) {
    fun resolve(): DirectRuntimeChildStart =
        try {
    ...
    companion object {
        fun admit(raw: String?): DirectRuntimeProcessIdAdmission {
            val value = raw?.toLongOrNull()?.takeIf { it > 0L } ?: return DirectRuntimeProcessIdAdmission.Rejected
 ...
 *
 * Establishes the exact detached child identity only after the fixed shell boundary exits successfully. Timeout,
 * malformed identity, interruption, and inaccessible process state remain closed finite outcomes.
 */
private fun Process.awaitDetachedChild(): DirectRuntimeChildStart {
    ...
    val processId =
        when (val admission = DirectRuntimeProcessId.admit(rawProcessId)) {
...
private class DirectRuntimeStartupSession(private val process: ProcessHandle) : AcceptedRuntimeStartupSession {
