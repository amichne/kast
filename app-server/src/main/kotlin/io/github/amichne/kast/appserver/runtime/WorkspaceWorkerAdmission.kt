...
internal value class WorkerMemoryMiB private constructor(val value: Long) {
    companion object {
        ...
        fun admit(raw: Long): Refinement<WorkerMemoryMiB, WorkerPolicyFailure> =
            if (raw >= 0) Refinement.Refined(WorkerMemoryMiB(raw))
            else Refinement.Rejected(WorkerPolicyFailure.MEMORY_REJECTED)
    }
}

internal enum class WorkerPolicyFailure {
    RESIDENT_LIMIT_REJECTED,
    STARTUP_LIMIT_REJECTED,
    MEMORY_REJECTED,
    MEMORY_OVERFLOW,
}

/** Declared reservation accounting, not a claim about observed process footprint or physical fit. */
internal class DeclaredWorkerMemory
private constructor(
    ...
    fun matches(other: DeclaredWorkerMemory): Boolean =
        heap == other.heap && nativeAllowance == other.nativeAllowance && gradleAllowance == other.gradleAllowance
    ...
    companion object {
        fun admit(
            ...
            nativeMiB: Long,
            gradleMiB: Long,
        ): Refinement<DeclaredWorkerMemory, WorkerPolicyFailure> {
            val native =
                when (val admitted = WorkerMemoryMiB.admit(nativeMiB)) {
                    is Refinement.Refined -> admitted.value
                    is Refinement.Rejected -> return admitted
                }
            val gradle =
                when (val admitted = WorkerMemoryMiB.admit(gradleMiB)) {
                    is Refinement.Refined -> admitted.value
                    is Refinement.Rejected -> return admitted
                }
            val heapMiB = heap.mebibytes.toLong()
            if (nativeMiB > Long.MAX_VALUE - heapMiB || gradleMiB > Long.MAX_VALUE - heapMiB - nativeMiB)
                return Refinement.Rejected(WorkerPolicyFailure.MEMORY_OVERFLOW)
            val total =
                when (val admitted = WorkerMemoryMiB.admit(heapMiB + nativeMiB + gradleMiB)) {
                    is Refinement.Refined -> admitted.value
                    is Refinement.Rejected -> return admitted
                }
...
internal class WorkerAdmissionPolicy
private constructor(
    ...
    companion object {
        fun admit(
            ...
            maximumStarting: Int,
            aggregateMiB: Long,
        ): Refinement<WorkerAdmissionPolicy, WorkerPolicyFailure> {
            if (
                maximumResident !in
                    1..io.github.amichne.kast.distribution.contract.configuration.WorkerCountLimit.Maximum
            )
                return Refinement.Rejected(WorkerPolicyFailure.RESIDENT_LIMIT_REJECTED)
            if (maximumStarting !in 1..maximumResident)
                return Refinement.Rejected(WorkerPolicyFailure.STARTUP_LIMIT_REJECTED)
            if (aggregateMiB <= 0) return Refinement.Rejected(WorkerPolicyFailure.MEMORY_REJECTED)
            val aggregate =
                when (val admitted = WorkerMemoryMiB.admit(aggregateMiB)) {
                    is Refinement.Refined -> admitted.value
                    is Refinement.Rejected -> return admitted
                }
...
internal value class WorkerReservationId private constructor(val value: UUID) {
    companion object {
        ...
        fun admit(raw: String): Refinement<WorkerReservationId, WorkerAdmissionFailure> =
            try {
                val parsed = UUID.fromString(raw)
                if (parsed.toString() == raw) Refinement.Refined(WorkerReservationId(parsed))
                else Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_UNKNOWN)
            } catch (_: IllegalArgumentException) {
                Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_UNKNOWN)
            }
    }
}

internal fun interface WorkerReservationIdSource {
    fun next(): WorkerReservationId
}

internal class WorkerReservation
...
internal class WorkspaceWorkerRoute
internal constructor(
...
internal enum class WorkerReservationPhase {
    RESERVED,
    STARTING,
    READY,
    QUARANTINED_STARTUP,
...
internal data class WorkerAdmissionSnapshot(
    val workers: List<WorkerReservationStatus>,
    val reserved: WorkerMemoryMiB,
    val starting: Int,
)
...
internal class WorkerAdmissionCoordinator(
    ...
    @Synchronized
    fun request(identity: WorkspaceWorkerIdentity, memory: DeclaredWorkerMemory): WorkerDemand {
        when (val admitted = admitIdentity(identity)) {
            is Refinement.Rejected -> return WorkerDemand.Rejected(admitted.failure)
            is Refinement.Refined -> Unit
        }
        if (admissionState == AdmissionState.DRAINING)
            return WorkerDemand.Rejected(WorkerAdmissionFailure.COORDINATOR_UNAVAILABLE)
        entries[identity.workspace.id]?.let { entry ->
            if (entry.reservation.identity != identity)
                return WorkerDemand.Rejected(WorkerAdmissionFailure.RESERVATION_IDENTITY_MISMATCH)
            when (entry.phase) {
                is Phase.QuarantinedStartup,
                is Phase.QuarantinedRuntime -> return WorkerDemand.Rejected(WorkerAdmissionFailure.RECOVERY_REQUIRED)
                else -> Unit
            }
            if (!entry.reservation.memory.matches(memory))
                return WorkerDemand.Rejected(WorkerAdmissionFailure.RESOURCE_CONFIGURATION_CONFLICT)
            return when (val phase = entry.phase) {
                is Phase.Ready -> WorkerDemand.Ready(phase.route)
                Phase.Reserved,
                is Phase.Starting -> WorkerDemand.Awaiting(entry.readiness)
                is Phase.QuarantinedStartup,
                is Phase.QuarantinedRuntime -> WorkerDemand.Rejected(WorkerAdmissionFailure.RECOVERY_REQUIRED)
            }
        }
        if (entries.size >= policy.maximumResident)
            return WorkerDemand.Rejected(WorkerAdmissionFailure.RESIDENT_CAPACITY_EXCEEDED)
        if (entries.values.count { it.phase.isStarting() } >= policy.maximumStarting)
            return WorkerDemand.Rejected(WorkerAdmissionFailure.STARTUP_CAPACITY_EXCEEDED)
        val reserved = entries.values.sumOf { it.reservation.memory.total.value }
        if (memory.total.value > policy.aggregate.value - reserved)
            return WorkerDemand.Rejected(WorkerAdmissionFailure.AGGREGATE_RESERVATION_EXCEEDED)
        val id = reservationIds.next()
        if (entries.values.any { it.reservation.id == id })
            return WorkerDemand.Rejected(WorkerAdmissionFailure.RESERVATION_IDENTITY_MISMATCH)
    ...
    @Synchronized
    fun consume(
        id: WorkerReservationId,
        identity: WorkspaceWorkerIdentity,
    ): Refinement<WorkerLaunchPermit, WorkerAdmissionFailure> {
        when (val admitted = admitIdentity(identity)) {
            is Refinement.Rejected -> return admitted
            is Refinement.Refined -> Unit
        }
        if (admissionState == AdmissionState.DRAINING)
            return Refinement.Rejected(WorkerAdmissionFailure.COORDINATOR_UNAVAILABLE)
        val entry =
            entries.values.singleOrNull { it.reservation.id == id }
                ?: return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_UNKNOWN)
        if (entry.reservation.identity != identity)
            return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_IDENTITY_MISMATCH)
        if (entry.phase != Phase.Reserved)
            return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_ALREADY_CONSUMED)
    ...
    ): Refinement<WorkspaceWorkerRoute, WorkerAdmissionFailure> {
        val entry =
            entries[permit.reservation.identity.workspace.id]
                ?: return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_UNKNOWN)
        val phase = entry.phase
        if (
            entry.reservation !== permit.reservation ||
                phase !is Phase.Starting ||
                phase.permit !== permit ||
    ...
    fun quarantine(
        ...
        failure: WorkerAdmissionFailure,
    ): Refinement<WorkerReservation, WorkerAdmissionFailure> {
        val entry =
            entries[reservation.identity.workspace.id]
                ?: return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_UNKNOWN)
        if (entry.reservation !== reservation)
            return Refinement.Rejected(WorkerAdmissionFailure.RESERVATION_IDENTITY_MISMATCH)
        entry.phase =
            if (entry.phase.isStarting()) Phase.QuarantinedStartup(failure) else Phase.QuarantinedRuntime(failure)
    ...
    suspend fun retire(
        reservation: WorkerReservation,
        authority: WorkerRetirementAuthority,
    ): Refinement<WorkerReservation, WorkerAdmissionFailure> {
        ...
        val observed =
            try {
                authority.observe(reservation)
            } catch (cancelled: kotlinx.coroutines.CancellationException) {
                throw cancelled
            } catch (_: Exception) {
                WorkerRetirementObservation.Unproven
            }
        return synchronized(this) {
            ...
            when {
                ...
                observed !is WorkerRetirementObservation.ExactRetired || observed.reservation !== reservation ->
                    Refinement.Rejected(WorkerAdmissionFailure.RETIREMENT_UNPROVEN)
                else -> {
    ...
    @Synchronized
    fun snapshot(): WorkerAdmissionSnapshot {
        val workers =
        ...
        val reserved =
            when (val admitted = WorkerMemoryMiB.admit(entries.values.sumOf { it.reservation.memory.total.value })) {
                is Refinement.Refined -> admitted.value
    ...
    private fun admitIdentity(
        identity: WorkspaceWorkerIdentity
    ): Refinement<WorkspaceWorkerIdentity, WorkerAdmissionFailure> =
        when {
            ...
            identity.serviceGeneration != serviceGeneration ->
                Refinement.Rejected(WorkerAdmissionFailure.SERVICE_GENERATION_MISMATCH)
            else -> Refinement.Refined(identity)
        }

    private fun Phase.isStarting(): Boolean =
        when (this) {
            Phase.Reserved,
            is Phase.Starting,
            is Phase.QuarantinedStartup -> true
            is Phase.Ready,
            is Phase.QuarantinedRuntime -> false
        }
}
