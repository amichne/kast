...
sealed interface DerivedTrustStoreMaterialization {
    data class Complete(val path: Path, val digest: String, val sources: Set<TrustProvenance>) :
...
object DerivedTrustStoreMaterializer {
    ...
    private fun materializeExclusively(
        directory: Path,
        ...
        for (donor in donors) {
            ...
            val store =
                if (donor.provider == null) java.security.KeyStore.getInstance(donor.type)
        ...
        if (java.nio.file.Files.exists(target, java.nio.file.LinkOption.NOFOLLOW_LINKS)) {
            ...
            if (
                aliases.any { !existing.isCertificateEntry(it) } ||
            ...
            val temporary =
                java.nio.file.Files.createTempFile(directory, ".trust-", ".tmp", *privateAttributes(directory))
            try {
    ...
    private fun privateAttributes(path: Path): Array<java.nio.file.attribute.FileAttribute<*>> =
        if (java.nio.file.Files.getFileStore(path).supportsFileAttributeView("posix"))
            arrayOf(java.nio.file.attribute.PosixFilePermissions.asFileAttribute(permissions))
        else emptyArray()
