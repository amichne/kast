...
internal class TopologyBackedTraversalOperations(
    ...
    override suspend fun run(plan: TraversalPlan): TraversalResult =
        observability.inSpan(KastSpanName.TRAVERSAL_RUN) { span ->
    ...
    private suspend fun runObserved(
        ...
        val workspace =
            when (
                val state =
                    trace.child(KastSpanName.TRAVERSAL_WORKSPACE) { span ->
                ...
                WorkspaceRuntimeState.Stopping ->
                    return rejected(TraversalRejection.OneHopRejected(RelationReadRejection.WORKSPACE_NOT_READY))
            }
        if (plan.start.lease != workspace.readLease) {
            return rejected(TraversalRejection.OneHopRejected(RelationReadRejection.STALE_GENERATION))
        }
        val snapshot =
            when (
                val eligible =
                    trace.child(KastSpanName.TRAVERSAL_SNAPSHOT_ELIGIBILITY) { span ->
                            ...
                            span.observe(
                                when (result) {
                                    ...
                                    is TopologySnapshotEligibility.Stale ->
                                        rejectedObservation(KastSpanFailure.TRAVERSAL_EVIDENCE_STALE)
                                    TopologySnapshotEligibility.Unavailable,
                                    is TopologySnapshotEligibility.Rejected ->
                                        rejectedObservation(KastSpanFailure.TRAVERSAL_EVIDENCE_UNAVAILABLE)
                                }
                            )
                        }
                    }
                ...
                is TopologySnapshotEligibility.Stale -> return rejected(TraversalRejection.RequiredEvidenceStale)
                TopologySnapshotEligibility.Unavailable,
                is TopologySnapshotEligibility.Rejected ->
        ...
        val compiler =
            when (
                val opened =
                    trace.child(KastSpanName.TRAVERSAL_SNAPSHOT_OPEN) { span ->
            ...
            when (val result = traversalOperations(relations).run(plan)) {
                is TraversalResult.Rejected ->
                    when (val reason = result.reason) {
                        ...
                        TraversalRejection.TraversalContractViolation -> result
                    }
                is TraversalResult.Complete,
                is TraversalResult.Qualified -> result
...
private fun TraversalResult.traceObservation(): KastSpanObservation =
    when (this) {
        is TraversalResult.Complete ->
            completeObservation(KastSpanMeasurement.RecordCount(exactCount(page.records.size)))
                ...
                when (val rejection = reason) {
                    is TraversalRejection.OneHopRejected ->
                        when (rejection.reason) {
                            RelationReadRejection.WORKSPACE_NOT_READY -> KastSpanFailure.TRAVERSAL_WORKSPACE_NOT_READY
                            RelationReadRejection.STALE_GENERATION -> KastSpanFailure.TRAVERSAL_STALE_GENERATION
                            else -> KastSpanFailure.TRAVERSAL_ONE_HOP
                        }
                    TraversalRejection.RequiredEvidenceStale -> KastSpanFailure.TRAVERSAL_EVIDENCE_STALE
                    TraversalRejection.RequiredEvidenceUnavailable -> KastSpanFailure.TRAVERSAL_EVIDENCE_UNAVAILABLE
                    TraversalRejection.ReaderContractViolation -> KastSpanFailure.TRAVERSAL_READER_CONTRACT
                    TraversalRejection.TraversalContractViolation -> KastSpanFailure.TRAVERSAL_CONTRACT
                }
            )
    }
...
private fun exactCount(value: Int): KastSpanCount =
    when (val parsed = KastSpanCount.parse(value.toLong())) {
