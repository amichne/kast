...
internal sealed interface CliOptionValue<out Value> {
    ...
    data class Present<Value>(val value: Value) : CliOptionValue<Value>
}
...
internal fun ParameterHolder.absolutePathOption(
    ...
    option(name, help = help, metavar = "absolute-path").convert("absolute-path") { raw ->
        val candidate =
            try {
                Path.of(raw)
 ...
 *
 * Establishes one member of the supplied closed value set. Unknown text becomes a Clikt usage rejection. Raw option
 * text is extracted only inside Clikt's conversion boundary.
 */
internal fun <Value : Any> ParameterHolder.closedChoiceOption(
    ...
    option(name, help = help, metavar = metavar).convert(metavar) { raw ->
 ...
 *
 * Establishes absence or exactly one supplied value. Duplicate calls become a Clikt usage rejection. Raw invocation
 * multiplicity remains inside Clikt's option boundary.
 */
 ...
 *
 * Establishes either one supplied value or the declared default. Duplicate calls become a Clikt usage rejection. Raw
 * invocation multiplicity remains inside Clikt's option boundary.
 */
internal fun <Value, Raw> NullableOption<Value, Raw>.defaultOnce(
    default: Value,
    defaultForHelp: String,
): OptionWithValues<Value, Value, Raw> =
    transformAll(defaultForHelp = defaultForHelp) { calls ->
        when (calls.size) {
            0 -> default
            1 -> calls.single()
            else -> fail("may be specified at most once")
