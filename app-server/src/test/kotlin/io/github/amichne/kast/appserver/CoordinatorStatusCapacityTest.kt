...
class CoordinatorStatusCapacityTest {
    @Test fun `passive transport admits the maximum valid worker status document`() = runBlocking {
        ...
        try {
            ...
            Files.writeString(command.readinessFile, BROKER_SERVICE_STATE_JSON.encodeToString(BrokerServiceStateDocument.serializer(),
                BrokerServiceStateDocument.Ready(BROKER_SERVICE_STATE_SCHEMA_VERSION, command.identity.value, generation, VENDORED_BROKER_VERSION)))
            val document = status(owner.installationId.value, owner.stateEpoch.value.toString(), generation, workerLaunchConfigurationIdentity(command.configuration), 256)
            ...
            val engine = embeddedServer(CIO, configure = { unixConnector(command.publicSocket.toString()) }, module = {
                install(WebSockets)
                routing { webSocket("/kast-runtime") {
                    val request = (incoming.receive() as Frame.Text).readText()
                    assertEquals("STATUS", Json.parseToJsonElement(request).jsonObject.getValue("action").jsonPrimitive.content)
    ...
    @Test fun `one extra worker exceeds admitted status capacity`() {
        val document = status("sha256:" + "0".repeat(64), UUID(0, 1).toString(), UUID(0, 2).toString(), "0".repeat(64), 257)
        assertTrue(CoordinatorStatusSnapshot.admit(document) is Refinement.Rejected)
    }

    private fun status(installation: String, epoch: String, generation: String, configuration: String, count: Int): JsonObject = buildJsonObject {
        val reservation = Long.MAX_VALUE / count
        put("status", "READY"); put("installationId", installation); put("stateEpoch", epoch)
        put("serviceGeneration", generation); put("configurationIdentity", configuration)
        put("reservedMiB", reservation * count); put("starting", 0); put("hostAttachment", "UNOBSERVED")
        putJsonArray("workers") { repeat(count) { index -> add(buildJsonObject {
            put("workspaceId", (index + 1).toString(16).padStart(64, '0'))
            put("reservationId", UUID(0, index.toLong() + 1).toString())
            put("phase", "QUARANTINED_RUNTIME"); put("reservedMiB", reservation)
            put("requestedHeapMiB", Int.MAX_VALUE); put("heapObservation", "UNOBSERVED")
