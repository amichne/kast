...
data class HostedCapability
private constructor(
    val operation: CanonicalOperation,
    val intents: Set<HostedCapabilityIntent>,
) {
    internal fun candidate(): HostedCapabilityCandidate =
        HostedCapabilityCandidate(
            operation.id.value,
...
sealed interface HostedCapabilitySetFailure {
    data class MalformedOperationId(val failure: PermanentIdentityFailure) : HostedCapabilitySetFailure

    data class UnknownOperation(val operationId: OperationId) : HostedCapabilitySetFailure

    data class UnsupportedIntent(val operation: CanonicalOperation) : HostedCapabilitySetFailure

    data class DuplicateOperation(val operation: CanonicalOperation) : HostedCapabilitySetFailure
...
class HostedCapabilitySet private constructor(val capabilities: List<HostedCapability>) {
    fun supports(operation: CanonicalOperation): Boolean = capabilities.any { it.operation == operation }
    ...
    internal fun candidates(): List<HostedCapabilityCandidate> = capabilities.map(HostedCapability::candidate)
    ...
    companion object {
        fun admit(raw: List<HostedCapabilityCandidate>): HostedCapabilitySetAdmission {
            ...
            raw.forEach { candidate ->
                val operationId =
                    when (val parsed = OperationId.parse(candidate.operationId)) {
                ...
                candidate.intents.forEach { identity ->
                    val intent =
                        HostedCapabilityIntent.entries.singleOrNull {
            ...
            return if (admitted == CanonicalHostedCapabilities.capabilities) {
                ...
                HostedCapabilitySetAdmission.Rejected(HostedCapabilitySetFailure.CanonicalProjectionMismatch)
            }
        }
    }
}
...
object CanonicalHostedCapabilities {
    ...
    val operations: Set<CanonicalOperation> =
        capabilities.mapTo(linkedSetOf()) {
            it.operation
        }
}
