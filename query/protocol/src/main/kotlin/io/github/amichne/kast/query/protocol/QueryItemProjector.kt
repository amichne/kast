...
import io.github.amichne.kast.protocol.contract.QueryCandidateFieldDocument
import io.github.amichne.kast.protocol.contract.QueryCandidateLocationDocument
...
internal class QueryItemProjector(private val authority: QueryReferenceAuthority) {
    fun projectItems(output: QueryOutputDocument, items: QueryResultSet): QueryProjection<QueryResultItemDocument> =
        when {
            output is QueryOutputDocument.Candidates && items is QueryResultSet.Candidates ->
                projectCandidates(output, items)
            output is QueryOutputDocument.Symbols && items is QueryResultSet.Symbols -> projectSymbols(output, items)
            else -> QueryProjection.Rejected
        }

    private fun projectCandidates(
        output: QueryOutputDocument.Candidates,
        items: QueryResultSet.Candidates,
            ...
            val boundedConnections =
                when (connections) {
                    is QueryProjection.Projected ->
                        BoundedProtocolList.create(connections.values).refinedForQueryOrNull()
                            ?: return@mapProjected null
