...
def prepare_read_fixture(workspace: Path, repo: Path) -> ReadFixture:
    ...
    diagnostic_fixture = target.parent / 'ReadDiagnosticPages.kt'
    shutil.copyfile(template / 'read-reliability/ReadDiagnosticPages.kt', diagnostic_fixture)
    ...
    files = [target, source_budget, call_fixture, diagnostic_fixture, enum_fixture, build, workspace / 'settings.gradle.kts']
    for name in ('core', 'logging'):
